...
// These launchers preserve the incumbent BF16 residual and centered-RMS
...
// input.  The second boundary has explicit K256 and lane-count-one R1
// publishers for the next layer's Attention input; neither route has a
// hidden fallback.
...
// Computes and publishes the MLP residual, applies the next layer's exact
// centered RMSNorm, then directly emits the next Attention projection's
// authenticated lane-count-one R1 A4 input.  `normalized_output_bf16` is an
// optional consumer-native publication for Linear-A/B, which remain BF16;
// pass nullptr with zero capacity when the next layer is Full Attention and
// consumes only packed_a.  Padded rows are zero-code / BF16-one-scale and
// never read left, right, norm_weight, or inverse_alpha. residual_output_bf16
// may exactly alias left_bf16 for the production in-place residual handoff;
// all other input/output aliases fail closed.
[[nodiscard]] int
