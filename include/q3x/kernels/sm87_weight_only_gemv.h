...
// Test-admission-only exact Decode QKV/Z/A/B rearchitecture. A 512-thread CTA
// contains two independently synchronized 256-thread row-quad workers. The
// fixed 1,024-CTA grid retains the production resident warp/owner population;
// each worker owns two row quads and keeps its five packed activation words
// only across those two tasks. Ordinary production dispatch never selects
// this entry point.
[[nodiscard]] int
...
[[nodiscard]] int
query_sm87_fp8_w8a16_m1_qkv_z_bf16_ab_dual_worker_resources_test_cuda(
...
// K128 slot preserves two ordered K64 async-copy groups and two independent
// 32-byte shared-B planes, reducing the C512 publication barriers from 80 to
// 40 without changing accumulation order or introducing B-row bank conflicts.
...
// (80 CTAs). C512 uses a Down-specialized M128xN256xK128 two-stage route
// (80 CTAs), presents four M128 token tiles consecutively per packed-weight
// panel, and bypasses L1 for its long activation stream. Only token_count=256
// or 512 and the exact aligned checkpoint shape are accepted; every near miss
// fails closed.
[[nodiscard]] int
