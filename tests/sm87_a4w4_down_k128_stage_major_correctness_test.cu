...
[[nodiscard]] bool run_resource_gate() {
  kernels::Sm87A4W4DownK128StageMajorResources resources{};
  if (!launch_ok(
          kernels::query_sm87_a4w4_down_k128_stage_major_resources_cuda(
              &resources),
          "query Down K128 stage-major resources")) {
    return false;
  }
  const bool gate =
      resources.registers_per_thread <= 255 &&
      resources.static_shared_bytes == 46'464U &&
  ...
  std::cout << "Down K128 stage-major resources: registers="
...
[[nodiscard]] bool run_small_ordered_equivalence() {
  ...
  constexpr std::size_t physical_groups = k_count / 64U;
  ...
  constexpr std::size_t a_scales_count =
      kernels::sm87_a4w4_consumer_k128_scale_capacity_elements(m_count,
                                                               k_count);
  constexpr std::size_t b_scales_count =
  ...
  for (std::size_t m = 0U; m < m_count; ++m) {
    ...
    for (std::size_t group = 0U; group < k128_groups; ++group) {
      a_scales[kernels::sm87_a4w4_consumer_k128_scale_offset(
          m, group, k128_groups)] =
          encode_bf16(static_cast<float>(1U + ((m + 2U * group) % 5U)) /
                      32.0F);
    }
  }
  for (std::size_t n = 0U; n < n_count; ++n) {
    ...
    for (std::size_t group = 0U; group < k128_groups; ++group) {
      b_scales[kernels::sm87_a4w4_consumer_k128_scale_offset(
          n, group, k128_groups)] =
          encode_bf16(static_cast<float>(1U + ((3U * n + group) % 7U)) /
                      64.0F);
    }
  }
  ...
  if (!device_a.allocate(a_bytes) || !device_b.allocate(b_bytes) ||
      ...
      !candidate_output.allocate(m_count * output_stride) ||
      !baseline_output.allocate(m_count * output_stride)) {
    std::cerr << "small Down K128 device allocation failed\n";
    return false;
  }
  if (!cuda_ok(cudaMemcpy(device_a.get(), packed_a.data(), a_bytes,
      ...
      !cuda_ok(cudaMemset(candidate_output.get(), 0x5a,
                          m_count * output_stride * sizeof(std::uint16_t)),
               "initialize small candidate guard") ||
      !cuda_ok(cudaMemset(baseline_output.get(), 0x5a,
                          m_count * output_stride * sizeof(std::uint16_t)),
               "initialize small baseline guard")) {
    return false;
  }

  const int short_capacity =
      kernels::launch_sm87_a4w4_down_k128_stage_major_bf16_cuda(
          device_a.get(), a_bytes - 1U, device_a_scales.get(),
          a_scales_count, device_b.get(), b_bytes, device_b_scales.get(),
          b_scales_count, m_count, n_count, k_count,
          candidate_output.get(), output_stride);
  if (short_capacity != static_cast<int>(cudaErrorInvalidValue)) {
    std::cerr << "Down K128 short A capacity did not fail closed\n";
    return false;
  }
  const int tail_shape =
      kernels::launch_sm87_a4w4_down_k128_stage_major_bf16_cuda(
  ...
  if (tail_shape != static_cast<int>(cudaErrorInvalidValue)) {
    std::cerr << "Down K128 M tail did not fail closed\n";
    return false;
  }
  ...
  if (!launch_ok(
          ...
          "launch small Down K128 stage-major") ||
      !launch_ok(kernels::launch_sm87_a4w4_prefill_gemm_k128_bf16_cuda(
                 ...
                 "launch existing small K128 baseline") ||
      !cuda_ok(cudaDeviceSynchronize(),
               "synchronize small Down K128 equivalence")) {
    return false;
  }

  std::vector<std::uint16_t> candidate(m_count * output_stride);
  std::vector<std::uint16_t> baseline(m_count * output_stride);
  if (!cuda_ok(cudaMemcpy(candidate.data(), candidate_output.get(),
                          candidate.size() * sizeof(candidate[0]),
                          cudaMemcpyDeviceToHost),
               "copy small candidate") ||
  ...
  for (std::size_t m = 0U; m < m_count; ++m) {
    for (std::size_t n = 0U; n < n_count; ++n) {
      ...
      if (candidate[offset] != baseline[offset] ||
        ...
        ++mismatches;
        if (mismatches <= 8U) {
          std::cerr << "small ordered mismatch m=" << m << " n=" << n
  ...
  if (mismatches != 0U) {
    std::cerr << "small ordered equivalence mismatches=" << mismatches
              << '\n';
    return false;
  }
  std::cout << "Down K128 stage-major bit-exact ordered equivalence passed: "
            << "M128 N256 K256\n";
  return true;
}

[[nodiscard]] bool run_large_real_layout_contract() {
  ...
  constexpr std::size_t b_scales_count =
      kernels::sm87_a4w4_consumer_k128_scale_capacity_elements(n_count,
                                                               k_count);
  constexpr std::size_t output_elements = m_count * output_stride;

  const std::uint16_t one = encode_bf16(1.0F);
  std::vector<std::uint16_t> a_scales(a_scales_count, one);
  std::vector<std::uint16_t> b_scales(b_scales_count, one);
  DeviceBuffer<std::uint8_t> device_a;
  DeviceBuffer<std::uint8_t> device_b;
  DeviceBuffer<std::uint16_t> device_a_scales;
  DeviceBuffer<std::uint16_t> device_b_scales;
  ...
  if (!device_a.allocate(a_bytes) || !device_b.allocate(b_bytes) ||
    ...
    std::cerr << "large Down K128 device allocation failed\n";
    return false;
  }
  // 0x11 is two signed +1 nibbles.  This exercises the complete real Down
  // consumer layout while keeping the exact result independently auditable.
  if (!cuda_ok(cudaMemset(device_a.get(), 0x11, a_bytes),
               "initialize large A layout") ||
      !cuda_ok(cudaMemset(device_b.get(), 0x11, b_bytes),
               "initialize large B layout") ||
      !cuda_ok(cudaMemcpy(device_a_scales.get(), a_scales.data(),
                          a_scales.size() * sizeof(a_scales[0]),
                          cudaMemcpyHostToDevice),
               "copy large A scales") ||
      ...
      !cuda_ok(cudaMemset(candidate_output.get(), 0x5a,
               ...
               "initialize large candidate guard") ||
      !cuda_ok(cudaMemset(baseline_output.get(), 0x5a,
                          output_elements * sizeof(std::uint16_t)),
               "initialize large baseline guard")) {
    return false;
  }
  ...
  if (!launch_ok(
          kernels::launch_sm87_a4w4_down_k128_stage_major_bf16_cuda(
              ...
              device_b_scales.get(), b_scales_count, m_count, n_count,
              k_count, candidate_output.get(), output_stride),
          "launch large real-layout stage-major") ||
      !launch_ok(kernels::launch_sm87_a4w4_prefill_gemm_k128_bf16_cuda(
                     device_a.get(), a_bytes, device_a_scales.get(),
                     a_scales_count, device_b.get(), b_bytes,
                     device_b_scales.get(), b_scales_count, m_count,
                     n_count, k_count, baseline_output.get(), output_stride),
                 "launch large existing K128 baseline") ||
      !cuda_ok(cudaDeviceSynchronize(),
               "synchronize large real-layout contract")) {
    return false;
  }
  ...
  const std::uint16_t expected = encode_bf16(static_cast<float>(k_count));
  std::size_t mismatches = 0U;
  for (std::size_t m = 0U; m < m_count; ++m) {
    for (std::size_t n = 0U; n < n_count; ++n) {
      const std::size_t offset = m * output_stride + n;
      if (candidate[offset] != baseline[offset] ||
          candidate[offset] != expected) {
        ++mismatches;
        if (mismatches <= 8U) {
          std::cerr << "large real-layout mismatch m=" << m
                    << " n=" << n
                    << " expected_bits=" << expected
                    << " candidate_bits=" << candidate[offset]
                    << " baseline_bits=" << baseline[offset] << '\n';
        }
      }
    }
    for (std::size_t n = n_count; n < output_stride; ++n) {
  ...
  if (mismatches != 0U) {
    std::cerr << "large real-layout mismatches=" << mismatches << '\n';
    return false;
  }
  std::cout << "Down K128 real-layout deterministic contract passed: M="
...
int main() {
  ...
  return run_resource_gate() && run_small_ordered_equivalence() &&
                 run_large_real_layout_contract()
             ? 0
             : 1;
}
