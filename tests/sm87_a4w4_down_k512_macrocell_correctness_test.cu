...
[[nodiscard]] bool resource_gate() {
  kernels::Sm87A4W4DownK512Resources resources{};
  const int status =
  ...
  const bool pass =
      ...
      resources.registers_per_thread <=
          static_cast<int>(kernels::kSm87A4W4DownK512MaximumRegisters) &&
  ...
  std::cout << "Down K512 resources: registers="
            << resources.registers_per_thread
...
[[nodiscard]] bool run_case(const std::size_t k_count) {
  ...
  // Two N128 cells exercise the one-CTA persistent loop and the shared-ring
  // release/re-fill boundary without pretending to be a performance shape.
  constexpr std::size_t n_count = 256U;
  constexpr std::size_t output_stride = n_count + 8U;
  const kernels::Sm87A4W4DownK512Plan plan =
      kernels::sm87_a4w4_down_k512_test_plan(m_count, n_count, k_count);
  if (plan.work_tiles != 2U) {
  ...
  if (plan.k512_groups > 1U && !observed_split_boundary) {
    std::cerr << "K" << k_count
              << ": fixture did not expose combined-vs-split K512 boundary\n";
    return false;
  }
  ...
  if (k_count == 512U) {
    const auto launch_invalid = [&](const std::uint8_t* a,
                                    const std::size_t a_capacity,
                                    const std::size_t m,
                                    const std::size_t output_capacity,
                                    std::uint16_t* output) {
      return kernels::launch_sm87_a4w4_down_k512_macrocell_test_bf16_cuda(
    ...
    const int short_a = launch_invalid(
        device_a.get(), a_bytes - 1U, m_count, output_count,
        device_output.get());
    const int unaligned = launch_invalid(
        device_a.get() + 1U, a_bytes - 1U, m_count, output_count,
        device_output.get());
    const int m_tail = launch_invalid(
        device_a.get(), a_bytes, m_count - 1U, output_count,
        device_output.get());
    const int short_output = launch_invalid(
        device_a.get(), a_bytes, m_count, output_count - 1U,
        device_output.get());
    const int alias_output = launch_invalid(
  ...
  if (launch_status != static_cast<int>(cudaSuccess) ||
    ...
    if (launch_status != static_cast<int>(cudaSuccess)) {
      std::cerr << "launch failed: "
                << cudaGetErrorName(static_cast<cudaError_t>(launch_status))
                << '\n';
    }
    return false;
  }
  ...
  std::cout << "Down K512 synthetic correctness passed: M=" << m_count
            << " N=" << n_count << " K=" << k_count << '\n';
  return true;
}
