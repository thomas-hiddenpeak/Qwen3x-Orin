...
[[nodiscard]] bool run_resource_gate() {
  kernels::Sm87A4W4DownCellV3Resources resources{};
  if (!launch_ok(
          kernels::query_sm87_a4w4_down_complete_cell_v3_resources_cuda(
              &resources),
          "query Down complete-cell v3 resources")) {
    return false;
  }
  const bool gate =
  ...
  std::cout << "Down complete-cell v3 resources: registers="
...
[[nodiscard]] bool run_shape(const std::size_t k_count) {
  ...
  if (k_count == 128U) {
    ...
    if (short_capacity != static_cast<int>(cudaErrorInvalidValue) ||
      ...
      std::cerr << "Down complete-cell v3 invalid calls did not fail closed\n";
      return false;
    }
  }
  ...
  std::cout << "Down complete-cell v3 bitwise ring case passed: M="
