...
constexpr char kAdmissionEnvironment[] =
...
constexpr std::array<std::size_t, 8U> kPromptTokenCounts{
    32U, 64U, 407U, 481U, 564U, 695U, 713U, 1'025U};
...
[[nodiscard]] bool run_case(runtime::ReferenceEngine& engine,
  ...
  if (unsetenv(kAdmissionEnvironment) != 0) {
    std::cerr << "failed to disable " << kAdmissionEnvironment << '\n';
    return false;
  }
  ...
  if (setenv(kAdmissionEnvironment, "1", 1) != 0) {
    std::cerr << "failed to enable " << kAdmissionEnvironment << '\n';
    return false;
  }
...
int main(const int argc, char** const argv) {
  ...
  const EnvironmentRestore restore_environment(kAdmissionEnvironment);
