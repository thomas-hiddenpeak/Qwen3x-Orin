...
// Four K128 groups exercise both ring slots and the steady-state recycle,
// while remaining a small synthetic correctness case rather than a timing
// surrogate for the real model.
inline constexpr std::size_t kK = 512U;
inline constexpr std::size_t kPhysicalGroups = kK / 64U;
inline constexpr std::size_t kK128Groups = kK / 128U;
...
inline constexpr std::size_t kAScaleElements =
    kernels::sm87_a4w4_consumer_k128_scale_capacity_elements(kM, kK);
...
inline constexpr std::size_t kBScaleElements =
    kernels::sm87_a4w4_consumer_k128_scale_capacity_elements(kN, kK);
    ...
    for (std::size_t group = 0U; group < kPhysicalGroups; ++group) {
      for (std::size_t byte = 0U; byte < 32U; ++byte) {
        ...
        result.a[kernels::sm87_a4w4_consumer_packed_offset(
            m, group, byte, kPhysicalGroups)] =
            kernels::sm87_a4w4_pack_signed_pair(even, odd);
      }
    }
    for (std::size_t group = 0U; group < kK128Groups; ++group) {
      result.a_scales[kernels::sm87_a4w4_consumer_k128_scale_offset(
          m, group, kK128Groups)] = encode_bf16(
...
[[nodiscard]] bool run_case() {
  ...
  if (!a.allocate(kAPackedBytes) || !a_scales.allocate(kAScaleElements) ||
      !gate.allocate(kBPackedBytes) ||
      !gate_scales.allocate(kBScaleElements) ||
      !up.allocate(kBPackedBytes) || !up_scales.allocate(kBScaleElements) ||
      !baseline.allocate(kOutputPackedBytes + kGuardBytes) ||
      !baseline_scales.allocate(kOutputScaleElements + kGuardScales) ||
      !candidate.allocate(kOutputPackedBytes + kGuardBytes) ||
      !candidate_scales.allocate(kOutputScaleElements + kGuardScales)) {
    std::cerr << "device allocation failed\n";
    return false;
  }
  if (!cuda_ok(cudaMemcpy(a.get(), payload.a.data(), kAPackedBytes,
                          cudaMemcpyHostToDevice), "copy A") ||
      !cuda_ok(cudaMemcpy(a_scales.get(), payload.a_scales.data(),
                          kAScaleElements * sizeof(std::uint16_t),
                          cudaMemcpyHostToDevice), "copy A scales") ||
      !cuda_ok(cudaMemcpy(gate.get(), payload.gate.data(), kBPackedBytes,
                          cudaMemcpyHostToDevice), "copy Gate") ||
      !cuda_ok(cudaMemcpy(gate_scales.get(), payload.gate_scales.data(),
                          kBScaleElements * sizeof(std::uint16_t),
                          cudaMemcpyHostToDevice), "copy Gate scales") ||
      !cuda_ok(cudaMemcpy(up.get(), payload.up.data(), kBPackedBytes,
  ...
  kernels::Sm87A4W4GateUpCellV2Resources resources{};
  if (!launch_ok(kernels::query_sm87_a4w4_gateup_cell_v2_resources_cuda(
                     &resources), "query candidate resources") ||
      resources.registers_per_thread > 128 ||
      resources.static_shared_bytes != 41'728U ||
  ...
  if (!launch_ok(kernels::launch_sm87_a4w4_gateup_paired_k128_cuda(
                     a.get(), kAPackedBytes, a_scales.get(), kAScaleElements,
                     gate.get(), kBPackedBytes, gate_scales.get(),
                     kBScaleElements, up.get(), kBPackedBytes,
                     up_scales.get(), kBScaleElements, kM, kN, kK,
                     kClipRatio, baseline.get(), kOutputPackedBytes,
                     baseline_scales.get(), kOutputScaleElements),
                 "launch established K128 baseline") ||
      !launch_ok(kernels::launch_sm87_a4w4_gateup_cell_v2_cuda(
                     a.get(), kAPackedBytes, a_scales.get(), kAScaleElements,
                     gate.get(), kBPackedBytes, gate_scales.get(),
                     kBScaleElements, up.get(), kBPackedBytes,
                     up_scales.get(), kBScaleElements, kM, kN, kK,
                     kClipRatio, candidate.get(), kOutputPackedBytes,
                     candidate_scales.get(), kOutputScaleElements),
                 "launch complete-cell v2") ||
      !cuda_ok(cudaDeviceSynchronize(), "synchronize kernels")) {
    return false;
  }

  std::vector<std::uint8_t> baseline_host(kOutputPackedBytes + kGuardBytes);
  std::vector<std::uint8_t> candidate_host(kOutputPackedBytes + kGuardBytes);
  ...
  for (std::size_t i = kOutputPackedBytes; i < baseline_host.size(); ++i) {
    if (baseline_host[i] != 0xa5U || candidate_host[i] != 0xa5U) {
      std::cerr << "packed-output guard modified at byte " << i << '\n';
      return false;
    }
  }
  for (std::size_t i = kOutputScaleElements;
       i < baseline_scale_host.size(); ++i) {
    if (baseline_scale_host[i] != 0xadadU ||
        candidate_scale_host[i] != 0xadadU) {
      std::cerr << "scale-output guard modified at element " << i << '\n';
      return false;
    }
  }
  if (baseline_host != candidate_host ||
    ...
    std::cerr << "bit mismatch: packed=" << code_mismatches
              << " scales=" << scale_mismatches << '\n';
    return false;
  }
  std::cout << "Gate+Up complete-cell v2 bit-exact: M=" << kM
            << " N=" << kN << " K=" << kK
