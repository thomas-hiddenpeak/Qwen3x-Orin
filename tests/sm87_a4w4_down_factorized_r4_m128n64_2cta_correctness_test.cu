...
void fill_scales(std::vector<std::uint16_t>& a_scales,
  ...
  for (std::size_t m = 0U; m < kLaunchM; ++m) {
    for (std::size_t lane = 0U; lane < 4U; ++lane) {
      a_scales[kernels::
          sm87_a4w4_down_factorized_r4_m128n64_scale_offset(m, lane)] =
  ...
  for (std::size_t n = 0U; n < kN; ++n) {
    for (std::size_t lane = 0U; lane < 4U; ++lane) {
      b_scales[kernels::
          sm87_a4w4_down_factorized_r4_m128n64_scale_offset(n, lane)] =
...
[[nodiscard]] bool run() {
  ...
  constexpr std::size_t b_scale_count = kernels::
      sm87_a4w4_down_factorized_r4_m128n64_scale_capacity_elements(kN);
  ...
  std::cout << "PASS: Down R4 M128N64 two-CTA release-shape correctness; "
               "P1853->P1920 tail, K-lane/stage boundaries, M/N owners, "
               "guards, nondefault stream, CUDA graph, launch negatives, "
               "and resource gates\n";
  return true;
}
