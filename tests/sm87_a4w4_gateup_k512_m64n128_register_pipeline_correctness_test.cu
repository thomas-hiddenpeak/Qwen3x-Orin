        ...
        for (std::size_t phase = 0U; phase < 8U; ++phase) {
          for (std::size_t lane = 0U; lane < 32U; ++lane) {
            ...
            const std::size_t gate_destination =
                kernels::sm87_a4w4_gateup_k512_projection_major_code_lane_offset(
            ...
            const std::size_t up_destination =
                kernels::sm87_a4w4_gateup_k512_projection_major_code_lane_offset(
                    fragment_n, group, phase,
                    kernels::kSm87A4W4GateUpK512ProjectionMajorUp,
                    lane, k512_groups);
            std::memcpy(result.projection_major_codes.data() +
                            gate_destination,
...
// It shares no projection-major layout, MMA ownership, fused edge, or
...
[[nodiscard]] bool capture_and_replay(
    ...
    GuardedDevice<std::uint8_t>& projection_major_codes,
  ...
  const int captured =
      kernels::
          launch_sm87_a4w4_gateup_k512_m64n128_register_pipeline_test_cuda(
              ...
              a_scales.payload_count(), projection_major_codes.payload(),
              projection_major_codes.payload_count(),
...
[[nodiscard]] bool run_case(const std::string& label,
  ...
  if (!a.initialize(payload.a, kByteGuard, kByteSentinel, label + " A") ||
      ...
      !projection_major_codes.initialize(
          payload.projection_major_codes, kByteGuard, kByteSentinel,
          label + " projection-major codes") ||
      ...
      [&](const std::size_t a_capacity,
        ...
        return kernels::
            launch_sm87_a4w4_gateup_k512_m64n128_register_pipeline_test_cuda(
                ...
                a_scale_capacity, projection_major_codes.payload(),
  ...
  if (!expect_invalid(
      ...
      !expect_invalid(
          launch_input_capacities(
              ...
              projection_major_codes.payload_count() - 1U,
              paired_scales.payload_count()),
          label + " short v3 code capacity") ||
      !expect_invalid(
          launch_input_capacities(
              a.payload_count(), a_scales.payload_count(),
              projection_major_codes.payload_count(),
      ...
      !expect_invalid(
          launch_candidate(projection_major_codes.payload_count(),
  ...
  if (graph_replay) {
    if (!candidate_packed.reset(stream.get()) ||
        ...
        !capture_and_replay(
            a, a_scales, projection_major_codes, paired_scales,
  ...
  if (!a.unchanged(label + " A") ||
      ...
      !projection_major_codes.unchanged(label + " v3 codes") ||
