...
int main() {
  TestContext test;
  test.expect(kernels::kSm87A4W4DownCellV2TileM == 128U &&
                  kernels::kSm87A4W4DownCellV2TileN == 128U &&
                  kernels::kSm87A4W4DownCellV2TileK == 128U,
              "Down complete cell owns M128N128K128");
  test.expect(kernels::kSm87A4W4DownCellV2Threads == 256U &&
                  kernels::kSm87A4W4DownCellV2PersistentCtas == 32U &&
                  kernels::kSm87A4W4DownCellV2CtasPerSm == 2U,
              "Down complete cell owns its two-CTA persistent launch");
  test.expect(kernels::kSm87A4W4DownCellV2AStages == 2U &&
                  kernels::kSm87A4W4DownCellV2BStages == 3U &&
                  kernels::kSm87A4W4DownCellV2SharedBytes == 42'240U &&
                  2U * kernels::kSm87A4W4DownCellV2SharedBytes <=
                      96U * 1'024U,
  ...
  constexpr auto plan = kernels::sm87_a4w4_down_complete_cell_v2_plan(
  ...
  const auto one = kernels::sm87_a4w4_down_complete_cell_v2_plan(
      128U, 128U, 128U);
  test.expect(one.work_tiles == 1U && one.k128_groups == 1U &&
                  one.launch_ctas == 1U,
              "one logical K128 group covers ring fill and drain");
  test.expect(kernels::sm87_a4w4_down_complete_cell_v2_plan(
                  127U, 128U, 128U)
                      .launch_ctas == 0U &&
                  kernels::sm87_a4w4_down_complete_cell_v2_plan(
                      128U, 64U, 128U)
                      .launch_ctas == 0U &&
                  kernels::sm87_a4w4_down_complete_cell_v2_plan(
                      128U, 128U, 64U)
  ...
  if (test.failures == 0) {
    std::cout << "SM87 Down complete-cell v2 host contract passed\n";
  }
