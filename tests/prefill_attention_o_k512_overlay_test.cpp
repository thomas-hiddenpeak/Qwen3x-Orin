...
[[nodiscard]] runtime::PrefillAttentionOK512BaseBinding make_base(
    const SyntheticSource& source) {
  runtime::PrefillAttentionOK512BaseBinding base;
...
void test_manifest(Test& test,
                   const runtime::PrefillAttentionOK512OverlayManifest& m) {
  test.expect(m.projections.size() == 64U &&
                  m.payload_bytes == 1'014'497'280ULL &&
                  m.physical_layout ==
                      runtime::kPrefillAttentionOK512OverlayLayout,
              "fixed 64-entry manifest and exact payload size");
  std::size_t linear = 0U;
  std::size_t full = 0U;
  for (std::size_t index = 0U; index < m.projections.size(); ++index) {
    const auto& entry = m.projections[index];
    linear += entry.family == "linear_o" ? 1U : 0U;
    full += entry.family == "full_o" ? 1U : 0U;
    test.expect(entry.layer_index == index && entry.ordinal == index &&
                    entry.sidecar_offset == index * 15'851'520ULL &&
                    entry.weight_bytes == 15'728'640ULL &&
                    entry.scale_bytes == 122'880ULL,
                "each O overlay entry has deterministic aligned byte layout");
  }
  test.expect(linear == 48U && full == 16U,
              "manifest has 48 linear and 16 full O projections");
  auto corrupted = m;
  ++corrupted.projections[3U].sidecar_offset;
  test.expect(
      !runtime::validate_prefill_attention_o_k512_overlay_manifest(corrupted),
...
void test_quantizer(Test& test) {
  ...
  for (std::size_t row = 0U; row < kRows; ++row) {
    for (std::size_t column = 0U; column < kColumns; ++column) {
      const float amplitude =
          static_cast<float>(1U + column / 128U) + row * 0.00390625F;
  ...
  const auto first = runtime::quantize_prefill_attention_o_k512_consumer_blocks(
      source.data(), kRows, kColumns, 0.75, packed.data(), packed.size(),
      scales.data(), scales.size());
  const auto second =
      runtime::quantize_prefill_attention_o_k512_consumer_blocks(
...
void test_policy_publication(
    Test& test,
    const runtime::PrefillAttentionOK512OverlayManifest& manifest) {
  const fs::path path = fs::temp_directory_path() /
                        ("q3x-attention-o-k512-policy-" +
                         std::to_string(::getpid()) + ".json");
  std::error_code ignored;
  fs::remove(path, ignored);
  const auto first =
      runtime::write_prefill_attention_o_k512_overlay_policy_template(
          manifest, path, 0.75, 1.0);
  test.expect(static_cast<bool>(first) &&
                  first.value->projections.size() == 64U &&
  ...
  const auto parsed = runtime::parse_prefill_attention_o_k512_overlay_policy(
      document, manifest);
  ...
  test.expect(!runtime::parse_prefill_attention_o_k512_overlay_policy(
                  corrupted, manifest),
...
void test_receipt_parser(
    ...
    const runtime::PrefillAttentionOK512OverlayManifest& manifest) {
  std::string document =
      "{\"schema\":\"q3x.prefill.attention-o-k512.publication-receipt\","
      "\"version\":{\"major\":1,\"minor\":0},"
      "\"mode\":\"production_calibrated\","
      "\"production_residency_eligible\":true,"
      "\"physical_layout\":\"sm87_s4_n64_packed_k64_scale_k512_attention_o_v1\","
      ...
      "\",\"payload_bytes\":1014497280,\"projection_count\":64}\n";
  runtime::PrefillAttentionOK512OverlayDiagnostic diagnostic;
  const auto receipt =
      runtime::parse_prefill_attention_o_k512_overlay_receipt(document,
                                                               diagnostic);
  ...
  test.expect(!runtime::parse_prefill_attention_o_k512_overlay_receipt(
                  document, diagnostic),
...
int main() {
  ...
  const auto built =
      runtime::build_qwen36_27b_prefill_attention_o_k512_overlay_manifest(
          source.manifest, source.shards, make_base(source));
  test.expect(static_cast<bool>(built),
              "synthetic pinned source builds K512 O overlay manifest");
  if (!built) {
  ...
  if (test.result() == 0) {
    std::cout << "Attention-O K512 overlay host contract passed\n";
  }
  return test.result();
}
