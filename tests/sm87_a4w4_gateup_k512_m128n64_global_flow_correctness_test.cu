...
[[nodiscard]] Payload make_payload(const std::size_t m_count,
  ...
  Payload result{
      ...
      std::vector<std::uint16_t>(
          kernels::sm87_a4w4_gateup_k512_m128n64_scale_capacity_elements(
              m_count, k_count)),
      std::vector<std::uint8_t>(
  ...
  for (std::size_t m = 0U; m < m_count; ++m) {
    ...
    for (std::size_t group = 0U; group < k512_groups; ++group) {
      result.a_scales[
          kernels::sm87_a4w4_gateup_k512_m128n64_scale_offset(
              m, group, k512_groups)] =
          encode_bf16(
              0.0021F *
...
// Independent scalar CUDA oracle.  It shares the public physical layout and
// K512 arithmetic boundary, but no candidate staging, warp mapping, or MMA.
__global__ void scalar_oracle(
  ...
  for (unsigned int group = 0U; group < k512_groups; ++group) {
    ...
    const float a_scale = decode_bf16_device(
        a_scales[kernels::sm87_a4w4_gateup_k512_m128n64_scale_offset(
            m, group, k512_groups)]);
...
[[nodiscard]] bool run_case(const std::string& label,
  ...
  const auto plan = kernels::sm87_a4w4_gateup_k512_m128n64_plan(
  ...
  if (!cuda_ok(cudaMemcpy(a.get(), payload.a.data(), payload.a.size(),
      ...
      !cuda_ok(cudaMemcpy(candidate.get(), initialized.data(),
                          ...
                          cudaMemcpyHostToDevice), "initialize candidate") ||
      !cuda_ok(cudaMemcpy(oracle.get(), initialized.data(),
                          initialized.size() * sizeof(std::uint16_t),
                          cudaMemcpyHostToDevice), "initialize oracle")) {
    return false;
  }

  const int short_capacity =
      kernels::launch_sm87_a4w4_gateup_k512_m128n64_test_bf16_cuda(
  ...
  if (short_capacity != static_cast<int>(cudaErrorInvalidValue)) {
    std::cerr << label << ": short full-weight capacity accepted\n";
    return false;
  }
  ...
  if (alias != static_cast<int>(cudaErrorInvalidValue)) {
    std::cerr << label << ": output/input alias accepted\n";
    return false;
  }
  ...
  if (!cuda_ok(cudaMemcpy(candidate_host.data(), candidate.get(),
                          ...
                          cudaMemcpyDeviceToHost), "copy candidate output") ||
      !cuda_ok(cudaMemcpy(oracle_host.data(), oracle.get(),
                          oracle_host.size() * sizeof(std::uint16_t),
                          cudaMemcpyDeviceToHost), "copy oracle output")) {
    return false;
  }
  ...
  std::cout << label << " bit-exact: M=" << m_count
            ...
            << " logical_cells=" << plan.work_cells
            << " cells_per_cta=" << plan.minimum_cells_per_cta << '/'
            << plan.maximum_cells_per_cta
            << " b_wave_bound=" << plan.b_wave_n_span_bound << '\n';
  return true;
}
...
int main() {
  ...
  constexpr auto real_plan =
      kernels::sm87_a4w4_gateup_k512_m128n64_plan(
          2'048U, 17'408U, 5'120U, 0U, 17'408U);
  static_assert(real_plan.m_tiles == 16U && real_plan.n_tiles == 136U &&
                real_plan.k512_groups == 10U &&
                real_plan.work_cells == 2'176U &&
                real_plan.launch_ctas == 16U);
  constexpr auto low_m_plan =
      kernels::sm87_a4w4_gateup_k512_m128n64_plan(
          128U, 17'408U, 5'120U, 0U, 17'408U);
  static_assert(low_m_plan.launch_ctas == 16U &&
                low_m_plan.minimum_cells_per_cta == 8U &&
                low_m_plan.maximum_cells_per_cta == 9U);
  constexpr auto p1920_plan =
      kernels::sm87_a4w4_gateup_k512_m128n64_plan(
          1'920U, 17'408U, 5'120U, 0U, 17'408U);
  static_assert(p1920_plan.m_tiles == 15U &&
                p1920_plan.work_cells == 2'040U &&
                p1920_plan.launch_ctas == 16U &&
                p1920_plan.minimum_cells_per_cta == 127U &&
                p1920_plan.maximum_cells_per_cta == 128U &&
                p1920_plan.b_wave_n_span_bound == 2U);
  constexpr auto p2176_plan =
      kernels::sm87_a4w4_gateup_k512_m128n64_plan(
          2'176U, 17'408U, 5'120U, 0U, 17'408U);
  static_assert(p2176_plan.m_tiles == 17U &&
                p2176_plan.work_cells == 2'312U &&
                p2176_plan.launch_ctas == 16U &&
                p2176_plan.minimum_cells_per_cta == 144U &&
                p2176_plan.maximum_cells_per_cta == 145U &&
                p2176_plan.b_wave_n_span_bound == 2U);

  kernels::Sm87A4W4GateUpK512M128N64Resources resources{};
  if (!launch_ok(
          kernels::query_sm87_a4w4_gateup_k512_m128n64_resources_cuda(
              &resources),
          "query M128N64 global-flow resources") ||
  ...
  if (!run_case("nonzero-N/two-phase", 128U, 256U, 128U, 128U,
                512U, 1U) ||
      !run_case("two-N-tile/two-scale", 128U, 384U, 128U, 256U,
                1'024U, 2U) ||
      !run_case("capped-work-cell-stride", 256U, 128U, 0U, 128U,
                512U, 1U) ||
      !run_case("natural-P1920-tail-balance", 1'920U, 256U, 0U,
                256U, 512U, 16U) ||
      !run_case("natural-P2176-tail-balance", 2'176U, 256U, 0U,
                256U, 512U, 16U)) {
    return 1;
  }
