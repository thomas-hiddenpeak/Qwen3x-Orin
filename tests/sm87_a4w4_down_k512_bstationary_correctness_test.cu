...
[[nodiscard]] int target_state() {
  ...
  if (properties.major != 8 || properties.minor != 7 ||
    ...
    std::cout << "SKIP: requires 16-SM SM87 with >=96 KiB shared\n";
    return 0;
  }
  return 1;
}
...
[[nodiscard]] bool resource_gate() {
  kernels::Sm87A4W4DownK512BStationaryResources resources{};
  const int status =
  ...
  const bool pass =
      ...
      resources.registers_per_thread <=
          static_cast<int>(
              kernels::kSm87A4W4DownK512BStationaryMaximumRegisters) &&
      resources.static_shared_bytes == 0U &&
      resources.dynamic_shared_bytes == 96U * 1'024U &&
      resources.configured_dynamic_shared_limit_bytes >= 96U * 1'024U &&
      resources.device_optin_shared_limit_bytes >= 96U * 1'024U &&
  ...
  std::cout << "Down K512 B-stationary resources: registers="
            << resources.registers_per_thread
...
[[nodiscard]] bool run_case(const std::size_t m_count,
                            const std::size_t n_count,
                            const std::size_t k_count,
                            const unsigned int maximum_launch_ctas,
                            const char* const label,
                            const bool validate_invalid_inputs) {
  // Synthetic values are used only for bitwise correctness.  Natural prompt
  // spans below preserve the real 15/17 M-tile scheduling boundary while N/K
  // may be reduced to keep exhaustive host reference work bounded.
  const std::size_t output_stride = n_count + 8U;
  const auto plan = kernels::sm87_a4w4_down_k512_bstationary_test_plan(
      m_count, n_count, k_count);
  if (plan.m_tiles !=
          m_count / kernels::kSm87A4W4DownK512BStationaryTileM ||
      plan.n_tiles !=
          n_count / kernels::kSm87A4W4DownK512BStationaryTileN ||
      plan.work_tiles != plan.m_tiles * plan.n_tiles) {
    std::cerr << label << ": invalid synthetic plan\n";
    return false;
  }
  ...
  const std::size_t b_bytes =
      kernels::sm87_a4w4_down_k512_packed_capacity_bytes(n_count, k_count);
  ...
  if (validate_invalid_inputs) {
    const int short_a =
        kernels::launch_sm87_a4w4_down_k512_bstationary_test_bf16_cuda(
            device_a.get(), a_bytes - 1U, device_a_scales.get(),
            a_scale_count, device_b.get(), b_bytes,
            device_b_scales.get(), b_scale_count, m_count, n_count,
            k_count, device_output.get(), output_stride, output_count,
            maximum_launch_ctas);
    const int m_tail =
        kernels::launch_sm87_a4w4_down_k512_bstationary_test_bf16_cuda(
            ...
            output_stride, output_count, maximum_launch_ctas);
    const int production_wrong_shape =
        kernels::launch_sm87_a4w4_down_k512_bstationary_bf16_cuda(
            device_a.get(), a_bytes, device_a_scales.get(), a_scale_count,
            device_b.get(), b_bytes, device_b_scales.get(), b_scale_count,
  ...
  for (std::size_t m = 0U; m < m_count; ++m) {
    for (std::size_t n = 0U; n < n_count; ++n) {
      ...
      if (actual[index] != expected[index]) {
        std::cerr << label << " bitwise mismatch at (" << m
    ...
    for (std::size_t n = n_count; n < output_stride; ++n) {
      if (actual[m * output_stride + n] != 0x7fc1U) {
        std::cerr << label << ": output padding overwritten\n";
        return false;
      }
    }
  }
  std::cout << "Down K512 B-stationary bitwise correctness passed: "
            << label << " M=" << m_count << " N=" << n_count
            << " K=" << k_count << '\n';
  return true;
}
...
int main() {
  ...
  if (!run_case(256U, 256U, 512U, 2U, "small-K512", true) ||
      !run_case(256U, 256U, 1'024U, 2U, "small-K1024", false) ||
      !run_case(1'920U, 128U, 512U, 15U, "natural-M1920", false) ||
      !run_case(2'176U, 128U, 512U, 16U, "natural-M2176", false)) {
    return 1;
  }
  return 0;
}
