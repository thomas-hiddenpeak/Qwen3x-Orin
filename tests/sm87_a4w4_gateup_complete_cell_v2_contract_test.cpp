...
[[nodiscard]] bool check_real_layout() {
  ...
  constexpr auto plan = kernels::sm87_a4w4_gateup_cell_v2_plan(m, n, k);
  ...
  return plan.m_tiles == 32U && plan.n_phases == 136U &&
         ...
         kernels::sm87_a4w4_consumer_k128_scale_offset(
             ...
             b_scales - 1U &&
         kernels::sm87_a4w4_consumer_k128_scale_offset(
             m - 1U, plan.n_phases - 1U, plan.n_phases) ==
             output_scales - 1U;
}

[[nodiscard]] bool check_rejections() {
  return kernels::sm87_a4w4_gateup_cell_v2_plan(65U, 128U, 256U)
                 .launch_ctas == 0U &&
         kernels::sm87_a4w4_gateup_cell_v2_plan(64U, 192U, 256U)
                 .launch_ctas == 0U &&
         kernels::sm87_a4w4_gateup_cell_v2_plan(64U, 128U, 192U)
                 .launch_ctas == 0U &&
         kernels::query_sm87_a4w4_gateup_cell_v2_resources_cuda(nullptr) ==
             static_cast<int>(cudaErrorInvalidValue) &&
         kernels::launch_sm87_a4w4_gateup_cell_v2_cuda(
...
[[nodiscard]] bool check_link_surface() {
  auto* volatile query =
      &kernels::query_sm87_a4w4_gateup_cell_v2_resources_cuda;
  auto* volatile launch = &kernels::launch_sm87_a4w4_gateup_cell_v2_cuda;
...
int main() {
  if (!check_real_layout()) {
    std::cerr << "real M2048/N17408/K5120 layout contract failed\n";
    return 1;
  }
  if (!check_rejections()) {
    std::cerr << "fail-closed contract failed\n";
    return 1;
  }
  if (!check_link_surface()) {
    std::cerr << "link surface contract failed\n";
    return 1;
  }
  std::cout << "Gate+Up complete-cell v2 real-shape layout contract passed\n";
  return 0;
}
