...
void fill_scales(std::vector<std::uint16_t>& a_scales,
  ...
  for (std::size_t m = 0U; m < kLaunchM; ++m) {
    for (std::size_t lane = 0U; lane < 4U; ++lane) {
      a_scales[kernels::
          sm87_a4w4_gateup_factorized_r4_m128n64_scale_offset(m, lane)] =
  ...
  for (std::size_t n = 0U; n < kN; ++n) {
    for (std::size_t lane = 0U; lane < 4U; ++lane) {
      const std::size_t offset = kernels::
          sm87_a4w4_gateup_factorized_r4_m128n64_scale_offset(n, lane);
      gate_scales[offset] =
...
[[nodiscard]] bool run() {
  ...
  constexpr std::size_t b_scale_count = kernels::
      sm87_a4w4_gateup_factorized_r4_m128n64_scale_capacity_elements(kN);
  constexpr std::size_t primary_count =
      kLaunchM * kernels::kSm87A4W4GateUpFactorizedR4M128N64PrimaryStride;
  constexpr std::size_t secondary_count =
      kLaunchM * kernels::kSm87A4W4GateUpFactorizedR4M128N64SecondaryStride;

  kernels::Sm87A4W4GateUpFactorizedR4M128N64Resources resources{};
  ...
  std::cout << "PASS: Gate+Up R4 M128N64 release-shape correctness; "
               "P1853->P1920 tail, N12288 seam, padding, guards, nondefault "
               "stream, launch negatives, and resource gates\n";
  return true;
}
