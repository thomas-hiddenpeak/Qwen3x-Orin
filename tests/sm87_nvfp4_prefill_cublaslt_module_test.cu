...
[[nodiscard]] bool expect_invalid_zero_node_capture(
    TestContext& test,
    q3x::kernels::Sm87Nvfp4PrefillCublasLtContext* const context,
  ...
  expect_invalid(q3x::kernels::launch_sm87_nvfp4_prefill_cublaslt_gate_c512(
  ...
  expect_invalid(q3x::kernels::launch_sm87_nvfp4_prefill_cublaslt_gate_c512(
                     context, packed, scales, kWeightScale2, activations,
                     kTokens, kRows, kColumns, scratch, kScratchBytes, scratch,
                     stream),
                 "scratch/output alias");
  expect_invalid(q3x::kernels::launch_sm87_nvfp4_prefill_cublaslt_gate_c512(
                     context, packed, scales, kWeightScale2, activations,
                     kTokens, kRows, kColumns,
                     const_cast<std::uint16_t*>(activations), kScratchBytes,
                     output, stream),
                 "activation/scratch alias");
...
int main() {
  ...
  if (properties.major != 8 || properties.minor != 7) {
    std::cout << "SKIP: exact module requires SM87\n";
    return 77;
  }

  q3x::kernels::Sm87Nvfp4PrefillCublasLtContext* context = nullptr;
  test.expect(
      q3x::kernels::create_sm87_nvfp4_prefill_cublaslt_context(nullptr) ==
          static_cast<int>(cudaErrorInvalidValue),
      "factory rejects null output pointer");
  const int create_status =
      q3x::kernels::create_sm87_nvfp4_prefill_cublaslt_context(&context);
  ...
  const int query_status =
      q3x::kernels::query_sm87_nvfp4_prefill_cublaslt_context(
  ...
  int ignored_rank = -1;
  test.expect(q3x::kernels::query_sm87_nvfp4_prefill_cublaslt_context(
  ...
  const int module_status =
      q3x::kernels::launch_sm87_nvfp4_prefill_cublaslt_gate_c512(
  ...
  test.expect(production_status == static_cast<int>(cudaSuccess),
              "launch production exact-C512 M128 path");
  ...
  ready = test.cuda_ok(cudaStreamSynchronize(stream),
          ...
          ready;
  q3x::kernels::destroy_sm87_nvfp4_prefill_cublaslt_context(context);
  q3x::kernels::destroy_sm87_nvfp4_prefill_cublaslt_context(nullptr);
  ready = test.cuda_ok(cudaStreamDestroy(stream), "destroy stream") && ready;

  std::cout << "NVFP4_PREFILL_CUBLASLT_MODULE: heuristic_rank="
