...
### Draft candidate: ordinary terminal-prefix liveness
...
adds a source-private generation adapter for SM87WeightOnly with Legacy-C512
...
path described in [the runner contract](REFERENCE_RUNNER.md#draft-candidate-private-terminal-prefix-execution).
...
same-ELF A/B control exists only with `BUILD_TESTING=ON`; the isolated OFF
candidate uses its compiled policy without an environment, CLI, or request
selector.
...
reset on the next request. This is a draft candidate contract, not P40000/O16
generation/API qualification or mainline promotion; the
