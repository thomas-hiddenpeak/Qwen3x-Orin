...
| `plan_and_receipts` | A default-off host plan declares C8000×5 traversal, two C8000×5120 hidden owners, one phase-aliased C8000×17408 scratch owner, recurrent owner identities, and panel/final events. A private startup foundation package regenerates this plan, authenticates the live 256-artifact/400-source target-AOT catalog, retains typed projection bindings, binds 96 live BF16 tensors as 48 canonical A/B pairs plus 128 live outer-norm vectors as 64 pairs, and mints non-caller-constructible V4-local resource seals. The first execution package copies the projection bindings by value and seals the complete BF16 A/B and norm catalogs once during construction; request execution does not repeat model/catalog/CUDA validation. It owns the exact 442,368,000-byte transient and 156,893,184-byte recurrent arenas. A narrow owner-held driver executes only layer-0 input norm on Main, records `NormReady`, waits on AbAux, executes BF16 A/B, and records `AbReady`. Since QKVZ is absent, Main does not wait on `AbReady`; Control joins both tails, physically observes `OwnerDrained`, and discards the unpublished request. PanelDone creates no host observation; final publication, safe discard, and exceptional drain remain the only physical receipt boundaries, and exceptional drain leaves the owner permanently poisoned. There is still no complete layer or whole-model receipt; the isolated receipt denies layer/panel/model/dispatch completion, `launcher_present=false`, no selector exists, and all CUDA constituents remain outside production dispatch. |
| `oracles` | The shared decoder covers every 16-bit packed word at unit scale and all 256 E4M3 codes on non-uniform words. Gate/Up and Down cover both canonical N128 halves plus M64/M37. BF16 A/B covers all two-by-48 outputs for patterned M64 compact versus direct-strided publication, every untouched scratch slot, immutable input/weights, exact caller/observed resource matching, managed/fake/offset allocation negatives, and one complete C8000 production-entry enqueue; old/V4 symbols are isolated in both build directions. Norm/residual covers C1 and special-value C65 bitwise comparison with the established exact bodies, residual and normalized plane ownership, input/weight/guard immutability, resource forgery, undersized and managed allocations, and both complete C8000 entry enqueues. The isolated live-CUDA composition test binds 96 BF16 projection tensors and 128 norm vectors, rejects a one-past norm before package construction, executes exactly one norm plus one A/B submission, observes the dual-tail `OwnerDrained` receipt, proves unpublished RequestState failure, and rejects repeat execution; its zero-valued synthetic weights are not real-checkpoint numerical evidence. FP8 covers every role/layout partition, both N128 halves, M64/M37, direct Full Q/G and private K/V scatter, interleaved-Q O gather, fixed `[4096,10240)` contiguous GDN-O reads, untouched scratch sentinels, resource forgery, layout mismatch, and undersized/foreign CUDA ranges over four K64 cells. Full-Attention preprocess covers C1/C65, first-position 0/16000, every Q/K element against an independent 256-thread tree, Gate/gap/prior/future-K immutability, multi-head NHD mapping, guards, allocation ranges, and resource/authority negatives. Fixed Attention covers C1/C65, first-position continuation, causal future poison, 6Q:1KV mapping, q==o versus disjoint output, Gate/gap immutability, complete-origin enforcement, resource/receipt negatives, and a nonzero patterned Q/K/V/Gate bitwise comparison against the public compact Q128-v4 body. Exact GDN covers C1, C65 across a C64 boundary, active/candidate state, pre-cancellation, resource tampering, and undersized/foreign CUDA ranges. Real-checkpoint BF16 A/B/full-K FP8, full-C8000 Attention/GDN, whole-state, mid-flight cancellation/rollback, logits/token, and external API oracles are absent. |
