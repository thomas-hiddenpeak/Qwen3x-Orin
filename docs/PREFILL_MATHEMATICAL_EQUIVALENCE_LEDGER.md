...
| `real_equivalence_proof` | Row-local projections/norms preserve their equations under a token partition. GDN carries the exact terminal state of panel `p` into `p+1`. Full Attention for a panel consumes every causally preceding committed K/V row plus the admitted current-panel prefix. These dependencies suffice over real numbers. A two-kernel GDN body instantiates the ordered recurrence for an independent panel admission, and a fixed C8000 unsplit Attention body instantiates the causal private-NHD traversal. One complete synthetic-T1 GDN layer-0 CUDA composition and one independent private Full Events T1 transaction now exist. The normal ExecutionPackage now derives that exact eight-step Full DAG from construction-sealed catalogs and owners in O(1), authenticates the enqueue receipt, and commits the matching move-only KV grant. No normal positive or real-checkpoint invocation has run, and the synthetic package proves only zero-enqueue rejection because it has no Full physical owner. Natural GDN layers 0--2 and the complete four-layer cohort, panel, request, and real-checkpoint compositions remain absent. |
...
| `state_transition_contract` | Two distinct 78,446,592-byte private convolution/GDN epochs are required. For each GDN layer, only its 61,440-byte convolution history copies active-to-candidate before in-place update; the recurrent matrix writes active-to-candidate directly. A whole-epoch pre-panel copy is forbidden. The candidate epoch must be fully assigned before the layer-63 swap and is discarded on failure. KV uses a private valid-end; canonical recurrent publication occurs after panel five and sequence length is the final non-fallible visibility fence. The synthetic-T1 GDN layer-0 executor proves candidate-grant commit followed by combined Events/RequestState physical drain and discard: normal closure preserves both bank identities/state epoch, and poison closure invalidates a pending grant. The Full Events transaction instead consumes one move-only KV grant bound to owner/request/panel/layer/allocation/offset/valid-end identity; it accepts that grant at most once, leaves the BF16 A/B cycle at its norm boundary, and permits RequestState to consume the same moved grant only after an authenticated eight-kernel enqueue. A replay or any injected accepted-prefix failure poisons, drains all three streams, and physically discards the pending grant. Neither T1 transaction proves a bank swap, private-KV publication, canonical publication, panel transition, or final sequence fence. |
| `plan_and_receipts` | A default-off host plan declares C8000×5 traversal, two C8000×5120 hidden owners, one phase-aliased C8000×17408 scratch owner, recurrent owner identities, and panel/final events. The private startup foundation authenticates the 256-artifact/400-source target-AOT catalog and construction-seals 48 complete GDN bindings (QKVZ, dedicated GDN-O, and four continuation weights), 64 Gate+Up/Down pairs, 48 BF16 A/B pairs, 64 input/post-norm pairs, and 16 natural Full bindings. Each Full binding covers Full-QKV, interleaved-Q Full-O, exact Q/K norm vectors, and the common four-resource observation bundle for Full-QKV, Full-O, in-place preprocess, and fixed C8000 Attention. The normal ExecutionPackage owns the exact 2,621,440,000-byte private KV arena and borrows the exact 67,108,864-byte shared Engine RoPE owner; together with transient/recurrent ownership and the 8,640,542,976-byte legacy arena, their staged reserve chain is exact. Normal startup/execution factories and the Engine lifecycle anchor are private and enforce `execution → startup → Engine RoPE → ModelWeights → complete target-AOT owner → resident`; they have only source/build authority because no pinned real-checkpoint Engine probe has executed them. The BUILD_TESTING-only GDN fixture executes layer 0 as `InputNorm → (QKVZ || A/B) → continuation(2 kernels + 1 asynchronous 61,440-byte D2D history copy) → GDN-O → ResidualPostNorm → GateUp → Down`, then drains and discards. The independent Full Events T1 fixture executes `InputNorm → FullQKV → preprocess → Attention → FullO → ResidualPostNorm → GateUp → Down` as exactly eight Main kernels and zero copies. Its semantic digest binds every typed argument, asset, resource geometry, and sealed identity; its receipt is request-owner authenticated, its grant is move-only/at-most-once, and its prefix ledger retains exactly 0--8 accepted kernels before failure. The normal package composer derives its pointers, aliases, stride, first position, assets, resources, and natural-layer identity from sealed state without a hot-path query, then commits the same grant only after the authenticated enqueue receipt. Its receipt denies physical, panel, model, and production completion. The physical 35-kernel/3-copy natural-layers-0--3 cohort, selector, and whole-model receipt do not exist. |
| `oracles` | The shared decoder covers every 16-bit packed word at unit scale and all 256 E4M3 codes on non-uniform words. Gate/Up and Down cover both canonical N128 halves plus M64/M37. BF16 A/B covers patterned compact-versus-strided publication, untouched scratch, immutable inputs/weights, resource negatives, and one complete C8000 enqueue. Norm/residual covers C1 and special-value C65 bitwise comparison, plane ownership, immutability, resource negatives, and both complete C8000 enqueues. FP8 covers every role/layout partition, direct role scatter/gather, untouched slots, and allocation/resource negatives. Full-Attention preprocess and fixed Attention retain their C1/C65, causal, layout, immutability, and bitwise constituent oracles. Exact GDN retains its C1/C65 recurrence, active/candidate, cancellation, and resource oracles. The GDN live-CUDA composition verifies nine submissions plus one 61,440-byte D2D copy, owner-mediated drain, unchanged banks/state epoch, repeat rejection, and poison invalidation without publication. The Full Events CUDA composition verifies exact 8/0 closure, semantic-digest rejection for pointer/asset/resource substitutions, foreign and stale authority rejection, at-most-once replay, every accepted prefix 0--8, unchanged A/B events/cycle state, and terminal three-stream RequestState discard. Live allocations with synthetic weights make both T1 scheduling/lifetime evidence, not real-checkpoint numerical evidence. Positive real-owner probe execution, complete-C8000 real-checkpoint GDN/Full equivalence, the physical first four-layer cohort, whole-state rollback, logits/token, and external API oracles are absent. |
| `quantitative_scope` | The plan caps transient activation storage at 442,368,000 bytes, declares 156,893,184 bytes for two recurrent epochs, and binds an exact 2,621,440,000-byte private KV arena: normal ExecutionPackage ownership is exactly 3,220,701,184 bytes. The shared Engine RoPE owner is exactly 67,108,864 bytes, so the anchored V4 total is 3,287,810,048 bytes and the future-allocation reserve including the existing 8,640,542,976-byte legacy arena is 11,928,353,024 bytes before the caller reserve. The isolated Full Events transaction is eight kernels/zero copies; the still-missing natural-layers-0--3 cohort is 35 kernels/three 61,440-byte copies. The transformation removes no model MAC and has no measured API effect. Xorg/GNOME/Mutter GPU-device handles blocked the clean-host window before either a performance run or the pending real-checkpoint Engine-lifetime probe, so no new timing or positive real-owner witness exists. |
...
The composer success boundary is enqueue plus RequestState grant commit only:
it does not drain, publish private KV or canonical state, or attest device,
panel, model, API, numerical, performance, release, or production completion.
Failure terminal-drains all three streams and discards the candidate. The only
package-level composer test so far is the synthetic zero-enqueue fail-closed
case; it is not a positive Full owner or real-checkpoint oracle. Complete-GDN
Events transaction hardening remains the next P0 before the physical
`GDN0 → GDN1 → GDN2 → Full3` cohort.
