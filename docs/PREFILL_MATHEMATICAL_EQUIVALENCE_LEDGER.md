...
| `oracles` | Constituent decoder, projection, norm, preprocess, Attention and GDN synthetic bit/resource oracles remain as previously declared. Complete-GDN Events tests cover authority, opaque receipt/owner match, replay, substitutions and accepted prefixes; Full Events now additionally covers strict normal-versus-Synthetic-T1 identities and O(1) five-by-16 replay. The joined CUDA oracle uses live synthetic allocations, complete KV/RoPE and natural shared hidden/scratch ownership. Success proves exact `GDN0 -> GDN1 -> GDN2 -> Full3` enqueue order, 35 kernels/three copies, four grant commits, no publication and one normal combined drain/discard. The minimum failure proves an exact 27/3 prefix, zero Full kernels, retained pending Full grant, poison-terminal invalidation and no publication. These are T1 correctness/dependency/scheduling/security/lifetime oracles only. Normal package runtime-positive execution, positive real-owner probe, real-checkpoint full-C8000 numerical equivalence, panel commit/rollback, logits/token and external API oracles are absent. |
...
the pending Full grant. The next P0 is the owner-only panel-commit bridge,
construction-sealed Embedding/final-norm/exact-LM-head-M1 request boundaries,
the fixed 64-layer/five-panel package transaction, cold rearm, final
`B -> A`/`FinalPublish`/sequence fence and proportional real-checkpoint gates;
