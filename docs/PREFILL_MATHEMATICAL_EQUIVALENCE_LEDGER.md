...
resource-seal API exists, and normal Execution construction now calls it once
after the four source-private resource queries. It retains the all-or-nothing
binding catalog, an independent catalog fold, exact 160,008-byte portable
pinned staging, and the fixed 507,144-byte scratch alias span. Synthetic-T1
keeps those owners and identities unbound. This proves construction ownership,
not leaf invocation or whole-request equivalence. `FinalRepresentationReady`
names only a diagnostic
...
| `real_equivalence_proof` | Row-local projections/norms preserve their equations under a token partition. GDN carries the exact terminal state of panel `p` into `p+1`. Full Attention for a panel consumes every causally preceding committed K/V row plus the admitted current-panel prefix. These dependencies suffice over real numbers. A two-kernel GDN body instantiates the ordered recurrence for an independent panel admission, and a fixed C8000 unsplit Attention body instantiates the causal private-NHD traversal. One same-request/same-panel Synthetic-T1 fixture now physically joins natural `GDN0 -> GDN1 -> GDN2 -> Full3`, reusing natural hidden parity/scratch and complete KV/RoPE allocations. It owner-authenticates and move-commits three GDN grants plus one Full grant as 35 kernels and three history copies before one combined drain/discard with no publication. The normal ExecutionPackage independently derives both the nine-kernel/one-copy GDN DAG and the eight-kernel Full DAG from construction-sealed catalogs and owners in O(1), but neither normal composer has a runtime-positive or real-checkpoint numerical invocation. The separate fixed schema-v5 real-checkpoint Engine construction probe does not execute either DAG or change the equivalence proof. The owner-only atomic rearm/panel-close lifecycle exists at a seeded-ledger state-machine boundary; the full five-panel request and real-checkpoint numerical composition remain absent. |
...
| `plan_and_receipts` | A default-off host plan declares C8000×5 traversal, two C8000×5120 hidden owners, one phase-aliased C8000×17408 scratch owner, recurrent owner identities, and panel/final events. The private startup foundation construction-seals 48 GDN, 64 MLP, 48 BF16 A/B, 64 norm, and 16 Full bindings. It now also seals one immutable request-boundary source catalog against the authenticated real Resident root, independently from the 256-entry projection catalog, covering Embedding, final norm, canonical LM-head plus both scalar provenances, and the greedy specification. Host-test authority cannot mint the normal execution catalog. Normal Execution construction performs all four source-private resource queries, consumes the all-or-nothing resource seal once, retains the binding catalog and independent fold, owns zero-initialized 160,008-byte `cudaHostAllocPortable` staging, and binds exact phase-exclusive token/finalizer aliases over the first 507,144 scratch bytes. Every construction rollback checks `cudaFreeHost`; the fixed schema-v5 real-checkpoint probe at `4f34c19` observed the stale registration become invalid after Engine destruction. The normal ExecutionPackage owns the exact 2,621,440,000-byte private KV arena and borrows the exact 67,108,864-byte shared Engine RoPE owner under the staged lifetime chain; that root now has bounded real-checkpoint construction/catalog/ownership observation while its strict source lifetime status remains `fail`. GDN and Full Events receipts are opaque, owner-authenticated, accepted-prefix preserving, and directly indexed in five-by-48 and five-by-16 replay ledgers. The normal Full outer commit receipt now cross-checks the normal domain, zero synthetic source, every package/Full/MLP/norm/RoPE/resource identity, request/grant/KV range and exact nested Events receipt, but it has no runtime-positive invocation. The joined Synthetic-T1 fixture uses complete KV/RoPE and physically executes the 35-kernel/three-copy four-layer cohort, then one physical completion/drain receipt and RequestState discard; it mints no panel, model, release, or production receipt. The selector, 64-by-5 package executor, request-boundary submission receipt, and whole-model receipt do not exist. |
| `oracles` | Constituent decoder, projection, norm, preprocess, Attention and GDN synthetic bit/resource oracles remain as previously declared. Complete-GDN Events tests cover authority, opaque receipt/owner match, replay, substitutions and accepted prefixes; Full Events additionally covers strict normal-versus-Synthetic-T1 identities and O(1) five-by-16 replay. The joined CUDA oracle uses live synthetic allocations, complete KV/RoPE and natural shared hidden/scratch ownership. Success proves exact `GDN0 -> GDN1 -> GDN2 -> Full3` enqueue order, 35 kernels/three copies, four grant commits, no publication and one normal combined drain/discard. The minimum failure proves an exact 27/3 prefix, zero Full kernels, retained pending Full grant, poison-terminal invalidation and no publication. Owner-lifecycle CUDA tests physically sample the runtime recurrent zero and verify fresh epoch/panel-0 atomic admission, exact-ledger panel commit/replay rejection, healthy terminal rearm, poison permanence, and unpublished final discard. Their 560-kernel panel ledger is deliberately seeded rather than physically executed, so it is state-machine evidence only. Static/schema contracts pin the four request-boundary query names and order, Startup seal, portable-pinned live query, checked rollback/free source, exact byte ledgers, and stale-address lifetime-probe semantics. The fixed real-checkpoint Engine probe now adds exact normal 48/64/16/1 catalog, KV/RoPE/boundary ownership, reserve-chain, destruction, and pinned-release observations; its source remains `fail` solely at memory recovery, and the post-exit resource preflight is not `no_owner_leak`. These are T0/T1 correctness/dependency/scheduling/security and bounded construction/lifetime observations. Normal package runtime-positive execution, real-checkpoint full-C8000 numerical equivalence, physical whole-panel/request execution, logits/token and external API oracles are absent. |
| `quantitative_scope` | The plan caps transient activation storage at 442,368,000 bytes, declares 156,893,184 bytes for two recurrent epochs, and binds an exact 2,621,440,000-byte private KV arena: normal ExecutionPackage device ownership is exactly 3,220,701,184 bytes. Exact 160,008-byte portable pinned staging raises total package ownership to 3,220,861,192 bytes without changing the device reserve. The shared Engine RoPE owner is exactly 67,108,864 bytes, so the anchored device ledger remains 3,287,810,048 bytes and total anchored device-plus-host ownership is 3,287,970,056 bytes; the future device-allocation reserve including the existing 8,640,542,976-byte legacy arena remains 11,928,353,024 bytes before the caller reserve. One secure GDN transaction is nine kernels plus one 61,440-byte copy; Full is eight kernels/zero copies. Joined success is 35 kernels plus three copies totaling 184,320 bytes; minimum failure is 27 kernels plus the same three copies and a pending Full-grant invalidation. The fixed schema-v5 probe matched these ownership ledgers but ended with a 419,917,824-byte free-memory gap against a 33,554,432-byte tolerance, so its source status is `fail`. The transformation removes no model MAC and has no measured API effect. No real-checkpoint numerical result or timing exists. |
...
Construction-lifetime addendum effective 2026-08-20: the fixed schema-v5
probe at `4f34c19` created, validated, and destroyed the pinned-real-checkpoint
Engine; matched the normal
48-complete-GDN, 64-MLP-pair, 16-Full-Attention, and one request-boundary
catalogs plus exact KV/RoPE/boundary ownership and reserve ledgers; and observed
the 160,008-byte pinned staging owner released. Its source status remains
`fail` solely because the 419,917,824-byte post-destruction free-memory gap
exceeds the 33,554,432-byte tolerance. The passing root post-exit resource
preflight is not a Jetson `nvmap` `no_owner_leak` classification and does not
rewrite the source result. All other absence and authority statements in those
rows remain current: neither normal composer nor boundary leaf ran, full-C8000
and whole-request numerical oracles remain absent, and the observation grants
no numerical, generation, API, timing, performance, release, or production
authority. Exact evidence is frozen in the
...
normal Execution resource-seal API is now consumed exactly once during normal
package construction, with the private catalog, portable pinned staging, and
phase aliases retained under the Engine lifetime root. Thus construction
ownership exists, but no leaf invocation, submission receipt, or request edge
exists, and the addendum has no whole-panel/request,
...
no runtime-positive normal owner or real-checkpoint numerical oracle has run.
Full Events
now strictly separates normal and Synthetic-T1 authority and uses an O(1)
five-by-16 replay ledger. Events T1 physically joins
`GDN0 -> GDN1 -> GDN2 -> Full3` as 35 kernels plus three copies on one
request/panel, complete KV/RoPE and one normal combined drain/discard, while
...
boundary. Owner direction on 2026-08-20 pauses Prefill performance work at the
construction checkpoint on `archive/v4-construction-ownership-20260820`.
When explicitly resumed, the next P0 is the fixed 64-layer/five-panel package
transaction and rollback using the already-sealed boundary owners, followed by
request-boundary leaf invocation and final
...
`PrefillStateCommitted` while excluding LM-head/argmax/D2H. The frozen strict
lifetime recovery failure must be preserved and reconciled if formal lifecycle
acceptance is needed; the remaining proportional real-checkpoint numerical
gates follow. No synthetic timing or normal-prefix probe precedes that
composition. `FinalRepresentationReady` remains a diagnostic
