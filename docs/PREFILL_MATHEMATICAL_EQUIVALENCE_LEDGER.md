...
| `plan_and_receipts` | A default-off host plan declares C8000×5 traversal, two C8000×5120 hidden owners, one phase-aliased C8000×17408 scratch owner, recurrent owner identities, and panel/final events. A private startup foundation package regenerates this plan, authenticates the live 256-artifact/400-source target-AOT catalog, retains typed projection bindings, binds 96 live BF16 tensors as 48 canonical A/B pairs plus 128 live outer-norm vectors as 64 pairs, and mints non-caller-constructible V4-local resource seals. Its construction-only GDN-QKVZ seam requires all 48 natural bindings and exact current CUDA allocation/upload-receipt agreement; the fake host catalog fails at ordinal zero. The normal startup and execution factories are private, and the named synthetic-T1 wrapper is absent from the core static library. The private synthetic branch remains in the BUILD_TESTING-only archive solely for its CUDA fixture and grants no production authority. An Engine-private composition root now calls only those normal factories under the existing default-off V3 real-owner harness, requires all 48 QKVZ bindings plus a nonzero catalog identity, and enforces `execution → startup → ModelWeights → complete target-AOT owner → resident` teardown. Its retained-memory admission adds the exact 442,368,000-byte transient and 156,893,184-byte recurrent arenas, or 599,261,184 bytes. This path has not executed in a pinned real-checkpoint Engine probe, so the positive real-owner 48-binding witness remains open. The first execution package copies the projection bindings by value and seals the complete BF16 A/B and norm catalogs once during construction; request execution does not repeat model/catalog/CUDA validation. A narrow owner-held driver executes layer-0 input norm on Main and records `NormReady`; AbAux waits, executes BF16 A/B into `[16384,16480)`, and records `AbReady`; Main independently executes fixed GDN-QKVZ into `[0,16384)` and immediately waits on that `AbReady` generation. Raw ready-event records and caller-filled QKVZ submission are outside the driver surface. Control joins both tails, physically observes `OwnerDrained`, and discards the unpublished request. PanelDone creates no host observation; final publication, safe discard, and exceptional drain remain the only physical receipt boundaries, and exceptional drain leaves the owner permanently poisoned. There is still no complete layer or whole-model receipt; the isolated receipt records its synthetic-T1 source/catalog provenance and denies layer/panel/model/dispatch completion, `launcher_present=false`, no selector exists, and all CUDA constituents remain outside production dispatch. |
| `oracles` | The shared decoder covers every 16-bit packed word at unit scale and all 256 E4M3 codes on non-uniform words. Gate/Up and Down cover both canonical N128 halves plus M64/M37. BF16 A/B covers all two-by-48 outputs for patterned M64 compact versus direct-strided publication, every untouched scratch slot, immutable input/weights, exact caller/observed resource matching, managed/fake/offset allocation negatives, and one complete C8000 production-entry enqueue; old/V4 symbols are isolated in both build directions. Norm/residual covers C1 and special-value C65 bitwise comparison with the established exact bodies, residual and normalized plane ownership, input/weight/guard immutability, resource forgery, undersized and managed allocations, and both complete C8000 entry enqueues. The isolated live-CUDA composition test binds 96 BF16 projection tensors and 128 norm vectors, rejects a one-past norm before package construction, proves the normal factory rejects the fake QKVZ catalog before execution allocation, then uses a live 83,886,080-byte synthetic-T1 FP8 allocation to execute exactly one norm, one fixed QKVZ, and one A/B submission. It observes Main waiting on `AbReady`, verifies every row of `[16480,17408)` remains untouched, closes the dual-tail `OwnerDrained` receipt, proves unpublished RequestState failure, and rejects repeat execution. Its synthetic zero-valued weights are not real-checkpoint numerical evidence. FP8 covers every role/layout partition, both N128 halves, M64/M37, direct Full Q/G and private K/V scatter, interleaved-Q O gather, fixed `[4096,10240)` contiguous GDN-O reads, untouched scratch sentinels, resource forgery, layout mismatch, and undersized/foreign CUDA ranges over four K64 cells. Full-Attention preprocess covers C1/C65, first-position 0/16000, every Q/K element against an independent 256-thread tree, Gate/gap/prior/future-K immutability, multi-head NHD mapping, guards, allocation ranges, and resource/authority negatives. Fixed Attention covers C1/C65, first-position continuation, causal future poison, 6Q:1KV mapping, q==o versus disjoint output, Gate/gap immutability, complete-origin enforcement, resource/receipt negatives, and a nonzero patterned Q/K/V/Gate bitwise comparison against the public compact Q128-v4 body. Exact GDN covers C1, C65 across a C64 boundary, active/candidate state, pre-cancellation, resource tampering, and undersized/foreign CUDA ranges. Positive real-owner 48-GDN-QKVZ catalog execution, real-checkpoint BF16 A/B/full-K FP8, full-C8000 Attention/GDN, whole-state, mid-flight cancellation/rollback, logits/token, and external API oracles are absent. |
| `quantitative_scope` | The plan caps transient activation storage at 442,368,000 bytes and declares 156,893,184 bytes for two recurrent epochs. The Engine harness now reserves their exact 599,261,184-byte sum in addition to its existing retained ownership. It removes no model MAC and has no measured API effect. |
