...
| `state_transition_contract` | Two distinct 78,446,592-byte private convolution/GDN epochs are required. For each GDN layer, only its 61,440-byte convolution history copies active-to-candidate before in-place update; the recurrent matrix writes active-to-candidate directly. A whole-epoch pre-panel copy is forbidden. The candidate epoch must be fully assigned before the layer-63 swap and is discarded on failure. KV uses a private valid-end; canonical recurrent publication occurs after panel five and sequence length is the final non-fallible visibility fence. The synthetic-T1 layer-0 executor now proves candidate-grant commit followed by combined Events/RequestState physical drain and discard: normal closure preserves both bank identities/state epoch, and poison closure invalidates a pending grant. It deliberately proves no bank swap, canonical publication, panel transition, or final sequence fence. |
| `plan_and_receipts` | A default-off host plan declares C8000×5 traversal, two C8000×5120 hidden owners, one phase-aliased C8000×17408 scratch owner, recurrent owner identities, and panel/final events. The private startup foundation authenticates the 256-artifact/400-source target-AOT catalog and construction-seals 48 complete GDN bindings (QKVZ, dedicated GDN-O, and four continuation weights), 64 Gate+Up/Down pairs, 48 BF16 A/B pairs, and 64 input/post-norm pairs. Normal startup/execution factories and the Engine lifecycle anchor are private and enforce `execution → startup → ModelWeights → complete target-AOT owner → resident`; they have only construction/build evidence because no pinned real-checkpoint Engine probe has executed them. The BUILD_TESTING-only synthetic-T1 fixture executes layer 0 as `InputNorm → (QKVZ || A/B) → continuation(2 kernels + 1 asynchronous 61,440-byte D2D history copy) → GDN-O → ResidualPostNorm → GateUp → Down`. Its authenticated layer receipt counts exactly nine kernels plus one copy, records the candidate grant, and discards without changing banks/state epoch or publishing canonical/Decode state. The Events owner mediates the physical drain identity, RequestState consumes it, and the complete receipt requires matching `physical_owner_drain_receipt_identity` plus `physical_execution_receipt_issued=true`. A separate poison fixture proves a pending grant becomes invalid only after terminal physical quiescence. The receipt grants `layer_complete=true` but denies panel/model/dispatch completion; no selector or whole-model receipt exists. |
| `oracles` | The shared decoder covers every 16-bit packed word at unit scale and all 256 E4M3 codes on non-uniform words. Gate/Up and Down cover both canonical N128 halves plus M64/M37. BF16 A/B covers patterned compact-versus-strided publication, untouched scratch, immutable inputs/weights, resource negatives, and one complete C8000 enqueue. Norm/residual covers C1 and special-value C65 bitwise comparison, plane ownership, immutability, resource negatives, and both complete C8000 enqueues. FP8 covers every role/layout partition, direct role scatter/gather, untouched slots, and allocation/resource negatives. Full-Attention preprocess and fixed Attention retain their C1/C65, causal, layout, immutability, and bitwise constituent oracles. Exact GDN retains its C1/C65 recurrence, active/candidate, cancellation, and resource oracles. The live-CUDA synthetic-T1 composition additionally executes the complete layer-0 sequence, verifies exactly nine kernel submissions plus one 61,440-byte D2D copy, authenticates the owner-mediated physical receipt against RequestState, preserves active/candidate bank identities and state epoch, rejects repeat execution, and proves poison-drain invalidation of a pending grant without publication. Live allocations with zero-valued synthetic weights and a synthetic source identity make this scheduling/lifetime evidence, not real-checkpoint numerical evidence. Positive real-owner catalog execution, real-checkpoint BF16 A/B/full-K FP8/full-C8000 GDN-layer equivalence, complete Attention layer, whole-state rollback, logits/token, and external API oracles are absent. |
| `quantitative_scope` | The plan caps transient activation storage at 442,368,000 bytes and declares 156,893,184 bytes for two recurrent epochs. The Engine harness now reserves their exact 599,261,184-byte sum in addition to its existing retained ownership. It removes no model MAC and has no measured API effect. The latest measurement admission failed closed on Xorg/GNOME/Mutter GPU-device handles before any timed work, so it contributes no performance number. |
