...
| `real_equivalence_proof` | Row-local projections/norms preserve their equations under a token partition. GDN carries the exact terminal state of panel `p` into `p+1`. Full Attention for a panel consumes every causally preceding committed K/V row plus the admitted current-panel prefix. These dependencies suffice over real numbers. A two-kernel GDN body instantiates the ordered recurrence for an independent panel admission, and a fixed C8000 unsplit Attention body instantiates the causal private-NHD traversal. The old package fixture executes one complete Synthetic-T1 GDN layer 0 and terminally drains it. The secure Events T1 transaction now executes natural GDN ordinals 0, 1, and 2 on one request/panel as 27 kernels plus three history copies, commits three fresh grants, and reaches the Full3 authorization boundary; it does not submit Full3. An independent private Full Events T1 transaction still exists. The normal ExecutionPackage now derives both the nine-kernel/one-copy GDN DAG and the eight-kernel Full DAG from construction-sealed catalogs and owners in O(1), authenticates the enqueue receipt, and commits the matching move-only state grant without a success drain or publication. No positive normal-package or real-checkpoint invocation has run. The physical four-layer cohort, panel, request, and real-checkpoint compositions remain absent. |
...
| `state_transition_contract` | Two distinct 78,446,592-byte private convolution/GDN epochs are required. For each GDN layer, only its 61,440-byte convolution history copies active-to-candidate before in-place update; the recurrent matrix writes active-to-candidate directly. A whole-epoch pre-panel copy is forbidden. The candidate epoch must be fully assigned before the layer-63 swap and is discarded on failure. KV uses a private valid-end; canonical recurrent publication occurs after panel five and sequence length is the final non-fallible visibility fence. Complete-GDN Events now consumes one move-only grant bound to owner/request/panel/natural layer/recurrent allocation/banks/slices and admits it through one direct O(1) `panel * 48 + ordinal` slot in a five-by-48 ledger. The slot is reserved before the first enqueue. Its opaque authenticated receipt and owner second matcher permit RequestState to consume that same moved grant only after the exact nine-kernel/one-copy prefix is accepted. A success records candidate-enqueued state only; it does not drain, swap banks, complete a panel/model, or publish. A replay or any accepted-prefix failure poisons, drains all three streams, and physically discards the pending candidate while retaining the exact partial ledger. The old package one-shot is explicitly Synthetic-T1-only and always terminally drains/discards. The Full Events transaction retains the analogous move-only KV-grant, authenticated eight-kernel, at-most-once boundary. Neither T1 route proves private-KV or canonical publication, a panel transition, or the final sequence fence. |
| `plan_and_receipts` | A default-off host plan declares C8000×5 traversal, two C8000×5120 hidden owners, one phase-aliased C8000×17408 scratch owner, recurrent owner identities, and panel/final events. The private startup foundation authenticates the 256-artifact/400-source target-AOT catalog and construction-seals 48 complete GDN bindings (QKVZ, dedicated GDN-O, and four continuation weights), 64 Gate+Up/Down pairs, 48 BF16 A/B pairs, 64 input/post-norm pairs, and 16 natural Full bindings. Each Full binding covers Full-QKV, interleaved-Q Full-O, exact Q/K norm vectors, and the common four-resource observation bundle for Full-QKV, Full-O, in-place preprocess, and fixed C8000 Attention. The normal ExecutionPackage owns the exact 2,621,440,000-byte private KV arena and borrows the exact 67,108,864-byte shared Engine RoPE owner; together with transient/recurrent ownership and the 8,640,542,976-byte legacy arena, their staged reserve chain is exact. Normal startup/execution factories and the Engine lifecycle anchor are private and enforce `execution → startup → Engine RoPE → ModelWeights → complete target-AOT owner → resident`; they have only source/build authority because no pinned real-checkpoint Engine probe has executed them. Complete-GDN Events separates normal sealed-catalog and Synthetic-T1 authority, binds grant plus package/GDN/BF16-A/B/norm/MLP/resource identities, fieldwise-digests every typed argument and seven resource snapshots, returns an opaque owner-authenticated nine-kernel/one-copy receipt, and retains every reachable accepted prefix. Its five-by-48 replay ledger is directly indexed. The normal reusable GDN composer derives natural layer, hidden parity, pointers, recurrent slices, assets, and resources from the sealed catalogs in O(1), owner-matches the receipt, then move-commits the same grant; success does not drain or publish. The old BUILD_TESTING one-shot accepts only Synthetic-T1 and terminally drains/discards. The independent Full Events T1 fixture executes `InputNorm → FullQKV → preprocess → Attention → FullO → ResidualPostNorm → GateUp → Down` as exactly eight Main kernels and zero copies, and its normal composer retains the analogous O(1), authenticated enqueue-plus-move-commit boundary. Neither normal composer has a positive package invocation. The physical 35-kernel/3-copy natural-layers-0--3 cohort, selector, and whole-model receipt do not exist. |
| `oracles` | The shared decoder covers every 16-bit packed word at unit scale and all 256 E4M3 codes on non-uniform words. Gate/Up and Down cover both canonical N128 halves plus M64/M37. BF16 A/B covers patterned compact-versus-strided publication, untouched scratch, immutable inputs/weights, resource negatives, and one complete C8000 enqueue. Norm/residual covers C1 and special-value C65 bitwise comparison, plane ownership, immutability, resource negatives, and both complete C8000 enqueues. FP8 covers every role/layout partition, direct role scatter/gather, untouched slots, and allocation/resource negatives. Full-Attention preprocess and fixed Attention retain their C1/C65, causal, layout, immutability, and bitwise constituent oracles. Exact GDN retains its C1/C65 recurrence, active/candidate, cancellation, and resource oracles. Complete-GDN Events CUDA tests cover normal-versus-Synthetic-T1 authority rules, opaque receipt construction, owner second match, replay with zero extra work, foreign/stale grants, identity/pointer/resource substitution, and every reachable accepted prefix including the history copy and partial continuation before terminal drain/discard. One same-request/same-panel Synthetic-T1 sequence commits GDN ordinals 0, 1, and 2 as exactly 27 kernels plus three copies, restores the A/B cycle after each, and reaches Full3 authorization without executing Full3. The old package one-shot still proves one terminally drained Synthetic-T1 layer. The Full Events CUDA composition independently verifies exact 8/0 closure, semantic-digest substitution rejection, foreign/stale authority, at-most-once replay, prefixes 0--8, unchanged A/B state, and terminal discard. Live allocations with synthetic weights make these scheduling/security/lifetime evidence, not a normal package positive or real-checkpoint numerical evidence. Positive normal GDN/Full package composition, positive real-owner probe execution, complete-C8000 real-checkpoint GDN/Full equivalence, the physical first four-layer cohort, whole-state rollback, logits/token, and external API oracles are absent. |
| `quantitative_scope` | The plan caps transient activation storage at 442,368,000 bytes, declares 156,893,184 bytes for two recurrent epochs, and binds an exact 2,621,440,000-byte private KV arena: normal ExecutionPackage ownership is exactly 3,220,701,184 bytes. The shared Engine RoPE owner is exactly 67,108,864 bytes, so the anchored V4 total is 3,287,810,048 bytes and the future-allocation reserve including the existing 8,640,542,976-byte legacy arena is 11,928,353,024 bytes before the caller reserve. One secure GDN transaction is nine kernels plus one 61,440-byte copy; the same-panel Events T1 sequence is 27 kernels plus three such copies and stops at Full3 authorization. The isolated Full Events transaction is eight kernels/zero copies. The still-missing physical natural-layers-0--3 cohort is 35 kernels/three copies. The transformation removes no model MAC and has no measured API effect. Xorg/GNOME/Mutter GPU-device handles blocked the clean-host window before either a performance run or the pending real-checkpoint Engine-lifetime probe, so no new timing or positive real-owner witness exists. |
...
or production completion. GDN success is exactly nine kernels plus one
61,440-byte D2D history copy; Full success is eight kernels and zero copies.
Failure terminal-drains all three streams and discards the candidate. The
secure GDN Events transaction and reusable normal composer are implemented,
but package-level composer coverage remains negative Synthetic-T1 rejection;
no positive normal owner or real-checkpoint oracle has run. Events T1 proves
GDN0--2 as 27 kernels plus three copies and reaches Full3 authorization only.
The next P0 is a positive normal-package GDN composition, followed by the
physical `GDN0 → GDN1 → GDN2 → Full3` 35-kernel/three-copy cohort.
