...
alignment gate 时退回两个有序 M8 launch。M17..M32 先发出 M16，再以 M8 和 M1..M7
处理 tail，因此 M18 为 M16+M2、M32 为 M16+M16。reference backend 和 BF16 weight
保留逐行 M1 fallback，因此接口语义不依赖 optimized backend。outer C17..C32 tile 中的
