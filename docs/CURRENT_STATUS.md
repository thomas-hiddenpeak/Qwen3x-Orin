...
unroll; its only redundancy is the 6x GQA key read, which is L2-absorbed and
whose merge was already rejected for occupancy collapse. The bit-exact local
optimization route is therefore exhausted, and the remaining gap is dominated
by the weight-read floor.
