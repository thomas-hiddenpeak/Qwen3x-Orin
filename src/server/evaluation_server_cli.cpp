...
bool parse_evaluation_server_arguments(
  ...
  for (int index = 2; index < argc; ++index) {
    ...
    if (argument == "--host") {
      ...
      if (!options
               ...
               .empty() ||
          value.empty()) {
        error = "--prefill-mlp-k512-projection-major-gateup-canonical-down-"
                "payload requires one non-empty FILE";
        return false;
      }
      options
          .prefill_mlp_k512_projection_major_gateup_canonical_down_payload_path =
      ...
      if (!options
          ...
          value.empty()) {
        error = "--prefill-mlp-k512-projection-major-gateup-canonical-down-"
                "policy requires one non-empty FILE";
        return false;
      }
      options
          .prefill_mlp_k512_projection_major_gateup_canonical_down_policy_path =
      ...
      if (!options
          ...
          value.empty()) {
        error = "--prefill-mlp-k512-projection-major-gateup-canonical-down-"
                "receipt requires one non-empty FILE";
        return false;
      }
      options
          .prefill_mlp_k512_projection_major_gateup_canonical_down_receipt_path =
      ...
      if (!options.prefill_mlp_factorized_lane_r4_payload_path.empty() ||
          value.empty()) {
        error = "--prefill-mlp-factorized-lane-r4-payload requires one "
                "non-empty FILE";
        return false;
      }
      options.prefill_mlp_factorized_lane_r4_payload_path = value;
    } else if (argument ==
               "--prefill-mlp-factorized-lane-r4-policy") {
      if (!options.prefill_mlp_factorized_lane_r4_policy_path.empty() ||
          value.empty()) {
        error = "--prefill-mlp-factorized-lane-r4-policy requires one "
                "non-empty FILE";
        return false;
      }
      options.prefill_mlp_factorized_lane_r4_policy_path = value;
    } else if (argument ==
               "--prefill-mlp-factorized-lane-r4-receipt") {
      if (!options.prefill_mlp_factorized_lane_r4_receipt_path.empty() ||
          value.empty()) {
        error = "--prefill-mlp-factorized-lane-r4-receipt requires one "
                "non-empty FILE";
        return false;
      }
      options.prefill_mlp_factorized_lane_r4_receipt_path = value;
  ...
  if (mlp_k512_projection_major_payload !=
    ...
    error = "--prefill-mlp-k512-projection-major-gateup-canonical-down-"
            "payload, --prefill-mlp-k512-projection-major-gateup-canonical-"
            "down-policy, and --prefill-mlp-k512-projection-major-gateup-"
            "canonical-down-receipt are required together";
    return false;
  }
  if (mlp_k512_projection_major_payload && !a4_payload) {
    error = "the projection-major-GateUp/canonical-Down K512 MLP overlay "
  ...
  if (mlp_factorized_r4_payload != mlp_factorized_r4_policy ||
    ...
    error = "--prefill-mlp-factorized-lane-r4-payload, "
            "--prefill-mlp-factorized-lane-r4-policy, and "
            "--prefill-mlp-factorized-lane-r4-receipt are required "
            "together";
    return false;
  }
  if (mlp_factorized_r4_payload && (!a4_payload || !a4_receipt)) {
    error = "the factorized-lane R4 MLP execution bundle requires the "
            "independently authenticated K256 A4 Attention inventory";
    return false;
  }
