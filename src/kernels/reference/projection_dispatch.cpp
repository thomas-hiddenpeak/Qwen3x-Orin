...
[[nodiscard]] bool fp8_qkvz_balanced_persistent_admission_enabled() noexcept {
  // Keep the structural candidate unreachable from ordinary production
  // invocations.  The immutable process-wide snapshot is also CUDA Graph
  // friendly: capture and replay observe one stable projection topology.
  static const bool enabled = []() noexcept {
    const char* const value = std::getenv(
        "Q3X_RUN_FP8_QKVZ_BALANCED_PERSISTENT_ADMISSION");
...
int launch_linear_attention_qkv_z_ab_to_bf16_cuda(
  ...
  if (fp8_qkvz_balanced_persistent_admission_enabled()) {
    return kernels::
