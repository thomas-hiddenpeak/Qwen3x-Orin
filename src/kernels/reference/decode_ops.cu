...
#if defined(Q3X_ENABLE_REFERENCE_RUNNER_INTERNAL_TEST_SEAMS)
constexpr unsigned int kSelectorExactPersistentAttentionV1QueryTile = 8U;
#endif
...
// Q8 expansion of the incumbent GenericQT2 kernel above: one CTA still owns
// one KV head, its six warps still own the same six query heads, and every
// query retains the same scalar FP32 FMA, warp reduction, key-order online
// softmax, FP32 P*V, and BF16->sigmoid-gate->BF16 publication sequence.  The
// only changed ownership is that one resident K/V tile feeds eight adjacent
// queries instead of two.  It is compiled only with internal test seams.
__global__ __launch_bounds__(kBulkGqaThreads, 1)
void selector_exact_persistent_attention_v1_q8_generic_suffix_kernel(
  ...
  static_assert(kWordsPerLane == 4U);
  static_assert(kValuesPerLane == 8U);
  static_assert(kSelectorExactPersistentAttentionV1QueryTile == 8U);
  ...
  const unsigned int query_tile_count =
      token_count / kSelectorExactPersistentAttentionV1QueryTile;
  for (unsigned int query_tile = blockIdx.x;
       query_tile < query_tile_count; query_tile += gridDim.x) {
    const unsigned int first_query_token =
        query_tile * kSelectorExactPersistentAttentionV1QueryTile;

  float query_values[kSelectorExactPersistentAttentionV1QueryTile]
                    [kValuesPerLane];
  float accumulators[kSelectorExactPersistentAttentionV1QueryTile]
                    [kValuesPerLane];
  float maxima[kSelectorExactPersistentAttentionV1QueryTile];
  float denominators[kSelectorExactPersistentAttentionV1QueryTile];
#pragma unroll
  for (unsigned int local_query = 0U;
       local_query < kSelectorExactPersistentAttentionV1QueryTile;
       ++local_query) {
    const unsigned int token = first_query_token + local_query;
    const bool valid_query = token < token_count;
    maxima[local_query] = -__int_as_float(0x7f800000);
...
#pragma unroll
  ...
  const unsigned int last_query_token =
      first_query_token + kSelectorExactPersistentAttentionV1QueryTile - 1U <
              token_count
          ? first_query_token +
                kSelectorExactPersistentAttentionV1QueryTile - 1U
          : token_count - 1U;
  const unsigned int causal_kv_length = first_position + last_query_token + 1U;
  for (unsigned int kv_tile_start = 0U;
       kv_tile_start < causal_kv_length;
...
#if defined(Q3X_ENABLE_REFERENCE_RUNNER_INTERNAL_TEST_SEAMS)
[[nodiscard]] bool
valid_selector_exact_persistent_attention_v1_q8_suffix_arguments(
    const std::uint16_t* const query,
    const std::uint16_t* const key_cache,
    const std::uint16_t* const value_cache,
    const std::uint16_t* const gate,
  ...
  if (!admitted_suffix ||
      token_count < kSelectorExactPersistentAttentionV1QueryTile ||
      token_count % kSelectorExactPersistentAttentionV1QueryTile != 0U ||
      first_position > kBulkGqaMaximumSequence - token_count ||
      query == nullptr || key_cache == nullptr || value_cache == nullptr ||
      gate == nullptr || output == nullptr) {
    return false;
...
launch_selector_exact_persistent_attention_v1_q8_generic_suffix_cuda(
    ...
    void* const cuda_stream) noexcept {
  if (!valid_selector_exact_persistent_attention_v1_q8_suffix_arguments(
          query_suffix, key_cache, value_cache, gate_suffix, first_position,
          token_count, output_suffix)) {
    return static_cast<int>(cudaErrorInvalidValue);
  }
  const dim3 blocks(
...
query_selector_exact_persistent_attention_v1_q8_resources_cuda(
  ...
  resources->threads_per_block = static_cast<int>(kBulkGqaThreads);
  return static_cast<int>(cudaSuccess);
}
#endif
