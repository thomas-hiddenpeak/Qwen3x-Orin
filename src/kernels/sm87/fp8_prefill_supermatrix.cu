...
static_assert(kActivationVectorsPerStage == 512U);
static_assert(kDynamicSharedBytes == 98'304U);
...
struct Fp8SupermatrixRegisterStage {
  InlineM16K16Activation activations[4];
  std::uint32_t packed_weights[4];
};
...
decode_fp8x4_to_bf16x4(
    const std::uint32_t packed,
    const std::uint16_t* const decoded_weights) {
  // The sidecar byte order is [v0,v2,v1,v3]. Gather the exact BF16 codebook
  // entries directly into the two m16n8 B registers [v0,v1] and [v2,v3].
  // The table is seeded once per persistent CTA, outside every N/K task.
...
__device__ __forceinline__ void load_m16k16_activation_fragment(
  ...
  // One ldmatrix.x4 consumes the four 8x8 quadrants of a logical M16xK16
  // row-major tile. Each row's eight 16-byte K64 chunks are XOR-permuted by
  // row at cp.async time, so select the physical chunk that owns this
  // quadrant. This keeps the fragment register-ready and removes the old
  // WMMA shared-load instruction/control sequence.
  const unsigned int quadrant = lane / 8U;
  const unsigned int row =
      (lane % 8U) + (quadrant & 1U) * 8U;
  const unsigned int logical_chunk =
      k16 * 2U + (quadrant >> 1U);
  const unsigned int physical_chunk = logical_chunk ^ (row & 7U);
  const std::uint16_t* const source =
      shared_activations +
      (m_panel * 16U + row) * kSharedLeadingDimension +
      physical_chunk * 8U;
  const unsigned int shared_address =
...
__device__ __forceinline__ std::uint32_t load_shared_u32(
...
__device__ __forceinline__ void load_supermatrix_register_stage(
    Fp8SupermatrixRegisterStage& stage,
    const std::uint16_t* const shared_activations,
    const std::uint32_t* const shared_weights,
    const unsigned int k16, const unsigned int thread,
    const unsigned int lane) {
#pragma unroll
  for (unsigned int m_panel = 0U; m_panel < 4U; ++m_panel) {
    load_m16k16_activation_fragment(
        stage.activations[m_panel], shared_activations, m_panel, k16,
        lane);
  }
#pragma unroll
  for (unsigned int n_panel = 0U; n_panel < 4U; ++n_panel) {
    stage.packed_weights[n_panel] = load_shared_u32(
        shared_weights +
        (k16 * kN8PanelsPerWarp + n_panel) * kThreads + thread);
  }
}
...
__device__ __forceinline__ void issue_pipeline_stage(
  ...
  static_assert(kSharedActivationChunksPerToken == 8U);
...
#pragma unroll
  for (unsigned int pass = 0U; pass < 2U; ++pass) {
    ...
    const unsigned int physical_chunk = chunk ^ (token & 7U);
    cp_async_cg_shared_global_16(
        pipeline->activations[shared_slot] +
            token * kSharedActivationChunksPerToken + physical_chunk,
...
fp8_prefill_supermatrix_m64_n256_k64_kernel(
  ...
  decoded_weights[thread] = decode_e4m3fn_to_bf16_bits(
...
#pragma unroll 1
    for (unsigned int stage = 0U; stage < kKStageCount; ++stage) {
      ...
      // Pair the global->shared ring with an independent two-slot
      // shared->register pipeline. The next K16 slice is resident in
      // registers before the current FP8 decode/MMA chain begins.
      Fp8SupermatrixRegisterStage register_stages[2];
      load_supermatrix_register_stage(
          register_stages[0],
          reinterpret_cast<const std::uint16_t*>(shared_a), shared_b, 0U,
          thread, lane);
#pragma unroll
      for (unsigned int k16 = 0U; k16 < kK16Groups; ++k16) {
        const unsigned int current_slot = k16 & 1U;
        const unsigned int next_slot = current_slot ^ 1U;
        if (k16 + 1U < kK16Groups) {
          load_supermatrix_register_stage(
              register_stages[next_slot],
              reinterpret_cast<const std::uint16_t*>(shared_a), shared_b,
              k16 + 1U, thread, lane);
        }
...
#pragma unroll
        for (unsigned int n_panel = 0U; n_panel < 4U; ++n_panel) {
          decoded_b[n_panel] = decode_fp8x4_to_bf16x4(
...
#pragma unroll
        for (unsigned int m_panel = 0U; m_panel < 4U; ++m_panel) {
          const InlineM16K16Activation activation =
              register_stages[current_slot].activations[m_panel];
#pragma unroll
          for (unsigned int n_panel = 0U; n_panel < 4U; ++n_panel) {
            mma_m16n8k16_bf16(
                accumulators[m_panel][n_panel], activation.x0,
                activation.x1, activation.x2, activation.x3,
    ...
    const float output_scale = selected.weight_scale;
#pragma unroll
    for (unsigned int m_panel = 0U; m_panel < 4U; ++m_panel) {
#pragma unroll
      for (unsigned int n_panel = 0U; n_panel < 4U; ++n_panel) {
...
int launch_sm87_fp8_prefill_supermatrix_pack_cuda(
  ...
  if (nblocks >
      ...
      nblocks > std::numeric_limits<unsigned int>::max() / kstage_count) {
    return invalid_value();
  }
  ...
  for (std::size_t index = 0U; index < partition_count; ++index) {
    ...
    for (std::size_t prior = 0U; prior < index; ++prior) {
      ...
      if (byte_ranges_overlap(partition.register_feed_sidecar,
                              ...
                              prior_sidecar_bytes) ||
          byte_ranges_overlap(partition.register_feed_sidecar,
                              sidecar_bytes, partitions[prior].output,
                              prior_output_bytes) ||
