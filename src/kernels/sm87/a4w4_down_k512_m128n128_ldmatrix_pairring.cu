...
load_a_ldmatrix_x4(const std::uint8_t* const shared_a,
  ...
  // This is the bitwise-proven operand-probe mapping.  The four providers
  // address {M0..7/K0..31, M8..15/K0..31, M0..7/K32..63,
  // M8..15/K32..63}, which is already the native MMA A register order.
...
load_b_ldmatrix_x2(const std::uint8_t* const shared_b,
                   const unsigned int lane) noexcept {
  // This is the bitwise-proven operand-probe B mapping.  x2 consumes lanes
  // 0..15; the upper half-warp supplies duplicate in-bounds addresses.
...
__device__ __forceinline__ void accumulate_k512_group(
  ...
  Sm87A4W4Accumulator
      partials[kSm87A4W4DownK512WarpTileN / 8U]
              [kSm87A4W4DownK512WarpTileM / 16U]{};

  // K64 is the reuse dimension: two A x4 loads feed all eight N8 fragments,
  // while one B x2 load feeds the two M16 fragments.  Per K512 group this is
  // exactly 16 LDSM.x4 + 64 LDSM.x2 + 128 IMMA, replacing the incumbent's
  // 640 scalar operand LDS instructions without changing operand bytes.
#pragma unroll
...
#pragma unroll
    for (unsigned int plane = 0U;
         plane < kSm87A4W4DownK512K64PerStage; ++plane) {
      const Sm87A4W4AFragment a_fragment0 = load_a_ldmatrix_x4(
          stage.a[plane] + local_m_start * kPackedK64Bytes, lane);
      const Sm87A4W4AFragment a_fragment1 = load_a_ldmatrix_x4(
          stage.a[plane] +
              (local_m_start + 16U) * kPackedK64Bytes,
          lane);
#pragma unroll
      for (unsigned int fragment_n = 0U;
           fragment_n < kSm87A4W4DownK512WarpTileN / 8U;
           ++fragment_n) {
        ...
        sm87_a4w4_mma_m16n8k64(
            partials[fragment_n][0U], a_fragment0, b_fragment);
        sm87_a4w4_mma_m16n8k64(
            partials[fragment_n][1U], a_fragment1, b_fragment);
      }
    }
  }

  // Preserve the incumbent scale/load/FMA order fragment by fragment.
#pragma unroll
  for (unsigned int fragment_n = 0U;
    ...
    const Sm87A4W4Accumulator& partial0 = partials[fragment_n][0U];
    const Sm87A4W4Accumulator& partial1 = partials[fragment_n][1U];
...
void q3x_sm87_a4w4_down_k512_m128n128_ldmatrix_pairring_kernel(
    const std::uint8_t* const packed_a,
    const std::uint16_t* const a_k512_scales_bf16,
    const std::uint8_t* const packed_b,
    const std::uint16_t* const b_k512_scales_bf16,
  ...
  for (unsigned int work_tile = blockIdx.x; work_tile < work_tile_count;
    ...
    // Publish the first complete K512 pair.  Thereafter pair 0 (slots 0/1)
    // and pair 1 (slots 2/3) alternate: while group g consumes one pair,
    // group g+1 is copied into the other.  The barrier after wait_group 0
    // simultaneously releases g and publishes g+1.
    ...
    for (unsigned int group = 0U; group < k512_group_count; ++group) {
      ...
      if (has_next) {
        ...
        const unsigned int next_pair = 2U * (next_group & 1U);
        const unsigned int next_phase = 2U * next_group;
        issue_k256_stage(shared.stage[next_pair], packed_a, packed_b,
                         m_tile_start, n_tile_start, next_phase,
                         physical_k64_group_count);
        issue_k256_stage(shared.stage[next_pair + 1U], packed_a,
                         packed_b, m_tile_start, n_tile_start,
                         next_phase + 1U, physical_k64_group_count);
      }

      const unsigned int current_pair = 2U * (group & 1U);
      accumulate_k512_group(
          shared.stage[current_pair], shared.stage[current_pair + 1U],
...
[[nodiscard]] int launch_impl(
  ...
  const int resource_status =
      query_sm87_a4w4_down_k512_m128n128_ldmatrix_pairring_resources_cuda(&resources);
...
int query_sm87_a4w4_down_k512_m128n128_ldmatrix_pairring_resources_cuda(
    Sm87A4W4DownK512M128N128LdmatrixPairringResources* const resources) noexcept {
  if (resources == nullptr) {
    return static_cast<int>(cudaErrorInvalidValue);
  }
  *resources = Sm87A4W4DownK512M128N128LdmatrixPairringResources{};
  cudaDeviceProp properties{};
...
int launch_sm87_a4w4_down_k512_m128n128_ldmatrix_pairring_bf16_cuda(
    const std::uint8_t* const packed_a,
    const std::size_t packed_a_capacity_bytes,
    const std::uint16_t* const a_k512_scales_bf16,
    const std::size_t a_scale_capacity_elements,
...
int launch_sm87_a4w4_down_k512_m128n128_ldmatrix_pairring_test_bf16_cuda(
    const std::uint8_t* const packed_a,
    const std::size_t packed_a_capacity_bytes,
    const std::uint16_t* const a_k512_scales_bf16,
    const std::size_t a_scale_capacity_elements,
