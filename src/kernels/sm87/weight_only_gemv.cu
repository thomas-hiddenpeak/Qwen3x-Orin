...
constexpr std::size_t kBf16SiluLookupTableElements = 1U << 16U;
...
// Return the exact BF16 encodings for one packed E2M1 byte. This deliberately
// reuses the exhaustively checked x8 PRMT constructor above; inlining plus the
// constant .x selection lets ptxas discard the unused six values. The M=1
// gate/up experiment consumes one pair at a time so only four row-pair words
// are live around the original FP32 multiply/FFMA sequence.
__device__ __forceinline__ std::uint32_t
decode_e2m1x2_to_bf16x2_prmt(const std::uint8_t packed) {
  return decode_e2m1x8_to_bf16x8_prmt(
...
// Test-only table-free E2M1 twin. Keep this physically separate from the
// production phase above so the baseline instantiation and its SASS identity
// cannot change during the experiment.
template <bool CtaLocalOutput = false>
__device__ __forceinline__ void
nvfp4_w4a16_gemv_bf16_gate_up_pair_activation_staged_coarsened_512_table_free_e2m1_test_phase(
    const std::uint8_t* const packed_weights,
    const std::uint8_t* const block_scales, const float weight_scale_2,
    std::uint16_t* const output, const ulonglong2* const staged_activation,
    const float* const decoded_scales, const unsigned int lane,
    const unsigned int warp) {
  constexpr std::uint32_t kRows = 17'408U;
  constexpr std::uint32_t kColumns = 5'120U;
  constexpr std::uint32_t kBlocks = 32U;
...
#pragma unroll 1
    for (std::uint32_t packed_column =
      ...
      const std::uint32_t scale_column =
          ...
          (lane & 1U) * kScaleIterationStride;
      std::uint32_t local_raw_scale_codes =
          static_cast<std::uint32_t>(__ldcs(row0_scales + scale_column)) |
          (static_cast<std::uint32_t>(__ldcs(row1_scales + scale_column))
           << 8U) |
          (static_cast<std::uint32_t>(__ldcs(row2_scales + scale_column))
           << 16U) |
          (static_cast<std::uint32_t>(__ldcs(row3_scales + scale_column))
           << 24U);
      const std::uint32_t partner_raw_scale_codes = __shfl_xor_sync(
          0xffff'ffffU, local_raw_scale_codes, 1);
      const std::uint32_t odd_lane_mask = 0U - (lane & 1U);
...
#pragma unroll
      for (unsigned int phase = 0U; phase < 2U; ++phase) {
        ...
        const std::uint32_t phase_packed_column =
            packed_column + phase * kPackedIterationStride;
        const std::uint32_t packed0 = __ldcs(
            reinterpret_cast<const unsigned int*>(
                row0_weights + phase_packed_column));
        const std::uint32_t packed1 = __ldcs(
            reinterpret_cast<const unsigned int*>(
                row1_weights + phase_packed_column));
        const std::uint32_t packed2 = __ldcs(
            reinterpret_cast<const unsigned int*>(
                row2_weights + phase_packed_column));
        const std::uint32_t packed3 = __ldcs(
            reinterpret_cast<const unsigned int*>(
                row3_weights + phase_packed_column));
        const std::uint32_t first_column =
            phase_packed_column * kNvFp4ValuesPerByte;
...
#pragma unroll
          for (unsigned int pair = 0U; pair < 2U; ++pair) {
            const unsigned int packed_byte = half * 2U + pair;
            const unsigned int byte_shift = packed_byte * 8U;
            const std::uint32_t decoded_pair0 =
                decode_e2m1x2_to_bf16x2_prmt(static_cast<std::uint8_t>(
                    packed0 >> byte_shift));
            const std::uint32_t decoded_pair1 =
                decode_e2m1x2_to_bf16x2_prmt(static_cast<std::uint8_t>(
                    packed1 >> byte_shift));
            const std::uint32_t decoded_pair2 =
                decode_e2m1x2_to_bf16x2_prmt(static_cast<std::uint8_t>(
                    packed2 >> byte_shift));
            const std::uint32_t decoded_pair3 =
                decode_e2m1x2_to_bf16x2_prmt(static_cast<std::uint8_t>(
                    packed3 >> byte_shift));
#pragma unroll
            for (unsigned int pair_value = 0U; pair_value < 2U;
                 ++pair_value) {
              const unsigned int value = pair * 2U + pair_value;
              const unsigned int value_shift = pair_value * 16U;
              const std::uint16_t encoded_activation =
                  static_cast<std::uint16_t>(
                      (packed_activation >> (value * 16U)) & 0xffffU);
              const float decoded_activation =
                  decode_bf16(encoded_activation);
              const float decoded_weight0 = decode_bf16(
                  static_cast<std::uint16_t>(decoded_pair0 >> value_shift));
              const float decoded_weight1 = decode_bf16(
                  static_cast<std::uint16_t>(decoded_pair1 >> value_shift));
              const float decoded_weight2 = decode_bf16(
                  static_cast<std::uint16_t>(decoded_pair2 >> value_shift));
              const float decoded_weight3 = decode_bf16(
                  static_cast<std::uint16_t>(decoded_pair3 >> value_shift));
              accumulators0[value] =
                  fmaf(decoded_weight0 * block_scale0, decoded_activation,
                       accumulators0[value]);
              accumulators1[value] =
                  fmaf(decoded_weight1 * block_scale1, decoded_activation,
                       accumulators1[value]);
              accumulators2[value] =
                  fmaf(decoded_weight2 * block_scale2, decoded_activation,
                       accumulators2[value]);
              accumulators3[value] =
                  fmaf(decoded_weight3 * block_scale3, decoded_activation,
                       accumulators3[value]);
            }
          }
        }
...
// Test-only scale-layout twin of the selected Decode gate/up phase. The
// sidecar keeps the canonical number of E4M3 bytes but stores each scale
// column as four adjacent-row bytes. One streaming U32 load therefore
// replaces the four strided U8 scale loads; packed weights, arithmetic order,
// BF16 boundaries, topology, and publication contract remain unchanged.
__device__ __forceinline__ void
nvfp4_w4a16_gemv_bf16_gate_up_pair_activation_staged_coarsened_512_scale_aosoa4_phase(
    const std::uint8_t* const packed_weights,
    const std::uint8_t* const scale_aosoa4, const float weight_scale_2,
    std::uint16_t* const output, const ulonglong2* const staged_activation,
    const float* const decoded_weights, const float* const decoded_scales,
    const unsigned int lane, const unsigned int warp) {
  constexpr std::uint32_t kRows = 17'408U;
  ...
  constexpr std::uint32_t kPackedRowStride =
      kRowStride * kPackedColumns;
  constexpr std::uint32_t kScaleQuadStride =
      (kRowStride / 4U) * kScaleColumns;
  static_assert(kBlocks * kWarps ==
                kNvFp4M1RowQuadMaximumBlocks * kWarpsPerBlock);
  ...
  const std::uint32_t first_row =
      4U * (static_cast<std::uint32_t>(blockIdx.x) * kWarps + warp);
  std::uint32_t packed_row_offset = first_row * kPackedColumns;
  std::uint32_t scale_quad_offset = (first_row / 4U) * kScaleColumns;
  std::uint32_t local_row0 = warp * 4U;
#pragma unroll 1
  for (std::uint32_t row0 = first_row; row0 < kRows;
       row0 += kRowStride) {
...
#pragma unroll 1
    for (std::uint32_t packed_column =
      ...
      std::uint32_t local_raw_scale_codes =
          nvfp4_test_cache_load_u32<NvFp4TestCachePolicy::kStreaming>(
              scale_aosoa4 +
              (static_cast<std::size_t>(scale_quad_offset + scale_column) *
               sizeof(std::uint32_t)));
      const std::uint32_t partner_raw_scale_codes = __shfl_xor_sync(
          0xffff'ffffU, local_raw_scale_codes, 1);
      const std::uint32_t odd_lane_mask = 0U - (lane & 1U);
...
#pragma unroll
    ...
    packed_row_offset += kPackedRowStride;
    scale_quad_offset += kScaleQuadStride;
    local_row0 += kRowsPerCtaPerStride;
  }
}
...
// Test-only production twin that removes only the 16-entry E2M1 float
// codebook. Exact E2M1 BF16 bits are constructed with the exhaustively
// checked PRMT path inside the projection loop, converted back to exact FP32,
// and multiplied by the unchanged FP32 E4M3FN scale before the original FFMA.
// Residual/RMSNorm, reductions, independently rounded gate/up boundaries,
// CTA-local staging, SiLU epilogue, and 32x512 topology remain unchanged.
__global__ __launch_bounds__(512, 2) void
nvfp4_w4a16_gemv_bf16_residual_norm_gate_up_silu_dead_up_table_free_e2m1_test_kernel(
    const std::uint8_t* const gate_packed_weights,
    const std::uint8_t* const gate_block_scales,
    const float gate_weight_scale_2,
    const std::uint8_t* const up_packed_weights,
  ...
  for (unsigned int pair_phase = 0U; pair_phase < 2U; ++pair_phase) {
    nvfp4_w4a16_gemv_bf16_gate_up_pair_activation_staged_coarsened_512_table_free_e2m1_test_phase<
        true>(pair_phase == 0U ? gate_packed_weights : up_packed_weights,
              ...
              pair_phase == 0U ? gate_weight_scale_2 : up_weight_scale_2,
              pair_phase == 0U ? staged_gate : staged_up,
              staged_activation, norm_partial_or_decoded_scales, lane, warp);
  }
...
// Test-only cold initialization and independent direct-reference kernels for
// the exact BF16-domain SiLU lookup screen. Both kernels intentionally spell
// out the selected device expression independently so the test can compare
// all 65,536 FP32 results bit-for-bit without relying on a host math library.
__global__ void bf16_silu_lookup_table_init_test_kernel(float* const table) {
  const std::uint32_t index =
      blockIdx.x * blockDim.x + threadIdx.x;
  if (index < kBf16SiluLookupTableElements) {
    const float gate = decode_bf16(static_cast<std::uint16_t>(index));
    table[index] = gate / (1.0F + expf(-gate));
  }
}

__global__ void bf16_silu_direct_reference_test_kernel(
    float* const output) {
  const std::uint32_t index =
      blockIdx.x * blockDim.x + threadIdx.x;
  if (index < kBf16SiluLookupTableElements) {
...
// Test-only exact-M1 candidate. This is a distinct copy of the currently
// selected 32x512 streaming dead-up path: residual/RMSNorm, both rounded
// projection phases, CTA-local BF16 staging, topology, and publication are
// unchanged. Only the SiLU epilogue consumes the cold-initialized table using
// the staged gate's raw BF16 bits instead of issuing expf/div instructions.
__global__ __launch_bounds__(512, 2) void
nvfp4_w4a16_gemv_bf16_residual_norm_gate_up_silu_dead_up_silu_table_test_kernel(
    const std::uint8_t* const gate_packed_weights,
    const std::uint8_t* const gate_block_scales,
    const float gate_weight_scale_2,
    const std::uint8_t* const up_packed_weights,
...
#pragma unroll 1
  ...
  for (std::uint32_t local_row = threadIdx.x;;
    ...
    const std::uint16_t gate_bits = staged_gate[local_row];
    const float up = decode_bf16(staged_up[local_row]);
    gate_output[row] = encode_bf16_rne(silu_table[gate_bits] * up);
  }
}

// Test-only dead-up twin consuming row-quad AoSoA4 scale sidecars. The
// selected production kernel and ABI remain untouched while this candidate
// isolates the scale-load transaction shape.
__global__ __launch_bounds__(512, 2) void
nvfp4_w4a16_gemv_bf16_residual_norm_gate_up_silu_dead_up_scale_aosoa4_test_kernel(
    const std::uint8_t* const gate_packed_weights,
    const std::uint8_t* const gate_scale_aosoa4,
    const float gate_weight_scale_2,
    const std::uint8_t* const up_packed_weights,
    const std::uint8_t* const up_scale_aosoa4,
    const float up_weight_scale_2,
    const std::uint16_t* const residual_left,
    const std::uint16_t* const residual_right,
    const std::uint16_t* const norm_weight, const float epsilon,
...
#pragma unroll 1
  for (unsigned int pair_phase = 0U; pair_phase < 2U; ++pair_phase) {
    nvfp4_w4a16_gemv_bf16_gate_up_pair_activation_staged_coarsened_512_scale_aosoa4_phase(
        pair_phase == 0U ? gate_packed_weights : up_packed_weights,
        pair_phase == 0U ? gate_scale_aosoa4 : up_scale_aosoa4,
        pair_phase == 0U ? gate_weight_scale_2 : up_weight_scale_2,
        pair_phase == 0U ? staged_gate : staged_up, staged_activation,
        decoded_weights, norm_partial_or_decoded_scales, lane, warp);
  }
...
void launch_nvfp4_residual_norm_gate_up_silu_dead_up_table_free_e2m1_test_unchecked(
    const std::uint8_t* const gate_packed_weights,
    const std::uint8_t* const gate_block_scales,
    const float gate_weight_scale_2,
    const std::uint8_t* const up_packed_weights,
...
void launch_nvfp4_residual_norm_gate_up_silu_dead_up_silu_table_test_unchecked(
    const std::uint8_t* const gate_packed_weights,
    const std::uint8_t* const gate_block_scales,
    const float gate_weight_scale_2,
    const std::uint8_t* const up_packed_weights,
  ...
  nvfp4_w4a16_gemv_bf16_residual_norm_gate_up_silu_dead_up_silu_table_test_kernel
      <<<kCoarsenedBlocks, kCoarsenedThreads, 0U, stream>>>(
          gate_packed_weights, gate_block_scales, gate_weight_scale_2,
          up_packed_weights, up_block_scales, up_weight_scale_2,
          residual_left, residual_right, norm_weight, epsilon, silu_table,
          residual_output, gate_output);
}

void launch_nvfp4_residual_norm_gate_up_silu_dead_up_scale_aosoa4_test_unchecked(
    const std::uint8_t* const gate_packed_weights,
    const std::uint8_t* const gate_scale_aosoa4,
    const float gate_weight_scale_2,
    const std::uint8_t* const up_packed_weights,
  ...
  nvfp4_w4a16_gemv_bf16_residual_norm_gate_up_silu_dead_up_scale_aosoa4_test_kernel
      <<<kCoarsenedBlocks, kCoarsenedThreads, 0U, stream>>>(
          gate_packed_weights, gate_scale_aosoa4, gate_weight_scale_2,
          up_packed_weights, up_scale_aosoa4, up_weight_scale_2,
          residual_left, residual_right, norm_weight, epsilon,
          residual_output, gate_output);
}
...
int launch_sm87_nvfp4_w4a16_residual_norm_gate_up_silu_dead_up_table_free_e2m1_test_cuda(
    const std::uint8_t* const gate_packed_weights,
    const std::uint8_t* const gate_block_scales,
    const float gate_weight_scale_2,
    const std::uint8_t* const up_packed_weights,
  ...
  const auto stream = reinterpret_cast<cudaStream_t>(cuda_stream);
  (void)cudaGetLastError();
  launch_nvfp4_residual_norm_gate_up_silu_dead_up_table_free_e2m1_test_unchecked(
      gate_packed_weights, gate_block_scales, gate_weight_scale_2,
      up_packed_weights, up_block_scales, up_weight_scale_2, residual_left,
      residual_right, norm_weight, epsilon, residual_output, gate_output,
      stream);
...
int launch_sm87_bf16_silu_lookup_table_init_test_cuda(
    float* const table, const std::size_t elements,
    void* const cuda_stream) noexcept {
  constexpr unsigned int kThreadsPerBlock = 256U;
  constexpr unsigned int kBlocks =
      static_cast<unsigned int>(kBf16SiluLookupTableElements /
                                kThreadsPerBlock);
  constexpr std::size_t kTableBytes =
      kBf16SiluLookupTableElements * sizeof(float);
  static_assert((kBf16SiluLookupTableElements % kThreadsPerBlock) == 0U);
  if (table == nullptr || elements != kBf16SiluLookupTableElements ||
      !pointer_is_aligned<alignof(float)>(table) ||
      byte_range_overflows(table, kTableBytes)) {
    return invalid_value();
  }
  const auto stream = reinterpret_cast<cudaStream_t>(cuda_stream);
  (void)cudaGetLastError();
  bf16_silu_lookup_table_init_test_kernel
      <<<kBlocks, kThreadsPerBlock, 0U, stream>>>(table);
  return static_cast<int>(cudaGetLastError());
}

int launch_sm87_bf16_silu_direct_reference_test_cuda(
    float* const output, const std::size_t elements,
    void* const cuda_stream) noexcept {
  constexpr unsigned int kThreadsPerBlock = 256U;
  constexpr unsigned int kBlocks =
      static_cast<unsigned int>(kBf16SiluLookupTableElements /
  ...
  constexpr std::size_t kTableBytes =
      kBf16SiluLookupTableElements * sizeof(float);
  static_assert((kBf16SiluLookupTableElements % kThreadsPerBlock) == 0U);
  if (output == nullptr || elements != kBf16SiluLookupTableElements ||
      !pointer_is_aligned<alignof(float)>(output) ||
      byte_range_overflows(output, kTableBytes)) {
    return invalid_value();
  }
  const auto stream = reinterpret_cast<cudaStream_t>(cuda_stream);
  (void)cudaGetLastError();
  bf16_silu_direct_reference_test_kernel
      <<<kBlocks, kThreadsPerBlock, 0U, stream>>>(output);
  return static_cast<int>(cudaGetLastError());
}

int launch_sm87_nvfp4_w4a16_residual_norm_gate_up_silu_dead_up_silu_table_test_cuda(
    const std::uint8_t* const gate_packed_weights,
    const std::uint8_t* const gate_block_scales,
    const float gate_weight_scale_2,
    const std::uint8_t* const up_packed_weights,
  ...
  const bool aligned =
      ...
      (reinterpret_cast<std::uintptr_t>(up_packed_weights) %
       alignof(std::uint32_t)) == 0U &&
      pointer_is_aligned<alignof(float)>(silu_table) &&
      (reinterpret_cast<std::uintptr_t>(residual_left) %
       alignof(std::uint16_t)) == 0U &&
      (reinterpret_cast<std::uintptr_t>(residual_right) %
       alignof(std::uint16_t)) == 0U &&
      ...
      (reinterpret_cast<std::uintptr_t>(up_workspace) %
       alignof(std::uint16_t)) == 0U;
  if (!supported_shape || silu_table == nullptr || !aligned ||
      byte_range_overflows(silu_table, kTableBytes)) {
    return invalid_value();
  }
  ...
  const std::size_t input_bytes = columns * sizeof(std::uint16_t);
  const std::size_t output_bytes = rows * sizeof(std::uint16_t);
  if (ranges_overlap(silu_table, kTableBytes, gate_packed_weights,
                     packed_bytes) ||
      ranges_overlap(silu_table, kTableBytes, gate_block_scales,
                     scale_bytes) ||
      ranges_overlap(silu_table, kTableBytes, up_packed_weights,
                     packed_bytes) ||
      ranges_overlap(silu_table, kTableBytes, up_block_scales,
                     scale_bytes) ||
      ranges_overlap(silu_table, kTableBytes, residual_left, input_bytes) ||
      ranges_overlap(silu_table, kTableBytes, residual_right, input_bytes) ||
      ranges_overlap(silu_table, kTableBytes, norm_weight, input_bytes) ||
      ranges_overlap(silu_table, kTableBytes, residual_output,
                     input_bytes) ||
      ranges_overlap(silu_table, kTableBytes, gate_output, output_bytes) ||
      ranges_overlap(silu_table, kTableBytes, up_workspace, output_bytes)) {
    return invalid_value();
  }

  const auto stream = reinterpret_cast<cudaStream_t>(cuda_stream);
  (void)cudaGetLastError();
  launch_nvfp4_residual_norm_gate_up_silu_dead_up_silu_table_test_unchecked(
      gate_packed_weights, gate_block_scales, gate_weight_scale_2,
      up_packed_weights, up_block_scales, up_weight_scale_2, residual_left,
      residual_right, norm_weight, epsilon, silu_table, residual_output,
      gate_output, stream);
  return static_cast<int>(cudaGetLastError());
}

// Test-only row-quad AoSoA4 block-scale entry point. Each sidecar has the
// same byte count as its canonical scale tensor, so the established overlap
// and size validation applies unchanged; the extra alignment check covers
// the candidate's U32 scale loads.
int launch_sm87_nvfp4_w4a16_residual_norm_gate_up_silu_dead_up_scale_aosoa4_test_cuda(
    const std::uint8_t* const gate_packed_weights,
    const std::uint8_t* const gate_scale_aosoa4,
    const float gate_weight_scale_2,
    const std::uint8_t* const up_packed_weights,
  ...
  const int validation = validate_nvfp4_residual_norm_gate_up_silu_launch(
      gate_packed_weights, gate_scale_aosoa4, gate_weight_scale_2,
      up_packed_weights, up_scale_aosoa4, up_weight_scale_2, residual_left,
      residual_right, norm_weight, epsilon, rows, columns, residual_output,
      gate_output, up_workspace);
  if (validation != static_cast<int>(cudaSuccess)) {
    return validation;
  ...
  const bool aligned =
      ...
      (reinterpret_cast<std::uintptr_t>(up_packed_weights) %
       alignof(std::uint32_t)) == 0U &&
      (reinterpret_cast<std::uintptr_t>(gate_scale_aosoa4) %
       alignof(std::uint32_t)) == 0U &&
      (reinterpret_cast<std::uintptr_t>(up_scale_aosoa4) %
       alignof(std::uint32_t)) == 0U &&
      (reinterpret_cast<std::uintptr_t>(residual_left) %
       alignof(std::uint16_t)) == 0U &&
      (reinterpret_cast<std::uintptr_t>(residual_right) %
  ...
  const auto stream = reinterpret_cast<cudaStream_t>(cuda_stream);
  (void)cudaGetLastError();
  launch_nvfp4_residual_norm_gate_up_silu_dead_up_scale_aosoa4_test_unchecked(
      gate_packed_weights, gate_scale_aosoa4, gate_weight_scale_2,
      up_packed_weights, up_scale_aosoa4, up_weight_scale_2, residual_left,
      residual_right, norm_weight, epsilon, residual_output, gate_output,
      stream);
...
int query_sm87_nvfp4_w4a16_m1_residual_norm_gate_up_silu_dead_up_table_free_e2m1_resources_test_cuda(
    int* const registers_per_thread,
    std::size_t* const static_shared_bytes,
    std::size_t* const local_bytes,
    int* const maximum_threads_per_block,
...
int query_sm87_nvfp4_w4a16_m1_residual_norm_gate_up_silu_dead_up_silu_table_resources_test_cuda(
    int* const registers_per_thread,
    std::size_t* const static_shared_bytes,
    std::size_t* const local_bytes,
    int* const maximum_threads_per_block,
...
int query_sm87_nvfp4_w4a16_m1_residual_norm_gate_up_silu_dead_up_scale_aosoa4_resources_test_cuda(
    int* const registers_per_thread,
    std::size_t* const static_shared_bytes,
    std::size_t* const local_bytes,
    int* const maximum_threads_per_block,
