...
// Default-off structural resource experiment.  One 256-thread CTA owns an
// M128N256 cell.  A single K256 A exchange slot is handed into registers;
// four independent K256 B slots form the long-lived ring.
...
// Explicit fields make every retained fragment a named register object.  No
// runtime A-fragment index is allowed in the consumer loop.
...
static_assert(sizeof(BRingSlot) == 33'280U);
static_assert(sizeof(SharedStorage) == 149'760U);
...
// 16 KiB codes: one 16-byte vector from every thread for each ordered K64
// plane.  Sixteen threads additionally publish one eight-row scale vector.
...
// 32 KiB codes: four 64-thread panel cohorts.  Each thread copies two
// 16-byte vectors for its panel through all four ordered K64 planes, exactly
// eight code copies/thread.  This is the complete 256-row B stage.
template <Sm87A4W4AttentionK256Topology Topology>
...
#pragma unroll
  ...
  if (threadIdx.x < 32U) {
    ...
    cp_async_16(
        slot.scale + 64U * scale_slot + 8U * row_vector,
...
__device__ __forceinline__ void accumulate_n8(
  ...
  const unsigned int local_n = n128_half * kWarpTileN + kFragmentOffset;
...
#define Q3X_ACCUMULATE_BAND(output, partial, scale0, scale1)              \
  ...
  Q3X_ACCUMULATE_BAND(output_m0, partial_m0, a.m0_scale0, a.m0_scale1);
  Q3X_ACCUMULATE_BAND(output_m1, partial_m1, a.m1_scale0, a.m1_scale1);
...
__device__ __forceinline__ void store_cell(
    const Float4 (&output_m0)[16U],
    const Float4 (&output_m1)[16U],
    const unsigned int macro_cell,
    const unsigned int m_tile_start,
    std::uint16_t* const output0,
    const unsigned int stride0,
...
__device__ __forceinline__ void run_fixed_cell_kernel(
  ...
  for (unsigned int work = blockIdx.x; work < work_cell_count;
    ...
    // Commit A0 first, then B0..B3.  The initial wait retires A0 and B0
    // while retaining the future-B groups where the group count permits.
    ...
    for (unsigned int group = 0U; group < k256_group_count; ++group) {
      ...
      accumulate_group(resident, shared.b[group & 3U], output_m0,
                       output_m1);
      __syncthreads();  // release the consumed B slot
      ...
      if (group + kBSlots < k256_group_count) {
        issue_b_ring_slot<Topology>(
            ...
            physical_k64_group_count);
        // The just-issued future B is the newest group.  Retire every older
        // group (including A_next) while leaving that B in flight.
        cp_async_wait<1U>();
      } else {
        // In the final four groups there is no future-B commit after A_next;
        // wait_group 1 would incorrectly leave A_next unpublished.
        cp_async_wait<0U>();
      }
      __syncthreads();  // publish A_next and the next consumable B slot
    }
...
// same dataflow and are emitted only after Linear passed that gate.
extern "C" __global__ __launch_bounds__(kThreads, 1)
void q3x_sm87_a4w4_attention_k256_m128n256_a_exchange_b4_linear_kernel(
    const std::uint8_t* const packed_a,
    const std::uint16_t* const a_k256_scales_bf16,
    const std::uint8_t* const packed_b0,
    const std::uint16_t* const b_scales0,
  ...
  run_fixed_cell_kernel<Sm87A4W4AttentionK256Topology::kLinearQkvZ>(
...
extern "C" __global__ __launch_bounds__(kThreads, 1)
void q3x_sm87_a4w4_attention_k256_m128n256_a_exchange_b4_full_kernel(
    const std::uint8_t* const packed_a,
    const std::uint16_t* const a_k256_scales_bf16,
    const std::uint8_t* const packed_b0,
    const std::uint16_t* const b_scales0,
  ...
  extern __shared__ __align__(16) unsigned char dynamic_shared[];
  auto& shared = *reinterpret_cast<SharedStorage*>(dynamic_shared);
  run_fixed_cell_kernel<Sm87A4W4AttentionK256Topology::kFullQkv>(
      shared, packed_a, a_k256_scales_bf16, packed_b0, b_scales0,
      packed_b1, b_scales1, packed_b2, b_scales2, k256_group_count,
      physical_k64_group_count, output0, stride0, output1, stride1,
      output2, stride2, m_tile_count, work_cell_count);
...
extern "C" __global__ __launch_bounds__(kThreads, 1)
void q3x_sm87_a4w4_attention_k256_m128n256_a_exchange_b4_o_kernel(
    const std::uint8_t* const packed_a,
    const std::uint16_t* const a_k256_scales_bf16,
    const std::uint8_t* const packed_b0,
    const std::uint16_t* const b_scales0,
  ...
  extern __shared__ __align__(16) unsigned char dynamic_shared[];
  auto& shared = *reinterpret_cast<SharedStorage*>(dynamic_shared);
  run_fixed_cell_kernel<Sm87A4W4AttentionK256Topology::kAttentionO>(
      shared, packed_a, a_k256_scales_bf16, packed_b0, b_scales0,
      packed_b1, b_scales1, packed_b2, b_scales2, k256_group_count,
      physical_k64_group_count, output0, stride0, output1, stride1,
      output2, stride2, m_tile_count, work_cell_count);
...
[[nodiscard]] cudaError_t configure_kernel(const Kernel kernel) noexcept {
...
[[nodiscard]] cudaError_t configure_topology(
  ...
  if (topology == Sm87A4W4AttentionK256Topology::kLinearQkvZ) {
    return configure_kernel(
        q3x_sm87_a4w4_attention_k256_m128n256_a_exchange_b4_linear_kernel);
  }
  if (topology == Sm87A4W4AttentionK256Topology::kFullQkv) {
    return configure_kernel(
        q3x_sm87_a4w4_attention_k256_m128n256_a_exchange_b4_full_kernel);
  }
...
[[nodiscard]] cudaError_t merge_resources(
  ...
  status = cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      ...
      static_cast<int>(kSm87A4W4AttentionK256AExchangeB4Threads),
      kSm87A4W4AttentionK256AExchangeB4SharedBytes);
  if (status != cudaSuccess) {
    return status;
  }
...
int query_sm87_a4w4_attention_k256_m128n256_a_exchange_b4_resources_cuda(
  ...
  *resources = Sm87A4W4AttentionK256Resources{};
  ...
  for (const auto topology : {
    ...
    const cudaError_t status = configure_topology(topology);
    if (status != cudaSuccess) {
      return static_cast<int>(status);
    }
  }
...
#define Q3X_MERGE_KERNEL(kernel)                                         \
  do {                                                                   \
...
int launch_sm87_a4w4_attention_k256_m128n256_a_exchange_b4_bf16_cuda(
  ...
  if (projection_count !=
      ...
      projections == nullptr || plan.launch_ctas == 0U ||
  ...
  constexpr dim3 block(
      static_cast<unsigned int>(
          kSm87A4W4AttentionK256AExchangeB4Threads));
#define Q3X_LAUNCH_CANDIDATE(kernel)                                     \
  kernel<<<grid, block,                                                   \
