...
void q3x_sm87_a4w4_gateup_down_k512_edge_m64n128_kernel(
    const std::uint8_t* const packed_a,
    const std::uint16_t* const a_k512_scales_bf16,
    const std::uint8_t* const packed_gate_b,
    const std::uint16_t* const gate_b_k512_scales_bf16,
...
[[nodiscard]] cudaError_t configure_dynamic_shared() noexcept {
  return cudaFuncSetAttribute(
      q3x_sm87_a4w4_gateup_down_k512_edge_m64n128_kernel,
      cudaFuncAttributeMaxDynamicSharedMemorySize,
      static_cast<int>(kSm87A4W4GateUpDownEdgeDynamicSharedBytes));
}
...
[[nodiscard]] int launch_impl(
  ...
  Sm87A4W4GateUpDownEdgeResources resources{};
  const int resource_status =
      query_sm87_a4w4_gateup_down_k512_edge_resources_cuda(&resources);
  ...
  q3x_sm87_a4w4_gateup_down_k512_edge_m64n128_kernel<<<
      ...
      static_cast<unsigned int>(kSm87A4W4GateUpDownEdgeThreads),
      kSm87A4W4GateUpDownEdgeDynamicSharedBytes, stream>>>(
      packed_a, a_k512_scales_bf16, packed_gate_b,
...
int query_sm87_a4w4_gateup_down_k512_edge_resources_cuda(
    Sm87A4W4GateUpDownEdgeResources* const resources) noexcept {
  if (resources == nullptr) {
    return static_cast<int>(cudaErrorInvalidValue);
  }
  *resources = Sm87A4W4GateUpDownEdgeResources{};
  cudaDeviceProp properties{};
  ...
  status = cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      &active_blocks,
      q3x_sm87_a4w4_gateup_down_k512_edge_m64n128_kernel,
      static_cast<int>(kSm87A4W4GateUpDownEdgeThreads),
      kSm87A4W4GateUpDownEdgeDynamicSharedBytes);
  if (status != cudaSuccess) {
    return static_cast<int>(status);
  }
  ...
  if (resources->registers_per_thread <= 0 ||
      resources->registers_per_thread >
          static_cast<int>(kSm87A4W4GateUpDownEdgeMaximumRegisters) ||
      ...
      resources->active_blocks_per_sm !=
          static_cast<int>(kSm87A4W4GateUpDownEdgeCtasPerSm)) {
    return static_cast<int>(cudaErrorLaunchOutOfResources);
  }
  return static_cast<int>(cudaSuccess);
}

int launch_sm87_a4w4_gateup_down_k512_edge_cuda(
    const std::uint8_t* const packed_a,
    const std::size_t packed_a_capacity_bytes,
    const std::uint16_t* const a_k512_scales_bf16,
    const std::size_t a_scale_capacity_elements,
...
int launch_sm87_a4w4_gateup_down_k512_edge_test_cuda(
    const std::uint8_t* const packed_a,
    const std::size_t packed_a_capacity_bytes,
    const std::uint16_t* const a_k512_scales_bf16,
    const std::size_t a_scale_capacity_elements,
