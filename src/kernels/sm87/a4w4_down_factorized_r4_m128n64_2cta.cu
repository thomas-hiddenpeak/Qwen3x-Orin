...
// Resource-first two-CTA/SM skeleton only; no runner or API route owns this
// kernel while the architecture is being qualified.
...
inline constexpr unsigned int kThreads = static_cast<unsigned int>(
    kSm87A4W4DownFactorizedR4M128N64Threads);
inline constexpr unsigned int kK64PerStage = static_cast<unsigned int>(
    kSm87A4W4DownFactorizedR4M128N64K64PerStage);
...
inline constexpr unsigned int kPhysicalK64Groups =
    static_cast<unsigned int>(
        ...
        kSm87A4W4DownFactorizedR4M128N64PhysicalK64);
...
// A warp owns one M16N64 output row.  The named fields keep the whole-lane
...
static_assert(sizeof(ProjectionS32) == 32U * sizeof(std::int32_t));
static_assert(sizeof(ProjectionFp32) == 32U * sizeof(float));
...
#define Q3X_R4_DOWN_M128N64_MMA(N)                                      \
  do {                                                                    \
...
#pragma unroll
  for (unsigned int plane = 0U; plane < kK64PerStage; ++plane) {
    ...
    Q3X_R4_DOWN_M128N64_MMA(0);
    Q3X_R4_DOWN_M128N64_MMA(1);
    Q3X_R4_DOWN_M128N64_MMA(2);
    Q3X_R4_DOWN_M128N64_MMA(3);
    Q3X_R4_DOWN_M128N64_MMA(4);
    Q3X_R4_DOWN_M128N64_MMA(5);
    Q3X_R4_DOWN_M128N64_MMA(6);
    Q3X_R4_DOWN_M128N64_MMA(7);
  }

#undef Q3X_R4_DOWN_M128N64_MMA
}
...
__device__ __forceinline__ void fold_lane(
    ...
    const unsigned int n_tile_start,
    const unsigned int warp_m16) noexcept {
#define Q3X_R4_DOWN_M128N64_FOLD(N)                                     \
  fold_lane_fragment<N##U>(                                              \
      partial.n##N, cross.n##N, a_scales_bf16, b_scales_bf16,            \
      factor_lane, m_tile_start, n_tile_start, warp_m16)
  Q3X_R4_DOWN_M128N64_FOLD(0);
  Q3X_R4_DOWN_M128N64_FOLD(1);
  Q3X_R4_DOWN_M128N64_FOLD(2);
  Q3X_R4_DOWN_M128N64_FOLD(3);
  Q3X_R4_DOWN_M128N64_FOLD(4);
  Q3X_R4_DOWN_M128N64_FOLD(5);
  Q3X_R4_DOWN_M128N64_FOLD(6);
  Q3X_R4_DOWN_M128N64_FOLD(7);
#undef Q3X_R4_DOWN_M128N64_FOLD
}
...
__device__ __forceinline__ void store_output(
    ...
    const unsigned int warp_m16,
    std::uint16_t* const output_bf16) noexcept {
#define Q3X_R4_DOWN_M128N64_STORE(N)                                    \
  store_fragment<N##U>(                                                  \
      value.n##N, logical_token_count, m_tile_start, n_tile_start,        \
      warp_m16, output_bf16)
  Q3X_R4_DOWN_M128N64_STORE(0);
  Q3X_R4_DOWN_M128N64_STORE(1);
  Q3X_R4_DOWN_M128N64_STORE(2);
  Q3X_R4_DOWN_M128N64_STORE(3);
  Q3X_R4_DOWN_M128N64_STORE(4);
  Q3X_R4_DOWN_M128N64_STORE(5);
  Q3X_R4_DOWN_M128N64_STORE(6);
  Q3X_R4_DOWN_M128N64_STORE(7);
#undef Q3X_R4_DOWN_M128N64_STORE
}

}  // namespace

extern "C" __global__
    __launch_bounds__(kSm87A4W4DownFactorizedR4M128N64Threads,
                      kSm87A4W4DownFactorizedR4M128N64CtasPerSm)
void q3x_sm87_a4w4_down_factorized_r4_m128n64_kernel(
    const std::uint8_t* const packed_a,
  ...
  // fifteen M128 owners cover the exact P1920 launch surface; every CTA walks
  // the 1,200-cell space with the fixed thirty-two-CTA stride.
  for (unsigned int work = blockIdx.x;
       work < kSm87A4W4DownFactorizedR4M128N64WorkTiles;
       work += gridDim.x) {
    const unsigned int n_tile =
        work / kSm87A4W4DownFactorizedR4M128N64MOwners;
    const unsigned int m_owner =
        work -
        n_tile * kSm87A4W4DownFactorizedR4M128N64MOwners;
    ...
    for (unsigned int stage = 0U;
         stage <
         kSm87A4W4DownFactorizedR4M128N64TotalK256Stages;
         ++stage) {
...
[[nodiscard]] cudaError_t configure_kernel() noexcept {
  cudaError_t status = cudaFuncSetAttribute(
      q3x_sm87_a4w4_down_factorized_r4_m128n64_kernel,
...
struct ResourceCache final {
  Sm87A4W4DownFactorizedR4M128N64Resources resources{};
...
[[nodiscard]] ResourceCache build_resource_cache() noexcept {
  ...
  status = cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      &active_blocks,
      q3x_sm87_a4w4_down_factorized_r4_m128n64_kernel,
      static_cast<int>(kSm87A4W4DownFactorizedR4M128N64Threads),
      kSm87A4W4DownFactorizedR4M128N64DynamicSharedBytes);
  if (status != cudaSuccess) {
    cache.status = static_cast<int>(status);
    return cache;
  ...
  const bool passes =
      ...
      resources.registers_per_thread <=
          static_cast<int>(
              kSm87A4W4DownFactorizedR4M128N64MaximumRegisters) &&
      ...
      resources.active_blocks_per_sm >=
          static_cast<int>(kSm87A4W4DownFactorizedR4M128N64CtasPerSm) &&
...
int query_sm87_a4w4_down_factorized_r4_m128n64_resources_cuda(
    Sm87A4W4DownFactorizedR4M128N64Resources* const resources) noexcept {
...
int launch_sm87_a4w4_down_factorized_r4_m128n64_bf16_cuda(
    const std::uint8_t* const packed_a,
  ...
  const Sm87A4W4DownFactorizedR4M128N64Plan plan =
  ...
  q3x_sm87_a4w4_down_factorized_r4_m128n64_kernel<<<
      kSm87A4W4DownFactorizedR4M128N64PersistentCtas,
      kSm87A4W4DownFactorizedR4M128N64Threads,
      kSm87A4W4DownFactorizedR4M128N64DynamicSharedBytes, stream>>>(
...
#if defined(Q3X_SM87_A4W4_DOWN_R4_M128N64_RESOURCE_MAIN)
int main() {
  q3x::kernels::Sm87A4W4DownFactorizedR4M128N64Resources resources{};
  const int status =
      q3x::kernels::
