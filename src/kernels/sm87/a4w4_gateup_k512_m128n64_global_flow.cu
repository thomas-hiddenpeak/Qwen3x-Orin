#include "q3x/kernels/sm87_a4w4_gateup_k512_m128n64_global_flow.h"
...
inline constexpr unsigned int kPackedK64Bytes =
    static_cast<unsigned int>(
        kSm87A4W4GateUpK512M128N64PackedK64Bytes);

struct alignas(16) Sm87A4W4GateUpK512M128N64Stage final {
  std::uint8_t a[kSm87A4W4GateUpK512M128N64K64PerCopy]
                [kSm87A4W4GateUpK512M128N64TileM *
                 kSm87A4W4GateUpK512M128N64PackedK64Bytes];
  std::uint8_t gate[kSm87A4W4GateUpK512M128N64K64PerCopy]
                   [kSm87A4W4GateUpK512M128N64PhaseN *
                    kSm87A4W4GateUpK512M128N64PackedK64Bytes];
  std::uint8_t up[kSm87A4W4GateUpK512M128N64K64PerCopy]
                 [kSm87A4W4GateUpK512M128N64PhaseN *
                  kSm87A4W4GateUpK512M128N64PackedK64Bytes];
};

struct alignas(16) Sm87A4W4GateUpK512M128N64ScaleSlot final {
  std::uint16_t a[kSm87A4W4GateUpK512M128N64TileM];
  std::uint16_t gate[kSm87A4W4GateUpK512M128N64PhaseN];
  std::uint16_t up[kSm87A4W4GateUpK512M128N64PhaseN];
};

struct alignas(16) Sm87A4W4GateUpK512M128N64Pipeline final {
  Sm87A4W4GateUpK512M128N64Stage
      stage[kSm87A4W4GateUpK512M128N64Stages];
  Sm87A4W4GateUpK512M128N64ScaleSlot
      scale[kSm87A4W4GateUpK512M128N64ScaleSlots];
};

union alignas(16) Sm87A4W4GateUpK512M128N64Shared final {
  Sm87A4W4GateUpK512M128N64Pipeline pipeline;
  float product[kSm87A4W4GateUpK512M128N64TileM]
               [kSm87A4W4GateUpK512M128N64PhaseN];
};

struct alignas(16) Sm87A4W4GateUpK512M128N64Float4 final {
  float x0{};
  float x1{};
  float x2{};
  float x3{};
...
static_assert(sizeof(Sm87A4W4GateUpK512M128N64Stage) ==
              kSm87A4W4GateUpK512M128N64StageBytes);
static_assert(sizeof(Sm87A4W4GateUpK512M128N64ScaleSlot) ==
              kSm87A4W4GateUpK512M128N64ScaleSlotBytes);
static_assert(sizeof(Sm87A4W4GateUpK512M128N64Pipeline) ==
              kSm87A4W4GateUpK512M128N64SharedBytes);
static_assert(sizeof(Sm87A4W4GateUpK512M128N64Shared) ==
              kSm87A4W4GateUpK512M128N64SharedBytes);
static_assert(sizeof(Sm87A4W4GateUpK512M128N64Float4) == 16U);
...
// One K256 stage is 32 KiB.  Across 512 threads each lane issues two A
// vectors and one vector for each projection: four 16-byte cp.async.cg
// operations.  No A byte is duplicated between Gate and Up.
__device__ __forceinline__ void issue_k256_codes(
    Sm87A4W4GateUpK512M128N64Stage& stage,
  ...
  constexpr unsigned int kAVectorsPerPlane =
      static_cast<unsigned int>(
          kSm87A4W4GateUpK512M128N64TileM * kPackedK64Bytes / 16U);
  constexpr unsigned int kBVectorsPerPlane =
      static_cast<unsigned int>(
          kSm87A4W4GateUpK512M128N64PhaseN * kPackedK64Bytes / 16U);
  constexpr unsigned int kAVectors =
      static_cast<unsigned int>(
          kSm87A4W4GateUpK512M128N64K64PerCopy) *
      kAVectorsPerPlane;
  ...
  static_assert(kAVectors ==
                2U * kSm87A4W4GateUpK512M128N64Threads);
  static_assert(kBVectors == kSm87A4W4GateUpK512M128N64Threads);

#pragma unroll
  for (unsigned int iteration = 0U; iteration < 2U; ++iteration) {
    const unsigned int a_vector =
        threadIdx.x +
        iteration * kSm87A4W4GateUpK512M128N64Threads;
...
__device__ __forceinline__ void issue_k512_scales(
    Sm87A4W4GateUpK512M128N64ScaleSlot& slot,
  ...
  constexpr unsigned int kAVectors =
      kSm87A4W4GateUpK512M128N64TileM / kRowsPerVector;
  constexpr unsigned int kBVectors =
      kSm87A4W4GateUpK512M128N64PhaseN / kRowsPerVector;
  static_assert(kAVectors == 16U);
...
__device__ __forceinline__ void issue_even_k256_and_scales(
    Sm87A4W4GateUpK512M128N64Stage& stage,
    Sm87A4W4GateUpK512M128N64ScaleSlot& scale,
    const std::uint8_t* const packed_a,
    const std::uint16_t* const a_k512_scales_bf16,
    const std::uint8_t* const packed_gate_b,
    const std::uint16_t* const gate_b_k512_scales_bf16,
...
__device__ __forceinline__ void accumulate_k256_stage(
    const Sm87A4W4GateUpK512M128N64Stage& stage,
  ...
  const unsigned int projection_warp =
      warp % kSm87A4W4GateUpK512M128N64ProjectionWarps;
  const unsigned int fragment_m_start = (projection_warp / 2U) * 32U;
  const unsigned int fragment_n_start = (projection_warp % 2U) * 32U;
  const std::uint8_t (*b)[kSm87A4W4GateUpK512M128N64PhaseN *
                          kSm87A4W4GateUpK512M128N64PackedK64Bytes] =
      warp < kSm87A4W4GateUpK512M128N64ProjectionWarps
          ? stage.gate
          : stage.up;
...
#pragma unroll
  for (unsigned int plane = 0U;
       plane < kSm87A4W4GateUpK512M128N64K64PerCopy; ++plane) {
    ...
    const Sm87A4W4AFragment a_fragment1 =
        sm87_a4w4_load_a_fragment_swizzled_shared(
            ...
            lane);
#pragma unroll
    for (unsigned int fragment_n = 0U; fragment_n < 4U;
         ++fragment_n) {
      const Sm87A4W4BFragment b_fragment =
          sm87_a4w4_load_b_fragment_swizzled_shared(
              b[plane] +
                  (fragment_n_start + fragment_n * 8U) *
                      kPackedK64Bytes,
              lane);
      sm87_a4w4_mma_m16n8k64(partials[fragment_n], a_fragment0,
                             b_fragment);
      sm87_a4w4_mma_m16n8k64(partials[4U + fragment_n], a_fragment1,
                             b_fragment);
    }
  }
}
...
__device__ __forceinline__ void apply_k512_group(
    Sm87A4W4GateUpK512M128N64Float4 (&accumulators)[8U],
    const Sm87A4W4Accumulator (&partials)[8U],
    const Sm87A4W4GateUpK512M128N64ScaleSlot& scale) noexcept {
  const unsigned int lane = threadIdx.x % kSm87A4W4WarpThreads;
  const unsigned int warp = threadIdx.x / kSm87A4W4WarpThreads;
  const unsigned int projection_warp =
      warp % kSm87A4W4GateUpK512M128N64ProjectionWarps;
...
#pragma unroll
  for (unsigned int fragment = 0U; fragment < 8U; ++fragment) {
    const unsigned int m_fragment = fragment / 4U;
    const unsigned int n_fragment = fragment % 4U;
    const unsigned int local_m_start =
        fragment_m_start + m_fragment * 16U;
    ...
    const unsigned int local_n0 =
        fragment_n_start + n_fragment * 8U + coordinate0.n;
    const unsigned int local_n1 =
        fragment_n_start + n_fragment * 8U + coordinate1.n;
    ...
    accumulators[fragment].x0 = __fmaf_rn(
        static_cast<float>(partials[fragment].x0), scale00,
        accumulators[fragment].x0);
    accumulators[fragment].x1 = __fmaf_rn(
        static_cast<float>(partials[fragment].x1), scale01,
        accumulators[fragment].x1);
    accumulators[fragment].x2 = __fmaf_rn(
        static_cast<float>(partials[fragment].x2), scale10,
        accumulators[fragment].x2);
    accumulators[fragment].x3 = __fmaf_rn(
        static_cast<float>(partials[fragment].x3), scale11,
        accumulators[fragment].x3);
  }
}

__device__ __forceinline__ void exchange_and_store_product(
    Sm87A4W4GateUpK512M128N64Shared& shared,
    const Sm87A4W4GateUpK512M128N64Float4 (&accumulators)[8U],
    const unsigned int m_tile_start,
  ...
  if (warp < kSm87A4W4GateUpK512M128N64ProjectionWarps) {
#pragma unroll
    for (unsigned int fragment = 0U; fragment < 8U; ++fragment) {
      const unsigned int m_fragment = fragment / 4U;
      const unsigned int n_fragment = fragment % 4U;
#pragma unroll
      for (unsigned int output = 0U; output < 4U; ++output) {
        ...
        const float values[4U] = {
            accumulators[fragment].x0, accumulators[fragment].x1,
            accumulators[fragment].x2, accumulators[fragment].x3};
        shared.product[fragment_m_start + m_fragment * 16U +
                           coordinate.m]
                      [fragment_n_start + n_fragment * 8U +
                           coordinate.n] = values[output];
      }
    }
  }
  __syncthreads();
...
#pragma unroll
      for (unsigned int output = 0U; output < 4U; ++output) {
        ...
        const unsigned int local_m =
            fragment_m_start + m_fragment * 16U + coordinate.m;
        const unsigned int local_n =
            fragment_n_start + n_fragment * 8U + coordinate.n;
        const float values[4U] = {
            accumulators[fragment].x0, accumulators[fragment].x1,
            accumulators[fragment].x2, accumulators[fragment].x3};
        output_bf16[
...
extern "C" __global__
    __launch_bounds__(kSm87A4W4GateUpK512M128N64Threads,
                      kSm87A4W4GateUpK512M128N64CtasPerSm)
void q3x_sm87_a4w4_gateup_k512_m128n64_global_flow_kernel(
    const std::uint8_t* const packed_a,
    const std::uint16_t* const a_k512_scales_bf16,
    const std::uint8_t* const packed_gate_b,
    const std::uint16_t* const gate_b_k512_scales_bf16,
  ...
  auto& shared =
      *reinterpret_cast<Sm87A4W4GateUpK512M128N64Shared*>(
          dynamic_shared);

  // N-major work-cell order keeps every simultaneous CTA round within a
  // small set of adjacent B windows.  Grid-striding over the complete cell
  // count distributes natural M tails across all 16 CTAs: P1920 performs
  // 127/128 cells per CTA and P2176 performs 144/145, rather than assigning
  // one owner a second complete N sweep.
  for (unsigned int work = blockIdx.x; work < work_cell_count;
    ...
    const unsigned int m_tile_start =
        m_tile * kSm87A4W4GateUpK512M128N64TileM;
      // Complete one N64 phase before allocating the second phase's
      // accumulators.  This is the register-feasible realization of the
      // logical M128N128 cell; it intentionally reloads A for phase one.
#pragma unroll
      for (unsigned int phase = 0U;
           phase < kSm87A4W4GateUpK512M128N64PhasesPerTile; ++phase) {
        const unsigned int absolute_n_tile_start =
            absolute_n_start +
            n_tile * kSm87A4W4GateUpK512M128N64TileN +
            phase * kSm87A4W4GateUpK512M128N64PhaseN;
        const unsigned int output_n_tile_start =
            n_tile * kSm87A4W4GateUpK512M128N64TileN +
            phase * kSm87A4W4GateUpK512M128N64PhaseN;
        Sm87A4W4GateUpK512M128N64Float4 accumulators[8U]{};
        ...
        for (unsigned int group = 0U; group < k512_group_count;
             ++group) {
          ...
          apply_k512_group(
              ...
              shared.pipeline.scale[
                  group % kSm87A4W4GateUpK512M128N64ScaleSlots]);
          __syncthreads();
...
[[nodiscard]] cudaError_t configure_dynamic_shared() noexcept {
  return cudaFuncSetAttribute(
      q3x_sm87_a4w4_gateup_k512_m128n64_global_flow_kernel,
      cudaFuncAttributeMaxDynamicSharedMemorySize,
      static_cast<int>(kSm87A4W4GateUpK512M128N64SharedBytes));
}
...
[[nodiscard]] int launch_impl(
  ...
  const Sm87A4W4GateUpK512M128N64Plan plan =
  ...
  if (plan.launch_ctas == 0U || maximum_launch_ctas == 0U ||
      (require_model_shape &&
       (!sm87_a4w4_gateup_k512_m128n64_is_model_plan(plan) ||
        token_count > 4'096U)) ||
      ...
      output_row_stride_elements >
          std::numeric_limits<unsigned int>::max() ||
      !sm87_a4w4_gateup_k512_m128n64_product_fits(
  ...
  const std::size_t required_a_scales =
      sm87_a4w4_gateup_k512_m128n64_scale_capacity_elements(token_count,
                                                             input_size);
  ...
  q3x_sm87_a4w4_gateup_k512_m128n64_global_flow_kernel
      <<<launch_ctas,
         static_cast<unsigned int>(
             kSm87A4W4GateUpK512M128N64Threads),
         kSm87A4W4GateUpK512M128N64SharedBytes, stream>>>(
          packed_a, a_k512_scales_bf16, packed_gate_b,
          gate_b_k512_scales_bf16, packed_up_b,
          up_b_k512_scales_bf16,
...
int query_sm87_a4w4_gateup_k512_m128n64_resources_cuda(
    Sm87A4W4GateUpK512M128N64Resources* const resources) noexcept {
  if (resources == nullptr) {
    return static_cast<int>(cudaErrorInvalidValue);
  }
  *resources = Sm87A4W4GateUpK512M128N64Resources{};
  cudaDeviceProp properties{};
  ...
  status = cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      &active_blocks,
      q3x_sm87_a4w4_gateup_k512_m128n64_global_flow_kernel,
      static_cast<int>(kSm87A4W4GateUpK512M128N64Threads),
      kSm87A4W4GateUpK512M128N64SharedBytes);
  if (status != cudaSuccess) {
    return static_cast<int>(status);
  }
  ...
  if (resources->registers_per_thread <= 0 ||
      resources->registers_per_thread >
          static_cast<int>(
              kSm87A4W4GateUpK512M128N64MaximumRegisters) ||
      ...
      resources->active_blocks_per_sm <
          static_cast<int>(kSm87A4W4GateUpK512M128N64CtasPerSm)) {
    return static_cast<int>(cudaErrorLaunchOutOfResources);
  }
  return static_cast<int>(cudaSuccess);
}

int launch_sm87_a4w4_gateup_k512_m128n64_bf16_cuda(
    const std::uint8_t* const packed_a,
    const std::size_t packed_a_capacity_bytes,
    const std::uint16_t* const a_k512_scales_bf16,
    const std::size_t a_scale_capacity_elements,
...
int launch_sm87_a4w4_gateup_k512_m128n64_test_bf16_cuda(
    const std::uint8_t* const packed_a,
    const std::size_t packed_a_capacity_bytes,
    const std::uint16_t* const a_k512_scales_bf16,
    const std::size_t a_scale_capacity_elements,
