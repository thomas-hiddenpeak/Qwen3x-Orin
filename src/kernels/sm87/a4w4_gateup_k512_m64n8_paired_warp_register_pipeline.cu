...
std::atomic<bool> g_paired_warp_pipeline_resources_ready{false};

struct alignas(16) RegisterPipelineAStage final {
  std::uint8_t plane
      [kSm87A4W4GateUpK512M64N8PairedWarpRegisterPipelineK64PerGroup]
      [kSm87A4W4GateUpK512M64N8PairedWarpRegisterPipelineTileM *
       kPackedK64Bytes];
};

struct alignas(16) RegisterPipelineScaleSlot final {
  std::uint16_t a[kSm87A4W4GateUpK512M64N8PairedWarpRegisterPipelineTileM];
  std::uint16_t paired_b
      [kSm87A4W4GateUpK512M64N8PairedWarpRegisterPipelineComputeTileN][2U];
};
...
struct alignas(16) RegisterPipelineSlots final {
  RegisterPipelineSlot
      slot[kSm87A4W4GateUpK512M64N8PairedWarpRegisterPipelineAStages];
};

struct alignas(16) RegisterPipelineEdge final {
  std::uint32_t pair
      [kSm87A4W4GateUpK512M64N8PairedWarpRegisterPipelineTileM][8U][32U];
};
...
static_assert(sizeof(RegisterPipelineAStage) ==
              kSm87A4W4GateUpK512M64N8PairedWarpRegisterPipelineAStageBytes);
static_assert(sizeof(RegisterPipelineScaleSlot) ==
              kSm87A4W4GateUpK512M64N8PairedWarpRegisterPipelineScaleSlotBytes);
static_assert(sizeof(RegisterPipelineSlot) ==
              kSm87A4W4GateUpK512M64N8PairedWarpRegisterPipelineSlotBytes);
static_assert(sizeof(RegisterPipelineSlots) ==
              kSm87A4W4GateUpK512M64N8PairedWarpRegisterPipelinePipelineBytes);
static_assert(sizeof(RegisterPipelineEdge) ==
              kSm87A4W4GateUpK512M64N8PairedWarpRegisterPipelineEdgeBytes);
static_assert(sizeof(RegisterPipelineShared) ==
              kSm87A4W4GateUpK512M64N8PairedWarpRegisterPipelineDynamicSharedBytes);
static_assert(sizeof(PairedBFragment) ==
              kSm87A4W4GateUpK512FragmentNativePairSlotBytes);
static_assert(offsetof(PairedBFragment, gate) == 0U &&
              offsetof(PairedBFragment, up) == 8U);
...
__device__ __forceinline__ void issue_a_codes(
  ...
  static_assert(kVectors == 2U *
                                kSm87A4W4GateUpK512M64N8PairedWarpRegisterPipelineThreads);
#pragma unroll
...
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ >= 800
  // The aligned AoS16 slot is consumed exactly once by this CTA.  One
  // transaction supplies both projections while .cg preserves L1 for A/edge.
...
load_paired_warp_b_fragment(
    ...
    const unsigned int k512_group_count) noexcept {
  return load_paired_b_fragment_cg(
      paired_b_codes +
      sm87_a4w4_gateup_k512_fragment_native_code_slot_offset(
          fragment_n, k512_group, plane, lane, k512_group_count));
}
...
__device__ __forceinline__ void accumulate_k512_plane_outer(
  ...
  const unsigned int fragment_n = absolute_n_tile_start + warp * 8U;
  PairedBFragment current = load_paired_warp_b_fragment(
      paired_b_codes, fragment_n, k512_group, 0U, lane,
      k512_group_count);
#pragma unroll 1
  for (unsigned int plane = 0U; plane < 7U; ++plane) {
    // The next aligned 128-bit LDG is independent of all eight current-plane
    // IMMA instructions, giving the scheduler an explicit register lookahead.
    const PairedBFragment next = load_paired_warp_b_fragment(
        paired_b_codes, fragment_n, k512_group, plane + 1U, lane,
        k512_group_count);
#pragma unroll
    for (unsigned int panel = 0U; panel < 4U; ++panel) {
      const Sm87A4W4AFragment a = load_a_ldmatrix_x4(
          ...
          lane);
      sm87_a4w4_mma_m16n8k64(partials[0U][panel], a, current.gate);
      sm87_a4w4_mma_m16n8k64(partials[1U][panel], a, current.up);
    }
    current = next;
  }
#pragma unroll
  for (unsigned int panel = 0U; panel < 4U; ++panel) {
    const Sm87A4W4AFragment a = load_a_ldmatrix_x4(
        stage.plane[7U] + panel * 16U * kPackedK64Bytes, lane);
    sm87_a4w4_mma_m16n8k64(partials[0U][panel], a, current.gate);
    sm87_a4w4_mma_m16n8k64(partials[1U][panel], a, current.up);
  }
}

template <unsigned int Projection>
__device__ __forceinline__ void apply_k512_projection(
...
#pragma unroll
  for (unsigned int panel = 0U; panel < 4U; ++panel) {
    ...
    const Sm87A4W4Accumulator& partial = partials[Projection][panel];
    Float4& output = accumulator[Projection][panel];
...
__device__ __forceinline__ void store_paired_warp_bf16_edge(
...
#pragma unroll
  for (unsigned int panel = 0U; panel < 4U; ++panel) {
    ...
    const unsigned int column =
        cell_in_edge * 128U + warp * 8U + coordinate0.n;
    const Float4& gate = accumulator[0U][panel];
    const Float4& up = accumulator[1U][panel];
...
__device__ __forceinline__ void compute_n128_cell(
  ...
  issue_k512_group(shared.pipeline.slot[0U], packed_a,
  ...
  if (input_k512_group_count > 1U) {
    issue_k512_group(shared.pipeline.slot[1U], packed_a,
                     a_k512_scales_bf16, paired_b_scales_bf16,
  ...
  for (unsigned int group = 0U; group < input_k512_group_count; ++group) {
    ...
    accumulate_k512_plane_outer(
        slot.a, paired_b_codes,
        absolute_n_tile_start, group, input_k512_group_count,
        partials);
    apply_k512_projection<0U>(accumulator, partials, slot.scale);
    apply_k512_projection<1U>(accumulator, partials, slot.scale);
...
#pragma unroll 1
  for (unsigned int cell = 0U; cell < 4U; ++cell) {
    compute_n128_cell(
        ...
        paired_b_codes, paired_b_scales_bf16,
        logical_token_count, m_tile_start,
        edge_n_start + cell * 128U, cell,
        input_k512_group_count);
  }
  // All four warp-local producers must publish the complete M64N512 edge
  // before the row-oriented quantizer consumes it.
  __syncthreads();
  quantize_edge_cell(
...
extern "C" __global__
    __launch_bounds__(
        kSm87A4W4GateUpK512M64N8PairedWarpRegisterPipelineThreads,
        kSm87A4W4GateUpK512M64N8PairedWarpRegisterPipelineCtasPerSm)
void q3x_sm87_a4w4_gateup_k512_m64n8_paired_warp_register_pipeline_kernel(
    const std::uint8_t* const packed_a,
    const std::uint16_t* const a_k512_scales_bf16,
    const std::uint8_t* const paired_b_codes,
    const std::uint16_t* const paired_b_scales_bf16,
  ...
  // Sweep N-major so all resident CTAs consume the same paired-v2 B
...
[[nodiscard]] cudaError_t configure_dynamic_shared() noexcept {
  return cudaFuncSetAttribute(
      q3x_sm87_a4w4_gateup_k512_m64n8_paired_warp_register_pipeline_kernel,
...
[[nodiscard]] int launch_impl(
  ...
  if (capture_status != cudaStreamCaptureStatusNone) {
    if (!g_paired_warp_pipeline_resources_ready.load(
            std::memory_order_acquire)) {
      return static_cast<int>(cudaErrorNotReady);
    }
  } else if (!g_paired_warp_pipeline_resources_ready.load(
                 std::memory_order_acquire)) {
...
int query_sm87_a4w4_gateup_k512_m64n8_paired_warp_register_pipeline_resources_cuda(
    Sm87A4W4GateUpK512M64N8PairedWarpRegisterPipelineResources* const resources)
    noexcept {
  if (resources == nullptr) {
    return static_cast<int>(cudaErrorInvalidValue);
  }
  *resources = Sm87A4W4GateUpK512M64N8PairedWarpRegisterPipelineResources{};
  cudaDeviceProp properties{};
  ...
  g_paired_warp_pipeline_resources_ready.store(
      true, std::memory_order_release);
  return static_cast<int>(cudaSuccess);
}

int launch_sm87_a4w4_gateup_k512_m64n8_paired_warp_register_pipeline_cuda(
    const std::uint8_t* const packed_a,
    const std::size_t packed_a_capacity_bytes,
    const std::uint16_t* const a_k512_scales_bf16,
    const std::size_t a_scale_capacity_elements,
  ...
  return launch_impl(
      ...
      static_cast<unsigned int>(
          kSm87A4W4GateUpK512M64N8PairedWarpRegisterPipelinePersistentCtas),
      true, cuda_stream);
}

int launch_sm87_a4w4_gateup_k512_m64n8_paired_warp_register_pipeline_test_cuda(
    const std::uint8_t* const packed_a,
    const std::size_t packed_a_capacity_bytes,
    const std::uint16_t* const a_k512_scales_bf16,
    const std::size_t a_scale_capacity_elements,
