...
constexpr std::string_view kManifestSchema =
    "q3x.prefill.attention-o-k512.overlay-manifest";
constexpr std::string_view kPolicySchema =
    "q3x.prefill.attention-o-k512.calibration-policy";
constexpr std::string_view kReceiptSchema =
    "q3x.prefill.attention-o-k512.publication-receipt";

[[nodiscard]] PrefillAttentionOK512OverlayDiagnostic make_diagnostic(
    const PrefillAttentionOK512OverlayErrorCode code, std::string context,
    std::string message, std::string expected = {}, std::string actual = {},
    const int system_error = 0) {
  PrefillAttentionOK512OverlayDiagnostic result;
  result.code = code;
...
[[nodiscard]] PrefillAttentionOK512OverlayDiagnostic hash_fd(
  ...
  try {
    ...
    while (offset < bytes) {
      ...
      if (!pread_exact(fd, buffer.data(), count, offset, error)) {
        return make_diagnostic(
            PrefillAttentionOK512OverlayErrorCode::kIoFailure, "sha256",
            "failed to read file while hashing", {}, {}, error);
      }
      if (!hash.update(buffer.data(), count)) {
        return make_diagnostic(
            PrefillAttentionOK512OverlayErrorCode::kArithmeticOverflow,
    ...
    return make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kAllocationFailure, "sha256",
...
void write_base(std::ostream& output,
                const PrefillAttentionOK512BaseBinding& base) {
...
[[nodiscard]] bool valid_base(
    const PrefillAttentionOK512BaseBinding& base) noexcept {
...
[[nodiscard]] std::string manifest_body(
  ...
  for (const PrefillAttentionOK512OverlayEntry& entry :
...
[[nodiscard]] bool expected_o_family(const PrefillProjectionFamily family) {
  return family == PrefillProjectionFamily::kLinearO ||
         family == PrefillProjectionFamily::kFullO;
}
...
[[nodiscard]] bool parse_base(const json::Value& value,
                              PrefillAttentionOK512BaseBinding& base) {
  const auto* object = value.as_object();
  std::string kind;
...
[[nodiscard]] bool read_bounded_file(
    ...
    std::string& output, PrefillAttentionOK512OverlayDiagnostic& diagnostic) {
  UniqueFd input(::open(path.c_str(), O_RDONLY | O_CLOEXEC | O_NOFOLLOW));
  if (!input) {
    diagnostic = make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kIoFailure, path.string(),
  ...
  try {
    ...
    diagnostic = make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kAllocationFailure,
...
PrefillAttentionOK512OverlayManifestResult
build_qwen36_27b_prefill_attention_o_k512_overlay_manifest(
  ...
  PrefillAttentionOK512OverlayManifestResult result;
  if (!valid_base(required_base)) {
    result.diagnostic = make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kInvalidManifest,
  ...
  PrefillAttentionOK512OverlayManifest manifest;
  manifest.physical_layout = std::string(kPrefillAttentionOK512OverlayLayout);
  ...
  manifest.payload_bytes = kPrefillAttentionOK512OverlayPayloadBytes;
  manifest.projections.reserve(kPrefillAttentionOK512OverlayProjectionCount);
  for (const PrefillProjectionSidecarEntry& source :
       full.value->projections) {
    if (!expected_o_family(source.family)) {
      continue;
    }
    const std::uint32_t ordinal =
        static_cast<std::uint32_t>(manifest.projections.size());
    PrefillAttentionOK512OverlayEntry entry;
    entry.ordinal = ordinal;
    entry.layer_index = source.layer_index;
    entry.family = source.family == PrefillProjectionFamily::kLinearO
                       ? "linear_o"
                       : "full_o";
    entry.source_module = source.source_module;
    entry.source_sha256 = source.source_sha256;
    entry.output_size = kPrefillAttentionOK512OverlayOutputSize;
    entry.input_size = kPrefillAttentionOK512OverlayInputSize;
    entry.sidecar_offset =
        static_cast<std::uint64_t>(ordinal) *
        kPrefillAttentionOK512OverlayProjectionBytes;
    entry.weight_bytes = kPrefillAttentionOK512OverlayProjectionWeightBytes;
    entry.scale_bytes = kPrefillAttentionOK512OverlayProjectionScaleBytes;
    manifest.projections.emplace_back(std::move(entry));
  }
  manifest.manifest_sha256 = sha256_text(manifest_body(manifest));
  result.diagnostic =
      validate_prefill_attention_o_k512_overlay_manifest(manifest);
...
PrefillAttentionOK512OverlayDiagnostic
validate_prefill_attention_o_k512_overlay_manifest(
    const PrefillAttentionOK512OverlayManifest& manifest) {
  if (manifest.version_major != kPrefillAttentionOK512OverlayVersionMajor ||
      manifest.version_minor != kPrefillAttentionOK512OverlayVersionMinor ||
      manifest.physical_layout != kPrefillAttentionOK512OverlayLayout ||
    ...
    return make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kInvalidManifest, "manifest",
        "K512 Attention-O manifest header is invalid");
  }
  for (std::size_t index = 0U; index < manifest.projections.size(); ++index) {
    const auto& entry = manifest.projections[index];
    const bool full = ((index + 1U) % 4U) == 0U;
    const std::string expected_module =
        "model.language_model.layers." + std::to_string(index) +
        (full ? ".self_attn.o_proj" : ".linear_attn.out_proj");
    if (entry.ordinal != index || entry.layer_index != index ||
        entry.family != (full ? "full_o" : "linear_o") ||
        entry.source_module != expected_module ||
        !lower_sha256(entry.source_sha256) ||
        entry.output_size != kPrefillAttentionOK512OverlayOutputSize ||
        entry.input_size != kPrefillAttentionOK512OverlayInputSize ||
        entry.sidecar_offset !=
            index * kPrefillAttentionOK512OverlayProjectionBytes ||
        entry.weight_bytes !=
      ...
      return make_diagnostic(
          ...
          "fixed O projection inventory or byte layout is invalid");
    }
  }
  const std::string digest = sha256_text(manifest_body(manifest));
  if (digest != manifest.manifest_sha256) {
    return make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kDigestMismatch,
...
quantize_prefill_attention_o_k512_consumer_blocks(
  ...
  if (source_rows == nullptr || packed_signed_w4 == nullptr ||
    ...
    return make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kInvalidOption,
      ...
      for (std::size_t local_n = 0U; local_n < 64U; ++local_n) {
        ...
        for (std::size_t column = 0U; column < 512U; ++column) {
          ...
          if (!std::isfinite(value)) {
            return make_diagnostic(
                PrefillAttentionOK512OverlayErrorCode::kQuantizationFailure,
...
[[nodiscard]] PrefillAttentionOK512OverlayDiagnostic publish_read_only_file(
    const fs::path& target, const std::string_view bytes) {
  if (target.empty()) {
    return make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kInvalidOption, "output",
        "non-empty output path is required");
  }
  struct stat existing {};
  if (::lstat(target.c_str(), &existing) == 0 || errno != ENOENT) {
    return make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kPublicationConflict,
  ...
  if (!output) {
    return make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kIoFailure,
  ...
  if (::unlink(temporary.c_str()) != 0) {
    ...
    return make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kIoFailure, target.string(),
  ...
  if (!directory || ::fsync(directory.get()) != 0) {
    ...
    (void)::unlink(target.c_str());
    return make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kIoFailure, parent.string(),
...
PrefillAttentionOK512OverlayPolicyResult
parse_prefill_attention_o_k512_overlay_policy(
    const std::string_view document,
    const PrefillAttentionOK512OverlayManifest& manifest) {
  PrefillAttentionOK512OverlayPolicyResult result;
  const PrefillAttentionOK512OverlayDiagnostic manifest_diagnostic =
      validate_prefill_attention_o_k512_overlay_manifest(manifest);
  ...
  try {
    ...
    if (root == nullptr ||
      ...
      result.diagnostic = make_diagnostic(
          PrefillAttentionOK512OverlayErrorCode::kInvalidPolicy, "policy",
          "strict policy JSON schema mismatch");
      return result;
    }
    PrefillAttentionOK512OverlayPolicy policy;
    std::string schema;
    std::string mode;
    ...
    if (!json_string(*root, "schema", schema) || schema != kPolicySchema ||
        ...
        policy.version_major != kPrefillAttentionOK512OverlayVersionMajor ||
        policy.version_minor != kPrefillAttentionOK512OverlayVersionMinor ||
        ...
        packed_k != kPrefillAttentionOK512OverlayPackedK ||
        !json_uint(*root, "scale_group_size", scale_k) ||
        scale_k != kPrefillAttentionOK512OverlayScaleK ||
    ...
    if (projections == nullptr || projections->size() !=
                                      manifest.projections.size()) {
      result.diagnostic = make_diagnostic(
          PrefillAttentionOK512OverlayErrorCode::kInvalidPolicy,
          "policy.projections", "policy must cover all 64 O projections");
      return result;
    }
    ...
    for (std::size_t index = 0U; index < projections->size(); ++index) {
      ...
      PrefillAttentionOK512OverlayCalibration calibration;
      ...
      if (object == nullptr ||
          ...
          !json_uint(*object, "activation_scale_group_size",
                     activation_group) ||
          activation_group != kPrefillAttentionOK512OverlayScaleK ||
    ...
    result.diagnostic = make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kAllocationFailure, "policy",
...
write_prefill_attention_o_k512_overlay_policy_template(
  ...
  if (!manifest_diagnostic || output_path.empty() ||
    ...
    result.diagnostic = manifest_diagnostic
                            ? make_diagnostic(
                                  PrefillAttentionOK512OverlayErrorCode::
                                      kInvalidOption,
                                  "policy_template",
  ...
  result = parse_prefill_attention_o_k512_overlay_policy(document, manifest);
  if (!result) {
    return result;
  }
...
write_qwen36_27b_prefill_attention_o_k512_overlay_policy_template(
  ...
  if (!base_receipt.has_value() || !base_diagnostic ||
    ...
    result.diagnostic = make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kInvalidReceipt,
  ...
  return write_prefill_attention_o_k512_overlay_policy_template(
...
[[nodiscard]] std::string serialize_receipt(
    const PrefillAttentionOK512OverlayReceipt& receipt) {
  std::ostringstream output;
...
[[nodiscard]] PrefillAttentionOK512OverlayDiagnostic authenticate_sources(
  ...
  if (!sources.root) {
    return make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kSourceAuthenticationFailed,
...
[[nodiscard]] PrefillAttentionOK512OverlayDiagnostic reauthenticate_sources(
...
[[nodiscard]] PrefillAttentionOK512OverlayDiagnostic read_tensor(
  ...
  if (!checked_add(relative_offset, bytes, end) || end > locator.byte_size ||
    ...
    return make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kSourceTensorMismatch,
...
std::optional<PrefillAttentionOK512OverlayReceipt>
parse_prefill_attention_o_k512_overlay_receipt(
  ...
  try {
    ...
    if (root == nullptr ||
      ...
      diagnostic = make_diagnostic(
          PrefillAttentionOK512OverlayErrorCode::kInvalidReceipt, "receipt",
          "strict receipt JSON schema mismatch");
      return std::nullopt;
    }
    PrefillAttentionOK512OverlayReceipt receipt;
    std::string schema;
    std::string mode;
    std::uint64_t packed_k = 0U;
    std::uint64_t scale_k = 0U;
    ...
    if (!json_string(*root, "schema", schema) || schema != kReceiptSchema ||
        ...
        receipt.version_major != kPrefillAttentionOK512OverlayVersionMajor ||
        receipt.version_minor != kPrefillAttentionOK512OverlayVersionMinor ||
        !json_string(*root, "mode", mode) ||
        mode != "production_calibrated" || eligible == nullptr || !*eligible ||
        !json_string(*root, "physical_layout", receipt.physical_layout) ||
        receipt.physical_layout != kPrefillAttentionOK512OverlayLayout ||
...
convert_pinned_qwen36_27b_prefill_attention_o_k512_overlay(
  ...
  try {
    if (options.model_directory.empty() ||
      ...
      result.diagnostic = make_diagnostic(
          PrefillAttentionOK512OverlayErrorCode::kInvalidOption, "convert",
    ...
    const PrefillAttentionOK512OverlayPolicyResult parsed_policy =
    ...
    if (kPrefillAttentionOK512OverlayPayloadBytes >
    ...
    if (options.preallocate_output) {
      const int error = ::posix_fallocate(
          output.get(), 0,
          static_cast<off_t>(kPrefillAttentionOK512OverlayPayloadBytes));
      if (error != 0) {
        result.diagnostic = make_diagnostic(
            PrefillAttentionOK512OverlayErrorCode::kIoFailure,
    ...
    for (std::size_t projection_index = 0U;
      ...
      if (weight == nullptr || weight_scale == nullptr ||
          weight->dtype != io::safetensors::DType::kF8E4M3 ||
          weight->shape != std::vector<std::uint64_t>{
                               kPrefillAttentionOK512OverlayOutputSize,
                               kPrefillAttentionOK512OverlayInputSize} ||
          weight_scale->dtype != io::safetensors::DType::kF32 ||
          !weight_scale->shape.empty() || weight_scale->byte_size != 4U) {
        result.diagnostic = make_diagnostic(
            PrefillAttentionOK512OverlayErrorCode::kSourceTensorMismatch,
            entry.source_module,
            "O projection must be original FP8 E4M3 [5120,6144] plus F32 scale");
        return result;
      }
      std::array<std::uint8_t, 4U> scalar_bytes{};
      result.diagnostic = read_tensor(
          sources, *weight_scale, 0U, scalar_bytes.data(), scalar_bytes.size(),
      ...
      if (!std::isfinite(tensor_scale) || tensor_scale < 0.0F) {
        result.diagnostic = make_diagnostic(
            PrefillAttentionOK512OverlayErrorCode::kSourceTensorMismatch,
            entry.source_module + ".weight_scale",
            "source FP8 tensor scale is not finite/nonnegative");
        return result;
      }
      ...
      const std::size_t source_stride = input_size;
      const std::size_t packed_stride = input_size / 2U;
      const std::size_t scale_stride = input_size / 512U * 2U;
      std::vector<std::uint8_t> source(rows_per_chunk * source_stride);
      ...
      const std::uint64_t working =
          source.capacity() + decoded.capacity() * sizeof(float) +
          packed.capacity() + scales.capacity();
      ...
      for (std::uint64_t row = 0U; row < entry.output_size;
        ...
        const std::size_t source_bytes = rows * source_stride;
        result.diagnostic = read_tensor(
            sources, *weight, row * source_stride, source.data(),
            source_bytes, entry.source_module + ".weight", result.stats);
        if (!result.diagnostic) {
          return result;
        }
        for (std::size_t index = 0U; index < rows * input_size; ++index) {
          decoded[index] =
              quantization::decode_e4m3fn(source[index]) * tensor_scale;
        }
        const std::size_t packed_bytes = rows * packed_stride;
        const std::size_t scale_bytes = rows * scale_stride;
        result.diagnostic = quantize_prefill_attention_o_k512_consumer_blocks(
    ...
    if (result.stats.projections_converted !=
      ...
      result.diagnostic = make_diagnostic(
          ...
          "conversion did not write the complete 64-projection payload");
      return result;
    }
    ...
    receipt.payload_bytes = kPrefillAttentionOK512OverlayPayloadBytes;
    receipt.projection_count = kPrefillAttentionOK512OverlayProjectionCount;
    const std::string receipt_document = serialize_receipt(receipt);
    PrefillAttentionOK512OverlayDiagnostic receipt_parse_diagnostic;
    if (!parse_prefill_attention_o_k512_overlay_receipt(
    ...
    result.diagnostic = make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kAllocationFailure, "convert",
        "conversion allocation failed");
    return result;
  } catch (...) {
    result.diagnostic = make_diagnostic(
        PrefillAttentionOK512OverlayErrorCode::kIoFailure, "convert",
...
std::string_view to_string(
    const PrefillAttentionOK512OverlayErrorCode code) noexcept {
  switch (code) {
    case PrefillAttentionOK512OverlayErrorCode::kNone:
      return "none";
    case PrefillAttentionOK512OverlayErrorCode::kInvalidOption:
      return "invalid_option";
    case PrefillAttentionOK512OverlayErrorCode::kInvalidManifest:
      return "invalid_manifest";
    case PrefillAttentionOK512OverlayErrorCode::kInvalidPolicy:
      return "invalid_policy";
    case PrefillAttentionOK512OverlayErrorCode::kInvalidReceipt:
      return "invalid_receipt";
    case PrefillAttentionOK512OverlayErrorCode::kSourceAuthenticationFailed:
      return "source_authentication_failed";
    case PrefillAttentionOK512OverlayErrorCode::kSourceTensorMismatch:
      return "source_tensor_mismatch";
    case PrefillAttentionOK512OverlayErrorCode::kIoFailure:
      return "io_failure";
    case PrefillAttentionOK512OverlayErrorCode::kDigestMismatch:
      return "digest_mismatch";
    case PrefillAttentionOK512OverlayErrorCode::kArithmeticOverflow:
      return "arithmetic_overflow";
    case PrefillAttentionOK512OverlayErrorCode::kQuantizationFailure:
      return "quantization_failure";
    case PrefillAttentionOK512OverlayErrorCode::kPublicationConflict:
      return "publication_conflict";
    case PrefillAttentionOK512OverlayErrorCode::kAllocationFailure:
