...
#include "q3x/runtime/prefill_mlp_k512_overlay.h"
    ...
    defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N128_REGISTER_PIPELINE_ADMISSION)
#include "q3x/runtime/prefill_mlp_k512_fragment_native_overlay.h"
#endif
#if defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R1_ADMISSION)
#if !defined(Q3X_ENABLE_A4W4_FULL_PREFILL_ADMISSION)
#error "factorized-lane R1 admission requires the authenticated K256 A4 base"
#endif
...
[[nodiscard]] bool
prefill_mlp_factorized_lane_r1_environment_enabled() noexcept {
  if (optimized_prefill_dispatch_disabled()) {
    return false;
  }
  const char* const value = std::getenv(
...
struct PrefillMLPK512ProjectionMajorGateUpCanonicalDownEnginePaths final {
  bool requested = false;
  std::filesystem::path payload;
  std::filesystem::path policy;
  std::filesystem::path receipt;
...
[[nodiscard]] bool validate_prefill_mlp_k512_leaf_selectors(
  ...
  const bool fused_quantize_gate_selected =
      exact_environment_selector_enabled(
          "Q3X_RUN_A4W4_GATEUP_K512_M128N512_FUSED_QUANTIZE_ADMISSION");
  const bool register_pipeline_gate_selected =
      prefill_gateup_k512_m64n128_register_pipeline_environment_enabled();
  ...
  if (register_pipeline_gate_selected != projection_major_selected) {
    error = "the M64N128 register-pipeline GateUp selector and "
            "projection-major publication master must be enabled together";
    return false;
  }
  if (any_projection_major_selector) {
#if !defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N128_REGISTER_PIPELINE_ADMISSION)
    error = "this binary does not contain the M64N128 register-pipeline "
            "GateUp admission";
    return false;
#else
    if (!projection_major_publication_requested ||
        !projection_major_selected || !register_pipeline_gate_selected) {
      error = "the M64N128 register-pipeline GateUp admission requires its "
              "authenticated projection-major publication and runtime "
              "master";
      return false;
    }
    if (optimized_prefill_dispatch_disabled()) {
      error = "the M64N128 register-pipeline GateUp admission cannot run "
              "while optimized Prefill dispatch is disabled";
      return false;
    }
    if (mlp_k512_v1_publication_requested || mlp_k512_v1_selected ||
    ...
    constexpr std::array<const char*, 10U> kConflictingGateSelectors = {
        "Q3X_RUN_A4W4_GATEUP_DOWN_K512_EDGE_ADMISSION",
        "Q3X_RUN_A4W4_GATEUP_DOWN_K512_EDGE_M128N64_ADMISSION",
        "Q3X_RUN_A4W4_GATEUP_COMPLETE_CELL_V2_ADMISSION",
        "Q3X_RUN_A4W4_M128_STAGE_MAJOR_ADMISSION",
    ...
    for (const char* const selector : kConflictingGateSelectors) {
      if (exact_environment_selector_enabled(selector)) {
        error = "the M64N128 register-pipeline GateUp admission conflicts "
    ...
    if (pairring_selected ||
        prefill_down_k512_m16n64_v2_environment_enabled() ||
        prefill_legacy_down_selector_environment_enabled()) {
      error = "the projection-major K512 MLP route permits only its "
              "optional 16-warp pair-ring Down selector";
      return false;
    }
...
#if !defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M64N128_K256_ALTERNATING_ADMISSION)
  if (alternating_gate_selected || ldmatrix_pairfeed_gate_selected) {
    error = "this binary does not contain the M64N128 K256 Gate+Up "
            "experiment package";
    return false;
  }
#endif
  if (alternating_gate_selected || ldmatrix_pairfeed_gate_selected) {
    if (optimized_prefill_dispatch_disabled()) {
      error = "the selected M64N128 K256 Gate+Up implementation cannot run "
              "while optimized Prefill dispatch is disabled";
      return false;
    }
    if (!mlp_k512_v1_publication_requested || !mlp_k512_v1_selected ||
    ...
    for (const char* const selector : kConflictingGateSelectors) {
      if (exact_environment_selector_enabled(selector)) {
        error = "the selected M64N128 K256 Gate+Up implementation conflicts "
                "with every other Prefill Gate+Up selector";
        return false;
      }
    }
    if (exact_environment_selector_enabled(
            "Q3X_RUN_A4W4_DOWN_K512_M16N64_V2_ADMISSION")) {
      error = "the selected M64N128 K256 Gate+Up implementation conflicts "
              "with the M16N64-v2 Down selector";
      return false;
    }
  }
...
[[nodiscard]] bool resolve_prefill_mlp_k512_engine_paths(
    const ReferenceEngineOptions& options, PrefillMLPK512EnginePaths& paths,
    std::string& error) {
  const bool has_payload = !options.prefill_mlp_k512_payload_path.empty();
  const bool has_policy = !options.prefill_mlp_k512_policy_path.empty();
  const bool has_receipt = !options.prefill_mlp_k512_receipt_path.empty();
  paths.requested = has_payload || has_policy || has_receipt;
  if (!paths.requested) {
    return true;
  }
  ...
  paths.payload = options.prefill_mlp_k512_payload_path;
  paths.policy = options.prefill_mlp_k512_policy_path;
  paths.receipt = options.prefill_mlp_k512_receipt_path;
  if (paths.payload == paths.policy || paths.payload == paths.receipt ||
      paths.policy == paths.receipt) {
...
[[nodiscard]] bool
resolve_prefill_mlp_k512_projection_major_gateup_canonical_down_engine_paths(
    const ReferenceEngineOptions& options,
    PrefillMLPK512ProjectionMajorGateUpCanonicalDownEnginePaths& paths,
    std::string& error) {
  const bool has_payload =
      !options
  ...
  if (paths.payload == paths.policy || paths.payload == paths.receipt ||
      paths.policy == paths.receipt) {
    error = "projection-major-GateUp/canonical-Down K512 MLP payload, "
            "source-v1 policy, and receipt must be distinct paths";
    return false;
  }
  return true;
}

[[nodiscard]] bool resolve_prefill_mlp_factorized_lane_r1_engine_paths(
    const ReferenceEngineOptions& options,
    PrefillMLPFactorizedLaneR1EnginePaths& paths, std::string& error) {
  const bool has_payload =
  ...
  if (!has_payload || !has_policy || !has_receipt) {
    error = "factorized-lane R1 payload, policy, and receipt are required "
            "together";
    return false;
  }
  paths.payload = options.prefill_mlp_factorized_lane_r1_payload_path;
  paths.policy = options.prefill_mlp_factorized_lane_r1_policy_path;
  paths.receipt = options.prefill_mlp_factorized_lane_r1_receipt_path;
  if (paths.payload == paths.policy || paths.payload == paths.receipt ||
      paths.policy == paths.receipt) {
...
struct Sm87MLPK512Overlay final {
  ...
  Sm87MLPK512Overlay() noexcept = default;
  Sm87MLPK512Overlay(const Sm87MLPK512Overlay&) = delete;
  Sm87MLPK512Overlay& operator=(const Sm87MLPK512Overlay&) = delete;
  ~Sm87MLPK512Overlay() { release(); }
...
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N128_REGISTER_PIPELINE_ADMISSION)
struct Sm87MLPK512ProjectionMajorGateUpCanonicalDownPreparation final {
  bool enabled = false;
  std::size_t layers = 0U;
  std::uint64_t bytes = 0U;
  std::uint64_t copy_chunks = 0U;
...
struct Sm87MLPFactorizedLaneR1Overlay final {
  ...
  Sm87MLPFactorizedLaneR1Overlay() noexcept = default;
  Sm87MLPFactorizedLaneR1Overlay(
      const Sm87MLPFactorizedLaneR1Overlay&) = delete;
  Sm87MLPFactorizedLaneR1Overlay& operator=(
      const Sm87MLPFactorizedLaneR1Overlay&) = delete;
  ~Sm87MLPFactorizedLaneR1Overlay() { release(); }
...
struct Sm87MLPFactorizedLaneR1Preparation final {
  bool enabled = false;
  std::size_t layers = 0U;
  std::uint64_t bytes = 0U;
  std::uint64_t copy_chunks = 0U;
...
struct FactorizedLaneR1LockedFile final {
  int descriptor = -1;
  FactorizedLaneR1FileSnapshot before;

  FactorizedLaneR1LockedFile() noexcept = default;
  FactorizedLaneR1LockedFile(const FactorizedLaneR1LockedFile&) = delete;
  FactorizedLaneR1LockedFile& operator=(
      const FactorizedLaneR1LockedFile&) = delete;
  ~FactorizedLaneR1LockedFile() {
    if (descriptor >= 0) {
      (void)::close(descriptor);
    }
  }
...
[[nodiscard]] bool capture_factorized_lane_r1_snapshot(
    const int descriptor, FactorizedLaneR1FileSnapshot& snapshot,
    int& system_error) noexcept {
  struct stat status {};
...
[[nodiscard]] bool same_factorized_lane_r1_snapshot(
...
[[nodiscard]] bool open_factorized_lane_r1_file(
  ...
  if (file.descriptor < 0) {
    result.message = "failed to open an owner-held regular publication file";
    result.context = path.string();
    result.dependency_error = errno;
    return false;
  }
  if (::flock(file.descriptor, LOCK_SH | LOCK_NB) != 0) {
    result.message = "publication file is concurrently locked for mutation";
    result.context = path.string();
    result.dependency_error = errno;
    return false;
  }
  ...
  if (!capture_factorized_lane_r1_snapshot(
    ...
    result.message = "publication file failed its regular-file snapshot";
    result.context = path.string();
  ...
  if (bytes < minimum_bytes || bytes > maximum_bytes ||
    ...
    result.message = "publication file must have the exact bounded size and "
                     "be owner-held, read-only, and singly linked";
    result.context = path.string();
    return false;
  }
  if (!factorized_lane_r1_path_names_snapshot(
          path, file.before, snapshot_error)) {
    result.message = "publication path no longer names its locked descriptor";
    result.context = path.string();
    result.dependency_error = snapshot_error;
    return false;
  }
...
[[nodiscard]] bool revalidate_factorized_lane_r1_file(
  ...
  if (!capture_factorized_lane_r1_snapshot(
      ...
      !same_factorized_lane_r1_snapshot(file.before, after) ||
      !factorized_lane_r1_path_names_snapshot(path, after, snapshot_error)) {
    result.message = "publication file changed while it was authenticated";
    result.context = path.string();
    result.dependency_error = snapshot_error;
    return false;
  }
...
[[nodiscard]] bool read_factorized_lane_r1_document(
  ...
  try {
    ...
    result.message = "host allocation failed for publication metadata";
    result.context = path.string();
    return false;
  } catch (const std::length_error&) {
    result.message = "publication metadata length is not representable";
    result.context = path.string();
    return false;
  }
  int read_error = 0;
  if (!pread_exact_engine(file.descriptor, document.data(), bytes, 0U,
                          read_error)) {
    result.message = "publication metadata read was incomplete";
    result.context = path.string();
    result.dependency_error = read_error;
    return false;
  }
  return revalidate_factorized_lane_r1_file(path, file, result);
}

[[nodiscard]] std::string factorized_lane_r1_sha256(
...
prepare_sm87_mlp_factorized_lane_r1_overlay(
    const std::filesystem::path& model_directory,
    const PrefillMLPFactorizedLaneR1EnginePaths& paths,
    const PrefillA4EnginePaths& base_paths,
    const ResidentWeights& resident, const Sm87A4PrefillPreparation& base,
    const std::uint64_t minimum_free_bytes_after_prepare,
    ModelWeights& model_weights, Sm87MLPFactorizedLaneR1Overlay& owner) {
  Sm87MLPFactorizedLaneR1Preparation result;
  if (!paths.requested || !base_paths.requested || owner.data != nullptr ||
      owner.bytes != 0U) {
    result.message = "invalid factorized-lane R1 preparation state";
    result.context = "prefill_mlp_factorized_lane_r1.prepare";
    return result;
  }
  const std::array<std::filesystem::path, 3U> overlay_path_set = {
  ...
  for (const std::filesystem::path& overlay_path : overlay_path_set) {
    for (const std::filesystem::path& base_path : base_path_set) {
      if (overlay_path == base_path) {
        result.message = "factorized-lane R1 files must not alias any K256 "
                         "base publication file";
        result.context = overlay_path.string();
        return result;
      }
    }
  }
  if (!base.enabled || base.sidecar_kind != PrefillSidecarKind::kA4K256 ||
    ...
    result.message = "factorized-lane R1 requires the complete authenticated "
                     "K256 A4 base";
    result.context = "prefill_mlp_factorized_lane_r1.base";
    return result;
  }
  ...
  if (base_manifest.kind != PrefillSidecarKind::kA4K256 ||
    ...
    result.context = "prefill_mlp_factorized_lane_r1.base_identity";
    return result;
  }
  ...
  PrefillMLPFactorizedLaneManifestResult overlay_manifest_result =
  ...
  if (!overlay_manifest_result) {
    ...
    result.dependency_error =
        overlay_manifest_result.diagnostic.system_error != 0
            ? overlay_manifest_result.diagnostic.system_error
            : static_cast<int>(overlay_manifest_result.diagnostic.code);
    return result;
  }
  const PrefillMLPFactorizedLaneOverlayManifestBinding& overlay_manifest =
  ...
  PrefillMLPFactorizedLaneR1PolicyResult policy_result =
      parse_prefill_mlp_factorized_lane_r1_policy(policy_document,
                                                   overlay_manifest);
  if (!policy_result) {
    ...
    result.dependency_error = policy_result.diagnostic.system_error != 0
                                  ? policy_result.diagnostic.system_error
                                  : static_cast<int>(
                                        policy_result.diagnostic.code);
    return result;
  }
  const PrefillMLPFactorizedLaneR1Policy& policy = *policy_result.value;
  ...
  if (!receipt_result) {
    ...
    result.dependency_error = receipt_result.diagnostic.system_error != 0
                                  ? receipt_result.diagnostic.system_error
                                  : static_cast<int>(
                                        receipt_result.diagnostic.code);
    return result;
  }
  const PrefillMLPFactorizedLaneR1Receipt& receipt = *receipt_result.value;
  ...
  if (result.receipt_sha256.empty() ||
    ...
    result.message = "R1 receipt does not retain the authenticated-ABI-only "
                     "performance-upper-bound contract";
    result.context = paths.receipt.string();
    return result;
  }
  ...
  if (receipt.binding.payload.bytes >
      std::numeric_limits<std::size_t>::max()) {
    result.message = "R1 payload does not fit the host address space";
  ...
  (void)total_bytes;
  if (cuda_status != cudaSuccess) {
    result.message = "cudaMemGetInfo failed before R1 overlay residency";
    result.context = "prefill_mlp_factorized_lane_r1.cudaMemGetInfo_before";
    result.cuda_error = static_cast<int>(cuda_status);
    return result;
  }
  const std::uint64_t free_u64 = static_cast<std::uint64_t>(free_bytes);
  if (receipt.binding.payload.bytes > free_u64 ||
    ...
    result.message = "insufficient device memory for factorized-lane R1 "
                     "overlay residency";
    result.context = "prefill_mlp_factorized_lane_r1.memory_gate";
    return result;
  }
  ...
  if (cuda_status != cudaSuccess) {
    result.message = "cudaMalloc failed for the factorized-lane R1 arena";
    result.context = "prefill_mlp_factorized_lane_r1.cudaMalloc";
    result.cuda_error = static_cast<int>(cuda_status);
    return result;
  }
  constexpr std::size_t kCopyChunkBytes = 32U * 1024U * 1024U;
  ...
  cuda_status = cudaHostAlloc(&staging, staging_bytes, cudaHostAllocDefault);
  if (cuda_status != cudaSuccess) {
    result.message = "cudaHostAlloc failed for factorized-lane R1 staging";
    result.context = "prefill_mlp_factorized_lane_r1.cudaHostAlloc";
    result.cuda_error = static_cast<int>(cuda_status);
    owner.release();
    return result;
  }
  ...
  while (offset < receipt.binding.payload.bytes) {
    ...
    if (!pread_exact_engine(payload_file.descriptor, staging, count, offset,
                            read_error)) {
      result.message = "failed to read the held factorized-lane R1 payload";
      result.context = "prefill_mlp_factorized_lane_r1.payload_copy";
      result.dependency_error = read_error;
      owner.release();
      return result;
    }
    if (!copied_hash.update(staging, count)) {
      result.message = "R1 payload copy SHA-256 length overflowed";
      result.context = "prefill_mlp_factorized_lane_r1.payload_copy_sha256";
      owner.release();
      return result;
    }
    cuda_status = cudaMemcpy(
        ...
        cudaMemcpyHostToDevice);
    if (cuda_status != cudaSuccess) {
      result.message = "factorized-lane R1 payload H2D copy failed";
      result.context = "prefill_mlp_factorized_lane_r1.cudaMemcpy";
      result.cuda_error = static_cast<int>(cuda_status);
      owner.release();
      return result;
    }
  ...
  if (copied_digest != receipt.binding.payload.sha256 ||
    ...
    if (result.message.empty()) {
      result.message = "bytes copied to the R1 arena failed receipt SHA-256";
      result.context =
          "prefill_mlp_factorized_lane_r1.payload_copy_sha256";
    }
    owner.release();
    return result;
  }

  // Rehash the exact same locked descriptor after H2D.  This is deliberately
  // separate from the in-copy digest so a mutation cannot hide behind a
  // matching prefix or a stale pre-copy authentication pass.
  ...
  while (offset < receipt.binding.payload.bytes) {
    ...
    if (!pread_exact_engine(payload_file.descriptor, staging, count, offset,
      ...
      result.message = "R1 payload digest revalidation failed";
      result.context =
          "prefill_mlp_factorized_lane_r1.payload_revalidate_sha256";
      result.dependency_error = read_error;
      owner.release();
      return result;
    }
  ...
  if (verified_hash.finalize().hex() != receipt.binding.payload.sha256 ||
    ...
    if (result.message.empty()) {
      result.message = "R1 payload digest changed after its H2D copy";
      result.context =
          "prefill_mlp_factorized_lane_r1.payload_revalidate_sha256";
    }
    owner.release();
  ...
  if (cuda_status != cudaSuccess ||
      static_cast<std::uint64_t>(free_bytes) <
          minimum_free_bytes_after_prepare) {
    result.message = "R1 residency did not preserve the configured "
                     "free-memory reserve";
    result.context = "prefill_mlp_factorized_lane_r1.cudaMemGetInfo_after";
    result.cuda_error = cuda_status == cudaSuccess
                            ? 0
                            : static_cast<int>(cuda_status);
    owner.release();
  ...
  if (!model_weights.attach_prefill_mlp_factorized_lane_r1_sidecars(
          owner.data, payload_bytes, &overlay_manifest,
          &policy.binding)) {
    result.message = "ModelWeights rejected the authenticated factorized-"
                     "lane R1 inventory";
    result.context = "prefill_mlp_factorized_lane_r1.attach";
    owner.release();
    return result;
  }
...
prepare_sm87_mlp_k512_overlay(
    const std::filesystem::path& model_directory,
    const PrefillMLPK512EnginePaths& paths,
    const ResidentWeights& resident,
    const Sm87A4PrefillPreparation& base,
    const std::uint64_t minimum_free_bytes_after_prepare,
    ModelWeights& model_weights, Sm87MLPK512Overlay& owner) {
  Sm87MLPK512Preparation result;
  if (!paths.requested || owner.data != nullptr || owner.bytes != 0U) {
    result.message = "invalid K512 MLP preparation state";
    result.context = "prefill_mlp_k512.prepare";
    return result;
  }
  ...
  const std::optional<PrefillMLPK512OverlayReceipt> receipt =
      parse_prefill_mlp_k512_overlay_receipt(receipt_document,
                                             receipt_diagnostic);
  if (!receipt.has_value() || !receipt_diagnostic) {
    result.message = receipt_diagnostic.message.empty()
                         ? "strict K512 receipt parse failed"
  ...
  if (!same_mlp_k512_base_binding(receipt->required_base, actual_base)) {
    result.message = "K512 receipt does not bind the loaded K128 base";
    result.context = "prefill_mlp_k512.required_base";
    return result;
  }

  PrefillMLPK512OverlayManifestResult manifest_result =
  ...
  if (receipt->physical_layout != manifest.physical_layout ||
      ...
      receipt->projection_count != manifest.projections.size()) {
    result.message =
        "K512 receipt does not match the rebuilt 192-projection manifest";
    result.context = "prefill_mlp_k512.manifest";
    return result;
  }
  ...
  PrefillMLPK512OverlayPolicyResult policy_result =
      parse_prefill_mlp_k512_overlay_policy(policy_document, manifest);
  if (!policy_result ||
      policy_result.value->policy_sha256 != receipt->policy_sha256 ||
      policy_result.value->policy_bytes != receipt->policy_bytes ||
      !same_mlp_k512_base_binding(policy_result.value->required_base,
                                   actual_base)) {
    result.message = policy_result.diagnostic.message.empty()
                         ? "K512 policy does not match its receipt"
                         : policy_result.diagnostic.message;
  ...
  if (cuda_status != cudaSuccess) {
    result.message = "cudaMemGetInfo failed before K512 residency";
    result.context = "prefill_mlp_k512.cudaMemGetInfo_before";
    result.cuda_error = static_cast<int>(cuda_status);
    return result;
  }
  const std::uint64_t free_u64 = static_cast<std::uint64_t>(free_bytes);
  if (receipt->payload_bytes > free_u64 ||
      minimum_free_bytes_after_prepare > free_u64 - receipt->payload_bytes) {
    result.message = "insufficient device memory for the K512 overlay";
    result.context = "prefill_mlp_k512.memory_gate";
    return result;
  }
  ...
  if (cuda_status != cudaSuccess) {
    result.message = "cudaMalloc failed for the K512 overlay";
    result.context = "prefill_mlp_k512.cudaMalloc";
    result.cuda_error = static_cast<int>(cuda_status);
    return result;
  }
  constexpr std::size_t kCopyChunkBytes = 32U * 1024U * 1024U;
  ...
  if (cuda_status != cudaSuccess) {
    result.message = "cudaHostAlloc failed for K512 staging";
    result.context = "prefill_mlp_k512.cudaHostAlloc";
    result.cuda_error = static_cast<int>(cuda_status);
    owner.release();
    return result;
  }
  ...
  while (offset < receipt->payload_bytes) {
    ...
    if (!pread_exact_engine(payload_fd, staging, count, offset,
                            system_error)) {
      result.message = "failed to read the held K512 payload descriptor";
      result.context = "prefill_mlp_k512.payload_copy";
      result.dependency_error = system_error;
      owner.release();
      return result;
    }
    if (!copied_hash.update(staging, count)) {
      result.message = "K512 payload SHA-256 length overflowed";
      result.context = "prefill_mlp_k512.payload_sha256";
      owner.release();
      return result;
    }
    cuda_status = cudaMemcpy(owner.data + static_cast<std::size_t>(offset),
    ...
    if (cuda_status != cudaSuccess) {
      result.message = "K512 payload H2D copy failed";
      result.context = "prefill_mlp_k512.cudaMemcpy";
      result.cuda_error = static_cast<int>(cuda_status);
      owner.release();
      return result;
    }
  ...
  if (cuda_status != cudaSuccess ||
      static_cast<std::uint64_t>(free_bytes) <
          minimum_free_bytes_after_prepare) {
    result.message = "K512 residency did not preserve the memory reserve";
    result.context = "prefill_mlp_k512.cudaMemGetInfo_after";
    result.cuda_error = cuda_status == cudaSuccess
                            ? 0
                            : static_cast<int>(cuda_status);
    owner.release();
  ...
  if (!model_weights.attach_prefill_mlp_k512_sidecars(
          owner.data, payload_bytes, &manifest, &*policy_result.value)) {
    result.message = "ModelWeights rejected the K512 overlay inventory";
    result.context = "prefill_mlp_k512.attach";
    owner.release();
    return result;
  }
  owner.bytes = payload_bytes;
...
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N128_REGISTER_PIPELINE_ADMISSION)
[[nodiscard]] Sm87MLPK512ProjectionMajorGateUpCanonicalDownPreparation
prepare_sm87_mlp_k512_projection_major_gateup_canonical_down_overlay(
    const std::filesystem::path& model_directory,
    const PrefillMLPK512ProjectionMajorGateUpCanonicalDownEnginePaths& paths,
    const ResidentWeights& resident, const Sm87A4PrefillPreparation& base,
    const std::uint64_t minimum_free_bytes_after_prepare,
    ModelWeights& model_weights, Sm87MLPK512Overlay& owner) {
  Sm87MLPK512ProjectionMajorGateUpCanonicalDownPreparation result;
  if (!paths.requested || owner.data != nullptr || owner.bytes != 0U) {
    result.message =
        "invalid projection-major-GateUp/canonical-Down K512 MLP preparation state";
    result.context =
        "prefill_mlp_k512_projection_major_gateup_canonical_down.prepare";
    return result;
  }
  ...
  if (!base.enabled || base.sidecar_kind != PrefillSidecarKind::kA4K256 ||
      ...
      !resident_matches_pinned_identity(resident)) {
    result.message =
        "projection-major-GateUp/canonical-Down K512 MLP requires the authenticated "
        "K256 A4 base inventory";
    result.context =
        "prefill_mlp_k512_projection_major_gateup_canonical_down.base";
    return result;
  }
  ...
  PrefillMLPK512OverlayDiagnostic receipt_diagnostic;
  const std::optional<PrefillMLPK512ProjectionMajorGateUpCanonicalDownReceipt>
      receipt =
  ...
  if (!receipt.has_value() || !receipt_diagnostic) {
    result.message =
        receipt_diagnostic.message.empty()
            ? "strict projection-major-GateUp/canonical-Down K512 receipt parse failed"
            : receipt_diagnostic.message;
    result.context = receipt_diagnostic.context.empty()
                         ? paths.receipt.string()
                         : receipt_diagnostic.context;
  ...
  if (receipt->receipt_sha256 != core::sha256(receipt_document).hex()) {
    result.message = "projection-major K512 receipt SHA-256 verification failed";
    result.context =
        "prefill_mlp_k512_projection_major_gateup_canonical_down.receipt_sha256";
    return result;
  }
  if (!same_mlp_k512_base_binding(receipt->required_base, actual_base)) {
    result.message =
        "projection-major K512 receipt does not bind the loaded K256 A4 base";
    result.context =
        "prefill_mlp_k512_projection_major_gateup_canonical_down.required_base";
    return result;
  }
  ...
  if (receipt->source_checkpoint_id !=
      ...
      receipt->source_v1.payload_bytes !=
          kPrefillMLPK512OverlayPayloadBytes) {
    result.message =
        "projection-major receipt source-v1 binding does not match the rebuilt "
        "real-model manifest";
    result.context =
        "prefill_mlp_k512_projection_major_gateup_canonical_down.source_v1";
    return result;
  }
  ...
  if (!source_v1_policy_result ||
    ...
    result.message =
        source_v1_policy_result.diagnostic.message.empty()
            ? "source-v1 policy does not match the projection-major K512 receipt"
            : source_v1_policy_result.diagnostic.message;
    result.context = source_v1_policy_result.diagnostic.context.empty()
                         ? paths.policy.string()
                         : source_v1_policy_result.diagnostic.context;
  ...
  source_v1_receipt.projection_count =
      kPrefillMLPK512OverlayProjectionCount;
  PrefillMLPK512ProjectionMajorGateUpCanonicalDownManifestResult projection_major_manifest_result =
      build_prefill_mlp_k512_projection_major_gateup_canonical_down_manifest(
          source_v1_receipt, receipt->source_v1.receipt_sha256);
  if (!projection_major_manifest_result ||
      projection_major_manifest_result.value->physical_layout !=
          receipt->physical_layout ||
      projection_major_manifest_result.value->source_checkpoint_id !=
          receipt->source_checkpoint_id ||
      projection_major_manifest_result.value->source_config_sha256 !=
          receipt->source_config_sha256 ||
      projection_major_manifest_result.value->source_index_sha256 !=
          receipt->source_index_sha256 ||
      projection_major_manifest_result.value->manifest_sha256 !=
          receipt->manifest_sha256 ||
      projection_major_manifest_result.value->payload_bytes != receipt->payload_bytes ||
      projection_major_manifest_result.value->layer_count != receipt->layer_count) {
    result.message =
        projection_major_manifest_result.diagnostic.message.empty()
            ? "projection-major receipt does not match the rebuilt deterministic "
              "manifest"
            : projection_major_manifest_result.diagnostic.message;
    result.context = projection_major_manifest_result.diagnostic.context.empty()
                         ? "prefill_mlp_k512_projection_major_gateup_canonical_down."
                           "manifest"
                         : projection_major_manifest_result.diagnostic.context;
    result.dependency_error =
        static_cast<int>(projection_major_manifest_result.diagnostic.code);
    return result;
  }
  const PrefillMLPK512ProjectionMajorGateUpCanonicalDownManifest& projection_major_manifest =
      *projection_major_manifest_result.value;

  // Materialize all reporting strings before attaching non-owning views.
  // The final attach is the sole runtime-visible publication point.
  result.physical_layout = receipt->physical_layout;
  result.manifest_sha256 = projection_major_manifest.manifest_sha256;
  result.policy_sha256 = source_v1_policy_result.value->policy_sha256;
  result.payload_sha256 = receipt->payload_sha256;
  result.receipt_sha256 = receipt->receipt_sha256;
  result.source_v1_receipt_sha256 = receipt->source_v1.receipt_sha256;
  ...
  if (payload_fd < 0) {
    result.message =
        "failed to open the projection-major K512 payload as a regular file";
    result.context = paths.payload.string();
    result.dependency_error = errno;
    return result;
  }
  ...
  if (::flock(payload_fd, LOCK_SH | LOCK_NB) != 0) {
    result.message = "projection-major K512 payload is locked for publication mutation";
    result.context = paths.payload.string();
    result.dependency_error = errno;
    return result;
  }
  struct stat before {};
  if (::fstat(payload_fd, &before) != 0) {
    result.message = "projection-major K512 payload fstat failed";
    result.context = paths.payload.string();
    result.dependency_error = errno;
    return result;
  }
  if (!S_ISREG(before.st_mode) || before.st_size < 0 ||
      ...
      receipt->payload_bytes != kPrefillMLPK512FragmentNativePayloadBytes ||
      receipt->payload_bytes > std::numeric_limits<std::size_t>::max()) {
    result.message = "projection-major K512 payload identity or byte length is invalid";
    result.context = paths.payload.string();
    return result;
  }
  if ((before.st_mode & (S_IWUSR | S_IWGRP | S_IWOTH)) != 0 ||
      before.st_uid != ::geteuid() || before.st_nlink != 1) {
    result.message = "projection-major K512 payload must be owner-held, read-only, "
                     "and singly linked";
    result.context = paths.payload.string();
    return result;
  }
  ...
  (void)total_bytes;
  if (cuda_status != cudaSuccess) {
    result.message = "cudaMemGetInfo failed before projection-major K512 residency";
    result.context =
        "prefill_mlp_k512_projection_major_gateup_canonical_down.cudaMemGetInfo_before";
    result.cuda_error = static_cast<int>(cuda_status);
    return result;
  }
  const std::uint64_t free_u64 = static_cast<std::uint64_t>(free_bytes);
  if (receipt->payload_bytes > free_u64 ||
      minimum_free_bytes_after_prepare > free_u64 - receipt->payload_bytes) {
    result.message = "insufficient device memory for projection-major K512 residency";
    result.context =
        "prefill_mlp_k512_projection_major_gateup_canonical_down.memory_gate";
    return result;
  }
  ...
  cuda_status = cudaMalloc(reinterpret_cast<void**>(&owner.data),
                           payload_bytes);
  if (cuda_status != cudaSuccess) {
    result.message = "cudaMalloc failed for projection-major K512 overlay";
    result.context =
        "prefill_mlp_k512_projection_major_gateup_canonical_down.cudaMalloc";
    result.cuda_error = static_cast<int>(cuda_status);
    return result;
  }
  constexpr std::size_t kCopyChunkBytes = 32U * 1024U * 1024U;
  ...
  cuda_status = cudaHostAlloc(&staging, staging_bytes, cudaHostAllocDefault);
  if (cuda_status != cudaSuccess) {
    result.message = "cudaHostAlloc failed for projection-major K512 staging";
    result.context =
        "prefill_mlp_k512_projection_major_gateup_canonical_down.cudaHostAlloc";
    result.cuda_error = static_cast<int>(cuda_status);
    owner.release();
    return result;
  }
  ...
  while (offset < receipt->payload_bytes) {
    ...
    if (!pread_exact_engine(payload_fd, staging, count, offset,
                            system_error)) {
      result.message = "failed to read the held projection-major K512 payload";
      result.context =
          "prefill_mlp_k512_projection_major_gateup_canonical_down.payload_copy";
      result.dependency_error = system_error;
      owner.release();
      return result;
    }
    if (!copied_hash.update(staging, count)) {
      result.message = "projection-major K512 payload SHA-256 overflowed";
      result.context =
          "prefill_mlp_k512_projection_major_gateup_canonical_down.payload_sha256";
      owner.release();
      return result;
    }
    cuda_status = cudaMemcpy(owner.data + static_cast<std::size_t>(offset),
                             staging, count, cudaMemcpyHostToDevice);
    if (cuda_status != cudaSuccess) {
      result.message = "projection-major K512 payload H2D copy failed";
      result.context =
          "prefill_mlp_k512_projection_major_gateup_canonical_down.cudaMemcpy";
      result.cuda_error = static_cast<int>(cuda_status);
      owner.release();
      return result;
    }
  ...
  struct stat after {};
  if (::fstat(payload_fd, &after) != 0) {
    result.message = "projection-major K512 payload second fstat failed";
    result.context =
        "prefill_mlp_k512_projection_major_gateup_canonical_down.payload_sha256";
    result.dependency_error = errno;
    owner.release();
  ...
  if (copied_digest != receipt->payload_sha256 ||
      ...
      before.st_ctim.tv_nsec != after.st_ctim.tv_nsec) {
    result.message =
        "projection-major K512 payload changed or failed receipt SHA-256";
    result.context =
        "prefill_mlp_k512_projection_major_gateup_canonical_down.payload_sha256";
    owner.release();
    return result;
  ...
  if (cuda_status != cudaSuccess ||
      static_cast<std::uint64_t>(free_bytes) <
          minimum_free_bytes_after_prepare) {
    result.message =
        "projection-major K512 residency did not preserve memory reserve";
    result.context =
        "prefill_mlp_k512_projection_major_gateup_canonical_down.cudaMemGetInfo_after";
    result.cuda_error = cuda_status == cudaSuccess
                            ? 0
                            : static_cast<int>(cuda_status);
    owner.release();
  ...
  if (!model_weights
           .attach_prefill_mlp_k512_projection_major_gateup_canonical_down_sidecars(
               owner.data, payload_bytes, &projection_major_manifest,
               &source_v1_manifest, &*source_v1_policy_result.value)) {
    result.message =
        "ModelWeights rejected the projection-major K512 MLP inventory";
    result.context =
        "prefill_mlp_k512_projection_major_gateup_canonical_down.attach";
    owner.release();
    return result;
  }
  owner.bytes = payload_bytes;
  result.enabled = true;
  result.layers = static_cast<std::size_t>(projection_major_manifest.layer_count);
  result.bytes = receipt->payload_bytes;
  return result;
}
  ...
  [[nodiscard]] static BuildResult build(
...
#endif
    PrefillMLPK512EnginePaths prefill_mlp_k512_paths;
    std::string prefill_mlp_k512_path_error;
    if (!resolve_prefill_mlp_k512_engine_paths(
            options, prefill_mlp_k512_paths,
            prefill_mlp_k512_path_error)) {
      result.diagnostic = engine_diagnostic(
          ReferenceEngineError::kInvalidArgument,
          "prefill_mlp_k512_options", prefill_mlp_k512_path_error);
      return result;
    }
    if (prefill_mlp_k512_paths.requested && !prefill_a4_paths.requested) {
      result.diagnostic = engine_diagnostic(
          ReferenceEngineError::kInvalidArgument,
    ...
    if (prefill_mlp_k512_environment_enabled() &&
      ...
      result.diagnostic = engine_diagnostic(
          ReferenceEngineError::kInvalidArgument,
          "prefill_mlp_k512_options",
          "the K512 MLP runtime selector requires an authenticated overlay");
      return result;
    }
#if !defined(Q3X_ENABLE_A4W4_MLP_K512_ADMISSION)
    ...
    PrefillMLPK512ProjectionMajorGateUpCanonicalDownEnginePaths
        prefill_mlp_k512_projection_major_paths;
    std::string prefill_mlp_k512_projection_major_path_error;
    if (!resolve_prefill_mlp_k512_projection_major_gateup_canonical_down_engine_paths(
            options, prefill_mlp_k512_projection_major_paths,
            prefill_mlp_k512_projection_major_path_error)) {
      result.diagnostic = engine_diagnostic(
          ReferenceEngineError::kInvalidArgument,
    ...
    const bool prefill_mlp_k512_projection_major_selected =
        prefill_mlp_k512_projection_major_gateup_canonical_down_environment_enabled();
    PrefillMLPFactorizedLaneR1EnginePaths
        prefill_mlp_factorized_lane_r1_paths;
    std::string prefill_mlp_factorized_lane_r1_path_error;
    if (!resolve_prefill_mlp_factorized_lane_r1_engine_paths(
            options, prefill_mlp_factorized_lane_r1_paths,
            prefill_mlp_factorized_lane_r1_path_error)) {
      result.diagnostic = engine_diagnostic(
          ReferenceEngineError::kInvalidArgument,
    ...
    if (prefill_mlp_factorized_lane_r1_paths.requested &&
      ...
      result.diagnostic = engine_diagnostic(
          ReferenceEngineError::kInvalidArgument,
          "prefill_mlp_factorized_lane_r1_options",
          "the factorized-lane R1 triplet requires the complete "
          "authenticated K256 base and its runtime master selector");
      return result;
    }
    ...
    if (prefill_mlp_k512_projection_major_paths.requested &&
        !prefill_a4_paths.requested) {
      result.diagnostic = engine_diagnostic(
          ReferenceEngineError::kInvalidArgument,
          "prefill_mlp_k512_projection_major_gateup_canonical_down_options",
          "the projection-major-GateUp/canonical-Down K512 MLP overlay "
          "requires an explicit complete K256 A4 base");
      return result;
    }
    if (prefill_mlp_k512_projection_major_selected &&
        !prefill_mlp_k512_projection_major_paths.requested) {
      result.diagnostic = engine_diagnostic(
          ReferenceEngineError::kInvalidArgument,
          "prefill_mlp_k512_projection_major_gateup_canonical_down_options",
          "the projection-major-GateUp/canonical-Down runtime selector "
          "requires its authenticated publication");
      return result;
    }
    if (prefill_mlp_k512_projection_major_paths.requested &&
      ...
      result.diagnostic = engine_diagnostic(
          ReferenceEngineError::kInvalidArgument,
          "prefill_mlp_k512_projection_major_gateup_canonical_down_options",
          "the authenticated projection-major-GateUp/canonical-Down K512 "
          "MLP overlay requires its runtime master selector");
      return result;
    }
    ...
    if (prefill_mlp_factorized_lane_r1_selected &&
      ...
      result.diagnostic = engine_diagnostic(
          ReferenceEngineError::kInvalidArgument,
          "prefill_mlp_factorized_lane_r1_options",
          "the factorized-lane R1 route is mutually exclusive with the "
          "structural K256 MLP package and every K512 MLP publication or "
          "runtime master");
      return result;
    }
    if (prefill_mlp_factorized_lane_r4_selected &&
    ...
    if (selected_mlp_k512_layouts > 1U) {
      result.diagnostic = engine_diagnostic(
          ...
          "MLP K512 v1, fragment-native v2, paired-GateUp/canonical-Down, "
          "and projection-major-GateUp/canonical-Down publications/"
          "selectors are strictly mutually exclusive");
      return result;
    }
#if !defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R1_ADMISSION)
    if (prefill_mlp_factorized_lane_r1_paths.requested ||
    ...
    if (prefill_mlp_k512_projection_major_paths.requested ||
      ...
      result.diagnostic = engine_diagnostic(
          ReferenceEngineError::kInvalidArgument,
          "prefill_mlp_k512_projection_major_gateup_canonical_down_options",
          "this binary does not contain the M64N128 register-pipeline "
          "GateUp admission required by the projection-major K512 MLP "
          "publication");
      return result;
    }
#endif
    ...
    try {
      ...
      if (options.projection_backend ==
...
#if defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R1_ADMISSION)
        if (prefill_mlp_factorized_lane_r1_paths.requested) {
          const Clock::time_point r1_begin = Clock::now();
          const Sm87MLPFactorizedLaneR1Preparation preparation =
              prepare_sm87_mlp_factorized_lane_r1_overlay(
                  model_directory, prefill_mlp_factorized_lane_r1_paths,
                  prefill_a4_paths, *impl->resident_weights,
                  prefill_a4_preparation,
                  request_options.min_free_bytes_after_create,
                  *impl->model_weights,
                  impl->prefill_mlp_factorized_lane_r1_overlay);
          impl->load.prefill_mlp_factorized_lane_r1_overlay_milliseconds =
              elapsed_milliseconds(r1_begin);
          if (!preparation.enabled ||
            ...
            result.diagnostic = engine_diagnostic(
                ReferenceEngineError::kRunnerFactoryFailure,
                "prefill_mlp_factorized_lane_r1_prepare",
                preparation.message.empty()
                    ? "the authenticated factorized-lane R1 overlay did not "
                      "attach all 64 MLP layers"
                    : preparation.message,
                preparation.context);
            result.diagnostic.cuda_error = preparation.cuda_error;
            result.diagnostic.dependency_error =
          ...
          impl->load.prefill_mlp_factorized_lane_r1_overlay_enabled = true;
          impl->load.prefill_mlp_factorized_lane_r1_overlay_layers =
              preparation.layers;
          impl->load.prefill_mlp_factorized_lane_r1_overlay_bytes =
              preparation.bytes;
          impl->load.prefill_mlp_factorized_lane_r1_overlay_copy_chunks =
              preparation.copy_chunks;
          impl->load.prefill_mlp_factorized_lane_r1_overlay_layout =
              preparation.physical_layout;
          impl->load.prefill_mlp_factorized_lane_r1_overlay_manifest_sha256 =
              preparation.manifest_sha256;
          impl->load.prefill_mlp_factorized_lane_r1_overlay_policy_sha256 =
              preparation.policy_sha256;
          impl->load.prefill_mlp_factorized_lane_r1_overlay_payload_sha256 =
              preparation.payload_sha256;
          impl->load.prefill_mlp_factorized_lane_r1_overlay_receipt_sha256 =
        ...
        if (prefill_mlp_k512_paths.requested) {
          const Clock::time_point k512_begin = Clock::now();
          const Sm87MLPK512Preparation k512_preparation =
              prepare_sm87_mlp_k512_overlay(
                  model_directory, prefill_mlp_k512_paths,
                  *impl->resident_weights, prefill_a4_preparation,
                  request_options.min_free_bytes_after_create,
                  *impl->model_weights, impl->prefill_mlp_k512_overlay);
          impl->load.prefill_mlp_k512_overlay_milliseconds =
              elapsed_milliseconds(k512_begin);
          if (!k512_preparation.enabled ||
              k512_preparation.projections !=
                  kPrefillMLPK512OverlayProjectionCount ||
              k512_preparation.bytes != kPrefillMLPK512OverlayPayloadBytes) {
            result.diagnostic = engine_diagnostic(
                ReferenceEngineError::kRunnerFactoryFailure,
                "prefill_mlp_k512_prepare",
                k512_preparation.message.empty()
                    ? "the authenticated K512 overlay did not attach all "
                      "192 MLP projections"
                    : k512_preparation.message,
          ...
          impl->load.prefill_mlp_k512_overlay_enabled = true;
          impl->load.prefill_mlp_k512_overlay_projections =
              k512_preparation.projections;
          impl->load.prefill_mlp_k512_overlay_bytes =
              k512_preparation.bytes;
          impl->load.prefill_mlp_k512_overlay_copy_chunks =
              k512_preparation.copy_chunks;
          impl->load.prefill_mlp_k512_overlay_layout =
              k512_preparation.physical_layout;
          impl->load.prefill_mlp_k512_overlay_manifest_sha256 =
              k512_preparation.manifest_sha256;
          impl->load.prefill_mlp_k512_overlay_policy_sha256 =
              k512_preparation.policy_sha256;
...
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N128_REGISTER_PIPELINE_ADMISSION)
        if (prefill_mlp_k512_projection_major_paths.requested) {
          const Clock::time_point projection_major_begin = Clock::now();
          const Sm87MLPK512ProjectionMajorGateUpCanonicalDownPreparation
              preparation =
                  prepare_sm87_mlp_k512_projection_major_gateup_canonical_down_overlay(
                      model_directory,
                      prefill_mlp_k512_projection_major_paths,
          ...
          if (!preparation.enabled ||
            ...
            result.diagnostic = engine_diagnostic(
                ReferenceEngineError::kRunnerFactoryFailure,
                "prefill_mlp_k512_projection_major_gateup_canonical_down_prepare",
                preparation.message.empty()
                    ? "the authenticated projection-major-GateUp/"
                      "canonical-Down K512 overlay did not attach all 64 "
                      "MLP layers"
                    : preparation.message,
                preparation.context);
            result.diagnostic.cuda_error = preparation.cuda_error;
...
ReferenceOneShotResult generate_reference(
  ...
  a4_preflight_options.prefill_attention_o_k512_receipt_path =
      options.prefill_attention_o_k512_receipt_path;
  a4_preflight_options.prefill_mlp_k512_payload_path =
      options.prefill_mlp_k512_payload_path;
  a4_preflight_options.prefill_mlp_k512_policy_path =
  ...
  a4_preflight_options
      ...
      options
          .prefill_mlp_k512_projection_major_gateup_canonical_down_receipt_path;
  a4_preflight_options.prefill_mlp_factorized_lane_r1_payload_path =
      options.prefill_mlp_factorized_lane_r1_payload_path;
  a4_preflight_options.prefill_mlp_factorized_lane_r1_policy_path =
...
#endif
  PrefillMLPK512EnginePaths mlp_k512_preflight_paths;
  std::string mlp_k512_preflight_error;
  if (!resolve_prefill_mlp_k512_engine_paths(
          a4_preflight_options, mlp_k512_preflight_paths,
          mlp_k512_preflight_error) ||
    ...
    result.diagnostic = engine_diagnostic(
        ReferenceEngineError::kInvalidArgument, "one_shot_options",
        mlp_k512_preflight_error.empty()
            ? "K512 MLP requires its authenticated overlay and the explicit "
              "K128 A4 base"
            : mlp_k512_preflight_error);
    return result;
  }
  ...
  PrefillMLPK512ProjectionMajorGateUpCanonicalDownEnginePaths
      mlp_k512_projection_major_preflight_paths;
  std::string mlp_k512_projection_major_preflight_error;
  if (!resolve_prefill_mlp_k512_projection_major_gateup_canonical_down_engine_paths(
          a4_preflight_options,
          mlp_k512_projection_major_preflight_paths,
          mlp_k512_projection_major_preflight_error) ||
    ...
    result.diagnostic = engine_diagnostic(
        ReferenceEngineError::kInvalidArgument, "one_shot_options",
        mlp_k512_projection_major_preflight_error.empty()
            ? "projection-major-GateUp/canonical-Down K512 MLP requires "
              "its authenticated publication and the explicit complete "
              "K256 A4 base"
            : mlp_k512_projection_major_preflight_error);
    return result;
  }
  PrefillMLPFactorizedLaneR1EnginePaths
      mlp_factorized_lane_r1_preflight_paths;
  ...
  if (!resolve_prefill_mlp_factorized_lane_r1_engine_paths(
    ...
    result.diagnostic = engine_diagnostic(
        ReferenceEngineError::kInvalidArgument, "one_shot_options",
        mlp_factorized_lane_r1_preflight_error.empty()
  ...
  if (mlp_factorized_lane_r1_preflight_selected &&
    ...
    result.diagnostic = engine_diagnostic(
        ReferenceEngineError::kInvalidArgument, "one_shot_options",
        "factorized-lane R1 is mutually exclusive with the structural K256 "
...
#if !defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R1_ADMISSION)
  if (mlp_factorized_lane_r1_preflight_paths.requested ||
      mlp_factorized_lane_r1_preflight_selected) {
    result.diagnostic = engine_diagnostic(
        ReferenceEngineError::kInvalidArgument, "one_shot_options",
        "this binary does not contain the factorized-lane R1 admission");
...
#endif
#if !defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N128_REGISTER_PIPELINE_ADMISSION)
  if (mlp_k512_projection_major_preflight_paths.requested ||
      prefill_mlp_k512_projection_major_gateup_canonical_down_environment_enabled() ||
      prefill_gateup_k512_m64n128_register_pipeline_environment_enabled()) {
    result.diagnostic = engine_diagnostic(
        ReferenceEngineError::kInvalidArgument, "one_shot_options",
