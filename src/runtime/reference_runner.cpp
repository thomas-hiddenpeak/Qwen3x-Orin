...
exchange_selector_exact_persistent_attention_v1_route_receipt_for_test(
    const SelectorExactPersistentAttentionV1RouteReceipt receipt) noexcept {
  return std::exchange(
      g_selector_exact_persistent_attention_v1_route_receipt_for_test,
      receipt);
}
...
int launch_selector_exact_persistent_attention_v1_cuda(
    ...
    SelectorExactPersistentAttentionV1LaunchReceipt* const receipt,
  ...
  *receipt = {};
  const SelectorExactPersistentAttentionV1Plan plan =
      make_selector_exact_persistent_attention_v1_plan(first_position,
                                                       token_count);
  receipt->plan = plan;
  if (!plan.valid || query == nullptr || key_cache == nullptr ||
      value_cache == nullptr || gate == nullptr || output == nullptr) {
    return static_cast<int>(cudaErrorInvalidValue);
  }
  ...
  for (std::size_t index = 0U; index < plan.physical_submission_count;
       ++index) {
    const SelectorExactPersistentAttentionV1PhysicalSubmissionReceipt& span =
        plan.physical_submissions[index];
    if (span.first_position < plan.first_position ||
        (span.tactic !=
    ...
    const std::size_t element_offset =
        (static_cast<std::size_t>(span.first_position) - plan.first_position) *
        kQueryElements;
    const int status =
        launch_bulk_causal_gqa_sigmoid_gate_24_4_256_fixed_cuda(
            query + element_offset, key_cache, value_cache,
            gate + element_offset, span.first_position, span.token_count,
            output + element_offset, cuda_stream);
    ...
    receipt->physical_submissions[receipt->physical_submission_count++] = span;
#endif
  }
#if defined(Q3X_ENABLE_SELECTOR_EXACT_PERSISTENT_ATTENTION_V1_P40_TESTING)
...
ReferenceRunner::prefill_whole_request_layer_major_core(
  ...
  if (g_selector_exact_persistent_attention_v1_for_test) {
    const auto candidate_plan = reference_runner_detail::
        make_selector_exact_persistent_attention_v1_plan(
            immutable_topology.first_position, token_count);
    if (executor != LayerMajorLayerExecutor::kOperatorPanel ||
        projection_tactic !=
            LayerMajorPrefillProjectionTactic::kExactSegmentedC512 ||
        full_attention_tactic !=
            LayerMajorPrefillFullAttentionTactic::kExactSegmentedC512 ||
        immutable_topology.panel_count != 1U ||
        immutable_topology.panels[0U].first_position != 0U ||
        immutable_topology.panels[0U].token_count != token_count ||
        !reference_runner_detail::
      ...
      return fail_whole_request_prefill(runner_status(
          ReferenceRunnerError::kInvalidStepOptions,
          "whole_request_prefill_selector_exact_persistent_attention_v1_"
          "admission"));
    }
  }
#endif
  ...
  for (std::size_t layer = 0U; layer < kReferenceDecoderLayerCount;
    ...
    defined(Q3X_ENABLE_SELECTOR_EXACT_PERSISTENT_ATTENTION_V1_P40_TESTING)
            ...
            // It records all exact GenericQT2 spans there, so this legacy
            // panel accounting must not count them a second time.
          } else {
#else
          } else {
#endif
  ...
  if (selector_exact_persistent_attention_v1_full_attention_layer_hits != 0U) {
    ...
    if (selector_exact_persistent_attention_v1_full_attention_layer_hits !=
        ...
        selector_receipt.arithmetic_spans !=
            kLayerMajorPrefillFullLayerCount *
                expected_attention_plan.arithmetic_span_count ||
        selector_receipt.group_q64_submissions !=
            kLayerMajorPrefillFullLayerCount *
                expected_attention_plan.group_q64_span_count ||
        selector_receipt.generic_qt2_spans !=
            kLayerMajorPrefillFullLayerCount *
                expected_attention_plan.generic_qt2_span_count ||
        selector_receipt.generic_q8_suffix_submissions != 0U ||
        selector_receipt.fallback_submissions != 0U ||
        selector_receipt.persistent_ctas != 0U ||
        ...
        !reference_runner_detail::
            selector_exact_persistent_attention_v1_physical_receipt_matches_plan(
                expected_attention_plan,
                selector_receipt.physical_submission_count_per_panel,
                selector_receipt.physical_submissions) ||
    ...
    for (std::size_t index = 0U;
      ...
      if (layer_receipt.layer != 3U + 4U * index ||
          !reference_runner_detail::
              selector_exact_persistent_attention_v1_physical_receipt_matches_plan(
                  expected_attention_plan,
                  layer_receipt.physical_submission_count, spans)) {
        return fail_whole_request_prefill(runner_status(
            ReferenceRunnerError::kRouteEvidenceFailure,
...
ReferenceRunner::enqueue_prefill_layer_panel(
  ...
  if (expected == model::LayerType::kLinearAttention) {
    ...
    if (selector_exact_persistent_attention_v1 &&
      ...
      return fail_enqueue(runner_status(
          ReferenceRunnerError::kInvalidStepOptions,
          "prefill_operator_panel_selector_exact_persistent_attention_v1_"
          "tactic",
          layer));
    }
    if (selector_exact_persistent_attention_v1) {
      ...
      if (!check_cuda(
              reference_runner_detail::
                  launch_selector_exact_persistent_attention_v1_cuda(
                      ...
                      token_count, core_output, &candidate_receipt, stream_),
              "prefill_operator_panel_selector_exact_persistent_attention_"
              "v1",
              layer)) {
        return fail_enqueue(launch_failure);
      }
