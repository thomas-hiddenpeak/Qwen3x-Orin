...
#include "q3x/kernels/sm87_fp8_prefill_supermatrix.h"
    ...
    defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R4_ADMISSION)
...
#include "q3x/kernels/sm87_a4w4_gateup_down_k256_m128n256_pairfeed.h"
#endif
...
[[nodiscard]] bool
a4w4_gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_environment_enabled()
    noexcept {
  if (optimized_prefill_dispatch_disabled()) {
    return false;
  }
...
thread_local std::size_t
    g_a4w4_gateup_down_k512_edge_m64n128_k256_alternating_admission_hits =
        0U;
thread_local bool
    g_enable_a4w4_gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_admission =
        a4w4_gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_environment_enabled();
thread_local std::size_t
    g_a4w4_gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_admission_hits =
        0U;
#endif
    ...
    defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_16WARP_PAIRRING_ADMISSION)
[[nodiscard]] bool
a4w4_mlp_k256_m128n256_pairfeed_package_environment_enabled() noexcept {
  if (optimized_prefill_dispatch_disabled()) {
    return false;
  }
  const char* const value = std::getenv(
...
thread_local bool
    g_enable_a4w4_mlp_k256_m128n256_pairfeed_package_admission =
        a4w4_mlp_k256_m128n256_pairfeed_package_environment_enabled();
thread_local std::size_t
    g_a4w4_mlp_k256_m128n256_pairfeed_package_admission_hits = 0U;
#endif

#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M32N512_OWNER_ADMISSION)
[[nodiscard]] bool
a4w4_gateup_down_k512_edge_m32n512_owner_environment_enabled() noexcept {
  if (optimized_prefill_dispatch_disabled()) {
    return false;
  }
  const char* const value = std::getenv(
...
thread_local bool
    g_enable_a4w4_gateup_down_k512_edge_m32n512_owner_admission =
        a4w4_gateup_down_k512_edge_m32n512_owner_environment_enabled();
#endif

#if defined(Q3X_ENABLE_A4W4_MLP_K512_SHAPE_SEPARATED_MARLIN_PACKAGE_ADMISSION)
[[nodiscard]] bool
a4w4_mlp_k512_shape_separated_marlin_package_environment_enabled() noexcept {
  if (optimized_prefill_dispatch_disabled()) {
    return false;
  }
  const char* const value = std::getenv(
...
thread_local bool
    g_enable_a4w4_mlp_k512_shape_separated_marlin_package_admission =
        a4w4_mlp_k512_shape_separated_marlin_package_environment_enabled();
#endif

#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M128N128_PROJECTION_SERIAL_ADMISSION)
[[nodiscard]] bool
a4w4_gateup_k512_m128n128_projection_serial_environment_enabled() noexcept {
  if (optimized_prefill_dispatch_disabled()) {
    return false;
  }
  const char* const value = std::getenv(
...
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M128N512_FUSED_QUANTIZE_ADMISSION)
[[nodiscard]] bool
a4w4_gateup_k512_m128n512_fused_quantize_environment_enabled() noexcept {
  if (optimized_prefill_dispatch_disabled()) {
    return false;
  }
  const char* const value = std::getenv(
...
thread_local bool
    g_enable_a4w4_gateup_k512_m128n512_fused_quantize_admission =
        a4w4_gateup_k512_m128n512_fused_quantize_environment_enabled();
thread_local std::size_t
    g_a4w4_gateup_k512_m128n512_fused_quantize_admission_hits = 0U;
#endif

#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N128_REGISTER_PIPELINE_ADMISSION)
[[nodiscard]] bool
a4w4_mlp_k512_projection_major_gateup_canonical_down_environment_enabled()
    noexcept {
  if (optimized_prefill_dispatch_disabled()) {
    return false;
  }
...
[[nodiscard]] bool
a4w4_gateup_k512_m64n128_register_pipeline_environment_enabled() noexcept {
  if (optimized_prefill_dispatch_disabled()) {
    return false;
  }
  const char* const value = std::getenv(
...
thread_local bool
    g_enable_a4w4_mlp_k512_projection_major_gateup_canonical_down_admission =
        a4w4_mlp_k512_projection_major_gateup_canonical_down_environment_enabled();
thread_local bool
    g_enable_a4w4_gateup_k512_m64n128_register_pipeline_admission =
        a4w4_gateup_k512_m64n128_register_pipeline_environment_enabled();
thread_local std::size_t
    g_a4w4_gateup_k512_m64n128_register_pipeline_admission_hits = 0U;
#endif
...
[[nodiscard]] bool
a4w4_gateup_k512_m64n8_paired_warp_register_pipeline_environment_enabled()
    noexcept {
  if (optimized_prefill_dispatch_disabled()) {
    return false;
  }
...
[[nodiscard]] bool
a4w4_down_k512_m128n128_16warp_pairring_environment_enabled() noexcept {
  if (optimized_prefill_dispatch_disabled()) {
    return false;
  }
  const char* const value = std::getenv(
...
[[nodiscard]] bool
a4w4_down_k512_m128n128_16warp_pairring_l2_macro4x4_environment_enabled()
    noexcept {
  if (optimized_prefill_dispatch_disabled()) {
    return false;
  }
...
thread_local bool
    g_enable_a4w4_gateup_down_k512_edge_m128n512_paired_ldmatrix_admission =
        a4w4_gateup_down_k512_edge_m128n512_paired_ldmatrix_environment_enabled();
thread_local bool
    g_enable_a4w4_gateup_k512_m64n8_paired_warp_register_pipeline_admission =
        a4w4_gateup_k512_m64n8_paired_warp_register_pipeline_environment_enabled();
thread_local bool
...
thread_local bool
    g_enable_a4w4_down_k512_m128n128_16warp_pairring_admission =
        a4w4_down_k512_m128n128_16warp_pairring_environment_enabled();
thread_local bool
    g_enable_a4w4_down_k512_m128n128_16warp_pairring_l2_macro4x4_admission =
        a4w4_down_k512_m128n128_16warp_pairring_l2_macro4x4_environment_enabled();
thread_local std::size_t
    g_a4w4_gateup_down_k512_edge_m128n512_paired_ldmatrix_admission_hits =
        0U;
thread_local std::size_t
    g_a4w4_gateup_k512_m64n8_paired_warp_register_pipeline_admission_hits =
        0U;
thread_local std::size_t
    g_a4w4_down_k512_m128n128_ldmatrix_pairring_admission_hits = 0U;
thread_local std::size_t
    g_a4w4_down_k512_m128n128_16warp_pairring_admission_hits = 0U;

#if defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_16WARP_PAIRRING_ADMISSION)
[[nodiscard]] int launch_selected_a4w4_down_k512_m128n128_16warp_pairring(
...
#if defined(Q3X_ENABLE_A4W4_MLP_K512_SHAPE_SEPARATED_MARLIN_PACKAGE_ADMISSION)
  if (g_enable_a4w4_mlp_k512_shape_separated_marlin_package_admission) {
    return kernels::
        launch_sm87_a4w4_down_k512_m64n256_16warp_pairring_bf16_cuda(
            ...
            output_capacity_elements, cuda_stream);
  }
#endif
  if (g_enable_a4w4_down_k512_m128n128_16warp_pairring_l2_macro4x4_admission) {
    return kernels::
...
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
thread_local reference_runner_detail::A4W4AttentionSupermatrixAdmissionHits
    g_a4w4_attention_factorized_lane_r1_admission_hits{};
#endif
...
[[nodiscard]] reference_runner_detail::
    A4W4ProjectionMajorGateUpCanonicalDownSelectorQuery
a4w4_projection_major_gateup_canonical_down_selector_query(
    const bool projection_span) noexcept {
  reference_runner_detail::
      A4W4ProjectionMajorGateUpCanonicalDownSelectorQuery query;
  ...
  query.down_requested =
      query.master_requested &&
      g_enable_a4w4_down_k512_m128n128_16warp_pairring_admission;
  query.legacy_publication_requested =
...
[[nodiscard]] reference_runner_detail::
    A4W4DownK512M128N128Pairring16V1SelectorQuery
a4w4_down_k512_m128n128_16warp_pairring_v1_selector_query(
    const bool projection_span) noexcept {
  reference_runner_detail::
      A4W4DownK512M128N128Pairring16V1SelectorQuery query;
  query.requested =
      (g_enable_a4w4_down_k512_m128n128_16warp_pairring_admission ||
       g_enable_a4w4_down_k512_m128n128_16warp_pairring_l2_macro4x4_admission)
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N128_REGISTER_PIPELINE_ADMISSION)
  ...
  query.hybrid_requested =
      ...
      g_enable_a4w4_mlp_k512_projection_major_gateup_canonical_down_admission;
#endif
  query.incumbent_pairring_requested =
      g_enable_a4w4_down_k512_m128n128_ldmatrix_pairring_admission;
  query.conflicting_down_requested =
      g_enable_a4w4_down_m128_stage_major_admission ||
      (g_enable_a4w4_down_k512_m128n128_16warp_pairring_l2_macro4x4_admission &&
...
thread_local std::size_t
    g_gdn_factorized_lane_r1_direct_pack_launch_hits = 0U;
#if defined(Q3X_ENABLE_GDN_STATE_O_BV64_FUSION_ADMISSION)
[[nodiscard]] bool gdn_state_o_bv64_fusion_environment_enabled() noexcept {
  const char* const value = std::getenv(
      "Q3X_RUN_GDN_STATE_O_BV64_FUSION_ADMISSION");
  return value != nullptr && std::strcmp(value, "1") == 0;
}

thread_local bool g_enable_gdn_state_o_bv64_fusion_admission =
    gdn_state_o_bv64_fusion_environment_enabled();
thread_local std::size_t g_gdn_state_o_bv64_fused_launch_hits = 0U;
thread_local std::size_t
    g_gdn_state_o_bv64_fused_logical_token_hits = 0U;
#endif
...
#if defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R1_ADMISSION)
[[nodiscard]] bool valid_factorized_lane_r1_projection(
    const PrefillMLPFactorizedLaneLinearSidecarView& view,
    const std::size_t output_size,
    const std::size_t input_size) noexcept {
  return view.attached() && view.output_size == output_size &&
         view.input_size == input_size && view.lane_count == 1U &&
         view.packed_weight_capacity_bytes == output_size * input_size / 2U &&
         view.lane_scale_capacity_elements == output_size &&
         view.inverse_alpha_capacity_elements == input_size &&
         std::isfinite(view.activation_clip_ratio) &&
         view.activation_clip_ratio > 0.0F &&
         view.activation_clip_ratio <= 1.0F &&
         reinterpret_cast<std::uintptr_t>(view.packed_weight) % 16U == 0U &&
         reinterpret_cast<std::uintptr_t>(view.lane_scales) % 16U == 0U &&
         reinterpret_cast<std::uintptr_t>(view.inverse_alpha) %
                 alignof(float) ==
             0U;
}

// R1 is published only as one complete 64-layer transaction.  Rechecking the
// device views here makes the runner factory an independent fail-closed
// admission boundary and, in particular, proves that one Gate activation
// quantization can feed both paired projections.
[[nodiscard]] bool complete_factorized_lane_r1_mlp_attached(
    const ModelWeights& weights) noexcept {
  std::size_t layer_count = 0U;
  for (std::size_t layer = 0U; layer < kReferenceDecoderLayerCount; ++layer) {
    const PrefillMLPFactorizedLaneLayerView& mlp =
        weights.layer(layer).prefill_mlp_factorized_lane_r1;
    if (!mlp.attached() ||
        !valid_factorized_lane_r1_projection(
            mlp.gate, kReferenceIntermediateSize, kReferenceHiddenSize) ||
        !valid_factorized_lane_r1_projection(
            mlp.up, kReferenceIntermediateSize, kReferenceHiddenSize) ||
        !valid_factorized_lane_r1_projection(
            mlp.down, kReferenceHiddenSize, kReferenceIntermediateSize) ||
        mlp.gate.inverse_alpha_capacity_elements !=
            mlp.up.inverse_alpha_capacity_elements ||
        mlp.gate.activation_clip_ratio !=
            mlp.up.activation_clip_ratio) {
      return false;
    }
    ++layer_count;
  }
...
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
[[nodiscard]] bool valid_attention_factorized_lane_r1_projection(
    const PrefillAttentionFactorizedLaneR1LinearSidecarView& view,
    const std::size_t output_size,
    const std::size_t input_size) noexcept {
  return view.attached() && view.output_size == output_size &&
         view.input_size == input_size && view.lane_count == 1U &&
...
[[nodiscard]] bool complete_attention_factorized_lane_r1_attached(
    const ModelWeights& weights) noexcept {
  std::size_t projection_count = 0U;
  for (std::size_t layer = 0U; layer < kReferenceDecoderLayerCount; ++layer) {
    const auto& attention =
        weights.layer(layer).prefill_attention_factorized_lane_r1;
    const model::LayerType expected =
        reference_runner_detail::expected_reference_layer_type(layer);
    if (expected == model::LayerType::kLinearAttention) {
      if (!attention.linear_attached() ||
          !valid_attention_factorized_lane_r1_projection(
              attention.linear_qkv, kLinearQkvElements,
              kReferenceHiddenSize) ||
          !valid_attention_factorized_lane_r1_projection(
              attention.linear_z, kLinearValueElements,
              kReferenceHiddenSize) ||
          !valid_attention_factorized_lane_r1_projection(
              attention.linear_o, kReferenceHiddenSize,
              kLinearValueElements)) {
        return false;
      }
      projection_count += 3U;
    } else if (expected == model::LayerType::kFullAttention) {
      if (!attention.full_attached() ||
          !valid_attention_factorized_lane_r1_projection(
              attention.full_q, kFullQGateElements, kReferenceHiddenSize) ||
          !valid_attention_factorized_lane_r1_projection(
              attention.full_k, kFullKvElements, kReferenceHiddenSize) ||
          !valid_attention_factorized_lane_r1_projection(
              attention.full_v, kFullKvElements, kReferenceHiddenSize) ||
          !valid_attention_factorized_lane_r1_projection(
              attention.full_o, kReferenceHiddenSize,
              kFullQueryElements)) {
        return false;
      }
      projection_count += 4U;
  ...
  return projection_count == 208U;
}
#endif

#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
[[nodiscard]] bool prefill_r1_projection_plane_v2_range_within_payload(
    const PrefillR1ProjectionPlaneV2AttachmentView& attachment,
    const void* const pointer, const std::size_t bytes) noexcept {
  if (pointer == nullptr || bytes == 0U || !attachment.attached()) {
    return false;
  }
  const std::uintptr_t begin =
      reinterpret_cast<std::uintptr_t>(attachment.payload);
  ...
  const std::uintptr_t end = begin + attachment.payload_bytes;
  return address >= begin && address + bytes <= end;
}

[[nodiscard]] bool valid_prefill_r1_projection_plane_v2_logical(
    const PrefillR1ProjectionPlaneV2AttachmentView& attachment,
    const PrefillR1ProjectionPlaneV2LogicalCalibrationView& logical,
    const PrefillR1ProjectionPlaneV2LogicalIdentity identity,
    const std::size_t output_size, const std::size_t input_size,
    const std::uint32_t physical_ordinal,
    std::array<bool,
               kPrefillR1ProjectionPlaneV2ViewLogicalProjectionCount>&
        logical_ordinals) noexcept {
  if (!logical.attached() || logical.identity != identity ||
      logical.output_size != output_size || logical.input_size != input_size ||
      logical.physical_ordinal != physical_ordinal ||
      logical.logical_ordinal >= logical_ordinals.size() ||
      logical_ordinals[logical.logical_ordinal] ||
      reinterpret_cast<std::uintptr_t>(logical.inverse_alpha) %
              alignof(float) !=
          0U ||
      !prefill_r1_projection_plane_v2_range_within_payload(
          attachment, logical.inverse_alpha,
          logical.inverse_alpha_capacity_elements * sizeof(float))) {
    return false;
  }
  logical_ordinals[logical.logical_ordinal] = true;
  return true;
}

[[nodiscard]] bool valid_prefill_r1_projection_plane_v2_physical(
    const PrefillR1ProjectionPlaneV2AttachmentView& attachment,
    const PrefillR1ProjectionPlaneV2PhysicalView& physical,
    const PrefillR1ProjectionPlaneV2ViewLayout layout,
    const PrefillR1ProjectionPlaneV2LogicalIdentity primary_identity,
    const PrefillR1ProjectionPlaneV2LogicalIdentity secondary_identity,
    const std::size_t output_size, const std::size_t input_size,
    std::array<bool,
    ...
    std::array<bool,
               kPrefillR1ProjectionPlaneV2ViewLogicalProjectionCount>&
        logical_ordinals,
    std::size_t& physical_count, std::size_t& logical_count) noexcept {
  const bool paired =
      layout == PrefillR1ProjectionPlaneV2ViewLayout::kPairedGateUp;
  if (!physical.attached() || physical.physical_layout != layout ||
      physical.output_size != output_size || physical.input_size != input_size ||
      physical.logical_projection_count != (paired ? 2U : 1U) ||
      physical.physical_ordinal >= physical_ordinals.size() ||
      physical_ordinals[physical.physical_ordinal] ||
      reinterpret_cast<std::uintptr_t>(physical.codes) % 16U != 0U ||
      reinterpret_cast<std::uintptr_t>(physical.scales) % 16U != 0U ||
      !prefill_r1_projection_plane_v2_range_within_payload(
          attachment, physical.codes, physical.code_capacity_bytes) ||
      !prefill_r1_projection_plane_v2_range_within_payload(
          attachment, physical.scales,
          physical.scale_capacity_elements * sizeof(std::uint16_t)) ||
      !valid_prefill_r1_projection_plane_v2_logical(
          attachment, physical.primary, primary_identity, output_size,
          input_size, physical.physical_ordinal, logical_ordinals) ||
      (paired &&
       !valid_prefill_r1_projection_plane_v2_logical(
           attachment, physical.secondary, secondary_identity, output_size,
           input_size, physical.physical_ordinal, logical_ordinals)) ||
      (!paired && !physical.secondary.empty())) {
    return false;
  }
  physical_ordinals[physical.physical_ordinal] = true;
  ++physical_count;
  logical_count += paired ? 2U : 1U;
  return true;
}

// ModelWeights already authenticates the publication transaction. The runner
// repeats the complete device-view proof because it is the last boundary
// before launches: every ordinal must appear once, all 336 physical operands
// and 400 logical calibrations must match the fixed Qwen3.6 topology, and all
// referenced ranges must stay inside the one authenticated payload.
[[nodiscard]] bool complete_prefill_r1_projection_plane_v2_attached(
    const ModelWeights& weights) noexcept {
  const PrefillR1ProjectionPlaneV2AttachmentView& attachment =
      weights.prefill_r1_projection_plane_v2();
  if (!attachment.attached() ||
      reinterpret_cast<std::uintptr_t>(attachment.payload) % 256U != 0U) {
    return false;
  }
  std::array<bool,
             kPrefillR1ProjectionPlaneV2ViewPhysicalProjectionCount>
  ...
  const auto validate = [&](const PrefillR1ProjectionPlaneV2PhysicalView& view,
                            const PrefillR1ProjectionPlaneV2ViewLayout layout,
                            ...
                            const std::size_t output_size,
                            const std::size_t input_size) noexcept {
    return valid_prefill_r1_projection_plane_v2_physical(
        attachment, view, layout, primary_identity, secondary_identity,
        output_size, input_size, physical_ordinals, logical_ordinals,
        physical_count, logical_count);
  };
  for (std::size_t layer = 0U; layer < kReferenceDecoderLayerCount; ++layer) {
    const PrefillR1ProjectionPlaneV2LayerView& view =
        weights.layer(layer).prefill_r1_projection_plane_v2;
    const model::LayerType expected =
        reference_runner_detail::expected_reference_layer_type(layer);
    bool attention_valid = false;
    if (expected == model::LayerType::kLinearAttention) {
      attention_valid =
          view.linear_attached() &&
          validate(view.linear_qkv,
                   ...
                   kLinearQkvElements, kReferenceHiddenSize) &&
          validate(view.linear_z,
                   PrefillR1ProjectionPlaneV2ViewLayout::kAdjacentN8,
                   PrefillR1ProjectionPlaneV2LogicalIdentity::kLinearZ,
                   PrefillR1ProjectionPlaneV2LogicalIdentity::kNone,
                   kLinearValueElements, kReferenceHiddenSize) &&
          validate(view.linear_o,
                   PrefillR1ProjectionPlaneV2ViewLayout::kAdjacentN8,
                   PrefillR1ProjectionPlaneV2LogicalIdentity::kLinearO,
                   PrefillR1ProjectionPlaneV2LogicalIdentity::kNone,
                   kReferenceHiddenSize, kLinearValueElements);
    } else if (expected == model::LayerType::kFullAttention) {
      attention_valid =
          view.full_attached() &&
          validate(view.full_q,
                   PrefillR1ProjectionPlaneV2ViewLayout::kAdjacentN8,
                   PrefillR1ProjectionPlaneV2LogicalIdentity::kFullQ,
                   PrefillR1ProjectionPlaneV2LogicalIdentity::kNone,
                   kFullQGateElements, kReferenceHiddenSize) &&
          validate(view.full_k,
                   PrefillR1ProjectionPlaneV2ViewLayout::kAdjacentN8,
                   PrefillR1ProjectionPlaneV2LogicalIdentity::kFullK,
                   PrefillR1ProjectionPlaneV2LogicalIdentity::kNone,
                   kFullKvElements, kReferenceHiddenSize) &&
          validate(view.full_v,
                   PrefillR1ProjectionPlaneV2ViewLayout::kAdjacentN8,
                   PrefillR1ProjectionPlaneV2LogicalIdentity::kFullV,
                   PrefillR1ProjectionPlaneV2LogicalIdentity::kNone,
                   kFullKvElements, kReferenceHiddenSize) &&
          validate(view.full_o,
                   PrefillR1ProjectionPlaneV2ViewLayout::kAdjacentN8,
                   PrefillR1ProjectionPlaneV2LogicalIdentity::kFullO,
                   PrefillR1ProjectionPlaneV2LogicalIdentity::kNone,
                   kReferenceHiddenSize, kFullQueryElements);
    }
    if (!attention_valid || !view.attached() ||
        !validate(view.mlp_gate_up,
  ...
  return physical_count ==
         ...
         logical_count ==
             kPrefillR1ProjectionPlaneV2ViewLogicalProjectionCount &&
         std::all_of(physical_ordinals.begin(), physical_ordinals.end(),
                     [](const bool seen) { return seen; }) &&
         std::all_of(logical_ordinals.begin(), logical_ordinals.end(),
                     [](const bool seen) { return seen; });
}
#endif

#if defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R4_ADMISSION)
[[nodiscard]] bool valid_factorized_lane_r4_projection(
    const PrefillMLPFactorizedLaneR4LinearSidecarView& view,
    const std::size_t output_size,
    const std::size_t input_size) noexcept {
  return view.attached() && view.output_size == output_size &&
         view.input_size == input_size && view.lane_count == 4U &&
         view.packed_weight_capacity_bytes == output_size * input_size / 2U &&
         view.lane_scale_capacity_elements == output_size * 4U &&
         view.inverse_alpha_capacity_elements == input_size &&
         std::isfinite(view.activation_clip_ratio) &&
         view.activation_clip_ratio > 0.0F &&
         view.activation_clip_ratio <= 1.0F &&
         reinterpret_cast<std::uintptr_t>(view.packed_weight) % 16U == 0U &&
         reinterpret_cast<std::uintptr_t>(view.lane_scales) % 16U == 0U &&
         reinterpret_cast<std::uintptr_t>(view.inverse_alpha) % 16U == 0U;
}

[[nodiscard]] bool complete_factorized_lane_r4_mlp_attached(
    const ModelWeights& weights) noexcept {
  const auto k512_projection_empty = [](const LinearWeight& weight) noexcept {
    const PrefillMLPK512LinearSidecarView view =
        prefill_mlp_k512_sidecar_view(weight);
  ...
  for (std::size_t layer = 0U; layer < kReferenceDecoderLayerCount; ++layer) {
    const DecoderLayerWeights& layer_weights = weights.layer(layer);
    const PrefillMLPFactorizedLaneR4LayerView& mlp =
        layer_weights.prefill_mlp_factorized_lane_r4;
    if (!mlp.attached() ||
        !layer_weights.prefill_mlp_factorized_lane_r1.empty() ||
        !layer_weights.prefill_mlp_k512_fragment_native.empty() ||
        !k512_projection_empty(layer_weights.mlp.gate_proj) ||
        !k512_projection_empty(layer_weights.mlp.up_proj) ||
        !k512_projection_empty(layer_weights.mlp.down_proj) ||
        !valid_factorized_lane_r4_projection(
            mlp.gate, kReferenceIntermediateSize, kReferenceHiddenSize) ||
        !valid_factorized_lane_r4_projection(
            mlp.up, kReferenceIntermediateSize, kReferenceHiddenSize) ||
        !valid_factorized_lane_r4_projection(
        ...
        mlp.gate.inverse_alpha_capacity_elements !=
            mlp.up.inverse_alpha_capacity_elements ||
        mlp.gate.activation_clip_ratio != mlp.up.activation_clip_ratio) {
      return false;
    }
    ++layer_count;
  }
...
// The factorized package owns the complete MLP dataflow.  Any independently
// selected MLP/Marlin successor would make route ownership ambiguous, even if
// an individual branch happened not to accept the current shape.  Attention
// selectors are deliberately absent: authenticated K256 Attention remains the
// independent companion plane for R1/R4 and is not an R4 payload source.
#if defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R1_ADMISSION) || \
    defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION) || \
    defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R4_ADMISSION)
[[nodiscard]] bool factorized_lane_mlp_selector_conflict() noexcept {
  bool conflict = false;
#if defined(Q3X_ENABLE_NVFP4_MARLIN_PREFILL_ADMISSION)
  conflict = conflict || g_enable_nvfp4_marlin_prefill_admission;
#endif
#if defined(Q3X_ENABLE_FP8_MARLIN_PREFILL_ADMISSION)
  conflict = conflict || g_enable_fp8_marlin_prefill_admission;
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M64N128_K256_ALTERNATING_ADMISSION) && \
    defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_16WARP_PAIRRING_ADMISSION)
  conflict = conflict ||
...
#endif
#if defined(Q3X_ENABLE_A4W4_MLP_K512_ADMISSION)
  conflict = conflict || g_enable_a4w4_mlp_k512_admission ||
             g_enable_a4w4_mlp_k512_paired_gateup_canonical_down_admission ||
             g_enable_a4w4_gateup_down_k512_edge_m128n512_paired_ldmatrix_admission ||
             g_enable_a4w4_gateup_k512_m64n8_paired_warp_register_pipeline_admission ||
             g_enable_a4w4_down_k512_m128n128_ldmatrix_pairring_admission ||
             ...
             g_enable_a4w4_down_k512_m128n128_16warp_pairring_l2_macro4x4_admission;
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_ADMISSION)
  conflict = conflict || g_enable_a4w4_gateup_down_k512_edge_admission;
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M64N128_K256_ALTERNATING_ADMISSION)
  conflict = conflict ||
             g_enable_a4w4_gateup_down_k512_edge_m64n128_k256_alternating_admission ||
...
#endif
#if defined(Q3X_ENABLE_A4W4_DOWN_K512_M16N64_V2_ADMISSION)
  conflict = conflict || g_enable_a4w4_down_k512_m16n64_v2_admission;
#endif
#endif
#if defined(Q3X_ENABLE_A4W4_MLP_K512_FRAGMENT_NATIVE_ADMISSION)
  conflict = conflict || g_enable_a4w4_mlp_k512_fragment_native_admission;
#endif
  conflict = conflict || g_enable_a4w4_gateup_complete_cell_v2_admission ||
             g_enable_a4w4_m128_stage_major_admission ||
             g_enable_a4w4_down_m128_stage_major_admission;
#if defined(Q3X_ENABLE_A4W4_GATEUP_PROJECTION_V3_ADMISSION)
  conflict = conflict || g_enable_a4w4_gateup_projection_v3_admission;
#endif
#if defined(Q3X_ENABLE_A4W4_DOWN_COMPLETE_CELL_V2_ADMISSION)
  conflict = conflict || g_enable_a4w4_down_complete_cell_v2_admission;
#endif
#if defined(Q3X_ENABLE_A4W4_DOWN_COMPLETE_CELL_V3_ADMISSION)
  conflict = conflict || g_enable_a4w4_down_complete_cell_v3_admission;
#endif
  return conflict;
}
#endif
...
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N128_REGISTER_PIPELINE_ADMISSION)
[[nodiscard]] bool
complete_mlp_k512_projection_major_gateup_canonical_down_attached(
    const ModelWeights& weights) noexcept {
  std::size_t layer_count = 0U;
  for (std::size_t layer = 0U; layer < kReferenceDecoderLayerCount; ++layer) {
    const PrefillMLPK512FragmentNativeCompositeView& composite =
...
[[nodiscard]] std::size_t a4w4_scale_capacity_elements(
  ...
  return consumer == reference_runner_detail::A4W4PrefillConsumer::kK128
    ...
    defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
[[nodiscard]] reference_runner_detail::
...
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
[[nodiscard]] int launch_selected_a4w4_attention_factorized_lane_r1(
    ...
    const std::uint16_t* const input_scales,
    const std::size_t input_scale_capacity,
    const PrefillAttentionFactorizedLaneR1LinearSidecarView* const sidecars,
    std::uint16_t* const* const outputs,
    const std::size_t* const output_capacities,
    const std::size_t projection_count, const std::size_t token_count,
    void* const stream,
  ...
  const bool topology_matches =
      ...
      (topology == kernels::Sm87A4W4AttentionK256Topology::kAttentionO &&
       ...
       projection_count == 1U);
  if (!topology_matches || sidecars == nullptr || outputs == nullptr ||
      output_capacities == nullptr || packed_input == nullptr ||
      input_scales == nullptr || token_count == 0U) {
    return static_cast<int>(cudaErrorInvalidValue);
  }

  std::array<
      kernels::Sm87A4W4AttentionFactorizedLaneR1ProjectionView, 3U>
      projections{};
  for (std::size_t index = 0U; index < projection_count; ++index) {
    if (!sidecars[index].attached() || outputs[index] == nullptr) {
      return static_cast<int>(cudaErrorInvalidValue);
    }
    projections[index] = {
        sidecars[index].packed_weight,
        sidecars[index].packed_weight_capacity_bytes,
        sidecars[index].lane_scales,
        sidecars[index].lane_scale_capacity_elements,
        sidecars[index].output_size,
        outputs[index],
        sidecars[index].output_size,
  ...
  const int status = kernels::
      launch_sm87_a4w4_attention_factorized_lane_r1_m128n256_bf16_cuda(
          topology, packed_input, packed_input_capacity, input_scales,
          input_scale_capacity, token_count, projections.data(),
          projection_count,
...
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
[[nodiscard]] int launch_selected_prefill_r1_projection_plane_v2_attention(
    const bool admission_enabled,
    const kernels::Sm87A4W4AttentionK256Topology topology,
    const reference_runner_detail::A4W4AttentionSupermatrixFamily family,
    const std::uint8_t* const packed_input,
    ...
    const std::uint16_t* const input_scales,
    const std::size_t input_scale_capacity,
    const PrefillR1ProjectionPlaneV2PhysicalView* const physical_views,
    std::uint16_t* const* const outputs,
    const std::size_t* const output_capacities,
    const std::size_t projection_count, const std::size_t token_count,
    void* const stream,
  ...
  const bool topology_matches =
      ...
      (topology == kernels::Sm87A4W4AttentionK256Topology::kAttentionO &&
       family == reference_runner_detail::A4W4AttentionSupermatrixFamily::
                     kOutput &&
       projection_count == 1U);
  if (!topology_matches || physical_views == nullptr || outputs == nullptr ||
      output_capacities == nullptr || packed_input == nullptr ||
      input_scales == nullptr || token_count == 0U) {
    return static_cast<int>(cudaErrorInvalidValue);
  }
  ...
  for (std::size_t index = 0U; index < projection_count; ++index) {
    const PrefillR1ProjectionPlaneV2PhysicalView& physical =
        physical_views[index];
    if (!physical.attached() ||
        physical.physical_layout !=
            PrefillR1ProjectionPlaneV2ViewLayout::kAdjacentN8 ||
        physical.logical_projection_count != 1U || outputs[index] == nullptr) {
      return static_cast<int>(cudaErrorInvalidValue);
    }
    projections[index] = {
        physical.codes,
        physical.code_capacity_bytes,
        physical.scales,
        physical.scale_capacity_elements,
        physical.output_size,
        outputs[index],
        physical.output_size,
        output_capacities[index]};
  }
  ...
  local_full_hits.logical_projection_hits += logical;
  g_a4w4_full_prefill_admission_hits.logical_projection_hits += logical;
  ++local_v2_hits.attention_physical_launch_hits;
  local_v2_hits.logical_projection_hits += logical;
  return status;
}
#endif
...
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M32N512_OWNER_ADMISSION)
  return std::exchange(
      g_enable_a4w4_gateup_down_k512_edge_m32n512_owner_admission, enabled);
#else
  (void)enabled;
  return false;
#endif
...
ReferenceRunner& ReferenceRunner::operator=(ReferenceRunner&& other) noexcept {
  ...
  a4w4_prefill_consumer_ = std::exchange(
      ...
      reference_runner_detail::A4W4PrefillConsumer::kUnavailable);
  a4w4_projection_major_gateup_canonical_down_route_ = std::exchange(
      other.a4w4_projection_major_gateup_canonical_down_route_,
      reference_runner_detail::
          A4W4ProjectionMajorGateUpCanonicalDownRoute::kDisabled);
  a4w4_paired_gateup_canonical_down_route_ = std::exchange(
      other.a4w4_paired_gateup_canonical_down_route_,
      reference_runner_detail::
  ...
  attention_factorized_lane_r1_prefill_admission_enabled_ = std::exchange(
      other.attention_factorized_lane_r1_prefill_admission_enabled_, false);
  prefill_r1_projection_plane_v2_admission_enabled_ = std::exchange(
      other.prefill_r1_projection_plane_v2_admission_enabled_, false);
  prefill_r1_projection_plane_v2_request_hits_ = std::exchange(
      other.prefill_r1_projection_plane_v2_request_hits_, {});
  ...
  factorized_lane_r4_2cta_prefill_admission_enabled_ = std::exchange(
      other.factorized_lane_r4_2cta_prefill_admission_enabled_, false);
...
void ReferenceRunner::release() noexcept {
  ...
  attention_factorized_lane_r1_prefill_admission_enabled_ = false;
  prefill_r1_projection_plane_v2_admission_enabled_ = false;
...
ReferencePrefillTileOutcome ReferenceRunner::prefill_prefix_tile_impl(
  ...
  if (factorized_lane_r1_prefill_admission_enabled_ ||
      attention_factorized_lane_r1_prefill_admission_enabled_ ||
      factorized_lane_r4_prefill_admission_enabled_) {
    return fail_prefill_tile(runner_status(
        ReferenceRunnerError::kInvalidStepOptions,
        factorized_lane_r4_prefill_admission_enabled_
            ? "factorized_lane_r4_requires_whole_m_projection_span"
        : attention_factorized_lane_r1_prefill_admission_enabled_
            ? "attention_factorized_lane_r1_requires_whole_m_projection_span"
            : "factorized_lane_r1_requires_whole_m_projection_span"));
  }
#if defined(Q3X_ENABLE_A4W4_MLP_K512_ADMISSION)
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N128_REGISTER_PIPELINE_ADMISSION)
  if (a4w4_projection_major_gateup_canonical_down_route_ !=
      reference_runner_detail::
          A4W4ProjectionMajorGateUpCanonicalDownRoute::kDisabled) {
    return fail_prefill_tile(runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_mlp_k512_projection_major_gateup_canonical_down_requires_projection_span"));
  }
#endif
  if (a4w4_paired_gateup_canonical_down_route_ !=
      reference_runner_detail::A4W4PairedGateUpCanonicalDownRoute::kDisabled) {
    return fail_prefill_tile(runner_status(
        ReferenceRunnerError::kInvalidRunner,
  ...
  if (reference_runner_detail::
          select_a4w4_down_k512_m128n128_16warp_pairring_v1_route(
              a4w4_down_k512_m128n128_16warp_pairring_v1_selector_query(
                  false)) !=
      reference_runner_detail::
          A4W4DownK512M128N128Pairring16V1Route::kDisabled) {
    return fail_prefill_tile(runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_mlp_k512_down_m128n128_16warp_pairring_v1_requires_projection_span"));
  }
#endif
...
ReferenceRunnerStatus ReferenceRunner::execute_long_prefill_projection_span(
  ...
  const bool enable_gdn_chunk64_native_admission =
      g_enable_prefill_gdn_chunk64_native_admission;
#if defined(Q3X_ENABLE_GDN_STATE_O_BV64_FUSION_ADMISSION)
  const bool enable_gdn_state_o_bv64_fusion_admission =
      g_enable_gdn_state_o_bv64_fusion_admission;
#else
  [[maybe_unused]] constexpr bool
      enable_gdn_state_o_bv64_fusion_admission = false;
#endif
#endif
#if defined(Q3X_ENABLE_GDN_PREFILL_PROMPT_SPAN_MACRO_ADMISSION)
...
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
  reference_runner_detail::A4W4AttentionSupermatrixAdmissionHits
      local_attention_factorized_lane_r1_hits{};
#endif
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
  reference_runner_detail::A4W4AttentionSupermatrixAdmissionHits
      local_prefill_r1_projection_plane_v2_attention_hits{};
#endif
  reference_runner_detail::PrefillR1ProjectionPlaneV2AdmissionHits
      local_prefill_r1_projection_plane_v2_hits{};
...
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
  const auto quantize_attention_r1 =
      [this, &check_cuda, &local_hits, projection_token_count](
          const std::uint16_t* const input,
          const std::size_t input_row_stride,
          const std::size_t logical_token_count,
          const PrefillAttentionFactorizedLaneR1LinearSidecarView& sidecar,
          std::uint8_t* const packed, const std::size_t packed_capacity,
          std::uint16_t* const scales, const std::size_t scale_capacity,
          const char* const operation) noexcept {
        if (!sidecar.attached() || input == nullptr ||
            logical_token_count == 0U ||
            input_row_stride < sidecar.input_size) {
          return check_cuda(static_cast<int>(cudaErrorInvalidValue),
                            operation);
        }
        const int status =
            kernels::launch_sm87_a4w4_factorized_lane_quantize_bf16_cuda(
                ...
                logical_token_count * input_row_stride,
                sidecar.inverse_alpha,
                sidecar.inverse_alpha_capacity_elements,
                logical_token_count, projection_token_count,
                sidecar.input_size, 1U, sidecar.activation_clip_ratio,
...
#endif
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
  const auto quantize_prefill_r1_projection_plane_v2 =
      [this, &check_cuda, &local_hits, projection_token_count](
          const std::uint16_t* const input,
          const std::size_t input_row_stride,
          const std::size_t logical_token_count,
          ...
          std::uint16_t* const scales, const std::size_t scale_capacity,
          const char* const operation) noexcept {
        const auto& logical = physical.primary;
        if (!physical.attached() ||
            physical.physical_layout !=
                PrefillR1ProjectionPlaneV2ViewLayout::kAdjacentN8 ||
            !logical.attached() || input == nullptr ||
            logical_token_count == 0U ||
            input_row_stride < physical.input_size) {
          return check_cuda(static_cast<int>(cudaErrorInvalidValue),
                            operation);
        }
        const int status =
            kernels::launch_sm87_a4w4_factorized_lane_quantize_bf16_cuda(
                input, input_row_stride,
                logical_token_count * input_row_stride,
                logical.inverse_alpha,
                logical.inverse_alpha_capacity_elements,
                logical_token_count, projection_token_count,
                physical.input_size, 1U,
                static_cast<float>(logical.activation_clip_ratio), packed,
                packed_capacity, scales, scale_capacity, stream_);
        if (!check_cuda(status, operation)) {
          return false;
        }
        ++local_hits.activation_quantize_hits;
...
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
  const bool attention_factorized_lane_r1_selected =
      attention_factorized_lane_r1_prefill_admission_enabled_;
  const PrefillAttentionFactorizedLaneR1LayerView& attention_r1 =
      layer_weights.prefill_attention_factorized_lane_r1;
#else
  constexpr bool attention_factorized_lane_r1_selected = false;
#endif
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
  const bool prefill_r1_projection_plane_v2_selected =
      prefill_r1_projection_plane_v2_admission_enabled_;
  const PrefillR1ProjectionPlaneV2LayerView& attention_v2 =
      layer_weights.prefill_r1_projection_plane_v2;
#else
  constexpr bool prefill_r1_projection_plane_v2_selected = false;
#endif
  const bool attention_factorized_lane_any_selected =
      attention_factorized_lane_r1_selected ||
  ...
  const bool attention_input_norm_required = !attention_input_handoff_ready;

  if (attention_input_norm_required && !check_cuda(
          launch_headwise_centered_rms_norm_reference_cuda(
  ...
  if (layer_type == model::LayerType::kLinearAttention) {
    ...
    const bool linear_ab_projected = project_linear_pair(
        attention->in_proj_a, attention->in_proj_b, primary, token_count,
        linear_a, linear_b, "prefill_projection_span_linear_ab");
    bool attention_input_quantized = true;
    if (linear_ab_projected && !attention_input_handoff_ready) {
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
      if (prefill_r1_projection_plane_v2_selected) {
        attention_input_quantized =
            quantize_prefill_r1_projection_plane_v2(
                primary, kReferenceHiddenSize, token_count,
                attention_v2.linear_qkv, views_.prefill_a4_hidden_packed,
                hidden_packed_capacity, views_.prefill_a4_hidden_scales,
                hidden_scale_capacity,
                "prefill_projection_span_linear_input_r1_v2_quantize");
      } else
#endif
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
      if (attention_factorized_lane_r1_selected) {
        attention_input_quantized = quantize_attention_r1(
            primary, kReferenceHiddenSize, token_count,
            attention_r1.linear_qkv, views_.prefill_a4_hidden_packed,
            hidden_packed_capacity, views_.prefill_a4_hidden_scales,
            hidden_scale_capacity,
            "prefill_projection_span_linear_input_r1_quantize");
      } else
#endif
      {
        attention_input_quantized = quantize(
            primary, kReferenceHiddenSize, token_count,
    ...
    if (!linear_ab_projected || !attention_input_quantized) {
      return failure;
    }
...
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
    if (prefill_r1_projection_plane_v2_selected) {
      const std::array<PrefillR1ProjectionPlaneV2PhysicalView, 2U>
          physical_views{attention_v2.linear_qkv, attention_v2.linear_z};
      ...
      const std::size_t output_capacities[2U] = {
          ...
          static_cast<std::size_t>(
              request_plan.long_prefill_projection_secondary_bf16
                  .element_capacity)};
      if (!check_cuda(
              launch_selected_prefill_r1_projection_plane_v2_attention(
                  true,
                  kernels::Sm87A4W4AttentionK256Topology::kLinearQkvZ,
                  reference_runner_detail::A4W4AttentionSupermatrixFamily::
                      kLinearInput,
                  views_.prefill_a4_hidden_packed, hidden_packed_capacity,
                  views_.prefill_a4_hidden_scales, hidden_scale_capacity,
                  physical_views.data(), outputs, output_capacities,
                  physical_views.size(), projection_token_count, stream_,
                  local_hits,
                  local_prefill_r1_projection_plane_v2_attention_hits,
                  local_prefill_r1_projection_plane_v2_hits,
                  &attention_supermatrix_selected),
              "prefill_projection_span_linear_qkv_z_r1_v2")) {
        return failure;
      }
    }
#endif
    ...
    if (!attention_supermatrix_selected &&
        attention_factorized_lane_r1_selected) {
      const std::array<PrefillAttentionFactorizedLaneR1LinearSidecarView, 2U>
          sidecars{attention_r1.linear_qkv, attention_r1.linear_z};
      std::uint16_t* const outputs[2U] = {primary, secondary};
      const std::size_t output_capacities[2U] = {
          static_cast<std::size_t>(
              request_plan.long_prefill_projection_primary_bf16
          ...
          static_cast<std::size_t>(
              request_plan.long_prefill_projection_secondary_bf16
                  .element_capacity)};
      if (!check_cuda(
              launch_selected_a4w4_attention_factorized_lane_r1(
                  true,
                  kernels::Sm87A4W4AttentionK256Topology::kLinearQkvZ,
                  reference_runner_detail::A4W4AttentionSupermatrixFamily::
                      kLinearInput,
                  views_.prefill_a4_hidden_packed, hidden_packed_capacity,
                  views_.prefill_a4_hidden_scales, hidden_scale_capacity,
                  sidecars.data(), outputs, output_capacities,
                  sidecars.size(), projection_token_count, stream_,
                  local_hits, local_attention_factorized_lane_r1_hits,
                  &attention_supermatrix_selected),
              "prefill_projection_span_linear_qkv_z_factorized_r1")) {
        return failure;
      }
    }
#endif
#if defined(Q3X_ENABLE_A4W4_ATTENTION_SUPERMATRIX_ADMISSION)
    if (!attention_supermatrix_selected && !check_cuda(
            launch_selected_a4w4_linear_attention_supermatrix(
                a4w4_prefill_consumer_, views_.prefill_a4_hidden_packed,
                hidden_packed_capacity, views_.prefill_a4_hidden_scales,
                hidden_scale_capacity, qkv_sidecar, z_sidecar,
    ...
    bool gdn_factorized_lane_r1_direct_pack_selected = false;
#if (defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R1_ADMISSION) || \
     defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)) && \
    ...
    if (factorized_lane_r1_handoff_selected) {
      gdn_k256_direct_pack_selected =
          ...
          a4w4_prefill_consumer_ ==
              reference_runner_detail::A4W4PrefillConsumer::kK256 &&
          !attention_factorized_lane_any_selected;
      gdn_factorized_lane_r1_direct_pack_selected =
          !gdn_prompt_span_macro_selected &&
          a4w4_prefill_consumer_ ==
              reference_runner_detail::A4W4PrefillConsumer::kK256 &&
          attention_factorized_lane_any_selected;
      for (std::uint32_t tile_index = 0U;
        ...
        gdn_k256_direct_pack_selected =
            gdn_k256_direct_pack_selected && tile_supported;
        gdn_factorized_lane_r1_direct_pack_selected =
            gdn_factorized_lane_r1_direct_pack_selected && tile_supported;
      }
      if (!gdn_k256_direct_pack_selected &&
        ...
        return runner_status(
            ReferenceRunnerError::kInvalidRequestState,
            attention_factorized_lane_any_selected
                ? "prefill_projection_span_linear_gdn_r1_handoff_contract"
...
#if defined(Q3X_ENABLE_GDN_STATE_O_BV64_FUSION_ADMISSION)
    if (enable_gdn_state_o_bv64_fusion_admission &&
        (gdn_prompt_span_macro_selected ||
         !gdn_factorized_lane_r1_direct_pack_selected)) {
      return runner_status(
          ReferenceRunnerError::kInvalidRequestState,
          "prefill_projection_span_linear_gdn_state_o_bv64_fusion_contract",
          item.layer_index);
    }
#endif
...
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT) || \
    defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
        const float* direct_r1_inverse_alpha = nullptr;
        std::size_t direct_r1_inverse_alpha_capacity = 0U;
        float direct_r1_activation_clip_ratio = 0.0F;
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
        if (prefill_r1_projection_plane_v2_selected) {
          direct_r1_inverse_alpha =
              attention_v2.linear_o.primary.inverse_alpha;
          direct_r1_inverse_alpha_capacity =
              attention_v2.linear_o.primary.inverse_alpha_capacity_elements;
          direct_r1_activation_clip_ratio = static_cast<float>(
              attention_v2.linear_o.primary.activation_clip_ratio);
        }
#endif
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
        if (attention_factorized_lane_r1_selected) {
          direct_r1_inverse_alpha = attention_r1.linear_o.inverse_alpha;
          direct_r1_inverse_alpha_capacity =
              attention_r1.linear_o.inverse_alpha_capacity_elements;
          direct_r1_activation_clip_ratio =
              attention_r1.linear_o.activation_clip_ratio;
        }
#endif
#endif
...
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT) || \
    defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
        } else if (gdn_factorized_lane_r1_direct_pack_selected) {
          native_status =
              use_conv_compact_qk_fused_candidate
                  ? gdn_prefill_chunk64_native_detail::
                        launch_qk_preprocessed_factorized_lane_r1_a4(
                            prefill_gdn_chunk64_native_workspace_,
                            prefill_gdn_chunk64_native_workspace_bytes_,
                            conv_qkv, tile.token_count,
                            linear_a + local_first * kLinearScalarElements,
                            ...
                            direct_r1_inverse_alpha,
                            direct_r1_inverse_alpha_capacity,
                            local_first, token_count, projection_token_count,
                            direct_r1_activation_clip_ratio,
                            views_.prefill_a4_intermediate_packed,
                            intermediate_packed_capacity,
                            views_.prefill_a4_intermediate_scales,
                            intermediate_scale_capacity, stream_,
                            enable_gdn_state_o_bv64_fusion_admission &&
                                !prefill_r1_projection_plane_v2_selected)
                  : gdn_prefill_chunk64_native_detail::
                        launch_factorized_lane_r1_a4(
                            prefill_gdn_chunk64_native_workspace_,
                            prefill_gdn_chunk64_native_workspace_bytes_,
                            conv_qkv, tile.token_count,
                            linear_a + local_first * kLinearScalarElements,
        ...
        if (gdn_k256_direct_pack_selected) {
          ...
          if (!prefill_r1_projection_plane_v2_selected) {
            ++g_gdn_factorized_lane_r1_direct_pack_launch_hits;
          }
#if defined(Q3X_ENABLE_GDN_STATE_O_BV64_FUSION_ADMISSION)
          if (enable_gdn_state_o_bv64_fusion_admission &&
              !prefill_r1_projection_plane_v2_selected) {
            ++g_gdn_state_o_bv64_fused_launch_hits;
            g_gdn_state_o_bv64_fused_logical_token_hits +=
                tile.token_count;
          }
#endif
        }
      ...
      if (!gdn_k256_direct_pack_selected &&
          !gdn_factorized_lane_r1_direct_pack_selected && !check_cuda(
              static_cast<int>(cudaMemcpyAsync(
                  secondary + local_first * kLinearValueElements,
                  views_.projection[2],
    ...
    bool attention_o_k512_selected = false;
#if defined(Q3X_ENABLE_A4W4_ATTENTION_O_K512_ADMISSION)
    if (!attention_factorized_lane_any_selected && !check_cuda(
            launch_selected_a4w4_attention_o_k512(
    ...
    if (!gdn_k256_direct_pack_selected &&
        !gdn_factorized_lane_r1_direct_pack_selected &&
        !attention_o_k512_selected) {
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
      if (prefill_r1_projection_plane_v2_selected) {
        if (!quantize_prefill_r1_projection_plane_v2(
                secondary, kLinearValueElements, token_count,
                attention_v2.linear_o,
                views_.prefill_a4_intermediate_packed,
                intermediate_packed_capacity,
                views_.prefill_a4_intermediate_scales,
                intermediate_scale_capacity,
                "prefill_projection_span_linear_output_r1_v2_quantize")) {
          return failure;
        }
      } else
#endif
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
      if (attention_factorized_lane_r1_selected) {
        if (!quantize_attention_r1(
                secondary, kLinearValueElements, token_count,
                attention_r1.linear_o,
                views_.prefill_a4_intermediate_packed,
                intermediate_packed_capacity,
                views_.prefill_a4_intermediate_scales,
                intermediate_scale_capacity,
                "prefill_projection_span_linear_output_r1_quantize")) {
          return failure;
        }
      } else
#endif
...
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
    if (prefill_r1_projection_plane_v2_selected) {
      const std::array<PrefillR1ProjectionPlaneV2PhysicalView, 1U>
          physical_views{attention_v2.linear_o};
      ...
      if (!check_cuda(
              launch_selected_prefill_r1_projection_plane_v2_attention(
                  ...
                  intermediate_packed_capacity,
                  views_.prefill_a4_intermediate_scales,
                  intermediate_scale_capacity, physical_views.data(),
                  outputs, output_capacities, physical_views.size(),
                  projection_token_count, stream_, local_hits,
                  local_prefill_r1_projection_plane_v2_attention_hits,
                  local_prefill_r1_projection_plane_v2_hits,
                  &attention_output_supermatrix_selected),
              "prefill_projection_span_linear_output_r1_v2")) {
        return failure;
      }
    }
#endif
    ...
    if (!attention_output_supermatrix_selected &&
        attention_factorized_lane_r1_selected) {
      const std::array<PrefillAttentionFactorizedLaneR1LinearSidecarView, 1U>
          sidecars{attention_r1.linear_o};
      std::uint16_t* const outputs[1U] = {secondary};
      const std::size_t output_capacities[1U] = {
          static_cast<std::size_t>(
              request_plan.long_prefill_projection_secondary_bf16
      ...
      if (!check_cuda(
              launch_selected_a4w4_attention_factorized_lane_r1(
                  ...
                  local_attention_factorized_lane_r1_hits,
                  &attention_output_supermatrix_selected),
              "prefill_projection_span_linear_output_factorized_r1")) {
        return failure;
      }
    }
#endif
#if defined(Q3X_ENABLE_A4W4_ATTENTION_SUPERMATRIX_ADMISSION)
    if (!attention_o_k512_selected &&
        !attention_output_supermatrix_selected && !check_cuda(
            launch_selected_a4w4_attention_o_supermatrix(
                a4w4_prefill_consumer_,
                views_.prefill_a4_intermediate_packed,
                intermediate_packed_capacity,
    ...
    [[maybe_unused]] const std::size_t projected_span_key_capacity =
        pad_full_kv ? value_offset_elements
                    : projection_token_count * kFullKvElements;
    ...
    if (!attention_input_handoff_ready) {
      ...
      if (prefill_r1_projection_plane_v2_selected) {
        if (!quantize_prefill_r1_projection_plane_v2(
                primary, kReferenceHiddenSize, token_count,
                attention_v2.full_q, views_.prefill_a4_hidden_packed,
                hidden_packed_capacity, views_.prefill_a4_hidden_scales,
                hidden_scale_capacity,
                "prefill_projection_span_full_input_r1_v2_quantize")) {
          return failure;
        }
      } else
#endif
      ...
      if (attention_factorized_lane_r1_selected) {
        if (!quantize_attention_r1(
                primary, kReferenceHiddenSize, token_count,
                attention_r1.full_q, views_.prefill_a4_hidden_packed,
                hidden_packed_capacity, views_.prefill_a4_hidden_scales,
                hidden_scale_capacity,
                "prefill_projection_span_full_input_r1_quantize")) {
          return failure;
        }
      } else
#endif
...
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
    if (prefill_r1_projection_plane_v2_selected) {
      const std::array<PrefillR1ProjectionPlaneV2PhysicalView, 3U>
          physical_views{attention_v2.full_q, attention_v2.full_k,
                         attention_v2.full_v};
      ...
      if (!check_cuda(
              launch_selected_prefill_r1_projection_plane_v2_attention(
                  true, kernels::Sm87A4W4AttentionK256Topology::kFullQkv,
                  reference_runner_detail::A4W4AttentionSupermatrixFamily::
                      kFullInput,
                  views_.prefill_a4_hidden_packed, hidden_packed_capacity,
                  views_.prefill_a4_hidden_scales, hidden_scale_capacity,
                  ...
                  local_prefill_r1_projection_plane_v2_hits,
                  &attention_supermatrix_selected),
              "prefill_projection_span_full_q_k_v_r1_v2")) {
        return failure;
      }
    }
#endif
    ...
    if (!attention_supermatrix_selected &&
        attention_factorized_lane_r1_selected) {
      const std::array<PrefillAttentionFactorizedLaneR1LinearSidecarView, 3U>
          sidecars{attention_r1.full_q, attention_r1.full_k,
                   attention_r1.full_v};
      std::uint16_t* const outputs[3U] = {
          primary, projected_span_key, projected_span_value};
      const std::size_t output_capacities[3U] = {
      ...
      if (!check_cuda(
              launch_selected_a4w4_attention_factorized_lane_r1(
                  ...
                  views_.prefill_a4_hidden_packed, hidden_packed_capacity,
                  views_.prefill_a4_hidden_scales, hidden_scale_capacity,
                  sidecars.data(), outputs, output_capacities, sidecars.size(),
                  projection_token_count, stream_, local_hits,
                  local_attention_factorized_lane_r1_hits,
                  &attention_supermatrix_selected),
              "prefill_projection_span_full_q_k_v_factorized_r1")) {
        return failure;
      }
    }
#endif
    ...
    if (!attention_o_k512_selected) {
      ...
      if (prefill_r1_projection_plane_v2_selected) {
        if (!quantize_prefill_r1_projection_plane_v2(
                secondary, kFullQueryElements, token_count,
                attention_v2.full_o,
                views_.prefill_a4_intermediate_packed,
                intermediate_packed_capacity,
                views_.prefill_a4_intermediate_scales,
                intermediate_scale_capacity,
                "prefill_projection_span_full_output_r1_v2_quantize")) {
          return failure;
        }
      } else
#endif
      ...
      if (attention_factorized_lane_r1_selected) {
        if (!quantize_attention_r1(
                ...
                views_.prefill_a4_intermediate_scales,
                intermediate_scale_capacity,
                "prefill_projection_span_full_output_r1_quantize")) {
          return failure;
        }
      } else
#endif
    ...
    if (prefill_r1_projection_plane_v2_selected) {
      const std::array<PrefillR1ProjectionPlaneV2PhysicalView, 1U>
          physical_views{attention_v2.full_o};
      std::uint16_t* const outputs[1U] = {secondary};
      const std::size_t output_capacities[1U] = {
          static_cast<std::size_t>(
              request_plan.long_prefill_projection_secondary_bf16
      ...
      if (!check_cuda(
              launch_selected_prefill_r1_projection_plane_v2_attention(
                  ...
                  local_prefill_r1_projection_plane_v2_hits,
                  &attention_output_supermatrix_selected),
              "prefill_projection_span_full_output_r1_v2")) {
        return failure;
      }
    }
#endif
    ...
    if (!attention_output_supermatrix_selected &&
      ...
      if (!check_cuda(
              launch_selected_a4w4_attention_factorized_lane_r1(
                  ...
                  local_attention_factorized_lane_r1_hits,
                  &attention_output_supermatrix_selected),
              "prefill_projection_span_full_output_factorized_r1")) {
        return failure;
      }
    }
#endif
  ...
  if (factorized_lane_r1_handoff_selected) {
    ...
    const PrefillR1ProjectionPlaneV2PhysicalView& v2_gate_up =
        layer_weights.prefill_r1_projection_plane_v2.mlp_gate_up;
    ...
    if (!check_cuda(
            ...
            prefill_r1_projection_plane_v2_selected
                ? "prefill_projection_span_r1_v2_attention_to_gate_handoff"
  ...
  const PrefillA4LinearSidecarView down_sidecar =
      prefill_a4_sidecar_view(layer_weights.mlp.down_proj);
  bool factorized_lane_r1_selected = false;
  bool prefill_r1_projection_plane_v2_mlp_selected = false;
  bool factorized_lane_r4_selected = false;
#if defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R4_ADMISSION)
  if (factorized_lane_r4_prefill_admission_enabled_) {
    const PrefillMLPFactorizedLaneR4LayerView& factorized =
        layer_weights.prefill_mlp_factorized_lane_r4;
    constexpr std::size_t launch_token_count =
        kernels::kSm87A4W4GateUpFactorizedR4M128N64LaunchTokens;
    const auto incumbent_gate_plan =
        kernels::sm87_a4w4_gateup_factorized_r4_m128n64_plan(
            token_count, launch_token_count, kReferenceIntermediateSize,
            kReferenceHiddenSize);
    const auto incumbent_down_plan =
        kernels::sm87_a4w4_down_factorized_r4_m192n128_plan(
            token_count, launch_token_count, kReferenceHiddenSize,
            kReferenceIntermediateSize);
#if defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R4_2CTA_ADMISSION)
    static_assert(
        launch_token_count ==
        kernels::kSm87A4W4GateUpFactorizedR4M64N64TwoCtaLaunchTokens);
    static_assert(
        launch_token_count ==
        kernels::kSm87A4W4DownFactorizedR4M128N64LaunchTokens);
    static_assert(
        kernels::kSm87A4W4GateUpFactorizedR4M128N64PrimaryStride ==
        kernels::kSm87A4W4GateUpFactorizedR4M64N64TwoCtaPrimaryStride);
    static_assert(
        kernels::kSm87A4W4GateUpFactorizedR4M128N64SecondaryStride ==
        kernels::kSm87A4W4GateUpFactorizedR4M64N64TwoCtaSecondaryStride);
    const bool r4_2cta_selected =
        factorized_lane_r4_2cta_prefill_admission_enabled_;
    const auto r4_2cta_gate_plan =
        kernels::sm87_a4w4_gateup_factorized_r4_m64n64_2cta_plan(
            token_count, launch_token_count, kReferenceIntermediateSize,
            kReferenceHiddenSize);
    const auto r4_2cta_down_plan =
        kernels::sm87_a4w4_down_factorized_r4_m128n64_plan(
            token_count, launch_token_count, kReferenceHiddenSize,
            kReferenceIntermediateSize);
    const bool gate_plan_valid = r4_2cta_selected
                                     ? static_cast<bool>(r4_2cta_gate_plan)
                                     : static_cast<bool>(incumbent_gate_plan);
    const bool down_plan_valid = r4_2cta_selected
                                     ? static_cast<bool>(r4_2cta_down_plan)
                                     : static_cast<bool>(incumbent_down_plan);
#else
    constexpr bool r4_2cta_selected = false;
    const bool gate_plan_valid = static_cast<bool>(incumbent_gate_plan);
    const bool down_plan_valid = static_cast<bool>(incumbent_down_plan);
#endif
    const auto hidden_quantize_plan =
        kernels::sm87_a4w4_factorized_lane_quantize_plan(
            token_count, launch_token_count, kReferenceHiddenSize, 4U);
    const auto product_quantize_plan =
        kernels::sm87_a4w4_factorized_lane_quantize_plan(
            token_count, launch_token_count, kReferenceIntermediateSize, 4U);
    ...
    const std::size_t secondary_capacity = static_cast<std::size_t>(
        request_plan.long_prefill_projection_secondary_bf16.element_capacity);
    if (factorized_lane_r1_prefill_admission_enabled_ ||
        factorized_lane_mlp_selector_conflict() ||
        a4w4_prefill_consumer_ !=
            reference_runner_detail::A4W4PrefillConsumer::kK256 ||
        projection_backend_ != ProjectionBackend::kSm87WeightOnly ||
        !reference_runner_detail::factorized_lane_r4_prefill_prompt_supported(
            token_count, projection_token_count, span_capacity) ||
        projection_token_count != launch_token_count || !gate_plan_valid ||
        !down_plan_valid ||
        !hidden_quantize_plan || !product_quantize_plan ||
        !factorized.attached() ||
        !valid_factorized_lane_r4_projection(
            factorized.gate, kReferenceIntermediateSize,
            kReferenceHiddenSize) ||
        !valid_factorized_lane_r4_projection(
            factorized.up, kReferenceIntermediateSize,
            kReferenceHiddenSize) ||
        !valid_factorized_lane_r4_projection(
            factorized.down, kReferenceHiddenSize,
            kReferenceIntermediateSize) ||
        factorized.gate.inverse_alpha_capacity_elements !=
            factorized.up.inverse_alpha_capacity_elements ||
        factorized.gate.activation_clip_ratio !=
            factorized.up.activation_clip_ratio ||
        hidden_packed_capacity < hidden_quantize_plan.packed_capacity_bytes ||
        hidden_scale_capacity < hidden_quantize_plan.scale_capacity_elements ||
        intermediate_packed_capacity <
            product_quantize_plan.packed_capacity_bytes ||
        intermediate_scale_capacity <
            product_quantize_plan.scale_capacity_elements ||
        primary_capacity <
            launch_token_count *
                kernels::
        ...
        secondary_capacity <
            launch_token_count * kReferenceHiddenSize) {
      return runner_status(
          ReferenceRunnerError::kInvalidRequestState,
          "prefill_projection_span_factorized_lane_r4_contract",
          item.layer_index);
    }

    const auto launch_r4_gateup = [&]() noexcept -> int {
#if defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R4_2CTA_ADMISSION)
      if (r4_2cta_selected) {
        return kernels::
            launch_sm87_a4w4_gateup_factorized_r4_m64n64_2cta_bf16_cuda(
                ...
                launch_token_count, kReferenceIntermediateSize,
    ...
    const auto launch_r4_down = [&]() noexcept -> int {
#if defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R4_2CTA_ADMISSION)
      if (r4_2cta_selected) {
        return kernels::
            launch_sm87_a4w4_down_factorized_r4_m128n64_bf16_cuda(
                ...
                intermediate_packed_capacity,
                views_.prefill_a4_intermediate_scales,
                intermediate_scale_capacity, factorized.down.packed_weight,
                factorized.down.packed_weight_capacity_bytes,
                factorized.down.lane_scales,
                factorized.down.lane_scale_capacity_elements, token_count,
                launch_token_count, kReferenceHiddenSize,
                kReferenceIntermediateSize, secondary,
                kReferenceHiddenSize, secondary_capacity, stream_);
      }
#endif
      return kernels::launch_sm87_a4w4_down_factorized_r4_m192n128_bf16_cuda(
          views_.prefill_a4_intermediate_packed,
          intermediate_packed_capacity,
          views_.prefill_a4_intermediate_scales,
          intermediate_scale_capacity, factorized.down.packed_weight,
          ...
          factorized.down.lane_scale_capacity_elements, token_count,
          launch_token_count, kReferenceHiddenSize,
          kReferenceIntermediateSize, secondary, kReferenceHiddenSize,
          secondary_capacity, stream_);
    };
    ...
    if (!check_cuda(
            kernels::launch_sm87_a4w4_factorized_lane_quantize_bf16_cuda(
                ...
                views_.prefill_a4_hidden_scales, hidden_scale_capacity,
                stream_),
            "prefill_projection_span_factorized_lane_r4_input_quantize") ||
        !check_cuda(
            launch_r4_gateup(),
            r4_2cta_selected
                ? "prefill_projection_span_factorized_lane_r4_2cta_gateup"
                : "prefill_projection_span_factorized_lane_r4_gateup") ||
        !check_cuda(
            kernels::
                launch_sm87_a4w4_factorized_lane_quantize_bf16_split_cuda(
                    ...
                    token_count, launch_token_count, 4U,
                    factorized.down.activation_clip_ratio,
                    views_.prefill_a4_intermediate_packed,
                    intermediate_packed_capacity,
                    views_.prefill_a4_intermediate_scales,
                    intermediate_scale_capacity, stream_),
            "prefill_projection_span_factorized_lane_r4_product_quantize") ||
        !check_cuda(
            launch_r4_down(),
            r4_2cta_selected
                ? "prefill_projection_span_factorized_lane_r4_2cta_down"
                : "prefill_projection_span_factorized_lane_r4_down")) {
      return failure;
    }
    ...
    factorized_lane_r4_selected = true;
  }
#endif
#if defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R1_ADMISSION) || \
    defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
  if (prefill_r1_projection_plane_v2_selected) {
    const PrefillR1ProjectionPlaneV2LayerView& v2 =
        layer_weights.prefill_r1_projection_plane_v2;
    const PrefillR1ProjectionPlaneV2PhysicalView& gate_up = v2.mlp_gate_up;
    const PrefillR1ProjectionPlaneV2PhysicalView& down = v2.mlp_down;
    const std::size_t gate_launch_token_count =
        kernels::sm87_a4w4_gateup_factorized_launch_token_count(token_count);
    const std::size_t down_launch_token_count =
        kernels::sm87_a4w4_down_factorized_launch_token_count(token_count);
    const auto gate_plan = kernels::sm87_a4w4_gateup_factorized_lane_plan(
        token_count, gate_launch_token_count, kReferenceIntermediateSize,
        kReferenceHiddenSize);
    const auto down_plan = kernels::sm87_a4w4_down_factorized_lane_plan(
    ...
    const std::size_t secondary_capacity = static_cast<std::size_t>(
        request_plan.long_prefill_projection_secondary_bf16.element_capacity);
    const std::size_t gate_a_bytes =
        kernels::sm87_a4w4_gateup_factorized_packed_capacity_bytes(
            gate_launch_token_count, kReferenceHiddenSize);
    const std::size_t gate_a_scales =
        kernels::sm87_a4w4_gateup_factorized_scale_capacity_elements(
            gate_launch_token_count);
    const std::size_t down_a_bytes =
        kernels::sm87_a4w4_down_factorized_packed_capacity_bytes(
            down_launch_token_count, kReferenceIntermediateSize);
    const std::size_t down_a_scales =
        kernels::sm87_a4w4_down_factorized_scale_capacity_elements(
            down_launch_token_count);
    if (!v2.attached() || !gate_up.attached() || !down.attached() ||
        gate_up.physical_layout !=
            PrefillR1ProjectionPlaneV2ViewLayout::kPairedGateUp ||
        down.physical_layout !=
            PrefillR1ProjectionPlaneV2ViewLayout::kAdjacentN8 ||
        gate_up.output_size != kReferenceIntermediateSize ||
        gate_up.input_size != kReferenceHiddenSize ||
        down.output_size != kReferenceHiddenSize ||
        down.input_size != kReferenceIntermediateSize ||
        !factorized_lane_r1_gate_input_handoff_ready ||
        factorized_lane_mlp_selector_conflict() ||
        a4w4_prefill_consumer_ !=
            reference_runner_detail::A4W4PrefillConsumer::kK256 ||
        projection_backend_ != ProjectionBackend::kSm87WeightOnly ||
        ...
        down_launch_token_count > span_capacity || gate_plan.launch_ctas == 0U ||
        down_plan.launch_ctas == 0U || !product_finalize_plan ||
        ...
        gate_a_bytes == 0U || gate_a_scales == 0U || down_a_bytes == 0U ||
        down_a_scales == 0U || hidden_packed_capacity < gate_a_bytes ||
        hidden_scale_capacity < gate_a_scales ||
        intermediate_packed_capacity < down_a_bytes ||
        intermediate_scale_capacity < down_a_scales ||
        primary_capacity <
            token_count * kernels::kSm87A4W4GateUpFactorizedPrimaryStride ||
        secondary_capacity <
            token_count * kernels::kSm87A4W4GateUpFactorizedSecondaryStride ||
        secondary_capacity < token_count * kReferenceHiddenSize) {
      return runner_status(
          ReferenceRunnerError::kInvalidRequestState,
          "prefill_projection_span_r1_projection_plane_v2_mlp_contract",
          item.layer_index);
    }
    if (!check_cuda(
            kernels::
                launch_sm87_a4w4_gateup_factorized_lane_r1_v2_tile_max_bf16_cuda(
                    views_.prefill_a4_hidden_packed, hidden_packed_capacity,
                    views_.prefill_a4_hidden_scales, hidden_scale_capacity,
                    gate_up.codes, gate_up.code_capacity_bytes,
                    gate_up.scales, gate_up.scale_capacity_elements,
                    token_count, gate_launch_token_count,
                    kReferenceIntermediateSize, kReferenceHiddenSize, primary,
                    kernels::kSm87A4W4GateUpFactorizedPrimaryStride,
                    primary_capacity, secondary,
                    kernels::kSm87A4W4GateUpFactorizedSecondaryStride,
                    secondary_capacity, down.primary.inverse_alpha,
                    down.primary.inverse_alpha_capacity_elements,
                    views_.prefill_a4_gateup_r1_product_partial_max,
                    views_.prefill_a4_gateup_r1_product_partial_max_elements,
                    stream_),
            "prefill_projection_span_r1_projection_plane_v2_gateup") ||
        !check_cuda(
            kernels::launch_sm87_a4w4_gateup_r1_product_finalize_cuda(
                primary, kernels::kSm87A4W4GateUpFactorizedPrimaryStride,
                primary_capacity, secondary,
                kernels::kSm87A4W4GateUpFactorizedSecondaryStride,
                secondary_capacity, down.primary.inverse_alpha,
                down.primary.inverse_alpha_capacity_elements,
                ...
                views_.prefill_a4_gateup_r1_product_partial_max_elements,
                token_count, down_launch_token_count,
                static_cast<float>(down.primary.activation_clip_ratio),
                views_.prefill_a4_intermediate_packed,
                intermediate_packed_capacity,
                views_.prefill_a4_intermediate_scales,
                intermediate_scale_capacity, stream_),
            "prefill_projection_span_r1_projection_plane_v2_product_finalize") ||
        !check_cuda(
            kernels::launch_sm87_a4w4_down_factorized_lane_r1_v2_bf16_cuda(
                views_.prefill_a4_intermediate_packed,
                intermediate_packed_capacity,
                views_.prefill_a4_intermediate_scales,
                intermediate_scale_capacity, down.codes,
                down.code_capacity_bytes, down.scales,
                down.scale_capacity_elements, token_count,
                down_launch_token_count, kReferenceHiddenSize,
                kReferenceIntermediateSize, secondary, kReferenceHiddenSize,
                secondary_capacity, stream_),
            "prefill_projection_span_r1_projection_plane_v2_down")) {
      return failure;
    }
    ++local_hits.activation_quantize_hits;
    ++local_hits.paired_gate_up_hits;
    ...
    ++g_a4w4_full_prefill_admission_hits.generic_projection_hits;
    g_a4w4_full_prefill_admission_hits.logical_projection_hits += 3U;
    ++local_prefill_r1_projection_plane_v2_hits.gateup_launch_hits;
    ++local_prefill_r1_projection_plane_v2_hits.down_launch_hits;
    local_prefill_r1_projection_plane_v2_hits.logical_projection_hits += 3U;
    prefill_r1_projection_plane_v2_mlp_selected = true;
  }
#endif
  ...
  if (factorized_lane_r1_prefill_admission_enabled_) {
    ...
    const std::size_t down_a_scales =
        kernels::sm87_a4w4_down_factorized_scale_capacity_elements(
            down_launch_token_count);
    if (factorized_lane_mlp_selector_conflict() ||
        a4w4_prefill_consumer_ !=
            reference_runner_detail::A4W4PrefillConsumer::kK256 ||
        projection_backend_ != ProjectionBackend::kSm87WeightOnly ||
        !factorized.attached() ||
      ...
      return runner_status(
          ReferenceRunnerError::kInvalidRequestState,
          "prefill_projection_span_factorized_lane_r1_contract",
          item.layer_index);
    }
    ...
    factorized_lane_r1_selected = true;
  }
#endif
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M64N128_K256_ALTERNATING_ADMISSION) && \
    defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_16WARP_PAIRRING_ADMISSION)
  const bool mlp_k256_m128n256_pairfeed_package_requested =
      g_enable_a4w4_mlp_k256_m128n256_pairfeed_package_admission;
#else
  constexpr bool mlp_k256_m128n256_pairfeed_package_requested = false;
#endif
#if defined(Q3X_ENABLE_A4W4_MLP_K512_ADMISSION)
  ...
  const bool gateup_down_k512_edge_m64n128_k256_alternating_requested =
      g_enable_a4w4_gateup_down_k512_edge_m64n128_k256_alternating_admission;
  const bool gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_requested =
      g_enable_a4w4_gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_admission;
#else
  constexpr bool gateup_down_k512_edge_m64n128_k256_alternating_requested =
      false;
  constexpr bool gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_requested =
      false;
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M32N512_OWNER_ADMISSION)
  const bool gateup_down_k512_edge_m32n512_owner_requested =
      g_enable_a4w4_gateup_down_k512_edge_m32n512_owner_admission;
#else
  constexpr bool gateup_down_k512_edge_m32n512_owner_requested = false;
#endif
#if defined(Q3X_ENABLE_A4W4_MLP_K512_SHAPE_SEPARATED_MARLIN_PACKAGE_ADMISSION)
  const bool mlp_k512_shape_separated_marlin_package_requested =
      g_enable_a4w4_mlp_k512_shape_separated_marlin_package_admission;
#else
  constexpr bool mlp_k512_shape_separated_marlin_package_requested = false;
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M128N128_PROJECTION_SERIAL_ADMISSION)
  const bool gateup_k512_m128n128_projection_serial_requested =
      g_enable_a4w4_gateup_k512_m128n128_projection_serial_admission;
#else
  ...
  const bool gateup_k512_m128n512_fused_quantize_requested =
      g_enable_a4w4_gateup_k512_m128n512_fused_quantize_admission;
#else
  constexpr bool gateup_k512_m128n512_fused_quantize_requested = false;
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M128N64_ADMISSION)
  ...
  const reference_runner_detail::
      A4W4ProjectionMajorGateUpCanonicalDownRoute
          projection_major_gateup_canonical_down_route =
              a4w4_projection_major_gateup_canonical_down_route_;
  if (projection_major_gateup_canonical_down_route ==
      reference_runner_detail::
          A4W4ProjectionMajorGateUpCanonicalDownRoute::kInvalid) {
    return runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_projection_span_mlp_k512_projection_major_gateup_canonical_down_selector_contract",
        item.layer_index);
  }
#else
  [[maybe_unused]] constexpr reference_runner_detail::
      A4W4ProjectionMajorGateUpCanonicalDownRoute
          projection_major_gateup_canonical_down_route =
              reference_runner_detail::
                  A4W4ProjectionMajorGateUpCanonicalDownRoute::kDisabled;
  ...
  constexpr reference_runner_detail::A4W4PairedGateUpCanonicalDownRoute
      paired_gateup_canonical_down_route =
          reference_runner_detail::A4W4PairedGateUpCanonicalDownRoute::
              kDisabled;
#endif
  const bool paired_old_gate_selected =
      paired_gateup_canonical_down_route ==
          reference_runner_detail::A4W4PairedGateUpCanonicalDownRoute::
  ...
  const bool paired_new_gate_selected =
      paired_gateup_canonical_down_route ==
          reference_runner_detail::A4W4PairedGateUpCanonicalDownRoute::
  ...
  const bool paired_old_down_selected =
      paired_gateup_canonical_down_route ==
      reference_runner_detail::A4W4PairedGateUpCanonicalDownRoute::
          kOldGateLdmatrixPairringDown;
  const bool paired_new_down_selected =
      paired_gateup_canonical_down_route ==
      reference_runner_detail::A4W4PairedGateUpCanonicalDownRoute::
          kNewPairedWarpGate16WarpPairringDown;
  const bool gateup_down_k512_edge_m32n512_owner_gate_conflict =
      ...
      ;
#if defined(Q3X_ENABLE_A4W4_MLP_K512_ADMISSION)
  const bool gateup_down_k512_edge_m32n512_owner_macro_conflict =
      g_enable_a4w4_down_k512_m128n128_16warp_pairring_l2_macro4x4_admission
#if defined(Q3X_ENABLE_A4W4_ATTENTION_K256_M128N256_A_EXCHANGE_B4_EXPERIMENT)
      || g_enable_a4w4_attention_k256_m128n256_a_exchange_b4_l2_macro4x4_admission
#endif
      ;
#else
  constexpr bool gateup_down_k512_edge_m32n512_owner_macro_conflict = false;
#endif
  const reference_runner_detail::A4W4GateUpK512PairfeedImplementation
      gateup_k512_pairfeed_implementation =
          reference_runner_detail::
              select_a4w4_gateup_k512_pairfeed_implementation(
                  {gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_requested,
                   gateup_down_k512_edge_m32n512_owner_requested,
                   gateup_down_k512_edge_m32n512_owner_gate_conflict,
                   gateup_down_k512_edge_m32n512_owner_macro_conflict, true});
  if (gateup_k512_pairfeed_implementation ==
      reference_runner_detail::A4W4GateUpK512PairfeedImplementation::
          kInvalid) {
    return runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_projection_span_mlp_k512_gateup_down_edge_m32n512_owner_selector_contract",
        item.layer_index);
  }
#if defined(Q3X_ENABLE_A4W4_MLP_K512_ADMISSION)
  const reference_runner_detail::
...
#if defined(Q3X_ENABLE_A4W4_MLP_K512_ADMISSION)
  const reference_runner_detail::
      A4W4DownK512M128N128Pairring16V1Route
          down_m128n128_16warp_pairring_v1_route =
              reference_runner_detail::
                  select_a4w4_down_k512_m128n128_16warp_pairring_v1_route(
                      a4w4_down_k512_m128n128_16warp_pairring_v1_selector_query(
                          true));
  if (down_m128n128_16warp_pairring_v1_route ==
      reference_runner_detail::
          A4W4DownK512M128N128Pairring16V1Route::kInvalid) {
    return runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_projection_span_mlp_k512_down_m128n128_16warp_pairring_v1_selector_contract",
        item.layer_index);
  }
#else
  constexpr reference_runner_detail::
      A4W4DownK512M128N128Pairring16V1Route
          down_m128n128_16warp_pairring_v1_route =
              reference_runner_detail::
                  A4W4DownK512M128N128Pairring16V1Route::kDisabled;
#endif
  if (mlp_k512_shape_separated_marlin_package_requested &&
      (!mlp_k512_requested ||
       !gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_requested ||
       gateup_down_k512_edge_m32n512_owner_requested ||
       gateup_down_k512_edge_m32n512_owner_gate_conflict ||
       gateup_down_k512_edge_m32n512_owner_macro_conflict ||
       projection_major_gateup_canonical_down_route !=
           reference_runner_detail::
               A4W4ProjectionMajorGateUpCanonicalDownRoute::kDisabled ||
       down_m128n128_16warp_pairring_v1_route !=
           reference_runner_detail::
               A4W4DownK512M128N128Pairring16V1Route::kEnabled)) {
    return runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_projection_span_mlp_k512_shape_separated_marlin_package_selector_contract",
        item.layer_index);
  }
#if !defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M128N512_PAIRED_LDMATRIX_ADMISSION)
  if (paired_old_gate_selected) {
    return runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_projection_span_mlp_k512_old_paired_gate_not_built",
        item.layer_index);
  }
#endif
#if !defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N8_PAIRED_WARP_REGISTER_PIPELINE_ADMISSION)
  if (paired_new_gate_selected) {
    return runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_projection_span_mlp_k512_paired_warp_gate_not_built",
        item.layer_index);
  }
#endif
#if !defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_LDMATRIX_PAIRRING_ADMISSION)
  if (paired_old_down_selected) {
    return runner_status(
        ReferenceRunnerError::kInvalidRunner,
...
#endif
#if !defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_16WARP_PAIRRING_ADMISSION)
  if (paired_new_down_selected) {
    return runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_projection_span_mlp_k512_paired_warp_down_m128n128_16warp_pairring_not_built",
        item.layer_index);
  }
#endif
#if !defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_LDMATRIX_PAIRRING_ADMISSION)
  ...
  if (down_m128n128_16warp_pairring_v1_route ==
    ...
    return runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_projection_span_mlp_k512_down_m128n128_16warp_pairring_v1_not_built",
        item.layer_index);
  }
#endif
  ...
  if (gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_requested &&
      !mlp_k512_requested) {
    return runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_projection_span_mlp_k512_gateup_down_edge_m64n128_k256_ldmatrix_pairfeed_requires_v1",
        item.layer_index);
  }
  if (gateup_down_k512_edge_m64n128_k256_alternating_requested &&
      gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_requested) {
    return runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_projection_span_mlp_k512_gateup_down_edge_m64n128_k256_gate_selector_conflict",
        item.layer_index);
  }
  if (gateup_k512_m128n128_projection_serial_requested &&
      !mlp_k512_requested) {
    return runner_status(
        ReferenceRunnerError::kInvalidRunner,
  ...
  if (gateup_k512_m128n512_fused_quantize_requested &&
    ...
    return runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_projection_span_mlp_k512_gateup_m128n512_fused_quantize_requires_v1",
        item.layer_index);
  }
  ...
  if (gateup_k512_m128n512_fused_quantize_requested &&
    ...
    return runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_projection_span_mlp_k512_gateup_m128n512_fused_quantize_gate_selector_conflict",
        item.layer_index);
  }
  if ((gateup_down_k512_edge_m64n128_k256_alternating_requested ||
       gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_requested) &&
  ...
  if (mlp_k256_m128n256_pairfeed_package_requested &&
      (mlp_k512_requested || mlp_k512_fragment_native_requested ||
       gateup_down_k512_edge_requested ||
       gateup_down_k512_edge_m64n128_k256_alternating_requested ||
       gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_requested ||
       gateup_down_k512_edge_m32n512_owner_requested ||
       mlp_k512_shape_separated_marlin_package_requested ||
       gateup_k512_m128n128_projection_serial_requested ||
       gateup_k512_m128n64_same_cta_requested ||
       gateup_k512_m128n512_fused_quantize_requested ||
       gateup_down_k512_edge_m128n64_requested ||
       down_k512_m16n64_v2_requested || paired_old_gate_selected ||
       paired_new_gate_selected || paired_old_down_selected ||
       paired_new_down_selected ||
       projection_major_gateup_canonical_down_route !=
           reference_runner_detail::
               A4W4ProjectionMajorGateUpCanonicalDownRoute::kDisabled ||
       g_enable_a4w4_gateup_complete_cell_v2_admission ||
    ...
    return runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_projection_span_mlp_k256_m128n256_pairfeed_package_selector_conflict",
        item.layer_index);
  }
  const std::size_t gate_weight_capacity =
      kernels::sm87_a4w4_consumer_packed_capacity_bytes(
  ...
  bool mlp_k256_package_selected = false;
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M64N128_K256_ALTERNATING_ADMISSION) && \
    defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_16WARP_PAIRRING_ADMISSION)
  if (mlp_k256_m128n256_pairfeed_package_requested) {
    const std::size_t mlp_launch_token_count =
        kernels::sm87_a4w4_attention_k256_launch_token_count(token_count);
    const auto gateup_plan =
        kernels::sm87_a4w4_gateup_down_k256_m128n256_pairfeed_plan(
            token_count, mlp_launch_token_count,
            kReferenceIntermediateSize, kReferenceHiddenSize);
    const auto down_plan =
        kernels::sm87_a4w4_down_k256_m128n128_16warp_pairring_plan(
            mlp_launch_token_count, kReferenceHiddenSize,
            kReferenceIntermediateSize);
    const std::size_t down_weight_capacity =
        kernels::sm87_a4w4_attention_k256_packed_capacity_bytes(
            down_sidecar.output_size, down_sidecar.input_size);
    const std::size_t down_scale_capacity =
        kernels::sm87_a4w4_attention_k256_scale_capacity_elements(
            down_sidecar.output_size, down_sidecar.input_size);
    const std::size_t k256_hidden_packed_capacity =
        kernels::sm87_a4w4_attention_k256_packed_capacity_bytes(
            mlp_launch_token_count, kReferenceHiddenSize);
    const std::size_t k256_hidden_scale_capacity =
        kernels::sm87_a4w4_attention_k256_scale_capacity_elements(
            mlp_launch_token_count, kReferenceHiddenSize);
    const std::size_t k256_intermediate_packed_capacity =
        kernels::sm87_a4w4_attention_k256_packed_capacity_bytes(
            mlp_launch_token_count, kReferenceIntermediateSize);
    const std::size_t k256_intermediate_scale_capacity =
        kernels::sm87_a4w4_attention_k256_scale_capacity_elements(
            mlp_launch_token_count, kReferenceIntermediateSize);
    const std::size_t secondary_capacity = static_cast<std::size_t>(
        request_plan.long_prefill_projection_secondary_bf16
    ...
    if (a4w4_prefill_consumer_ !=
            reference_runner_detail::A4W4PrefillConsumer::kK256 ||
        !gate_sidecar.attached() || !up_sidecar.attached() ||
        !down_sidecar.attached() ||
        gate_sidecar.sidecar_kind != PrefillSidecarKind::kA4K256 ||
        up_sidecar.sidecar_kind != PrefillSidecarKind::kA4K256 ||
        down_sidecar.sidecar_kind != PrefillSidecarKind::kA4K256 ||
        gate_sidecar.output_size != kReferenceIntermediateSize ||
        gate_sidecar.input_size != kReferenceHiddenSize ||
        up_sidecar.output_size != kReferenceIntermediateSize ||
        up_sidecar.input_size != kReferenceHiddenSize ||
        down_sidecar.output_size != kReferenceHiddenSize ||
        down_sidecar.input_size != kReferenceIntermediateSize ||
        gate_sidecar.activation_clip_ratio !=
            up_sidecar.activation_clip_ratio ||
        mlp_launch_token_count == 0U ||
        mlp_launch_token_count != projection_token_count ||
        mlp_launch_token_count > span_capacity ||
        gateup_plan.launch_ctas !=
            kernels::kSm87A4W4GateUpDownK256M128N256PairfeedPersistentCtas ||
        down_plan.launch_ctas == 0U ||
        hidden_packed_capacity < k256_hidden_packed_capacity ||
        hidden_scale_capacity < k256_hidden_scale_capacity ||
        intermediate_packed_capacity < k256_intermediate_packed_capacity ||
        intermediate_scale_capacity < k256_intermediate_scale_capacity ||
        gate_weight_capacity !=
            kernels::sm87_a4w4_attention_k256_packed_capacity_bytes(
                kReferenceIntermediateSize, kReferenceHiddenSize) ||
        gate_scale_capacity !=
            kernels::sm87_a4w4_attention_k256_scale_capacity_elements(
                kReferenceIntermediateSize, kReferenceHiddenSize) ||
        up_weight_capacity != gate_weight_capacity ||
        up_scale_capacity != gate_scale_capacity ||
        down_weight_capacity == 0U || down_scale_capacity == 0U ||
        secondary_capacity <
            mlp_launch_token_count * kReferenceHiddenSize) {
      return runner_status(
          ReferenceRunnerError::kInvalidRequestState,
          "prefill_projection_span_mlp_k256_m128n256_pairfeed_package_contract",
          item.layer_index);
    }
    if (!quantize(
            primary, kReferenceHiddenSize, token_count,
        ...
        !check_cuda(
            kernels::
                launch_sm87_a4w4_gateup_down_k256_m128n256_pairfeed_cuda(
                    ...
                    hidden_packed_capacity,
                    views_.prefill_a4_hidden_scales,
                    hidden_scale_capacity, gate_sidecar.weight,
                    gate_weight_capacity, gate_sidecar.scales,
                    gate_scale_capacity, up_sidecar.weight,
                    up_weight_capacity, up_sidecar.scales,
                    up_scale_capacity, token_count,
                    mlp_launch_token_count, kReferenceIntermediateSize,
                    kReferenceHiddenSize,
                    down_sidecar.activation_clip_ratio,
                    ...
                    views_.prefill_a4_intermediate_scales,
                    intermediate_scale_capacity, stream_),
            "prefill_projection_span_mlp_k256_gateup_down_edge_m128n256_pairfeed") ||
        !check_cuda(
            kernels::
                launch_sm87_a4w4_down_k256_m128n128_16warp_pairring_bf16_cuda(
                    ...
                    intermediate_packed_capacity,
                    views_.prefill_a4_intermediate_scales,
                    intermediate_scale_capacity, down_sidecar.weight,
                    down_weight_capacity, down_sidecar.scales,
                    down_scale_capacity, token_count,
                    mlp_launch_token_count, kReferenceHiddenSize,
                    kReferenceIntermediateSize, secondary,
                    kReferenceHiddenSize, secondary_capacity, stream_),
    ...
    ++g_a4w4_full_prefill_admission_hits.generic_projection_hits;
    g_a4w4_full_prefill_admission_hits.logical_projection_hits += 3U;
    ++g_a4w4_mlp_k256_m128n256_pairfeed_package_admission_hits;
    mlp_k256_package_selected = true;
  }
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N128_REGISTER_PIPELINE_ADMISSION)
  if (!mlp_k256_package_selected &&
      (projection_major_gateup_canonical_down_route ==
          reference_runner_detail::
    ...
    const std::size_t secondary_capacity = static_cast<std::size_t>(
        request_plan.long_prefill_projection_secondary_bf16.element_capacity);
    const std::size_t mlp_launch_token_count =
        kernels::sm87_a4w4_gateup_down_edge_launch_token_count(token_count);
    const auto gate_plan = kernels::sm87_a4w4_gateup_down_edge_plan(
        token_count, mlp_launch_token_count, kReferenceIntermediateSize,
        kReferenceHiddenSize);
    ...
    if (!composite.attached() ||
      ...
      return runner_status(
          ReferenceRunnerError::kInvalidRequestState,
          "prefill_projection_span_mlp_k512_projection_major_gateup_canonical_down_contract",
          item.layer_index);
    }
    if (!check_cuda(
    ...
    ++g_a4w4_gateup_k512_m64n128_register_pipeline_admission_hits;
    ...
    if (projection_major_gateup_canonical_down_route ==
        reference_runner_detail::
            A4W4ProjectionMajorGateUpCanonicalDownRoute::kGateAndDown) {
      down_status = launch_selected_a4w4_down_k512_m128n128_16warp_pairring(
              views_.prefill_a4_intermediate_packed,
              intermediate_packed_capacity,
              views_.prefill_a4_intermediate_scales,
              intermediate_scale_capacity, composite.down_codes,
              composite.down_code_capacity_bytes, composite.down_scales,
              composite.down_scale_capacity_elements, token_count,
              mlp_launch_token_count, kReferenceHiddenSize,
              kReferenceIntermediateSize, secondary,
              kReferenceHiddenSize, secondary_capacity, stream_);
      down_stage =
          g_enable_a4w4_down_k512_m128n128_16warp_pairring_l2_macro4x4_admission
              ? "prefill_projection_span_mlp_k512_projection_major_down_m128n128_16warp_pairring_l2_macro4x4"
              : "prefill_projection_span_mlp_k512_projection_major_down_m128n128_16warp_pairring";
    } else
#endif
    {
      down_status = kernels::launch_sm87_a4w4_down_k512_macrocell_bf16_cuda(
          ...
          kReferenceHiddenSize, secondary_capacity, stream_);
      down_stage =
          "prefill_projection_span_mlp_k512_projection_major_canonical_down";
    }
    ...
    if (projection_major_gateup_canonical_down_route ==
        reference_runner_detail::
            A4W4ProjectionMajorGateUpCanonicalDownRoute::kGateAndDown) {
      ++g_a4w4_down_k512_m128n128_16warp_pairring_admission_hits;
    }
#endif
    local_hits.activation_quantize_hits += 2U;
    g_a4w4_full_prefill_admission_hits.activation_quantize_hits += 2U;
  ...
  if (!mlp_k512_selected &&
      (paired_old_gate_selected || paired_new_gate_selected)) {
    const PrefillMLPK512FragmentNativeCompositeView& composite =
        layer_weights.prefill_mlp_k512_fragment_native;
    const std::size_t secondary_capacity = static_cast<std::size_t>(
        request_plan.long_prefill_projection_secondary_bf16
    ...
    const std::size_t mlp_launch_token_count =
        kernels::sm87_a4w4_gateup_down_edge_launch_token_count(token_count);
    bool gate_plan_valid = false;
    bool gate_scratch_contract_valid = true;
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M128N512_PAIRED_LDMATRIX_ADMISSION)
    if (paired_old_gate_selected) {
      const auto gate_plan = kernels::
          sm87_a4w4_gateup_down_edge_m128n512_paired_ldmatrix_plan(
              token_count, mlp_launch_token_count,
              kReferenceIntermediateSize, kReferenceHiddenSize);
      gate_plan_valid = gate_plan.launch_ctas != 0U;
      gate_scratch_contract_valid =
    ...
    if (!composite.attached() ||
        ...
        mlp_launch_token_count != projection_token_count ||
        mlp_launch_token_count > span_capacity ||
        !gate_plan_valid || !gate_scratch_contract_valid ||
        down_plan.launch_ctas == 0U ||
#if defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_16WARP_PAIRRING_ADMISSION)
        (paired_new_down_selected &&
         !kernels::
    ...
    int gate_status = static_cast<int>(cudaErrorInvalidValue);
    const char* gate_stage = nullptr;
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N8_PAIRED_WARP_REGISTER_PIPELINE_ADMISSION)
    if (paired_new_gate_selected) {
      gate_status = kernels::
          launch_sm87_a4w4_gateup_k512_m64n8_paired_warp_register_pipeline_cuda(
              views_.prefill_a4_hidden_packed, hidden_packed_capacity,
              views_.prefill_a4_hidden_scales, hidden_scale_capacity,
              composite.gateup_codes,
              composite.gateup_code_capacity_bytes,
              composite.gateup_scales,
              composite.gateup_scale_capacity_elements, token_count,
              mlp_launch_token_count, kReferenceIntermediateSize,
              kReferenceHiddenSize, composite.down_activation_clip_ratio,
              views_.prefill_a4_intermediate_packed,
              intermediate_packed_capacity,
              views_.prefill_a4_intermediate_scales,
              intermediate_scale_capacity, stream_);
      gate_stage =
          "prefill_projection_span_mlp_k512_gateup_m64n8_paired_warp_register_pipeline";
    }
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M128N512_PAIRED_LDMATRIX_ADMISSION)
    if (paired_old_gate_selected) {
...
#endif
    if (!check_cuda(gate_status, gate_stage)) {
      return failure;
    }
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N8_PAIRED_WARP_REGISTER_PIPELINE_ADMISSION)
    if (paired_new_gate_selected) {
    ...
    if (paired_new_down_selected) {
      ...
      down_stage =
          g_enable_a4w4_down_k512_m128n128_16warp_pairring_l2_macro4x4_admission
              ? "prefill_projection_span_mlp_k512_paired_warp_down_m128n128_16warp_pairring_l2_macro4x4"
              : "prefill_projection_span_mlp_k512_paired_warp_down_m128n128_16warp_pairring";
    } else
#endif
#if defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_LDMATRIX_PAIRRING_ADMISSION)
    if (paired_old_down_selected) {
  ...
  if (!mlp_k512_selected && mlp_k512_requested) {
...
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M32N512_OWNER_ADMISSION)
    const auto gateup_down_edge_m32n512_owner_plan =
        kernels::sm87_a4w4_gateup_down_k512_edge_m32n512_owner_plan(
            token_count, mlp_launch_token_count,
            kReferenceIntermediateSize, kReferenceHiddenSize);
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M128N128_PROJECTION_SERIAL_ADMISSION)
    const auto gateup_m128n128_projection_serial_primary_plan =
    ...
    const auto gateup_m128n128_projection_serial_secondary_plan =
        kernels::sm87_a4w4_gateup_k512_m128n128_projection_serial_plan(
            token_count, mlp_launch_token_count,
            kReferenceIntermediateSize, kReferenceHiddenSize,
            kPrimaryProductColumns, kSecondaryProductColumns);
...
#endif
#if defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_16WARP_PAIRRING_ADMISSION)
    const auto down_m128n128_16warp_pairring_plan =
        kernels::sm87_a4w4_down_k512_plan(
            mlp_launch_token_count, kReferenceHiddenSize,
            kReferenceIntermediateSize);
#endif
    if (!gate_k512.attached() || !up_k512.attached() ||
...
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M32N512_OWNER_ADMISSION)
        (gateup_down_k512_edge_m32n512_owner_requested &&
         gateup_down_edge_m32n512_owner_plan.launch_ctas == 0U) ||
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M128N128_PROJECTION_SERIAL_ADMISSION)
        (gateup_k512_m128n128_projection_serial_requested &&
         (!kernels::
              sm87_a4w4_gateup_k512_m128n128_projection_serial_is_model_plan(
                  gateup_m128n128_projection_serial_primary_plan) ||
          !kernels::
              sm87_a4w4_gateup_k512_m128n128_projection_serial_is_model_plan(
...
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M128N512_FUSED_QUANTIZE_ADMISSION)
        (gateup_k512_m128n512_fused_quantize_requested &&
         gateup_m128n512_fused_quantize_plan.launch_ctas == 0U) ||
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M128N64_ADMISSION)
        (gateup_down_k512_edge_m128n64_requested &&
        ...
        (down_m128n128_16warp_pairring_v1_route ==
         ...
         (!kernels::
              sm87_a4w4_down_k512_m128n128_16warp_pairring_padding_contract(
                  token_count, mlp_launch_token_count) ||
          down_m128n128_16warp_pairring_plan.launch_ctas == 0U)) ||
#endif
        primary_capacity <
            mlp_launch_token_count * kPrimaryProductColumns ||
        secondary_capacity <
      ...
      if (!check_cuda(
              kernels::
                  launch_sm87_a4w4_gateup_k512_m128n128_projection_serial_bf16_cuda(
                      ...
                      stream_),
              "prefill_projection_span_mlp_k512_gateup_m128n128_projection_serial_primary") ||
          !check_cuda(
              kernels::
                  launch_sm87_a4w4_gateup_k512_m128n128_projection_serial_bf16_cuda(
                      views_.prefill_a4_hidden_packed,
                      ...
                      kRequestLongPrefillSecondaryWidth,
                      secondary_capacity, stream_),
              "prefill_projection_span_mlp_k512_gateup_m128n128_projection_serial_secondary") ||
          !check_cuda(
              kernels::launch_sm87_a4_quantize_bf16_k512_split_cuda(
    ...
    if (gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_requested) {
        ...
        if (!check_cuda(
                kernels::
                    launch_sm87_a4w4_gateup_down_k512_edge_m64n256_marlin_cuda(
                        ...
                        views_.prefill_a4_intermediate_scales,
                        intermediate_scale_capacity, stream_),
                "prefill_projection_span_mlp_k512_gateup_down_edge_m64n256_marlin_k64_b3")) {
          return failure;
        }
      } else
#endif
      ...
      if (gateup_k512_pairfeed_implementation ==
          reference_runner_detail::A4W4GateUpK512PairfeedImplementation::
              kM32N512Owner) {
        if (!check_cuda(
                kernels::launch_sm87_a4w4_gateup_down_k512_edge_m32n512_owner_cuda(
                    views_.prefill_a4_hidden_packed,
                    hidden_packed_capacity,
                    views_.prefill_a4_hidden_scales,
                    hidden_scale_capacity, gate_k512.weight,
                    ...
                    views_.prefill_a4_intermediate_scales,
                    intermediate_scale_capacity, stream_),
                "prefill_projection_span_mlp_k512_gateup_down_edge_m32n512_owner_k128_b4")) {
          return failure;
        }
      } else
#endif
      {
        if (!check_cuda(
                kernels::launch_sm87_a4w4_gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_cuda(
                    views_.prefill_a4_hidden_packed,
                    hidden_packed_capacity,
                    views_.prefill_a4_hidden_scales,
                    hidden_scale_capacity, gate_k512.weight,
                    ...
                    views_.prefill_a4_intermediate_scales,
                    intermediate_scale_capacity, stream_),
                "prefill_projection_span_mlp_k512_gateup_down_edge_m64n128_k256_ldmatrix_pairfeed")) {
          return failure;
        }
      }
      ++g_a4w4_gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_admission_hits;
    } else if (gateup_down_k512_edge_m64n128_k256_alternating_requested) {
      if (!check_cuda(
    ...
    if (down_m128n128_16warp_pairring_v1_route ==
      ...
      down_status = launch_selected_a4w4_down_k512_m128n128_16warp_pairring(
              ...
              down_k512_scale_capacity, token_count,
              mlp_launch_token_count, kReferenceHiddenSize,
              kReferenceIntermediateSize, secondary,
              kReferenceHiddenSize, secondary_capacity, stream_);
#if defined(Q3X_ENABLE_A4W4_MLP_K512_SHAPE_SEPARATED_MARLIN_PACKAGE_ADMISSION)
...
#endif
      {
        down_stage =
            g_enable_a4w4_down_k512_m128n128_16warp_pairring_l2_macro4x4_admission
                ? "prefill_projection_span_mlp_k512_down_m128n128_16warp_pairring_l2_macro4x4"
...
#endif
  if (!prefill_r1_projection_plane_v2_mlp_selected &&
      !factorized_lane_r1_selected && !factorized_lane_r4_selected &&
      !mlp_k512_selected &&
      !mlp_k256_package_selected) {
    if (!quantize(primary, kReferenceHiddenSize, token_count,
                  kReferenceHiddenSize, gate_sidecar.activation_clip_ratio,
                  views_.prefill_a4_hidden_packed, hidden_packed_capacity,
                  views_.prefill_a4_hidden_scales, hidden_scale_capacity,
  ...
  if (factorized_lane_r1_cross_layer_handoff_selected &&
...
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT) || \
    defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
    const float* next_attention_inverse_alpha = nullptr;
    std::size_t next_attention_inverse_alpha_capacity = 0U;
#endif
...
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
        const auto& next_v2 =
            next_layer_weights.prefill_r1_projection_plane_v2.linear_qkv;
        if (!next_v2.attached()) {
          return runner_status(
              ReferenceRunnerError::kInvalidRequestState,
              "prefill_projection_span_r1_v2_next_linear_attention_contract",
              item.layer_index);
        }
        next_attention_clip_ratio = static_cast<float>(
            next_v2.primary.activation_clip_ratio);
        next_attention_inverse_alpha = next_v2.primary.inverse_alpha;
        next_attention_inverse_alpha_capacity =
            next_v2.primary.inverse_alpha_capacity_elements;
#endif
      } else if (attention_factorized_lane_r1_selected) {
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
        const auto& next_r1 = next_layer_weights
                                  .prefill_attention_factorized_lane_r1
                                  .linear_qkv;
        if (!next_r1.attached()) {
          return runner_status(
              ReferenceRunnerError::kInvalidRequestState,
              "prefill_projection_span_r1_next_linear_attention_contract",
              item.layer_index);
        }
        next_attention_clip_ratio = next_r1.activation_clip_ratio;
        next_attention_inverse_alpha = next_r1.inverse_alpha;
        next_attention_inverse_alpha_capacity =
            next_r1.inverse_alpha_capacity_elements;
#endif
      } else {
...
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
        const auto& next_v2 =
            next_layer_weights.prefill_r1_projection_plane_v2.full_q;
        if (!next_v2.attached()) {
          return runner_status(
              ReferenceRunnerError::kInvalidRequestState,
              "prefill_projection_span_r1_v2_next_full_attention_contract",
              item.layer_index);
        }
        next_attention_clip_ratio = static_cast<float>(
            next_v2.primary.activation_clip_ratio);
...
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
        const auto& next_r1 =
            next_layer_weights.prefill_attention_factorized_lane_r1.full_q;
        if (!next_r1.attached()) {
          return runner_status(
              ReferenceRunnerError::kInvalidRequestState,
              "prefill_projection_span_r1_next_full_attention_contract",
              item.layer_index);
        }
        next_attention_clip_ratio = next_r1.activation_clip_ratio;
        next_attention_inverse_alpha = next_r1.inverse_alpha;
    ...
    const std::size_t primary_capacity = static_cast<std::size_t>(
        request_plan.long_prefill_projection_primary_bf16.element_capacity);
    const bool next_attention_requires_normalized_bf16 =
        std::holds_alternative<LinearAttentionWeights>(
            next_layer_weights.attention);
    int handoff_status = static_cast<int>(cudaErrorInvalidValue);
    const char* handoff_operation =
        "prefill_projection_span_r1_mlp_to_next_attention_handoff";
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT) || \
    defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
    if (attention_factorized_lane_any_selected) {
      handoff_status = kernels::
          launch_sm87_a4w4_mlp_residual_next_norm_attention_r1_quantize_cuda(
              output_hidden, secondary,
              next_layer_weights.input_layernorm.data,
              next_attention_inverse_alpha,
              next_attention_inverse_alpha_capacity, token_count,
              projection_token_count, kRmsEpsilon,
              next_attention_clip_ratio, output_hidden,
              token_count * kReferenceHiddenSize,
              next_attention_requires_normalized_bf16 ? primary : nullptr,
              next_attention_requires_normalized_bf16 ? primary_capacity : 0U,
              views_.prefill_a4_hidden_packed, hidden_packed_capacity,
              views_.prefill_a4_hidden_scales, hidden_scale_capacity,
              stream_);
      handoff_operation = prefill_r1_projection_plane_v2_selected
                              ? "prefill_projection_span_r1_v2_mlp_to_next_attention_handoff"
                              : "prefill_projection_span_r1_mlp_to_next_attention_r1_handoff";
    } else
#endif
    {
      handoff_status = kernels::
    ...
    if (!check_cuda(handoff_status, handoff_operation)) {
      return failure;
    }
  ...
  if (factorized_lane_r1_handoff_selected) {
    if (prefill_r1_projection_plane_v2_selected) {
      ++local_prefill_r1_projection_plane_v2_hits
            .handoff_package_launch_hits;
      if (next_attention_input_handoff_published) {
        ++local_prefill_r1_projection_plane_v2_hits
              .cross_layer_handoff_launch_hits;
      }
    } else {
...
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
  observed_attention_linear +=
      local_attention_factorized_lane_r1_hits.linear_input_launch_hits;
  observed_attention_full +=
      local_attention_factorized_lane_r1_hits.full_input_launch_hits;
  observed_attention_output +=
      local_attention_factorized_lane_r1_hits.output_launch_hits;
  observed_attention_logical +=
      local_attention_factorized_lane_r1_hits.logical_projection_hits;
#endif
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
  observed_attention_linear +=
      local_prefill_r1_projection_plane_v2_attention_hits
  ...
  observed_attention_full +=
      local_prefill_r1_projection_plane_v2_attention_hits
          .full_input_launch_hits;
  observed_attention_output +=
      local_prefill_r1_projection_plane_v2_attention_hits.output_launch_hits;
  observed_attention_logical +=
      local_prefill_r1_projection_plane_v2_attention_hits
  ...
  const bool incumbent_attention_none =
      local_attention_supermatrix_hits.linear_input_launch_hits == 0U &&
      local_attention_supermatrix_hits.full_input_launch_hits == 0U &&
      local_attention_supermatrix_hits.output_launch_hits == 0U &&
      local_attention_supermatrix_hits.logical_projection_hits == 0U;
  const bool requested_attention_supermatrix_selected =
      attention_factorized_lane_any_selected
          ? incumbent_attention_none
          : (!g_enable_a4w4_attention_supermatrix_admission ||
  ...
  const bool requested_attention_k256_selected =
      attention_factorized_lane_any_selected
          ? k256_attention_none && k256_a_exchange_b4_attention_none
          : (g_enable_a4w4_attention_k256_m128n256_admission
...
#endif
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
  const bool attention_r1_inputs_all =
      local_attention_factorized_lane_r1_hits.linear_input_launch_hits ==
          expected_attention_linear &&
      local_attention_factorized_lane_r1_hits.full_input_launch_hits ==
          expected_attention_full &&
      local_attention_factorized_lane_r1_hits.logical_projection_hits ==
          expected_attention_input_logical +
              local_attention_factorized_lane_r1_hits.output_launch_hits;
  const bool attention_r1_output_all =
      local_attention_factorized_lane_r1_hits.output_launch_hits ==
      expected_attention_output;
  const bool attention_r1_none =
      local_attention_factorized_lane_r1_hits.linear_input_launch_hits == 0U &&
      local_attention_factorized_lane_r1_hits.full_input_launch_hits == 0U &&
      local_attention_factorized_lane_r1_hits.output_launch_hits == 0U &&
      local_attention_factorized_lane_r1_hits.logical_projection_hits == 0U;
  const bool requested_attention_r1_selected =
      attention_factorized_lane_r1_selected
          ? attention_r1_inputs_all && attention_r1_output_all
          : attention_r1_none;
#else
  constexpr bool requested_attention_r1_selected = true;
#endif
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
  const bool attention_v2_inputs_all =
      local_prefill_r1_projection_plane_v2_attention_hits
              .linear_input_launch_hits == expected_attention_linear &&
      local_prefill_r1_projection_plane_v2_attention_hits
              .full_input_launch_hits == expected_attention_full &&
      local_prefill_r1_projection_plane_v2_attention_hits
              .logical_projection_hits ==
          expected_attention_input_logical +
              local_prefill_r1_projection_plane_v2_attention_hits
                  .output_launch_hits;
  const bool attention_v2_output_all =
      local_prefill_r1_projection_plane_v2_attention_hits.output_launch_hits ==
      expected_attention_output;
  const bool attention_v2_none =
      local_prefill_r1_projection_plane_v2_attention_hits
              .linear_input_launch_hits == 0U &&
      local_prefill_r1_projection_plane_v2_attention_hits
              .full_input_launch_hits == 0U &&
      local_prefill_r1_projection_plane_v2_attention_hits
              .output_launch_hits == 0U &&
      local_prefill_r1_projection_plane_v2_attention_hits
              .logical_projection_hits == 0U;
  const bool requested_attention_v2_selected =
      prefill_r1_projection_plane_v2_selected
          ? attention_v2_inputs_all && attention_v2_output_all
          : attention_v2_none;
#else
  constexpr bool requested_attention_v2_selected = true;
#endif
  if (attention_supermatrix_inputs_all) {
  ...
  down_cell_query.admission_enabled =
      g_enable_a4w4_down_complete_cell_v3_admission &&
      !prefill_r1_projection_plane_v2_mlp_selected &&
      !factorized_lane_r1_selected && !factorized_lane_r4_selected &&
      !mlp_k512_selected &&
      !mlp_k256_package_selected;
  expect_down_complete_cell_v2 =
...
#endif
  if (prefill_r1_projection_plane_v2_selected) {
    ++local_prefill_r1_projection_plane_v2_hits.layer_package_launch_hits;
  }
  const bool prefill_r1_projection_plane_v2_layer_accounting_valid =
      prefill_r1_projection_plane_v2_selected
          ? local_prefill_r1_projection_plane_v2_hits
                    .layer_package_launch_hits == 1U &&
                local_prefill_r1_projection_plane_v2_hits
                        .gateup_launch_hits == 1U &&
                local_prefill_r1_projection_plane_v2_hits.down_launch_hits ==
                    1U &&
                local_prefill_r1_projection_plane_v2_hits
                        .attention_physical_launch_hits == 2U &&
                local_prefill_r1_projection_plane_v2_hits
                        .logical_projection_hits == expected_logical &&
                local_prefill_r1_projection_plane_v2_hits
                        .handoff_package_launch_hits == 1U &&
                local_prefill_r1_projection_plane_v2_hits
                        .cross_layer_handoff_launch_hits ==
                    (factorized_lane_r1_cross_layer_handoff_selected &&
                             item.layer_index + 1U <
                         ...
                         : 0U)
          : local_prefill_r1_projection_plane_v2_hits
                        .layer_package_launch_hits == 0U &&
                local_prefill_r1_projection_plane_v2_hits
                        .gateup_launch_hits == 0U &&
                local_prefill_r1_projection_plane_v2_hits.down_launch_hits ==
                    0U &&
                local_prefill_r1_projection_plane_v2_hits
                        .attention_physical_launch_hits == 0U &&
                local_prefill_r1_projection_plane_v2_hits
                        .logical_projection_hits == 0U &&
                local_prefill_r1_projection_plane_v2_hits
                        .handoff_package_launch_hits == 0U &&
                local_prefill_r1_projection_plane_v2_hits
                        .cross_layer_handoff_launch_hits == 0U;
  if (!requested_attention_supermatrix_selected ||
      !requested_attention_k256_selected ||
      !requested_attention_o_k512_selected ||
      !requested_attention_r1_selected ||
      !requested_attention_v2_selected ||
      !prefill_r1_projection_plane_v2_layer_accounting_valid ||
      (!attention_supermatrix_inputs_all &&
       !attention_supermatrix_inputs_none) ||
      (!attention_supermatrix_output_all &&
       !attention_supermatrix_output_none) ||
      ...
      local_hits.activation_quantize_hits !=
          (prefill_r1_projection_plane_v2_mlp_selected ||
                   factorized_lane_r1_selected || factorized_lane_r4_selected ||
                   mlp_k512_selected || mlp_k256_package_selected
               ? 4U
               : 3U) ||
      ...
      local_hits.factorized_lane_r1_handoff_package_launch_hits !=
          (factorized_lane_r1_handoff_prefill_admission_enabled_ ? 1U : 0U) ||
      local_hits.factorized_lane_r1_cross_layer_handoff_launch_hits !=
          (factorized_lane_r1_handoff_prefill_admission_enabled_ &&
                   plan.projection_span_count == 1U &&
                   item.layer_index + 1U < kReferenceDecoderLayerCount
  ...
  if (prefill_r1_projection_plane_v2_selected) {
    auto& request_hits = prefill_r1_projection_plane_v2_request_hits_;
    request_hits.layer_package_launch_hits +=
        local_prefill_r1_projection_plane_v2_hits.layer_package_launch_hits;
    request_hits.gateup_launch_hits +=
        local_prefill_r1_projection_plane_v2_hits.gateup_launch_hits;
    request_hits.down_launch_hits +=
        local_prefill_r1_projection_plane_v2_hits.down_launch_hits;
    request_hits.attention_physical_launch_hits +=
        local_prefill_r1_projection_plane_v2_hits
            .attention_physical_launch_hits;
    request_hits.logical_projection_hits +=
        local_prefill_r1_projection_plane_v2_hits.logical_projection_hits;
    request_hits.handoff_package_launch_hits +=
        local_prefill_r1_projection_plane_v2_hits.handoff_package_launch_hits;
    request_hits.cross_layer_handoff_launch_hits +=
        local_prefill_r1_projection_plane_v2_hits
            .cross_layer_handoff_launch_hits;
  }
  return {};
...
ReferenceLongPrefillOutcome ReferenceRunner::prefill_layer_major_prompt(
  ...
  if (prefill_r1_projection_plane_v2_admission_enabled_ &&
      !projection_span_selected) {
    return fail_long_prefill(runner_status(
        ReferenceRunnerError::kInvalidStepOptions,
        "prefill_r1_projection_plane_v2_requires_whole_m_projection_span"));
  }
  if (factorized_lane_r4_prefill_admission_enabled_ &&
      (!projection_span_selected ||
       !reference_runner_detail::factorized_lane_r4_prefill_prompt_supported(
           prompt_token_count, r4_projection_padding.projection_token_count,
           projection_span_capacity))) {
    return fail_long_prefill(runner_status(
        ReferenceRunnerError::kInvalidStepOptions,
        "factorized_lane_r4_requires_p1793_p1920_projection_span"));
  }
#if defined(Q3X_ENABLE_GDN_PREFILL_PROMPT_SPAN_MACRO_ADMISSION)
  ...
  if (projection_span_selected) {
...
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M64N128_K256_ALTERNATING_ADMISSION) && \
    defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_16WARP_PAIRRING_ADMISSION)
    const std::size_t mlp_k256_m128n256_pairfeed_package_hits_before =
        g_a4w4_mlp_k256_m128n256_pairfeed_package_admission_hits;
#endif
#if defined(Q3X_ENABLE_A4W4_MLP_K512_ADMISSION)
    ...
    const std::size_t
        gateup_m128n512_paired_ldmatrix_hits_before =
            g_a4w4_gateup_down_k512_edge_m128n512_paired_ldmatrix_admission_hits;
    const std::size_t gateup_m64n8_paired_warp_register_pipeline_hits_before =
        g_a4w4_gateup_k512_m64n8_paired_warp_register_pipeline_admission_hits;
    const std::size_t down_m128n128_ldmatrix_pairring_hits_before =
        g_a4w4_down_k512_m128n128_ldmatrix_pairring_admission_hits;
    const std::size_t down_m128n128_16warp_pairring_hits_before =
        g_a4w4_down_k512_m128n128_16warp_pairring_admission_hits;
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N128_REGISTER_PIPELINE_ADMISSION)
    const std::size_t gateup_m64n128_register_pipeline_hits_before =
        g_a4w4_gateup_k512_m64n128_register_pipeline_admission_hits;
    const reference_runner_detail::
        A4W4ProjectionMajorGateUpCanonicalDownRoute
            request_projection_major_gateup_canonical_down_route =
                a4w4_projection_major_gateup_canonical_down_route_;
#endif
    const reference_runner_detail::A4W4PairedGateUpCanonicalDownRoute
    ...
    const reference_runner_detail::
        A4W4DownK512M128N128Pairring16V1Route
            request_down_m128n128_16warp_pairring_v1_route =
                reference_runner_detail::
                    select_a4w4_down_k512_m128n128_16warp_pairring_v1_route(
                        a4w4_down_k512_m128n128_16warp_pairring_v1_selector_query(
                            true));
    ...
    const std::size_t
        gateup_down_k512_edge_m64n128_k256_alternating_hits_before =
            g_a4w4_gateup_down_k512_edge_m64n128_k256_alternating_admission_hits;
    const std::size_t
        gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_hits_before =
            g_a4w4_gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_admission_hits;
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M128N128_PROJECTION_SERIAL_ADMISSION)
    const std::size_t
...
#endif
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
    const auto attention_factorized_lane_r1_hits_before =
        g_a4w4_attention_factorized_lane_r1_admission_hits;
#endif
#if defined(Q3X_ENABLE_GDN_CHUNK64_NATIVE_ADMISSION)
    ...
    const std::size_t gdn_k256_direct_pack_launch_hits_before =
        g_gdn_k256_direct_pack_launch_hits;
    const std::size_t
        gdn_factorized_lane_r1_direct_pack_launch_hits_before =
            g_gdn_factorized_lane_r1_direct_pack_launch_hits;
#if defined(Q3X_ENABLE_GDN_STATE_O_BV64_FUSION_ADMISSION)
    const std::size_t gdn_state_o_bv64_fused_launch_hits_before =
        g_gdn_state_o_bv64_fused_launch_hits;
    const std::size_t gdn_state_o_bv64_fused_logical_token_hits_before =
        g_gdn_state_o_bv64_fused_logical_token_hits;
#endif
#endif
#if defined(Q3X_ENABLE_GDN_PREFILL_PROMPT_SPAN_MACRO_ADMISSION)
    ...
    std::size_t gdn_factorized_lane_r1_direct_pack_launch_delta = 0U;
    std::size_t gdn_state_o_bv64_fused_launch_delta = 0U;
    std::size_t gdn_state_o_bv64_fused_logical_token_delta = 0U;
    ...
    gdn_accounting_valid =
        gdn_accounting_valid &&
        g_gdn_state_o_bv64_fused_launch_hits >=
            gdn_state_o_bv64_fused_launch_hits_before &&
        g_gdn_state_o_bv64_fused_logical_token_hits >=
            gdn_state_o_bv64_fused_logical_token_hits_before;
#endif
    if (gdn_accounting_valid) {
      ...
      gdn_k256_direct_pack_launch_delta =
          g_gdn_k256_direct_pack_launch_hits -
          gdn_k256_direct_pack_launch_hits_before;
      gdn_factorized_lane_r1_direct_pack_launch_delta =
          g_gdn_factorized_lane_r1_direct_pack_launch_hits -
          gdn_factorized_lane_r1_direct_pack_launch_hits_before;
#if defined(Q3X_ENABLE_GDN_STATE_O_BV64_FUSION_ADMISSION)
      gdn_state_o_bv64_fused_launch_delta =
          g_gdn_state_o_bv64_fused_launch_hits -
          gdn_state_o_bv64_fused_launch_hits_before;
      gdn_state_o_bv64_fused_logical_token_delta =
          g_gdn_state_o_bv64_fused_logical_token_hits -
          gdn_state_o_bv64_fused_logical_token_hits_before;
#endif
    }
    ...
    std::size_t attention_factorized_lane_r1_launch_delta = 0U;
    std::size_t attention_factorized_lane_r1_logical_delta = 0U;
...
#endif
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
    std::size_t attention_factorized_lane_r1_output_delta = 0U;
    attention_accounting_valid =
        attention_accounting_valid &&
        a4w4_attention_supermatrix_aggregate_delta(
            attention_factorized_lane_r1_hits_before,
            g_a4w4_attention_factorized_lane_r1_admission_hits, spans,
            attention_factorized_lane_r1_launch_delta,
            attention_factorized_lane_r1_logical_delta,
            attention_factorized_lane_r1_output_delta);
    attention_logical_delta += attention_factorized_lane_r1_logical_delta;
    attention_supermatrix_output_delta +=
        attention_factorized_lane_r1_output_delta;
#endif
    const std::size_t prefill_r1_projection_plane_v2_attention_physical_delta =
        prefill_r1_projection_plane_v2_request_hits_
            .attention_physical_launch_hits;
    const std::size_t prefill_r1_projection_plane_v2_mlp_logical_delta =
        3U * prefill_r1_projection_plane_v2_request_hits_
                 .layer_package_launch_hits;
    const bool prefill_r1_projection_plane_v2_attention_logical_valid =
        prefill_r1_projection_plane_v2_request_hits_.logical_projection_hits >=
        prefill_r1_projection_plane_v2_mlp_logical_delta;
    const std::size_t prefill_r1_projection_plane_v2_attention_logical_delta =
        prefill_r1_projection_plane_v2_attention_logical_valid
            ? prefill_r1_projection_plane_v2_request_hits_
                      .logical_projection_hits -
                  prefill_r1_projection_plane_v2_mlp_logical_delta
            : 0U;
    attention_accounting_valid =
        ...
        prefill_r1_projection_plane_v2_attention_logical_valid &&
        (prefill_r1_projection_plane_v2_admission_enabled_
             ? prefill_r1_projection_plane_v2_attention_physical_delta ==
                       spans * 128U &&
                   prefill_r1_projection_plane_v2_attention_logical_delta ==
                       spans * 208U
             : prefill_r1_projection_plane_v2_attention_physical_delta ==
                       0U &&
                   prefill_r1_projection_plane_v2_attention_logical_delta ==
    ...
    attention_accounting_valid =
        ...
        ((attention_supermatrix_launch_delta != 0U ? 1U : 0U) +
             (attention_k256_incumbent_launch_delta != 0U ? 1U : 0U) +
             (attention_k256_a_exchange_b4_launch_delta != 0U ? 1U : 0U) +
             (attention_factorized_lane_r1_launch_delta != 0U ? 1U : 0U) +
             (prefill_r1_projection_plane_v2_attention_physical_delta != 0U
                  ? 1U
                  : 0U) <=
         1U);
...
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M64N128_K256_ALTERNATING_ADMISSION) && \
    defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_16WARP_PAIRRING_ADMISSION)
    const std::size_t mlp_k256_m128n256_pairfeed_package_delta =
        g_a4w4_mlp_k256_m128n256_pairfeed_package_admission_hits >=
                mlp_k256_m128n256_pairfeed_package_hits_before
            ? g_a4w4_mlp_k256_m128n256_pairfeed_package_admission_hits -
                  mlp_k256_m128n256_pairfeed_package_hits_before
            : 0U;
    const bool mlp_k256_m128n256_pairfeed_package_accounting_valid =
        g_a4w4_mlp_k256_m128n256_pairfeed_package_admission_hits >=
            mlp_k256_m128n256_pairfeed_package_hits_before &&
        (mlp_k256_m128n256_pairfeed_package_delta == 0U ||
         mlp_k256_m128n256_pairfeed_package_delta == spans * 64U) &&
        (!g_enable_a4w4_mlp_k256_m128n256_pairfeed_package_admission ||
         mlp_k256_m128n256_pairfeed_package_delta == spans * 64U);
#else
    constexpr std::size_t mlp_k256_m128n256_pairfeed_package_delta = 0U;
    constexpr bool mlp_k256_m128n256_pairfeed_package_accounting_valid =
        true;
#endif
#if defined(Q3X_ENABLE_A4W4_MLP_K512_ADMISSION)
...
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N128_REGISTER_PIPELINE_ADMISSION)
    const std::size_t gateup_m64n128_register_pipeline_delta =
        g_a4w4_gateup_k512_m64n128_register_pipeline_admission_hits >=
                gateup_m64n128_register_pipeline_hits_before
            ? g_a4w4_gateup_k512_m64n128_register_pipeline_admission_hits -
                  gateup_m64n128_register_pipeline_hits_before
            : 0U;
    const bool projection_major_gate_and_down_selected =
        request_projection_major_gateup_canonical_down_route ==
        reference_runner_detail::
            A4W4ProjectionMajorGateUpCanonicalDownRoute::kGateAndDown;
    const std::size_t projection_major_down_hits_after =
        projection_major_gate_and_down_selected
            ? g_a4w4_down_k512_m128n128_16warp_pairring_admission_hits
            : down_m128n128_16warp_pairring_hits_before;
    const bool projection_major_gateup_canonical_down_accounting_valid =
        reference_runner_detail::
            a4w4_projection_major_gateup_canonical_down_accounting_valid(
                request_projection_major_gateup_canonical_down_route, spans,
                gateup_m64n128_register_pipeline_hits_before,
                g_a4w4_gateup_k512_m64n128_register_pipeline_admission_hits,
                down_m128n128_16warp_pairring_hits_before,
                projection_major_down_hits_after);
#else
    constexpr std::size_t gateup_m64n128_register_pipeline_delta = 0U;
    [[maybe_unused]] constexpr bool projection_major_gate_and_down_selected =
        false;
    constexpr bool
        projection_major_gateup_canonical_down_accounting_valid = true;
#endif
#if defined(Q3X_ENABLE_A4W4_MLP_K512_ADMISSION)
    const std::size_t gateup_m128n512_paired_ldmatrix_delta =
        g_a4w4_gateup_down_k512_edge_m128n512_paired_ldmatrix_admission_hits >=
            ...
            ? g_a4w4_gateup_down_k512_edge_m128n512_paired_ldmatrix_admission_hits -
                  gateup_m128n512_paired_ldmatrix_hits_before
            : 0U;
    const std::size_t gateup_m64n8_paired_warp_register_pipeline_delta =
        g_a4w4_gateup_k512_m64n8_paired_warp_register_pipeline_admission_hits >=
                gateup_m64n8_paired_warp_register_pipeline_hits_before
            ? g_a4w4_gateup_k512_m64n8_paired_warp_register_pipeline_admission_hits -
                  gateup_m64n8_paired_warp_register_pipeline_hits_before
            : 0U;
    ...
    const std::size_t down_m128n128_ldmatrix_pairring_hybrid_hits_after =
        request_paired_gateup_canonical_down_route ==
            ...
            ? g_a4w4_down_k512_m128n128_ldmatrix_pairring_admission_hits
            : down_m128n128_ldmatrix_pairring_hits_before;
    const std::size_t down_m128n128_16warp_pairring_hybrid_hits_after =
        request_paired_gateup_canonical_down_route ==
                reference_runner_detail::
                    A4W4PairedGateUpCanonicalDownRoute::
                        kNewPairedWarpGate16WarpPairringDown
    ...
    const bool paired_gateup_canonical_down_accounting_valid =
        reference_runner_detail::
            a4w4_paired_gateup_canonical_down_accounting_valid(
                ...
                down_m128n128_16warp_pairring_hits_before,
                down_m128n128_16warp_pairring_hybrid_hits_after);
        ...
        reference_runner_detail::
            a4w4_down_k512_m128n128_ldmatrix_pairring_v1_accounting_valid(
                ...
                request_paired_gateup_canonical_down_route ==
                    ...
                    ? down_m128n128_ldmatrix_pairring_hits_before
                    : g_a4w4_down_k512_m128n128_ldmatrix_pairring_admission_hits);
    const std::size_t down_m128n128_16warp_pairring_delta =
        g_a4w4_down_k512_m128n128_16warp_pairring_admission_hits >=
                down_m128n128_16warp_pairring_hits_before
            ? g_a4w4_down_k512_m128n128_16warp_pairring_admission_hits -
                  down_m128n128_16warp_pairring_hits_before
            : 0U;
    const bool down_m128n128_16warp_pairring_v1_accounting_valid =
        reference_runner_detail::
            a4w4_down_k512_m128n128_16warp_pairring_v1_accounting_valid(
                request_down_m128n128_16warp_pairring_v1_route, spans,
                mlp_k512_v1_delta,
                down_m128n128_16warp_pairring_hits_before,
                (projection_major_gate_and_down_selected ||
                 request_paired_gateup_canonical_down_route ==
    ...
    constexpr std::size_t down_m128n128_16warp_pairring_delta = 0U;
    constexpr bool
        down_m128n128_16warp_pairring_v1_accounting_valid = true;
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_ADMISSION)
...
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M128N128_PROJECTION_SERIAL_ADMISSION)
    const std::size_t gateup_m128n128_projection_serial_delta =
        g_a4w4_gateup_k512_m128n128_projection_serial_admission_hits >=
                gateup_m128n128_projection_serial_hits_before
            ? g_a4w4_gateup_k512_m128n128_projection_serial_admission_hits -
                  gateup_m128n128_projection_serial_hits_before
            : 0U;
    const bool gateup_m128n128_projection_serial_accounting_valid =
        g_a4w4_gateup_k512_m128n128_projection_serial_admission_hits >=
            gateup_m128n128_projection_serial_hits_before &&
        (gateup_m128n128_projection_serial_delta == 0U ||
         gateup_m128n128_projection_serial_delta == spans * 64U) &&
        (!g_enable_a4w4_gateup_k512_m128n128_projection_serial_admission ||
         gateup_m128n128_projection_serial_delta == spans * 64U) &&
        (gateup_m128n128_projection_serial_delta == 0U ||
         gateup_m128n128_projection_serial_delta == mlp_k512_v1_delta);
#else
    constexpr std::size_t gateup_m128n128_projection_serial_delta = 0U;
    ...
    const std::size_t gateup_m128n512_fused_quantize_delta =
        g_a4w4_gateup_k512_m128n512_fused_quantize_admission_hits >=
                gateup_m128n512_fused_quantize_hits_before
            ? g_a4w4_gateup_k512_m128n512_fused_quantize_admission_hits -
                  gateup_m128n512_fused_quantize_hits_before
            : 0U;
    const bool gateup_m128n512_fused_quantize_accounting_valid =
        g_a4w4_gateup_k512_m128n512_fused_quantize_admission_hits >=
            gateup_m128n512_fused_quantize_hits_before &&
        (gateup_m128n512_fused_quantize_delta == 0U ||
         gateup_m128n512_fused_quantize_delta == spans * 64U) &&
        (!g_enable_a4w4_gateup_k512_m128n512_fused_quantize_admission ||
         gateup_m128n512_fused_quantize_delta == spans * 64U) &&
        (gateup_m128n512_fused_quantize_delta == 0U ||
         gateup_m128n512_fused_quantize_delta == mlp_k512_v1_delta);
#else
    constexpr std::size_t gateup_m128n512_fused_quantize_delta = 0U;
    constexpr bool gateup_m128n512_fused_quantize_accounting_valid = true;
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M128N64_ADMISSION)
    ...
    const std::size_t mlp_k512_delta =
        ...
        gateup_m64n128_register_pipeline_delta +
        gateup_m64n8_paired_warp_register_pipeline_delta;
    const std::size_t factorized_lane_r1_package_delta =
        hits_after.factorized_lane_r1_package_launch_hits >=
                hits_before.factorized_lane_r1_package_launch_hits
            ? hits_after.factorized_lane_r1_package_launch_hits -
                  hits_before.factorized_lane_r1_package_launch_hits
            : 0U;
    const bool factorized_lane_r1_package_accounting_valid =
        hits_after.factorized_lane_r1_package_launch_hits >=
            hits_before.factorized_lane_r1_package_launch_hits &&
    ...
    const std::size_t factorized_lane_r4_package_delta =
        hits_after.factorized_lane_r4_package_launch_hits >=
            ...
            ? hits_after.factorized_lane_r4_package_launch_hits -
                  hits_before.factorized_lane_r4_package_launch_hits
            : 0U;
    const bool factorized_lane_r4_package_accounting_valid =
        hits_after.factorized_lane_r4_package_launch_hits >=
            hits_before.factorized_lane_r4_package_launch_hits &&
        factorized_lane_r4_package_delta ==
            (factorized_lane_r4_prefill_admission_enabled_
                 ? spans * kReferenceDecoderLayerCount
                 : 0U);
    const std::size_t factorized_lane_r4_2cta_package_delta =
        hits_after.factorized_lane_r4_2cta_package_launch_hits >=
                hits_before.factorized_lane_r4_2cta_package_launch_hits
            ? hits_after.factorized_lane_r4_2cta_package_launch_hits -
                  hits_before.factorized_lane_r4_2cta_package_launch_hits
            : 0U;
    const bool factorized_lane_r4_2cta_package_accounting_valid =
        hits_after.factorized_lane_r4_2cta_package_launch_hits >=
            hits_before.factorized_lane_r4_2cta_package_launch_hits &&
        factorized_lane_r4_2cta_package_delta ==
            (factorized_lane_r4_2cta_prefill_admission_enabled_
                 ? spans * kReferenceDecoderLayerCount
                 : 0U) &&
        factorized_lane_r4_2cta_package_delta <=
    ...
    const bool mlp_k512_accounting_valid =
        ...
        projection_major_gateup_canonical_down_accounting_valid &&
        paired_gateup_canonical_down_accounting_valid &&
        down_m128n128_ldmatrix_pairring_v1_accounting_valid &&
        down_m128n128_16warp_pairring_v1_accounting_valid &&
        ...
        (mlp_k512_v1_delta == 0U ||
         mlp_k512_fragment_native_delta == 0U) &&
        (down_m128n128_ldmatrix_pairring_delta == 0U ||
         down_m128n128_16warp_pairring_delta == 0U) &&
        (gateup_m128n512_paired_ldmatrix_delta == 0U ||
         (mlp_k512_v1_delta == 0U &&
          mlp_k512_fragment_native_delta == 0U &&
          gateup_down_k512_edge_delta == 0U &&
        ...
        (gateup_m64n8_paired_warp_register_pipeline_delta == 0U ||
         (mlp_k512_v1_delta == 0U &&
          ...
          gateup_m128n512_fused_quantize_delta == 0U &&
          gateup_down_k512_edge_m128n64_delta == 0U));
    const auto prefill_r1_projection_plane_v2_route =
        prefill_r1_projection_plane_v2_admission_enabled_
            ? reference_runner_detail::PrefillR1ProjectionPlaneV2Route::
                  kEnabled
            : reference_runner_detail::PrefillR1ProjectionPlaneV2Route::
                  kDisabled;
    const bool prefill_r1_projection_plane_v2_accounting_valid =
        reference_runner_detail::
            prefill_r1_projection_plane_v2_accounting_valid(
                prefill_r1_projection_plane_v2_route, spans,
                prefill_r1_projection_plane_v2_request_hits_);
    const std::size_t prefill_r1_projection_plane_v2_package_delta =
        prefill_r1_projection_plane_v2_request_hits_
            .layer_package_launch_hits;
    const bool activation_accounting_valid =
        ...
        hits_after.activation_quantize_hits -
            ...
            spans * 192U + mlp_k512_delta +
                ...
                factorized_lane_r1_package_delta +
                factorized_lane_r4_package_delta +
                prefill_r1_projection_plane_v2_package_delta;
    ...
    if (!activation_accounting_valid ||
        ...
        ((mlp_k512_delta != 0U ? 1U : 0U) +
             (mlp_k256_m128n256_pairfeed_package_delta != 0U ? 1U : 0U) +
             (factorized_lane_r1_package_delta != 0U ? 1U : 0U) +
             (factorized_lane_r4_package_delta != 0U ? 1U : 0U) +
             (prefill_r1_projection_plane_v2_package_delta != 0U ? 1U : 0U) >
         1U) ||
    ...
    value.attention_k256_m128n256_a_exchange_b4_logical_projection_hits =
        attention_k256_a_exchange_b4_logical_delta;
    value.attention_factorized_lane_r1_launch_hits =
        attention_factorized_lane_r1_launch_delta;
    value.attention_factorized_lane_r1_logical_projection_hits =
        attention_factorized_lane_r1_logical_delta;
    value.mlp_k256_m128n256_pairfeed_package_launch_hits =
        mlp_k256_m128n256_pairfeed_package_delta;
    ...
    value.prefill_r1_projection_plane_v2_gateup_launch_hits =
        prefill_r1_projection_plane_v2_request_hits_.gateup_launch_hits;
    value.prefill_r1_projection_plane_v2_down_launch_hits =
        prefill_r1_projection_plane_v2_request_hits_.down_launch_hits;
...
#if defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
  if (attention_factorized_lane_r1_prefill_admission_enabled_) {
    return fail_long_prefill(runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_attention_factorized_lane_r1_requires_projection_span"));
  }
#endif

#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M64N128_K256_ALTERNATING_ADMISSION) && \
    defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_16WARP_PAIRRING_ADMISSION)
  if (g_enable_a4w4_mlp_k256_m128n256_pairfeed_package_admission) {
    return fail_long_prefill(runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_mlp_k256_m128n256_pairfeed_package_requires_projection_span"));
  }
#endif
  ...
  if (g_enable_a4w4_gateup_down_k512_edge_m64n128_k256_ldmatrix_pairfeed_admission) {
    return fail_long_prefill(runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_mlp_k512_gateup_down_edge_m64n128_k256_ldmatrix_pairfeed_requires_projection_span"));
  }
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M32N512_OWNER_ADMISSION)
  if (g_enable_a4w4_gateup_down_k512_edge_m32n512_owner_admission) {
    return fail_long_prefill(runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_mlp_k512_gateup_down_edge_m32n512_owner_requires_projection_span"));
  }
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M128N128_PROJECTION_SERIAL_ADMISSION)
  // The projection-serial candidate owns only the authenticated whole-M K512
  // projection-span executor.  Never accept its selector and silently fall
  // back to a tiled or K128 Gate+Up route.
  if (g_enable_a4w4_gateup_k512_m128n128_projection_serial_admission) {
    return fail_long_prefill(runner_status(
        ReferenceRunnerError::kInvalidRunner,
...
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_K512_M128N512_FUSED_QUANTIZE_ADMISSION)
  // This structural candidate owns only the authenticated whole-M K512
  // projection-span executor.  Never accept its selector and silently fall
  // back to the tiled or K128 Gate+Up path.
  if (g_enable_a4w4_gateup_k512_m128n512_fused_quantize_admission) {
    return fail_long_prefill(runner_status(
        ReferenceRunnerError::kInvalidRunner,
        "prefill_mlp_k512_gateup_m128n512_fused_quantize_requires_projection_span"));
  }
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M128N64_ADMISSION)
  ...
  callbacks.finish_prompt = +[](void* const opaque,
    ...
    if (execution.a4w4_full_prefill_selected) {
      ...
      [[maybe_unused]] std::size_t
          attention_k256_a_exchange_b4_launch_delta = 0U;
      std::size_t attention_k256_a_exchange_b4_logical_delta = 0U;
      bool attention_accounting_valid = true;
#if defined(Q3X_ENABLE_A4W4_ATTENTION_SUPERMATRIX_ADMISSION)
      attention_accounting_valid =
...
ReferenceRunnerFactoryResult create_reference_runner(
...
#endif
#if !defined(Q3X_ENABLE_A4W4_ATTENTION_FACTORIZED_LANE_R1_EXPERIMENT)
  if (options.enable_attention_factorized_lane_r1_prefill_admission) {
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
        "attention_factorized_lane_r1_prefill_admission_build");
    return result;
  }
#endif
#if !defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
  if (options.enable_prefill_r1_projection_plane_v2_admission) {
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
        "prefill_r1_projection_plane_v2_admission_build");
    return result;
  }
#endif
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION) && \
  ...
  if (options.enable_prefill_r1_projection_plane_v2_admission) {
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
        "prefill_r1_projection_plane_v2_requires_gdn_chunk64_native_build");
    return result;
  }
#endif
  if (options.enable_prefill_r1_projection_plane_v2_admission &&
      (options.enable_factorized_lane_r1_prefill_admission ||
       options.enable_factorized_lane_r1_handoff_prefill_admission ||
       options.enable_attention_factorized_lane_r1_prefill_admission ||
       options.enable_factorized_lane_r4_prefill_admission ||
       options.enable_factorized_lane_r4_2cta_prefill_admission)) {
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
        "prefill_r1_projection_plane_v2_legacy_route_conflict");
    return result;
  }
  if (options.enable_prefill_r1_projection_plane_v2_admission &&
      (!options.enable_a4w4_full_prefill_admission ||
       options.projection_backend != ProjectionBackend::kSm87WeightOnly ||
       options.enable_trace || optimized_prefill_dispatch_disabled())) {
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
        "prefill_r1_projection_plane_v2_dependencies");
    return result;
  }
  if (options.enable_attention_factorized_lane_r1_prefill_admission &&
      (!options.enable_factorized_lane_r1_prefill_admission ||
    ...
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
        "attention_factorized_lane_r1_requires_mlp_r1_handoff_package");
    return result;
  }
  ...
  if (options.enable_factorized_lane_r4_prefill_admission) {
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
        "factorized_lane_r4_prefill_admission_build");
    return result;
  }
#endif
#if !defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R4_2CTA_ADMISSION)
  if (options.enable_factorized_lane_r4_2cta_prefill_admission) {
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
...
#endif
  const reference_runner_detail::FactorizedLaneR4TwoCtaPrefillRoute
      r4_2cta_route =
          reference_runner_detail::select_factorized_lane_r4_2cta_prefill_route(
              {options.enable_factorized_lane_r4_2cta_prefill_admission,
               options.enable_factorized_lane_r4_prefill_admission});
  if (r4_2cta_route == reference_runner_detail::
...
#if defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R4_ADMISSION) && \
    !defined(Q3X_ENABLE_A4W4_FULL_PREFILL_ADMISSION)
  if (options.enable_factorized_lane_r4_prefill_admission) {
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
        "factorized_lane_r4_full_prefill_build");
    return result;
  }
#endif
  if (options.enable_factorized_lane_r1_prefill_admission &&
    ...
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
        "factorized_lane_r1_r4_prefill_admission_conflict");
    return result;
  }
...
#if defined(Q3X_ENABLE_PREFILL_R1_PROJECTION_PLANE_V2_ADMISSION)
  if (options.enable_prefill_r1_projection_plane_v2_admission) {
    const bool complete_attachment =
        complete_prefill_r1_projection_plane_v2_attached(*weights);
    reference_runner_detail::PrefillR1ProjectionPlaneV2RouteQuery query;
    query.requested = true;
    query.full_a4_prefill_enabled =
        options.enable_a4w4_full_prefill_admission;
    query.sm87_weight_only =
        options.projection_backend == ProjectionBackend::kSm87WeightOnly;
    query.trace_enabled = options.enable_trace;
    query.optimized_dispatch_disabled = optimized_prefill_dispatch_disabled();
    query.k256_inventory =
        runner.a4w4_prefill_consumer_ ==
        reference_runner_detail::A4W4PrefillConsumer::kK256;
    query.complete_attachment = complete_attachment;
    query.projection_span = true;
    query.graph_capture = false;
    query.legacy_r1_requested =
        options.enable_factorized_lane_r1_prefill_admission ||
        options.enable_factorized_lane_r1_handoff_prefill_admission ||
        options.enable_attention_factorized_lane_r1_prefill_admission;
    query.r4_requested =
        options.enable_factorized_lane_r4_prefill_admission ||
        options.enable_factorized_lane_r4_2cta_prefill_admission;
    query.legacy_mlp_selector_conflict =
        factorized_lane_mlp_selector_conflict();
    query.legacy_attention_selector_conflict =
        options.enable_attention_factorized_lane_r1_prefill_admission;
    if (reference_runner_detail::select_prefill_r1_projection_plane_v2_route(
            query) != reference_runner_detail::
                          PrefillR1ProjectionPlaneV2Route::kEnabled) {
      result.diagnostic = runner_status(
          complete_attachment ? ReferenceRunnerError::kInvalidDependency
                              : ReferenceRunnerError::kInvalidModelWeights,
          "prefill_r1_projection_plane_v2_contract");
      return result;
    }
    kernels::Sm87A4W4FactorizedLaneQuantizeResources quantize_resources{};
    int resource_status =
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_r1_projection_plane_v2_quantize_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
    kernels::Sm87A4W4GateUpFactorizedLaneResources gateup_resources{};
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_r1_projection_plane_v2_gateup_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
    kernels::Sm87A4W4GateUpR1ProductFinalizeResources finalize_resources{};
    resource_status =
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_r1_projection_plane_v2_finalize_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
    kernels::Sm87A4W4DownFactorizedLaneResources down_resources{};
    resource_status = kernels::
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_r1_projection_plane_v2_down_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
    kernels::Sm87A4W4AttentionFactorizedLaneR1Resources
        attention_resources{};
    resource_status = kernels::
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_r1_projection_plane_v2_attention_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
    kernels::Sm87A4W4PrefillHandoffResources handoff_resources{};
    resource_status = kernels::query_sm87_a4w4_prefill_handoff_resources_cuda(
        &handoff_resources);
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_r1_projection_plane_v2_handoff_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
#if defined(Q3X_ENABLE_GDN_CHUNK64_NATIVE_ADMISSION)
    ...
    resource_status = gdn_prefill_chunk_o_bv64_detail::
        query_norm_factorized_lane_r1_a4_resources(
            &registers_per_thread, &static_shared_bytes, &local_bytes,
            &maximum_threads_per_block, &active_blocks_per_sm);
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_r1_projection_plane_v2_gdn_direct_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
#endif
  ...
  if (options.enable_factorized_lane_r1_prefill_admission) {
    if (!options.enable_a4w4_full_prefill_admission ||
        ...
        runner.a4w4_prefill_consumer_ !=
            reference_runner_detail::A4W4PrefillConsumer::kK256 ||
        !complete_factorized_lane_r1_mlp_attached(*weights) ||
        factorized_lane_mlp_selector_conflict()) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidModelWeights,
          "factorized_lane_r1_prefill_contract");
      return result;
    }
    kernels::Sm87A4W4FactorizedLaneQuantizeResources quantize_resources{};
    int resource_status =
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "factorized_lane_r1_quantize_resource_gate", kReferenceNoLayer,
          resource_status);
      return result;
    }
    kernels::Sm87A4W4GateUpFactorizedLaneResources gateup_resources{};
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "factorized_lane_r1_gateup_resource_gate", kReferenceNoLayer,
          resource_status);
      return result;
    }
    kernels::Sm87A4W4DownFactorizedLaneResources down_resources{};
    ...
    if (options.enable_factorized_lane_r1_handoff_prefill_admission) {
      ...
      if (options.enable_attention_factorized_lane_r1_prefill_admission) {
        ...
        if (resource_status != static_cast<int>(cudaSuccess)) {
          result.diagnostic = runner_status(
              ReferenceRunnerError::kInvalidDependency,
              "factorized_lane_r1_handoff_gdn_r1_resource_gate",
              kReferenceNoLayer, resource_status);
          return result;
        }
      }
  ...
  if (options.enable_attention_factorized_lane_r1_prefill_admission) {
    if (!options.enable_a4w4_full_prefill_admission ||
        ...
        runner.a4w4_prefill_consumer_ !=
            reference_runner_detail::A4W4PrefillConsumer::kK256 ||
        !complete_attention_factorized_lane_r1_attached(*weights)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidModelWeights,
          "attention_factorized_lane_r1_prefill_contract");
      return result;
    }
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "attention_factorized_lane_r1_resource_gate", kReferenceNoLayer,
          resource_status);
      return result;
    }
  }
#endif
#if defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R4_ADMISSION)
  const reference_runner_detail::FactorizedLaneR4PrefillRoute r4_route =
      reference_runner_detail::select_factorized_lane_r4_prefill_route(
          {options.enable_factorized_lane_r4_prefill_admission,
           options.enable_factorized_lane_r1_prefill_admission,
           options.enable_a4w4_full_prefill_admission,
           options.projection_backend == ProjectionBackend::kSm87WeightOnly,
           ...
           runner.a4w4_prefill_consumer_ ==
               reference_runner_detail::A4W4PrefillConsumer::kK256,
           complete_factorized_lane_r4_mlp_attached(*weights),
           factorized_lane_mlp_selector_conflict()});
  if (r4_route ==
      reference_runner_detail::FactorizedLaneR4PrefillRoute::kInvalid) {
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidModelWeights,
        "factorized_lane_r4_prefill_contract");
    return result;
  }
  if (r4_route ==
      reference_runner_detail::FactorizedLaneR4PrefillRoute::kEnabled) {
    kernels::Sm87A4W4FactorizedLaneQuantizeResources quantize_resources{};
    int resource_status =
        kernels::query_sm87_a4w4_factorized_lane_quantize_resources_cuda(
            &quantize_resources);
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "factorized_lane_r4_quantize_resource_gate", kReferenceNoLayer,
          resource_status);
      return result;
    }
#if defined(Q3X_ENABLE_A4W4_FACTORIZED_LANE_R4_2CTA_ADMISSION)
    if (r4_2cta_route == reference_runner_detail::
      ...
      kernels::Sm87A4W4GateUpFactorizedR4M64N64TwoCtaResources
          gateup_resources{};
      resource_status = kernels::
      ...
      if (resource_status != static_cast<int>(cudaSuccess)) {
        result.diagnostic = runner_status(
            ReferenceRunnerError::kInvalidDependency,
            "factorized_lane_r4_2cta_gateup_resource_gate",
            kReferenceNoLayer, resource_status);
        return result;
      }
      kernels::Sm87A4W4DownFactorizedR4M128N64Resources down_resources{};
      resource_status = kernels::
      ...
      if (resource_status != static_cast<int>(cudaSuccess)) {
        result.diagnostic = runner_status(
            ReferenceRunnerError::kInvalidDependency,
            "factorized_lane_r4_2cta_down_resource_gate", kReferenceNoLayer,
            resource_status);
        return result;
      }
    } else
#endif
    {
      kernels::Sm87A4W4GateUpFactorizedR4M128N64Resources gateup_resources{};
      resource_status = kernels::
      ...
      if (resource_status != static_cast<int>(cudaSuccess)) {
        result.diagnostic = runner_status(
            ReferenceRunnerError::kInvalidDependency,
            "factorized_lane_r4_gateup_resource_gate", kReferenceNoLayer,
            resource_status);
        return result;
      }
      kernels::Sm87A4W4DownFactorizedR4M192N128Resources down_resources{};
      resource_status = kernels::
      ...
      if (resource_status != static_cast<int>(cudaSuccess)) {
        result.diagnostic = runner_status(
            ReferenceRunnerError::kInvalidDependency,
            "factorized_lane_r4_down_resource_gate", kReferenceNoLayer,
            resource_status);
        return result;
      }
    }
  ...
  if (g_enable_a4w4_mlp_k256_m128n256_pairfeed_package_admission) {
    if (!options.enable_a4w4_full_prefill_admission ||
      ...
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidModelWeights,
          "prefill_mlp_k256_m128n256_pairfeed_package_contract");
      return result;
    }
    kernels::Sm87A4W4GateUpDownK256M128N256PairfeedResources
        gate_resources{};
    int resource_status = kernels::
        query_sm87_a4w4_gateup_down_k256_m128n256_pairfeed_resources_cuda(
            &gate_resources);
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_mlp_k256_m128n256_pairfeed_gate_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
    kernels::Sm87A4W4DownK256M128N128Pairring16Resources down_resources{};
    resource_status = kernels::
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_mlp_k256_down_m128n128_16warp_pairring_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
  }
...
#if defined(Q3X_ENABLE_A4W4_MLP_K512_SHAPE_SEPARATED_MARLIN_PACKAGE_ADMISSION)
  if (g_enable_a4w4_mlp_k512_shape_separated_marlin_package_admission) {
    kernels::Sm87A4W4GateUpDownEdgeM64N256MarlinResources gate_resources{};
    int resource_status = kernels::
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_mlp_k512_gateup_down_edge_m64n256_marlin_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
    kernels::Sm87A4W4DownK512M64N256Pairring16Resources down_resources{};
    resource_status = kernels::
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_mlp_k512_down_m64n256_16warp_pairring_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
  }
  ...
  if (g_enable_a4w4_gateup_down_k512_edge_m32n512_owner_admission) {
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_mlp_k512_gateup_down_edge_m32n512_owner_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
  }
  ...
  if (projection_major_gateup_canonical_down_route ==
    ...
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
        "prefill_mlp_k512_projection_major_gateup_canonical_down_selector_contract");
    return result;
  }
  if (projection_major_gateup_canonical_down_route !=
          reference_runner_detail::
    ...
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidModelWeights,
        "prefill_mlp_k512_projection_major_gateup_canonical_down_contract");
    return result;
  }
#if !defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_16WARP_PAIRRING_ADMISSION)
  if (projection_major_gateup_canonical_down_route ==
    ...
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
        "prefill_mlp_k512_projection_major_down_m128n128_16warp_pairring_build");
    return result;
  }
#endif
  if (projection_major_gateup_canonical_down_route !=
      reference_runner_detail::
          A4W4ProjectionMajorGateUpCanonicalDownRoute::kDisabled) {
    kernels::Sm87A4W4GateUpK512M64N128RegisterPipelineResources resources{};
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_mlp_k512_projection_major_gateup_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
#if defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_16WARP_PAIRRING_ADMISSION)
    if (projection_major_gateup_canonical_down_route ==
        reference_runner_detail::
            A4W4ProjectionMajorGateUpCanonicalDownRoute::kGateAndDown) {
      kernels::Sm87A4W4DownK512M128N128Pairring16Resources down_resources{};
      const int down_resource_status =
          kernels::
              query_sm87_a4w4_down_k512_m128n128_16warp_pairring_resources_cuda(
                  &down_resources);
      if (down_resource_status != static_cast<int>(cudaSuccess)) {
        result.diagnostic = runner_status(
            ReferenceRunnerError::kInvalidDependency,
            "prefill_mlp_k512_projection_major_down_resource_gate",
            kReferenceNoLayer, down_resource_status);
        return result;
      }
    }
#endif
  }
  runner.a4w4_projection_major_gateup_canonical_down_route_ =
      projection_major_gateup_canonical_down_route;
#else
  [[maybe_unused]] constexpr reference_runner_detail::
      A4W4ProjectionMajorGateUpCanonicalDownRoute
  ...
  if (paired_old_gate_selected) {
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
        "prefill_mlp_k512_old_paired_gate_build");
    return result;
  }
#endif
#if !defined(Q3X_ENABLE_A4W4_GATEUP_K512_M64N8_PAIRED_WARP_REGISTER_PIPELINE_ADMISSION)
  if (paired_new_gate_selected) {
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
        "prefill_mlp_k512_paired_warp_gate_build");
    return result;
  }
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M128N512_PAIRED_LDMATRIX_ADMISSION) || \
  ...
  if (paired_new_down_selected) {
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
        "prefill_mlp_k512_paired_warp_down_m128n128_16warp_pairring_build");
    return result;
  }
#endif
#if defined(Q3X_ENABLE_A4W4_GATEUP_DOWN_K512_EDGE_M128N512_PAIRED_LDMATRIX_ADMISSION)
  if (paired_old_gate_selected) {
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_mlp_k512_old_paired_gate_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
  }
  ...
  if (paired_new_gate_selected) {
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_mlp_k512_paired_warp_gate_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
  }
  ...
  if (paired_old_down_selected) {
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_mlp_k512_old_pairring_down_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
  }
  ...
  if (paired_new_down_selected) {
    ...
    if (resource_status != static_cast<int>(cudaSuccess)) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidDependency,
          "prefill_mlp_k512_paired_warp_down_resource_gate",
          kReferenceNoLayer, resource_status);
      return result;
    }
  }
#endif
  runner.a4w4_paired_gateup_canonical_down_route_ =
      paired_gateup_canonical_down_route;
  const reference_runner_detail::
      A4W4DownK512M128N128LdmatrixPairringV1Route
          down_m128n128_ldmatrix_pairring_v1_route =
  ...
  if (down_m128n128_16warp_pairring_v1_route ==
    ...
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
        "prefill_mlp_k512_down_m128n128_16warp_pairring_v1_selector_contract");
    return result;
  }
#if !defined(Q3X_ENABLE_A4W4_DOWN_K512_M128N128_16WARP_PAIRRING_ADMISSION)
  if (down_m128n128_16warp_pairring_v1_route ==
    ...
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
        "prefill_mlp_k512_down_m128n128_16warp_pairring_v1_build");
    return result;
  }
#else
  if (down_m128n128_16warp_pairring_v1_route ==
    ...
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidModelWeights,
        "prefill_mlp_k512_down_m128n128_16warp_pairring_v1_contract");
    return result;
  }
#endif
#endif
  ...
  if (options.enable_a4w4_full_prefill_admission &&
      runner.a4w4_prefill_consumer_ ==
          reference_runner_detail::A4W4PrefillConsumer::kK256 &&
      !options.enable_prefill_r1_projection_plane_v2_admission) {
    result.diagnostic = runner_status(
        ReferenceRunnerError::kInvalidDependency,
  ...
  runner.factorized_lane_r1_handoff_prefill_admission_enabled_ =
      options.enable_factorized_lane_r1_handoff_prefill_admission;
  runner.attention_factorized_lane_r1_prefill_admission_enabled_ =
      options.enable_attention_factorized_lane_r1_prefill_admission;
  runner.prefill_r1_projection_plane_v2_admission_enabled_ =
      options.enable_prefill_r1_projection_plane_v2_admission;
  ...
  if (options.enable_factorized_lane_r1_prefill_admission ||
      options.enable_prefill_r1_projection_plane_v2_admission) {
    const RequestMemoryPlan& plan = state->plan();
    const std::size_t span_capacity =
        plan.long_prefill_projection_span_capacity;
    const std::size_t hidden_packed_capacity =
        static_cast<std::size_t>(plan.prefill_a4_hidden_packed.byte_size);
    const std::size_t hidden_scale_capacity = static_cast<std::size_t>(
        plan.prefill_a4_hidden_scales_bf16.element_capacity);
    const std::size_t intermediate_packed_capacity =
        static_cast<std::size_t>(
    ...
    const std::size_t intermediate_scale_capacity =
        static_cast<std::size_t>(
            plan.prefill_a4_intermediate_scales_bf16.element_capacity);
    const std::size_t down_launch_token_count =
        kernels::sm87_a4w4_down_factorized_launch_token_count(span_capacity);
    const auto product_finalize_plan =
        kernels::sm87_a4w4_gateup_r1_product_finalize_plan(
            span_capacity, down_launch_token_count);
    if (plan.long_prefill_token_capacity == 0U || span_capacity == 0U ||
        span_capacity %
                kernels::kSm87A4W4DownFactorizedTileM !=
            0U ||
        span_capacity >
            kernels::kSm87A4W4DownFactorizedMaximumTokens ||
        hidden_packed_capacity <
            kernels::sm87_a4w4_gateup_factorized_packed_capacity_bytes(
                span_capacity, kReferenceHiddenSize) ||
        hidden_scale_capacity <
        ...
        intermediate_scale_capacity <
            kernels::sm87_a4w4_down_factorized_scale_capacity_elements(
                span_capacity) ||
        runner.views_.long_prefill_hidden[0] == nullptr ||
        runner.views_.long_prefill_hidden[1] == nullptr ||
        runner.views_.long_prefill_projection_primary == nullptr ||
        runner.views_.long_prefill_projection_secondary == nullptr ||
        runner.views_.prefill_a4_hidden_packed == nullptr ||
        runner.views_.prefill_a4_hidden_scales == nullptr ||
        runner.views_.prefill_a4_intermediate_packed == nullptr ||
        runner.views_.prefill_a4_intermediate_scales == nullptr ||
        (options.enable_prefill_r1_projection_plane_v2_admission &&
         (!product_finalize_plan ||
          runner.views_.prefill_a4_gateup_r1_product_partial_max == nullptr ||
          runner.views_.prefill_a4_gateup_r1_product_partial_max_elements <
              product_finalize_plan.partial_capacity_elements))) {
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidRequestState,
          options.enable_prefill_r1_projection_plane_v2_admission
              ? "prefill_r1_projection_plane_v2_whole_m_workspace_contract"
              : "factorized_lane_r1_whole_m_workspace_contract");
      return result;
    }
  }
#endif
  ...
  if (options.enable_attention_factorized_lane_r1_prefill_admission ||
    ...
    const std::size_t span_capacity =
        plan.long_prefill_projection_span_capacity;
    const std::size_t launch_token_count =
        kernels::sm87_a4w4_attention_k256_launch_token_count(span_capacity);
    const auto hidden_quantize_plan =
        kernels::sm87_a4w4_factorized_lane_quantize_plan(
            span_capacity, launch_token_count, kReferenceHiddenSize, 1U);
    const auto output_quantize_plan =
        kernels::sm87_a4w4_factorized_lane_quantize_plan(
            span_capacity, launch_token_count, kFullQueryElements, 1U);
    const std::size_t hidden_packed_capacity =
        static_cast<std::size_t>(plan.prefill_a4_hidden_packed.byte_size);
    const std::size_t hidden_scale_capacity = static_cast<std::size_t>(
        plan.prefill_a4_hidden_scales_bf16.element_capacity);
    ...
    if (span_capacity == 0U || launch_token_count != span_capacity ||
        !hidden_quantize_plan || !output_quantize_plan ||
        hidden_packed_capacity < hidden_quantize_plan.packed_capacity_bytes ||
        hidden_scale_capacity <
            hidden_quantize_plan.scale_capacity_elements ||
        intermediate_packed_capacity <
            output_quantize_plan.packed_capacity_bytes ||
        intermediate_scale_capacity <
            output_quantize_plan.scale_capacity_elements ||
        primary_capacity < launch_token_count * kFullQGateElements ||
        secondary_capacity < launch_token_count * kFullQueryElements ||
        secondary_capacity <
            launch_token_count * 2U * kFullKvElements ||
        runner.views_.long_prefill_hidden[0] == nullptr ||
        runner.views_.long_prefill_hidden[1] == nullptr ||
        runner.views_.long_prefill_projection_primary == nullptr ||
        runner.views_.long_prefill_projection_secondary == nullptr ||
      ...
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidRequestState,
          options.enable_prefill_r1_projection_plane_v2_admission
              ? "prefill_r1_projection_plane_v2_attention_workspace_contract"
              : "attention_factorized_lane_r1_whole_m_workspace_contract");
      return result;
    }
  }
#endif
  ...
  if (options.enable_factorized_lane_r4_prefill_admission) {
    ...
    static_assert(
        launch_token_count ==
        kernels::kSm87A4W4DownFactorizedR4M192N128LaunchTokens);
    const auto hidden_quantize_plan =
        kernels::sm87_a4w4_factorized_lane_quantize_plan(
            kernels::kSm87A4W4GateUpFactorizedR4M128N64MinimumLogicalTokens,
            launch_token_count, kReferenceHiddenSize, 4U);
    const auto product_quantize_plan =
        kernels::sm87_a4w4_factorized_lane_quantize_plan(
            kernels::kSm87A4W4GateUpFactorizedR4M128N64MinimumLogicalTokens,
            launch_token_count, kReferenceIntermediateSize, 4U);
    const std::size_t hidden_packed_capacity =
        static_cast<std::size_t>(plan.prefill_a4_hidden_packed.byte_size);
    const std::size_t hidden_scale_capacity = static_cast<std::size_t>(
        plan.prefill_a4_hidden_scales_bf16.element_capacity);
    ...
    const std::size_t secondary_capacity = static_cast<std::size_t>(
        plan.long_prefill_projection_secondary_bf16.element_capacity);
    if (plan.long_prefill_token_capacity < launch_token_count ||
        plan.long_prefill_projection_span_capacity < launch_token_count ||
        !hidden_quantize_plan || !product_quantize_plan ||
        hidden_packed_capacity < hidden_quantize_plan.packed_capacity_bytes ||
        hidden_scale_capacity <
            hidden_quantize_plan.scale_capacity_elements ||
        ...
        secondary_capacity <
            launch_token_count *
                kernels::kSm87A4W4GateUpFactorizedR4M128N64SecondaryStride ||
        secondary_capacity < launch_token_count * kReferenceHiddenSize ||
        runner.views_.long_prefill_hidden[0] == nullptr ||
        runner.views_.long_prefill_hidden[1] == nullptr ||
        runner.views_.long_prefill_projection_primary == nullptr ||
        runner.views_.long_prefill_projection_secondary == nullptr ||
      ...
      result.diagnostic = runner_status(
          ReferenceRunnerError::kInvalidRequestState,
          "factorized_lane_r4_whole_m_workspace_contract");
      return result;
    }
  }
#endif
