...
[[nodiscard]] bool supported_lane_count(
  ...
  return prefill_mlp_factorized_lane_qualification_role(lane_count) !=
...
PrefillMLPFactorizedLaneMetadataSerializationResult
serialize_prefill_mlp_factorized_lane_metadata(
  ...
  PrefillMLPFactorizedLaneMetadataSerializationResult result;
  if (!supported_lane_count(lane_count)) {
    result.error =
        PrefillMLPFactorizedLaneMetadataError::kUnsupportedLaneCount;
    return result;
  }
  if (inverse_alpha == nullptr && inverse_alpha_count != 0U) {
    result.error = PrefillMLPFactorizedLaneMetadataError::kNullInput;
    return result;
  }
  if (inverse_alpha_count >
      static_cast<std::size_t>(std::numeric_limits<std::uint64_t>::max())) {
    result.error = PrefillMLPFactorizedLaneMetadataError::kArithmeticOverflow;
    return result;
  }
  const std::uint64_t input_size =
      static_cast<std::uint64_t>(inverse_alpha_count);
  if (!structurally_valid_input_size(input_size, lane_count)) {
    result.error = PrefillMLPFactorizedLaneMetadataError::kInvalidInputSize;
    return result;
  }
  for (std::size_t index = 0U; index < inverse_alpha_count; ++index) {
    if (!std::isfinite(inverse_alpha[index]) ||
      ...
      result.error =
          PrefillMLPFactorizedLaneMetadataError::kInvalidInverseAlpha;
      return result;
    }
  }
  ...
  try {
    ...
    result.error = PrefillMLPFactorizedLaneMetadataError::kAllocationFailure;
    return result;
  ...
  result.error = PrefillMLPFactorizedLaneMetadataError::kNone;
  return result;
}

PrefillMLPFactorizedLaneMetadataParseResult
parse_prefill_mlp_factorized_lane_metadata(
  ...
  PrefillMLPFactorizedLaneMetadataParseResult result;
  if (bytes == nullptr && byte_count != 0U) {
    result.error = PrefillMLPFactorizedLaneMetadataError::kNullInput;
    return result;
  }
  if (byte_count < kPrefillA4FactorizedLaneMetadataHeaderBytes) {
    result.error = PrefillMLPFactorizedLaneMetadataError::kInvalidByteLength;
    return result;
  }
  if (!std::equal(
          reinterpret_cast<const std::uint8_t*>(
    ...
    result.error = PrefillMLPFactorizedLaneMetadataError::kInvalidMagic;
    return result;
  }
  ...
  if (version_major != kPrefillA4FactorizedLaneContractVersionMajor ||
    ...
    result.error = PrefillMLPFactorizedLaneMetadataError::kUnsupportedVersion;
    return result;
  }
  ...
  if (encoding != kPrefillA4FactorizedLaneInverseAlphaFp32Le) {
    result.error =
        PrefillMLPFactorizedLaneMetadataError::kUnsupportedEncoding;
    return result;
  }
  ...
  if (!supported_lane_count(expected_lane_count) ||
    ...
    result.error =
        PrefillMLPFactorizedLaneMetadataError::kExpectedShapeMismatch;
    return result;
  }
  ...
  if (!digest_equal(
    ...
    result.error = PrefillMLPFactorizedLaneMetadataError::kDigestMismatch;
    return result;
  }
  try {
