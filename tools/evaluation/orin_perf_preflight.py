...
class ProcessCollection:
    ...
    ignored_exit_races: list[dict[str, Any]]
...
class GpuFdAudit:
    ...
    inaccessible: list[dict[str, Any]]
    ignored_races: list[dict[str, Any]]
...
) -> tuple[Any | None, OSError | ParseError | None]:
    """Retry one /proc read so only persistent access failures are fatal."""

    last_error: OSError | ParseError | None = None
    for _ in range(2):
        try:
...
def collect_process_snapshot(proc_root: pathlib.Path) -> ProcessCollection:
    processes: dict[int, ProcessSnapshot] = {}
    inaccessible: list[dict[str, Any]] = []
    ignored_exit_races: list[dict[str, Any]] = []
    try:
    ...
    for entry in entries:
        value, error = _retry_os_read(lambda entry=entry: _read_process_snapshot(entry))
        pid = int(entry.name)
        if error is not None:
            if isinstance(error, FileNotFoundError):
                ignored_exit_races.append(
                    {"pid": pid, "stage": "process_snapshot", "kind": "process_exited"}
                )
            else:
                inaccessible.append(
                    {
            ...
            continue
        assert isinstance(value, ProcessSnapshot)
        if value.pid != pid:
            inaccessible.append(
                {
                    "pid": pid,
                    "stage": "process_snapshot",
                    "error": "PID changed during scan",
                    "observed_pid": value.pid,
                }
            )
            continue
        processes[value.pid] = value
    return ProcessCollection(processes, inaccessible, ignored_exit_races)
...
def _process_identity_status(
    proc_root: pathlib.Path, expected: ProcessSnapshot
) -> tuple[str, str | None]:
    """Return same, gone, replaced, or unreadable for an observed PID."""

    value, error = _retry_os_read(
        lambda: parse_proc_stat_line(
            (proc_root / str(expected.pid) / "stat").read_text(encoding="utf-8")
        )
    )
    if error is not None:
        if isinstance(error, FileNotFoundError):
    ...
    assert isinstance(value, ProcessSnapshot)
    if value.pid != expected.pid or value.start_time_ticks != expected.start_time_ticks:
        return "replaced", "PID was reused during the fd audit"
    return "same", None


def _fd_access_failure(
    ...
    """Classify a persistent fd-access error without hiding ownership gaps."""

    identity, identity_error = _process_identity_status(proc_root, process)
    record: dict[str, Any] = {
        "pid": process.pid,
        "stage": stage,
        "error": str(error),
        "error_type": type(error).__name__,
        "identity_after_error": identity,
    }
    if fd is not None:
        record["fd"] = fd
    if identity_error:
        record["identity_error"] = identity_error
    if identity == "gone":
        record["kind"] = "process_exited"
        return None, record
    # If identity is still present, or cannot itself be read, ownership was
    # not exhaustively audited. Both states must fail a formal preflight.
    return record, None


def scan_gpu_fd_holders(
    proc_root: pathlib.Path,
    processes: Mapping[int, ProcessSnapshot],
    ...
    for pid in sorted(processes):
        ...
        if identity != "same":
            identity_record: dict[str, Any] = {
                ...
                "identity_after_snapshot": identity,
            }
            if identity_error:
                identity_record["identity_error"] = identity_error
            if identity == "gone":
                identity_record["kind"] = "process_exited"
                ignored_races.append(identity_record)
            else:
                identity_record["error"] = (
                    "process identity could not be reconciled before fd scan"
                )
                inaccessible.append(identity_record)
            continue
        fd_root = proc_root / str(pid) / "fd"
        value, error = _retry_os_read(lambda fd_root=fd_root: list(fd_root.iterdir()))
        if error is not None:
            assert isinstance(error, OSError)
            incomplete, race = _fd_access_failure(
                proc_root, process, "fd_directory", error
            )
            if incomplete is not None:
                inaccessible.append(incomplete)
            if race is not None:
                ignored_races.append(race)
            continue
        ...
        for descriptor in value:
            target_value, target_error = _retry_os_read(
                lambda descriptor=descriptor: os.readlink(descriptor)
            )
            if target_error is not None:
                assert isinstance(target_error, OSError)
                if isinstance(target_error, FileNotFoundError):
                    identity, identity_error = _process_identity_status(
                        proc_root, process
                    )
                    ...
                    if identity_error:
                        missing_record["identity_error"] = identity_error
                    if identity == "same":
                        missing_record["kind"] = "descriptor_closed"
                        ignored_races.append(missing_record)
                    elif identity == "gone":
                        missing_record["kind"] = "process_exited"
                        ignored_races.append(missing_record)
                    else:
                        missing_record["error"] = (
                            "descriptor disappeared but process identity "
                            "could not be safely reconciled"
                        )
                        inaccessible.append(missing_record)
                    continue
                incomplete, race = _fd_access_failure(
                    proc_root,
                    process,
            ...
            scanned_fds += 1
            if is_gpu_specific_device(target_value):
                specific.add(target_value.removesuffix(" (deleted)"))
            elif is_gpu_auxiliary_device(target_value):
                auxiliary.add(target_value.removesuffix(" (deleted)"))
        ...
        while len(evidence["samples"]) < sample_count:
            ...
            if not events:
                if process.poll() is not None:
                    errors.append(
                        "tegrastats exited before enough samples: "
            ...
            if not chunk:
                if process.poll() is not None:
                    errors.append(
                        "tegrastats closed output before enough samples: "
                        f"status={process.returncode}"
                    )
                    break
                continue
...
def run_preflight(config: PreflightConfig) -> tuple[dict[str, Any], int]:
    ...
    try:
        ...
        first_collection = collect_process_snapshot(proc_root)
    except (OSError, ParseError, RuntimeError) as error:
        first_cpu_total = (0, 0)
        first_collection = ProcessCollection(
            {},
            [
                {
                    "pid": None,
                    "stage": "process_collection",
                    "error": str(error),
                    "error_type": type(error).__name__,
                }
            ],
    ...
    try:
        ...
        second_collection = collect_process_snapshot(proc_root)
    except (OSError, ParseError, RuntimeError) as error:
        second_cpu_total = (0, 0)
        second_collection = ProcessCollection(
            {},
            [
                {
                    "pid": None,
    ...
    if first_collection.inaccessible:
        collection_errors.append(
            "initial process audit incomplete: "
            f"{len(first_collection.inaccessible)} persistent unreadable entries"
        )
    if second_collection.inaccessible:
        collection_errors.append(
            "final process audit incomplete: "
            f"{len(second_collection.inaccessible)} persistent unreadable entries"
        )

    first_processes = first_collection.processes
    second_processes = second_collection.processes
    missing_allowed = [
    ...
    if second_collection.inaccessible or not gpu_audit.complete:
        unreadable_count = (
            len(second_collection.inaccessible) + len(gpu_audit.inaccessible)
        )
        collection_errors.append(
            "GPU handle audit incomplete: "
            f"{unreadable_count} persistent unreadable process/fd entries"
        )
    ...
    report: dict[str, Any] = {
        ...
        "tegrastats": tegra,
        "gpu_device_fd_audit": {
            "complete": second_collection.complete and gpu_audit.complete,
            "scanned_processes": gpu_audit.scanned_processes,
            "scanned_fds": gpu_audit.scanned_fds,
            "holders": gpu_audit.holders,
            "inaccessible_processes": second_collection.inaccessible,
            "inaccessible_fds": gpu_audit.inaccessible,
            "ignored_process_exit_races": (
                second_collection.ignored_exit_races + gpu_audit.ignored_races
            ),
            "matching_rule": (
        ...
        "cpu_audit": {
            ...
            "initial_process_scan_inaccessible": first_collection.inaccessible,
            "initial_process_exit_races": first_collection.ignored_exit_races,
            "final_process_scan_inaccessible": second_collection.inaccessible,
            "final_process_exit_races": second_collection.ignored_exit_races,
        },
        "decision": {
            "accepted": accepted,
            "result": result,
